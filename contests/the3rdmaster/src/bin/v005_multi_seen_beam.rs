...
const SEEN_KEEP_PER_KEY: usize = 3;
...
fn build_plan_beam(
    ...
    for depth in 0..MAX_BEAM_DEPTH {
        ...
        let mut seen = FxHashMap::<(Bits, i16), Vec<usize>>::default();
        ...
        for state in beam.into_iter() {
            ...
            for transition in transitions.into_iter().take(MAX_BRANCHES_PER_STATE) {
                ...
                if !seen_accepts(&mut seen, seen_key, next_cost) {
                    continue;
                }
...
fn seen_accepts(
    seen: &mut FxHashMap<(Bits, i16), Vec<usize>>,
    key: (Bits, i16),
    cost: usize,
) -> bool {
    let entry = seen.entry(key).or_default();
    if entry.iter().any(|&existing| existing == cost) {
        return false;
    }
    if entry.len() < SEEN_KEEP_PER_KEY {
        entry.push(cost);
        entry.sort_unstable();
        return true;
    }
    let worst = *entry.last().unwrap();
    if cost >= worst {
        return false;
    }
    entry.pop();
