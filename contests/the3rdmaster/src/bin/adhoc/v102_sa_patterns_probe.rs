...
struct ProbeStats {
    ...
    build_patterns_in_total: usize,
    build_patterns_out_total: usize,
...
impl ProbeStats {
    fn report(&self) -> String {
        ...
        let avg_patterns_in = if total_build_successes == 0 {
            0.0
        } else {
            self.build_patterns_in_total as f64 / total_build_successes as f64
        };
        let avg_patterns_out = if total_build_successes == 0 {
            0.0
        } else {
            self.build_patterns_out_total as f64 / total_build_successes as f64
        };
        format!(
            "baseline_cost={} initial_build_calls={} initial_build_successes={} search_iters={} candidate_build_attempts={} candidate_evals={} candidate_build_failures={} avg_patterns_in_per_eval={:.2} avg_patterns_out_per_eval={:.2} accepted_total={} accepted_better={} accepted_equal={} accepted_worse={} best_updates={} final_current_cost={} final_best_cost={} final_best_patterns={} elapsed_ms={}",
            self.baseline_cost,
...
fn solve(input: &Input) -> ProbeStats {
    ...
    let Some(mut current) = build_search_state(Vec::new(), input, &mut time_keeper, &mut stats) else {
    ...
    while time_keeper.step() {
        ...
        let Some(candidate) =
            build_search_state(next_patterns, input, &mut time_keeper, &mut stats)
        else {
...
fn build_search_state(
    ...
    stats.build_patterns_in_total += patterns.len();
    ...
    stats.build_patterns_out_total += normalized_patterns.len();
    Some(SearchState {
