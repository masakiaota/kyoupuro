...
if [ -n "$INPUT_FILE" ]; then
    INPUT_FILE_ABS=$(CDPATH= cd -- "$(dirname -- "$INPUT_FILE")" && pwd)/$(basename -- "$INPUT_FILE")
    case "$INPUT_FILE_ABS" in
        "$ROOT_DIR"/tools/in/*)
            OUTPUT_DIR="$OUTPUT_DIR/in"
            ;;
        "$ROOT_DIR"/tools/inB/*)
            OUTPUT_DIR="$OUTPUT_DIR/inB"
            ;;
        "$ROOT_DIR"/tools/in_generated/*)
            OUTPUT_DIR="$OUTPUT_DIR/in_generated"
            ;;
    esac
    mkdir -p "$OUTPUT_DIR"
fi
