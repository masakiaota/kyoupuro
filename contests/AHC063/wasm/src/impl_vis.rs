...
fn try_parse_input(input: &str) -> Result<Input, String> {
    std::panic::catch_unwind(|| parse_input(input))
        .map_err(|_| "Failed to parse input".to_owned())
}

fn fallback_turn_count(output: &str) -> usize {
    output
        .split_whitespace()
        .take(100000)
        .count()
...
pub fn generate(seed: i32) -> String {
    ...
    gen(seed, None, None, None).to_string()
}

pub fn calc_max_turn(input: &str, output: &str) -> usize {
    let fallback = fallback_turn_count(output);
    let Ok(input) = try_parse_input(input) else {
        return fallback;
    };
    match parse_output(&input, output) {
        Ok(out) => out.out.len().max(1),
        Err(_) => fallback,
    }
}

pub fn visualize(input: &str, output: &str, turn: usize) -> Result<(i64, String, String), String> {
    let input = try_parse_input(input)?;
    let out = match parse_output(&input, output) {
        Ok(out) => out,
        Err(err) => {
            let (_, _, svg) = vis(&input, &[], false);
            return Ok((0, err, svg));
        }
    };

    let t = turn.min(out.out.len());
    let (score, err, svg) = vis(&input, &out.out[..t], false);
    Ok((score, err, svg))
}
