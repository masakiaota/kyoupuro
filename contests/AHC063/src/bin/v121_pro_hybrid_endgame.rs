...
const MAX_TARGETS_ENDGAME: usize = 14;
const MAX_TARGETS_RESCUE: usize = 20;
...
const BUDGETS_ENDGAME_LIGHT: [(usize, usize); 2] = [(1_200, 18), (3_500, 22)];
const BUDGETS_ENDGAME_RESCUE: [(usize, usize); 2] = [(8_000, 24), (24_000, 30)];
const BUDGETS_RESCUE: [(usize, usize); 2] = [(16_000, 24), (60_000, 32)];
const ENDGAME_REMAINING_FOOD: usize = 10;
const ENDGAME_ELL_LEFT: usize = 14;
...
fn merge_solutions(
    acc: &mut Vec<BeamState>,
    mut add: Vec<BeamState>,
    input: &Input,
    ell: usize,
) {
    acc.append(&mut add);
    ...
    acc.sort_unstable_by_key(|bs| (next_stage_rank(&bs.state, input, ell + 1), bs.ops.len()));
    let mut uniq = Vec::with_capacity(STAGE_BEAM * 2);
    let mut seen: FxHashSet<Key> = FxHashSet::default();
    for s in acc.drain(..) {
        if seen.insert(encode_key(&s.state)) {
            uniq.push(s);
            if uniq.len() >= STAGE_BEAM * 2 {
                break;
            }
        }
    }
...
fn rescue_stage(
    ...
    for &idx in &order {
        ...
        let mut sols = if endgame_mode {
            let mut tmp = stage_search_bestfirst(bs, input, ell, &BUDGETS_ENDGAME_RESCUE, STAGE_BEAM, started);
            if !time_over(started) {
                let exact = collect_exact_solutions(bs, input, ell, target_color, MAX_TARGETS_RESCUE, started);
                merge_solutions(&mut tmp, exact, input, ell);
            }
            tmp
        } else {
        ...
        for bs in &beam {
            ...
            if !time_over(&started) {
                if endgame_mode {
                    ...
                    if !time_over(&started) {
                        let exact =
                            collect_exact_solutions(bs, input, ell, target_color, MAX_TARGETS_ENDGAME, &started);
                        merge_solutions(&mut sols, exact, input, ell);
                    }
                } else {
