...
fn inventory_try_safe_parked_build_fallback(
    ...
    inventory_try_transportable_build_from_parked(ctx, base, target, phases_left, seg_idx)
        .filter(|candidate| inventory_candidate_respects_active_constraint(base, candidate))
}

fn inventory_try_safe_current_build_fallback(
    ctx: &InventoryCtx<'_>,
    parked_base: &BeamState,
    current_base: &BeamState,
    target: &InventoryTarget<'_>,
    phases_left: usize,
    seg_idx: usize,
) -> Option<BeamState> {
    match inventory_build_target_exact_legacy_parked(ctx, current_base, target, phases_left) {
        InventoryBuildOutcome::Built(next_bs)
            if inventory_candidate_respects_active_constraint(parked_base, &next_bs) =>
        {
            return Some(next_bs);
        }
        _ => {}
    }
    inventory_try_transportable_build_from_current(ctx, current_base, target, phases_left, seg_idx)
        .filter(|candidate| inventory_candidate_respects_active_constraint(parked_base, candidate))
}
...
fn inventory_run_segment_phase(
    ...
    let (split_built, _prepared_for_rescue) = if base.state.len() == 5 && seg_idx > 0 {
        if let Some(next_bs) = inventory_try_direct_build_from_current(ctx, base, &target, phases_left)
        {
            ...
            if exact_prefix(&prepared.state, target.colors, target.protect_len) {
                if inventory_candidate_respects_active_constraint(base, &prepared) {
                    (prepared.clone(), Some(prepared))
                } else {
                    match inventory_try_safe_current_build_fallback(
                        ctx,
                        base,
                        &prepared,
                        &target,
                        ...
                        Some(next_bs) => (next_bs, Some(prepared)),
                        None => match inventory_try_safe_parked_build_fallback(
                            ctx,
                            base,
                            &target,
                            phases_left,
                ...
                match inventory_try_safe_current_build_fallback(
                    ...
                    Some(next_bs) => (next_bs, Some(prepared)),
                    None => match inventory_build_target_exact(ctx, &prepared, &target, phases_left)
                    {
                        InventoryBuildOutcome::Built(next_bs)
                            if inventory_candidate_respects_active_constraint(base, &next_bs) =>
                        {
                        ...
                        _ => match inventory_try_safe_parked_build_fallback(
                            ctx,
                            base,
                            &target,
                            phases_left,
        ...
        if exact_prefix(&prepared.state, target.colors, target.protect_len) {
            ...
            match inventory_build_target_exact(ctx, &prepared, &target, phases_left) {
                InventoryBuildOutcome::Built(next_bs) => (next_bs, None),
                _ => {
                    if base.state.len() == 5 {
                        match inventory_try_safe_parked_build_fallback(
                            ctx,
                            base,
                            &target,
                            phases_left,
    ...
    bs = match transport_to_entry_from_right(
        ...
        Some(next_bs) => next_bs,
        None => {
            if let Some(alt_built) = inventory_try_safe_parked_build_fallback(
                ctx,
                base,
                &target,
                phases_left,
