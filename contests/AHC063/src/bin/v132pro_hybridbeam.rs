...
impl InternalPosDeque {
    ...
    fn push_front_grow(&mut self, cell: Cell) {
        let x = encode_internal_pos_hash(cell);
        self.head = if self.head == 0 { MAX_LEN - 1 } else { self.head - 1 };
...
#[inline]
fn step_with_dropped(st: &State, dir: usize, dropped: &mut DroppedBuf) -> (State, u8, Option<usize>) {
...
fn compute_body_release_dist(st: &State) -> CellSearchResult {
    ...
    CellSearchResult { start: head, dist, prev }
}
...
#[inline]
fn empty_path_rank(n: usize, pos: &InternalPosDeque, target: Cell) -> (usize, usize, usize, usize) {
    (
...
fn navigate_to_goal_loose(
    ...
    while st.head() != goal || !restore_queue.is_empty() {
        ...
        *cnt += 1;
        ...
        let (ns, bite_idx) =
            advance_with_restore_queue(&st, dir, target, ell, &mut restore_queue, &mut dropped)?;
        if bite_idx.is_some() {
            bite_count += 1;
...
fn is_complete_exact(bs: &BeamState, input: &Input) -> bool {
    bs.state.len() == input.m && exact_prefix(&bs.state, input, input.m) && remaining_food_count(&bs.state) == 0
}
...
fn optimize_exact_suffix_with_simple(
    ...
    for &window in &SIMPLE_SUFFIX_WINDOWS {
        ...
        let start_bs = BeamState {
            state: st,
            ops: best.ops[..prefix_turns].to_string(),
        };
...
fn main() {
    ...
    print!("{out}");
    profile_counts::dump("v131pro_hybridbeam");
}
