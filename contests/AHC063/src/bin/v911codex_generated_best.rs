// v911codex_generated_best.rs
// in_generated 2000件だけで最良だった実装可能 ensemble
...
fn run_v012(input: &CommonInput) -> Candidate {
    ...
    let _elapsed_sec = started.elapsed().as_secs_f64();
...
fn run_v014(input: &CommonInput) -> Candidate {
    let started = Instant::now();
    let ops = v014_impl::solve_from_common(input);
    let _elapsed_sec = started.elapsed().as_secs_f64();
    let score = score_ops(input, &ops);
    Candidate {
        ops,
...
fn run_v202(input: &CommonInput, limit_sec: f64) -> Candidate {
    let started = Instant::now();
    let (ops, finished) = v202_impl::solve_from_common(input, limit_sec);
    let _elapsed_sec = started.elapsed().as_secs_f64();
    let score = score_ops(input, &ops);
    Candidate {
        ops,
...
fn run_v149(input: &CommonInput, limit_sec: f64) -> Candidate {
    let started = Instant::now();
    let (ops, finished) = v149_impl::solve_from_common(input, limit_sec);
    let _elapsed_sec = started.elapsed().as_secs_f64();
    let score = score_ops(input, &ops);
    Candidate {
        ops,
...
fn run_sequence(input: &CommonInput, order: &[SolverKind]) -> Candidate {
    let ensemble_start = Instant::now();
    let mut iter = order.iter().copied();
    let first = iter.next().unwrap_or(SolverKind::V202);
    let mut best = match first {
        SolverKind::V012 => run_v012(input),
        SolverKind::V014 => run_v014(input),
        SolverKind::V202 => run_v202(input, remaining_limit_sec(&ensemble_start)),
        SolverKind::V139 => run_v139(input, remaining_limit_sec(&ensemble_start)),
        SolverKind::V149 => run_v149(input, remaining_limit_sec(&ensemble_start)),
    };

    for solver in iter {
        let cand = match solver {
            SolverKind::V012 => run_v012(input),
            SolverKind::V014 => run_v014(input),
            SolverKind::V202 => {
                let limit = remaining_limit_sec(&ensemble_start);
                if limit <= 0.0 {
                    break;
                }
                run_v202(input, limit)
            }
            SolverKind::V139 => {
                let limit = remaining_limit_sec(&ensemble_start);
                if limit <= 0.0 {
            ...
            SolverKind::V149 => {
                ...
                run_v149(input, limit)
            }
        };
        let eligible = match solver {
            SolverKind::V012 | SolverKind::V014 => true,
            SolverKind::V202 | SolverKind::V139 | SolverKind::V149 => cand.finished,
        };
        if eligible {
...
fn select_solver_order(input: &CommonInput) -> &'static [SolverKind] {
    use Density3::{H, L, M};
    use SolverKind::{V012, V014, V139, V149, V202};

    match (input.n, classify_density3(input)) {
        (8..=9, L | M | H) => &[V202],
        (10, L) => &[V012, V014, V139],
        (10, M | H) => &[V202],
        (11, L) => &[V012, V014, V139],
        (11, M) => &[V202],
        (11, H) => &[V012, V014, V149],
        (12, L | M) => &[V012, V014, V139],
        (12, H) => &[V012, V014, V149],
        (13, L | M | H) => &[V012, V014, V149],
        (14, L) => &[V012, V014, V139],
        (14, M) => &[V012, V014, V149],
        (14, H) => &[V202],
        (15, L | M) => &[V012, V014, V149],
        (15, H) => &[V202],
        (16, L | M) => &[V012, V149],
        (16, H) => &[V139],
        _ => &[V202],
...
mod v202_impl {
    ...
    const SOLVE_CHECK_INTERVAL_LOG2: u32 = 6;
    ...
    const FINAL_MIN_LEFT_SEC: f64 = 0.012;
    const ANYTIME_MIN_LEFT_SEC: f64 = 0.035;
    const ANYTIME_CHUNK_MIN_SEC: f64 = 0.07;
    const ANYTIME_CHUNK_MAX_SEC: f64 = 0.32;
    const ANYTIME_SIMPLE_MIN_LEFT_SEC: f64 = 0.06;
    const ANYTIME_SUFFIX_WINDOWS: [usize; 14] =
        [8, 12, 16, 20, 24, 28, 32, 40, 48, 64, 80, 96, 128, 160];
    const BUDGETS_NORMAL_WIDE: [(usize, usize); 4] =
        [(6_000, 24), (18_000, 28), (54_000, 36), (140_000, 48)];
    const BUDGETS_LATE_WIDE: [(usize, usize); 4] =
        [(8_000, 24), (28_000, 32), (84_000, 40), (220_000, 56)];
    const BUDGETS_ENDGAME_WIDE: [(usize, usize); 3] = [(2_000, 20), (12_000, 28), (64_000, 44)];
    const BUDGETS_RESCUE_WIDE: [(usize, usize); 3] = [(24_000, 28), (96_000, 40), (320_000, 56)];
    const BUDGETS_NORMAL_DEEP: [(usize, usize); 4] =
        [(10_000, 28), (36_000, 36), (120_000, 48), (320_000, 64)];
    const BUDGETS_LATE_DEEP: [(usize, usize); 4] =
        [(14_000, 28), (48_000, 40), (180_000, 56), (480_000, 72)];
    const BUDGETS_ENDGAME_DEEP: [(usize, usize); 3] = [(4_000, 24), (18_000, 36), (120_000, 56)];
    const BUDGETS_RESCUE_DEEP: [(usize, usize); 3] = [(40_000, 32), (160_000, 48), (520_000, 72)];
    ...
    struct SearchConfig {
        stage_beam: usize,
        max_targets_per_stage: usize,
        max_targets_endgame: usize,
        max_targets_rescue: usize,
        suffix_stage_beam: usize,
        suffix_opt_targets: usize,
        simple_inject_per_state: usize,
        bite_candidate_width: usize,
        fastlane_min_left_sec: f64,
        fast_fallback_targets: usize,
        simple_stage_beam: usize,
        simple_candidate_width: usize,
        budgets_normal: &'static [(usize, usize)],
        budgets_late: &'static [(usize, usize)],
        budgets_endgame_light: &'static [(usize, usize)],
        budgets_rescue: &'static [(usize, usize)],
    }
    
    const DEFAULT_SEARCH_CONFIG: SearchConfig = SearchConfig {
        stage_beam: STAGE_BEAM,
        max_targets_per_stage: MAX_TARGETS_PER_STAGE,
        max_targets_endgame: MAX_TARGETS_ENDGAME,
        max_targets_rescue: MAX_TARGETS_RESCUE,
        suffix_stage_beam: SUFFIX_STAGE_BEAM,
        suffix_opt_targets: SUFFIX_OPT_TARGETS,
        simple_inject_per_state: SIMPLE_INJECT_PER_STATE,
        bite_candidate_width: BITE_CANDIDATE_WIDTH,
        fastlane_min_left_sec: FASTLANE_MIN_LEFT_SEC,
        fast_fallback_targets: FAST_FALLBACK_TARGETS,
        simple_stage_beam: SIMPLE_STAGE_BEAM,
        simple_candidate_width: SIMPLE_CANDIDATE_WIDTH,
        budgets_normal: &BUDGETS_NORMAL,
        budgets_late: &BUDGETS_LATE,
        budgets_endgame_light: &BUDGETS_ENDGAME_LIGHT,
        budgets_rescue: &BUDGETS_RESCUE,
    };
    
    const WIDE_SEARCH_CONFIG: SearchConfig = SearchConfig {
        stage_beam: 8,
        max_targets_per_stage: 14,
        max_targets_endgame: 36,
        max_targets_rescue: 44,
        suffix_stage_beam: 8,
        suffix_opt_targets: 20,
        simple_inject_per_state: 6,
        bite_candidate_width: 6,
        fastlane_min_left_sec: 0.10,
        fast_fallback_targets: 16,
        simple_stage_beam: 96,
        simple_candidate_width: 8,
        budgets_normal: &BUDGETS_NORMAL_WIDE,
        budgets_late: &BUDGETS_LATE_WIDE,
        budgets_endgame_light: &BUDGETS_ENDGAME_WIDE,
        budgets_rescue: &BUDGETS_RESCUE_WIDE,
    };
    
    const DEEP_SEARCH_CONFIG: SearchConfig = SearchConfig {
        stage_beam: 10,
        max_targets_per_stage: 18,
        max_targets_endgame: 48,
        max_targets_rescue: 56,
        suffix_stage_beam: 10,
        suffix_opt_targets: 28,
        simple_inject_per_state: 8,
        bite_candidate_width: 8,
        fastlane_min_left_sec: 0.08,
        fast_fallback_targets: 24,
        simple_stage_beam: 128,
        simple_candidate_width: 10,
        budgets_normal: &BUDGETS_NORMAL_DEEP,
        budgets_late: &BUDGETS_LATE_DEEP,
        budgets_endgame_light: &BUDGETS_ENDGAME_DEEP,
        budgets_rescue: &BUDGETS_RESCUE_DEEP,
    };
    ...
    fn extend_fastlane_one(
        ...
        if time_over(started) || time_left(started) < config.fastlane_min_left_sec {
            return None;
        }
    ...
    fn collect_exact_solutions_turn_focused(
        ...
        let mut out = Vec::with_capacity(keep_limit);
        let mut seen: FxHashSet<State> = FxHashSet::default();
        for bs in sols {
            if seen.insert(bs.state.clone()) {
                out.push(bs);
                if out.len() >= keep_limit {
                    break;
                }
            }
        }
    ...
    fn rescue_stage(
        ...
        for &idx in &order {
            ...
            if rescue_map.len() >= config.stage_beam * 2 {
                break;
            }
        }
        ...
        if out.len() > config.stage_beam {
            out.truncate(config.stage_beam);
        }
        out
    }
    ...
    fn trim_stage_beam(
        ...
        let mut out = Vec::with_capacity(beam_width);
        let mut seen: FxHashSet<State> = FxHashSet::default();
    ...
    fn grow_to_target_prefix_impl(
        ...
        match mode {
            GrowthMode::Base => {
                for ell in start_ell..goal_len {
                    ...
                    let budgets: &[(usize, usize)] = if goal_len - ell < 10 {
                        ...
                        config.budgets_normal
                    };
    
                    let short_seed = beam.iter().min_by_key(|bs| bs.ops.len()).cloned();
                    let quick_short = short_seed.as_ref().and_then(|bs| {
                        extend_fastlane_one(bs, input, goal_colors, ell, &started, config)
                    });
                    ...
                    for bs in &beam {
                        ...
                        if !time_over(&started) {
                            let simple_children = expand_simple_children(
                                ...
                                goal_colors,
                                ell,
                                config.simple_inject_per_state,
                            );
            ...
            GrowthMode::SuffixTurnFocused => {
                for ell in start_ell..goal_len {
                    if time_over(&started) || time_left(&started) < FINAL_MIN_LEFT_SEC {
                        break;
                    }
                    ...
                    for bs in &beam {
                        ...
                        let mut sols = collect_exact_solutions_turn_focused(
                            ...
                            ell,
                            target_color,
                            config.suffix_opt_targets,
                            config.suffix_stage_beam,
                            &started,
                        );
                    ...
                    if new_beam.len() > config.suffix_stage_beam {
                        new_beam.truncate(config.suffix_stage_beam);
                    }
                    beam = new_beam;
                }
            }
    ...
    fn reconstruct_exact_checkpoints(input: &Input, ops: &[Dir]) -> Vec<Option<(usize, State)>> {
        let goal_colors = &input.d[..input.m];
        ...
        for (t, &dir_u8) in ops.iter().enumerate() {
            ...
            st = ns;
            if ell < input.m && st.len() == ell + 1 && st.colors[..ell + 1] == goal_colors[..ell + 1] {
                ell += 1;
    ...
    fn is_complete_exact(bs: &BeamState, input: &Input) -> bool {
        bs.state.len() == input.m
            && bs.state.colors[..input.m] == input.d[..input.m]
    ...
    fn optimize_exact_suffix(input: &Input, base: BeamState, timer: &TimeKeeper) -> BeamState {
        ...
        for &window in &SUFFIX_OPT_WINDOWS {
            ...
            let Some((start_ell, start_bs)) = checkpoint_beamstate_for_window(input, &best, window)
            else {
                continue;
            };
    
            let cand = solve_suffix_turn_focused(
                input,
                start_bs,
                start_ell,
                timer,
    ...
    fn optimize_exact_suffix_with_simple(
        ...
        for &window in &SIMPLE_SUFFIX_WINDOWS {
            ...
            let cand = solve_simple_beam_from(
                ...
                start_bs,
                start_ell,
                DEFAULT_SEARCH_CONFIG.simple_stage_beam,
                DEFAULT_SEARCH_CONFIG.simple_candidate_width,
                started,
    ...
    fn schedule_anytime_chunk(timer: &TimeKeeper) -> f64 {
        let usable = (timer.exact_remaining_sec() - FINAL_MIN_LEFT_SEC).max(0.0);
        if usable <= 0.0 {
            0.0
        } else if usable <= ANYTIME_CHUNK_MIN_SEC {
            usable
        } else {
            (usable * 0.55).clamp(ANYTIME_CHUNK_MIN_SEC, ANYTIME_CHUNK_MAX_SEC)
        }
    }
    
    fn run_simple_suffix_pass(
        input: &Input,
        start_bs: BeamState,
        start_ell: usize,
        timer: &TimeKeeper,
        ...
        let min_left_sec =
            (local_time_limit_sec * 0.18).clamp(FINAL_MIN_LEFT_SEC, ANYTIME_SIMPLE_MIN_LEFT_SEC);
        let _budget = push_search_budget(timer, local_time_limit_sec.max(0.0), FINAL_MIN_LEFT_SEC);
        let local_started = Instant::now();
        solve_simple_beam_from(
            input,
            start_bs,
            start_ell,
            config.simple_stage_beam,
            config.simple_candidate_width,
            &local_started,
            min_left_sec,
        )
    }
    
    fn optimize_exact_anytime(input: &Input, base: BeamState, timer: &TimeKeeper) -> BeamState {
        let goal_colors = &input.d[..input.m];
        let mut best = base;
        let mut round = 0usize;
    
        while timer.exact_remaining_sec() > ANYTIME_MIN_LEFT_SEC {
            let chunk = schedule_anytime_chunk(timer);
            if chunk <= 0.0 {
                break;
            }
    
            if !is_complete_exact(&best, input) {
                let init_state = State::initial(input);
                let cand = grow_to_target_prefix(
                    input,
                    init_state.clone(),
                    goal_colors,
                    timer,
                ...
                if beam_score_key(&cand, goal_colors) < beam_score_key(&best, goal_colors) {
                    best = cand;
                }
                round += 1;
                continue;
            ...
            let window = ANYTIME_SUFFIX_WINDOWS[round % ANYTIME_SUFFIX_WINDOWS.len()];
            let Some((start_ell, start_bs)) = checkpoint_beamstate_for_window(input, &best, window)
            else {
                round += 1;
                continue;
            ...
            let cand = match round % 3 {
                0 => solve_suffix_turn_focused(
                    input,
                    start_bs,
                    start_ell,
                    timer,
                ...
                _ => {
                    run_simple_suffix_pass(input, start_bs, start_ell, timer, DEEP_SEARCH_CONFIG, chunk)
                }
            };
    ...
    fn solve(input: &Input) -> Ops {
        let timer = TimeKeeper::new(TIME_LIMIT_SEC, SOLVE_CHECK_INTERVAL_LOG2);
        ...
        let anytime_opt = optimize_exact_anytime(input, simple_opt.clone(), &timer);
        let mut best = choose_best_beamstate(
            vec![base, turn_opt, simple_opt, anytime_opt],
            &input.d[..input.m],
        );
        if best.ops.len() > MAX_TURNS {
            best.ops.truncate(MAX_TURNS);
    ...
    fn from_common_input(common: &crate::CommonInput) -> Input {
        let mut d = [0_u8; MAX_LEN];
        for (dst, src) in d.iter_mut().zip(common.goal_colors.iter().copied()) {
            *dst = src;
        }
        let mut food = [0_u8; MAX_CELLS];
        for (dst, src) in food.iter_mut().zip(common.food.iter().copied()) {
            *dst = src;
        }
        Input {
            n: common.n,
...
mod v014_impl {
    ...
    // - beam 評価を「次色へ進めるか → ここまでの総手数 → 次色までの距離」にしていることは、それなりに効いている。
    ...
    const MAX_TURNS: usize = 100_000;
    const BEAM_WIDTH: usize = 108;
    const DIRECT_BEAM_WIDTH: usize = 96;
    const BITE_BEAM_WIDTH: usize = 12;
    ...
    fn solve_beam(input: &Input) -> Ops {
        debug_assert_eq!(DIRECT_BEAM_WIDTH + BITE_BEAM_WIDTH, BEAM_WIDTH);
        ...
        while ell < input.m {
            ...
            let mut bite_children = Vec::new();
            for parent in &beam {
                for child in expand_one_node(parent, input, ell) {
                    match child.lane {
                        ChildLane::Direct => direct_children.push(child),
                        ChildLane::Bite => bite_children.push(child),
                    }
                }
            }
            if direct_children.is_empty() && bite_children.is_empty() {
                break;
            }

            direct_children.sort_by(|a, b| a.score.cmp(&b.score));
            bite_children.sort_by(|a, b| a.score.cmp(&b.score));

            let direct_take = direct_children.len().min(DIRECT_BEAM_WIDTH);
            let bite_take = bite_children.len().min(BITE_BEAM_WIDTH);

            let mut next_children = Vec::with_capacity(BEAM_WIDTH);
            next_children.extend(direct_children.drain(..direct_take));
            next_children.extend(bite_children.drain(..bite_take));

            if next_children.len() < BEAM_WIDTH {
                let mut remainder = Vec::with_capacity(direct_children.len() + bite_children.len());
                remainder.extend(direct_children);
                remainder.extend(bite_children);
                remainder.sort_by(|a, b| a.score.cmp(&b.score));

                let fill = (BEAM_WIDTH - next_children.len()).min(remainder.len());
                next_children.extend(remainder.into_iter().take(fill));
            }
    ...
    fn collect_direct_children(parent: &BeamNode, input: &Input, ell: usize) -> Vec<BeamNode> {
        ...
        let paths = collect_top_k_target_paths(
            ...
            input.n,
            target_color,
            DIRECT_CANDIDATE_WIDTH,
        );

        let mut out = Vec::with_capacity(paths.len());
        for path in paths {
            let Some(child) = build_child_from_base(&parent.state, &parent.ops, input.n, &path)
            else {
                continue;
            };
            out.push(child);
    ...
    fn expand_one_node(parent: &BeamNode, input: &Input, ell: usize) -> Vec<ScoredNode> {
        let direct_children = collect_direct_children(parent, input, ell);
        let bite_children = collect_first_bite_children(parent, input, ell);

        let mut out = Vec::with_capacity(direct_children.len() + bite_children.len());
        for child in direct_children {
            let score = evaluate_node(&child.state, input, ell + 1, child.ops.len());
            out.push(ScoredNode {
                node: child,
                score,
                lane: ChildLane::Direct,
            });
        }
        for child in bite_children {
            let score = evaluate_node(&child.state, input, ell + 1, child.ops.len());
            out.push(ScoredNode {
                node: child,
                score,
...
mod v012_impl {
    ...
    fn expand_one_node(parent: &BeamNode, input: &Input, ell: usize) -> Vec<ScoredNode> {
        let target_color = input.d[ell];
        let paths =
            collect_top_k_target_paths(&parent.state, input.n, target_color, CANDIDATE_WIDTH);
...
mod v139_impl {
...

            {
                ...
                for &dir1_u8 in dirs1.as_slice() {
                    ...
                    for &dir2_u8 in dirs2.as_slice() {
                        ...
                        let mut ops = start_bs.ops.clone();
                        let prefix =
                            prefix_plan.get_or_insert_with(|| reconstruct_plan(&nodes, idx));
                        ops.reserve(prefix.len() + recover_ops.len() + 2);
                        ops.extend_from_slice(prefix);
                        ops.push(dir1 as Dir);
                        ops.extend_from_slice(&recover_ops);
    ...
    fn solve_simple_beam_from(
        ...
        while ell < input.m {
            ...
            for parent in &beam {
                ...
                let mut children = expand_simple_children(
                    parent,
                    input,
                    &input.d[..input.m],
                    ell,
            ...
            all_children.sort_unstable_by_key(|bs| {
                simple_beam_rank(&bs.state, input, ell + 1, bs.ops.len())
            });
    ...
    fn try_target_exact(
        ...
        for &goal in cand.as_slice() {
            ...
            if let Some(b1) =
                navigate_to_goal_loose(bs, input, goal_colors, goal, target, ell, started)
            {
                if let Some(b2) =
                    shrink_to_ell(&b1, input, goal_colors, ell, target, target_color, started)
                {
...
mod v149_impl {
    ...
    fn inventory_normalize_departed_len5(
        ...
        let mut eat_counts =
            Vec::with_capacity(INVENTORY_DEPART_NORMALIZE_NODE_LIMIT.min(8192) + 8);
        let mut bite_counts =
            Vec::with_capacity(INVENTORY_DEPART_NORMALIZE_NODE_LIMIT.min(8192) + 8);
        nodes.push(InventoryStateNode {
    ...
    #[inline]
    fn can_commit_inventory_entry_from_right_state(
        st: &State,
        ...
        F: FnMut(&BeamState) -> bool,
    {
        ...
        for idx in eval_list {
            ...
            let cur = nodes[idx].state.clone();
            if let Some(grown) =
                grow_to_target_prefix(input, cur, goal_colors, timer, build_limit_sec)
            {
                if exact_prefix(&grown.state, goal_colors, goal_colors.len()) {
                    if let Some(next_bs) =
                        inventory_append_launch_and_build(base, &nodes, idx, grown)
    ...
    fn inventory_transport_finish_from_node_with_suffix(
        ...
        let finish =
            inventory_transport_finish_kind(&suffix.state, protect_colors, protect_len, geom)?;
        let mut ops = reconstruct_inventory_state_path(nodes, idx);
    ...
    fn inventory_transport_plan_with_limits(
        ...
        while let Some(Reverse((_, _, idx))) = pq.pop() {
            ...
            for &dir_u8 in legal_dirs(&cur).as_slice() {
                ...
                if inventory_is_entry_cell(geom, ns.head()) {
                    if inventory_transport_finish_kind(&ns, protect_colors, protect_len, geom)
                        .is_some()
                    {
                        let child = nodes.len();
                        nodes.push(InventoryStateNode {
    ...
    fn inventory_transport_oracle_direct_plan(
        ...
        for &(goal_dist, goal) in &candidates {
            ...
            for (step_idx, &dir_u8) in rev.iter().enumerate() {
                let (ns, _, bite_idx) =
                    step_with_dropped(&cur_state, dir_u8 as usize, &mut dropped);
        ...
        if let Some(path) = reconstruct_cell_search_path(&bfs, goal) {
            ...
            if slice.len() >= 2 {
                if let Some(dir) = dir_between_cells(st.n, slice[0], slice[1]) {
                    if inventory_transport_step_state(st, dir, protect_colors, protect_len)
                        .is_some()
                    {
                        return Some(dir);
                    }
    ...
    fn inventory_transport_to_goal_plan_with_limits(
        ...
        while let Some(Reverse((_, _, idx))) = pq.pop() {
            ...
            for &dir_u8 in legal_dirs(&cur).as_slice() {
                let Some((ns, ate, bit)) = inventory_transport_step_state(
                    &cur,
                    dir_u8 as usize,
                    protect_colors,
                    protect_len,
    ...
    fn inventory_transport_plan_with_repair_limits(
        ...
        while let Some(Reverse((_, _, idx))) = pq.pop() {
            ...
            for &dir_u8 in legal_dirs(&cur).as_slice() {
                let dir = dir_u8 as usize;
                let Some((ns, seg, ate, bit)) =
                    inventory_transport_step_state_with_repair_for_entry(
                        &cur,
                        input,
                        dir,
                        protect_colors,
    ...
    fn transport_to_cell_preserving_prefix(
        ...
        let suffix = inventory_transport_to_goal_plan(
            &st,
            input,
            protect_colors,
            protect_len,
    ...
    fn inventory_transport_harvest_entry(
...

        {
            ...
            if let Some(next_bs) = transport_to_cell_preserving_prefix(
                bs,
                input,
                tail_colors,
                protect_len,
    ...
    fn inventory_run_segment_phase(
        ...
        bs = match place_inventory_segment(
            &bs,
            seg_idx,
            ctx.input.n,
            target.colors,
    ...
    fn inventory_build_tail_exact(
        ...
        let _goal_hash = push_goal_prefix_hash(target.colors);
        let _constraint =
            push_movement_constraint(movement_constraint_with_min_col(2 * ctx.stock_cnt));
...
fn select_ensemble_candidate(input: &CommonInput) -> Candidate {
    run_sequence(input, select_solver_order(input))
}
