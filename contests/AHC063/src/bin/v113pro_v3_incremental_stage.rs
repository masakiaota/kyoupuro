// v113pro_v3_incremental_stage.rs
...
const DIRECT_BUDGETS: [(usize, usize); 2] = [(10, 2_000), (16, 8_000)];
const RESCUE_DIRECT_BUDGET: (usize, usize) = (20, 20_000);
const QUICK_DIRECT_DEPTH: usize = 12;
const QUICK_DIRECT_NODES: usize = 3_000;
const QUICK_DIRECT_WINDOW_SEC: f64 = 0.01;
...
fn reconstruct_direct_path(parents: &[usize], moves: &[usize], mut idx: usize) -> String {
    let mut rev = Vec::new();
    while parents[idx] != usize::MAX {
        rev.push(DIRS[moves[idx]].2);
        idx = parents[idx];
    }
    rev.reverse();
...
fn safe_capture_exact_solutions(
    start_bs: &BeamState,
    input: &Input,
    ell: usize,
    max_depth: usize,
    ...
    let start = &start_bs.state;
    if ell >= input.m || !exact_prefix(start, &input.d, ell) {
        return Vec::new();
    }
    let target = input.d[ell];
    ...
    let mut sol_states = FxHashSet::<Key>::default();
    let mut goal_depth: Option<usize> = None;
    let mut nodes = 0usize;

    while let Some(idx) = q.pop_front() {
        if nodes >= max_nodes || started.elapsed().as_secs_f64() >= deadline_sec {
            break;
        }
        nodes += 1;
        ...
        if goal_depth.is_some_and(|gd| dep >= gd) {
            continue;
        }
        ...
        for dir in legal_dirs(&st) {
            ...
            if ate != 0 {
                if ate == target && exact_prefix(&ns, &input.d, ell + 1) {
                    if goal_depth.is_none() {
                        goal_depth = Some(nd);
                    }
                    if Some(nd) == goal_depth {
                        let key = encode_key(&ns);
                        if sol_states.insert(key) {
                            let mut ops = start_bs.ops.clone();
                            ops.push_str(&reconstruct_direct_path(&parents, &moves, idx));
                            ops.push(DIRS[dir].2);
                            sols.push(BeamState { state: ns, ops });
...
fn quick_direct_len(
    ...
    ell: usize,
    started: &Instant,
    cache: &mut FxHashMap<(Key, usize), usize>,
) -> usize {
    if ell >= input.m {
        return 0;
    }

    let state_key = encode_key(st);
    if let Some(&v) = cache.get(&(state_key.clone(), ell)) {
        return v;
    }

    let now = started.elapsed().as_secs_f64();
    let deadline_sec = (now + QUICK_DIRECT_WINDOW_SEC).min(TIME_LIMIT_SEC);
    let probe = BeamState {
        state: st.clone(),
        ops: String::new(),
    };
    let sols = safe_capture_exact_solutions(
        &probe,
        input,
        ell,
        QUICK_DIRECT_DEPTH,
    ...
    let ans = if let Some(bs) = sols.first() {
        bs.ops.len()
    } else {
        1_000_000_000
    };
    cache.insert((state_key, ell), ans);
    ans
}
...
fn solve(input: &Input) -> String {
    ...
    let mut quick_cache: FxHashMap<(Key, usize), usize> = FxHashMap::default();
    ...
    for ell in 5..input.m {
        ...
        for bs in &beam {
            ...
            if exact_prefix(&bs.state, &input.d, ell) {
                for &(max_depth, max_nodes) in &DIRECT_BUDGETS {
                    if time_over(&started) {
                        break;
                    }
                    let direct = safe_capture_exact_solutions(
                        bs,
                        input,
                        ell,
                        max_depth,
            ...
            if !time_over(&started) {
                let cand = stage_search_bestfirst(bs, input, ell, budgets, STAGE_BEAM, &started);
                got_general = !cand.is_empty();
                sols.extend(cand);
            }

            if !got_general && !time_over(&started) {
        ...
        new_beam.sort_unstable_by_key(|bs| {
            (
                next_stage_rank(&bs.state, input, ell + 1),
                quick_direct_len(&bs.state, input, ell + 1, &started, &mut quick_cache),
                bs.ops.len(),
            )
        });
    ...
    while !time_over(&started) {
        let ell = lcp(&best.state.colors, &input.d);
        if ell >= input.m || !exact_prefix(&best.state, &input.d, ell) {
            break;
        }
        ...
        let direct = safe_capture_exact_solutions(
            ...
            input,
            ell,
            RESCUE_DIRECT_BUDGET.0,
            RESCUE_DIRECT_BUDGET.1,
            1,
            &started,
            TIME_LIMIT_SEC,
        );
    ...
    let mut ans = best.ops;
    if ans.len() > MAX_TURNS {
        ans.truncate(MAX_TURNS);
    }
    ans
}
