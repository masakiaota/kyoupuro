// v135pro_hybridbeam_opsoccmanhattan.rs
// メモ:
...
fn reconstruct_plan(nodes: &[Node], mut idx: usize) -> Ops {
    let mut rev = Vec::new();
    ...
    let total_len = rev.iter().map(|seg| seg.len()).sum();
    let mut out = Ops::with_capacity(total_len);
    for seg in rev {
...
fn reconstruct_quick_plan(nodes: &[QuickSearchNode], mut idx: usize) -> Ops {
    let mut rev = Vec::new();
        ...
        for &dir_u8 in dirs.as_slice() {
            ...
            if child.state.len() >= ell + 1 {
                ...
                if good {
                    ...
                    let quick_ops = reconstruct_quick_plan(&nodes, goal_idx);
                    ops.extend_from_slice(&quick_ops);
            ...
            if let Some(dir2) = target_adjacent(&st, input.d[ell]) {
                ...
                if bite2.is_none() && exact_prefix(&ns2, input, ell + 1) {
                    if solkeys.insert(ns2.clone()) {
                        ...
                        plan.push(dir2 as Dir);

                        let mut ops = start_bs.ops.clone();
                        ops.extend_from_slice(&plan);
...

        {
            ...
            for &dir1_u8 in dirs1.as_slice() {
                ...
                for &dir2_u8 in dirs2.as_slice() {
                    ...
                    plan.push(dir1 as Dir);
                    plan.extend_from_slice(&recover_ops);
                    plan.push(dir2 as Dir);
