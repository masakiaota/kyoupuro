// v015_detour_beam.rs
// 考察メモ:
// - direct / first-bite で child が作れない parent に対する救済として collect_detour_rejoin_children を追加した。
// - detour 中は prefix を壊さずに数手だけ探索し、exact_prefix(ell) に戻れた地点から既存の direct 探索へ合流させる。
// - detour の局所 path は path_bits(u16, 2bit/手) で持つので、深さ上限と型幅の整合に注意が必要である。
// - 今回追加した collect_detour_rejoin_children は、詰み回避の能力はかなり低い。
...
// detour も同じ packed local path(u16, 2bit/手) を使うため、DETOUR_DEPTH_LIMIT は 8 以下に保つこと。
const DETOUR_DEPTH_LIMIT: usize = 7;
const DETOUR_CANDIDATE_WIDTH: usize = 8;
const DETOUR_NON_TARGET_LIMIT: u8 = 128;
type Dir = u8;
type Ops = Vec<Dir>;
const DIR_U: Dir = 0;
const DIR_D: Dir = 1;
...
#[inline]
fn prefix_ok(state: &State, input: &Input, ell: usize) -> bool {
    matches_prefix_len(&state.colors, &input.d, state.len().min(ell))
}

#[inline]
fn exact_prefix(state: &State, input: &Input, ell: usize) -> bool {
    state.len() == ell && matches_prefix_len(&state.colors, &input.d, ell)
}
...
fn solve_beam(input: &Input) -> Ops {
    ...
    while ell < input.m {
        ...
        beam = next_children.into_iter().map(|scored| scored.node).collect();
        ell += 1;
    }
...
#[inline]
fn local_detour_key(state: &State) -> LocalDetourKey {
    LocalDetourKey {
        pos_len: state.pos.len(),
        pos_hash1: state.pos.hash1,
        pos_hash2: state.pos.hash2,
...
fn append_local_dir(path_bits: u16, depth: u8, dir: Dir) -> u16 {
    ...
    debug_assert!(2 * (usize::from(depth) + 1) <= u16::BITS as usize);
...
/// prefix を壊さない範囲で数手だけ detour を探索し、
/// exact_prefix(ell) に戻れた地点から既存の direct 探索で d[ell] を食う child を集める。
/// 途中で current target を食う枝は見ず、自己bite は 1 回だけ許して復旧後に finish を試す。
fn collect_detour_rejoin_children(parent: &BeamNode, input: &Input, ell: usize) -> Vec<BeamNode> {
    const DETOUR_BITE_LIMIT: u8 = 1;
    ...
    visited.insert(local_detour_key(&parent.state));

    while let Some(node) = q.pop_front() {
        if out.len() >= DETOUR_CANDIDATE_WIDTH {
            break;
        }

        // ちょうど ell 個の prefix に戻れているなら、ここから通常 direct で finish を試す。
        if exact_prefix(&node.state, input, ell) {
            if let Some(path) = find_path_to_nearest_target_food(&node.state, input.n, target_color)
            {
                if let Some(child) = build_child_from_detour_base(
                    parent,
                    &node.state,
                    node.path_bits,
                    node.depth,
        ...
        if usize::from(node.depth) == DETOUR_DEPTH_LIMIT {
            continue;
        }
        ...
        for &dir in &ALL_DIRS {
            ...
            let step_result = step(&node.state, input.n, dir);
            // detour 中に current target を食う枝は他の候補生成器に任せる。
            if step_result.ate == Some(target_color) {
                continue;
            }
            ...
            let mut next_non_target_used = node.non_target_used;
            // non-target の誤飲は許すが、上限を超える枝は広げない。
            if let Some(ate_color) = step_result.ate {
                if ate_color != target_color {
                    if next_non_target_used >= DETOUR_NON_TARGET_LIMIT {
                        continue;
                    }
                    next_non_target_used += 1;
                }
            }

            if step_result.bite_idx.is_some() {
                // self-bite は 1 回だけ許し、復旧して exact_prefix(ell) に戻れた場合だけ finish を試す。
                let next_bite_used = node.bite_used + 1;
                if next_bite_used > DETOUR_BITE_LIMIT {
                    continue;
                }
                ...
                let Some(child) = build_child_from_detour_repaired_base(
                    parent,
                    &repaired.state,
                    next_path_bits,
                    next_depth,
                ...
                // bite 後の途中状態は継続探索せず、その場で finish 可否だけを見る。
                continue;
            }

            // 通常の detour 途中状態は、prefix をまだ壊していない枝だけキューに残す。
            if !prefix_ok(&step_result.state, input, ell) {
                continue;
            }

            if !visited.insert(local_detour_key(&step_result.state)) {
                continue;
            }
            ...
            q.push_back(LocalDetourNode {
                ...
                path_bits: next_path_bits,
                depth: next_depth,
                non_target_used: next_non_target_used,
                bite_used: node.bite_used,
            });
        }
    }
...
fn expand_one_node(parent: &BeamNode, input: &Input, ell: usize) -> Vec<ScoredNode> {
    ...
    let detour_children = if direct_children.is_empty() && bite_children.is_empty() {
        collect_detour_rejoin_children(parent, input, ell)
    } else {
        Vec::new()
    };

    let mut out =
        Vec::with_capacity(direct_children.len() + bite_children.len() + detour_children.len());
...
fn build_child_from_detour_base(
    parent: &BeamNode,
    exact_state: &State,
    local_path_bits: u16,
    local_depth: u8,
    ...
    let mut child_state = exact_state.clone();
    let mut child_ops = parent.ops.clone();
    extend_ops_with_local_path(&mut child_ops, local_path_bits, local_depth);
    if !apply_path_with_turn_cap(&mut child_state, &mut child_ops, n, path, MAX_TURNS) {
        return None;
...
fn build_child_from_detour_repaired_base(
    parent: &BeamNode,
    repaired_state: &State,
    local_path_bits: u16,
    local_depth: u8,
