...
fn plan_color_goal_quick(
    ...
    while let Some(cur_idx) = q.pop_front() {
        ...
        for dir in legal_dirs(nodes[cur_idx].state.as_ref()) {
            let (sim, ate, bite_idx, _) = step(nodes[cur_idx].state.as_ref(), dir);
            ...
            let key = (state_hash64(child.state.as_ref()), non_target, bite);
            if seen
                .get(&key)
        ...
        if exact_prefix(&st, &input.d, ell) {
            if let Some(dir2) = target_adjacent(&st, input.d[ell]) {
                ...
                if bite2.is_none() && exact_prefix(&ns2, &input.d, ell + 1) {
                    if !beam_contains_state(&sols, &ns2) {
