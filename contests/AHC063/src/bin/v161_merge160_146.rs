...
fn inventory_try_depart_empty4_static(
    ...
    while let Some(idx) = q.pop_front() {
        ...
        for nxt in next_vec {
            ...
            let Some(dir) = dir_between_cells(n, cur, nxt) else {
                continue;
            };
            let node = (nxt, next_used, next_used_len, depth + 1, Some(idx), dir as u8);
...
fn inventory_depart_from_parked_search(
    ...
    let empty4 = inventory_try_depart_empty4_static(base, goal_colors, min_allowed_col);
...
fn inventory_try_launch_direct(
    ...
    let mut eat_counts = Vec::with_capacity(INVENTORY_LAUNCH_NODE_LIMIT.min(8192) + 8);
    nodes.push(InventoryStateNode {
        state: base.state.clone(),
        parent: None,
        mv: 0,
    ...
    let root_gap = goal_colors.len().saturating_sub(lcp_state(&base.state, goal_colors));
...
fn inventory_prepare_build_start(
    ...
    let departed = inventory_depart_from_parked(base, input, goal_colors, timer, min_allowed_col)?;
    if departed.state.len() == 5 {
        return Some(departed);
    }
    inventory_normalize_departed_len5(&departed, goal_colors, timer, min_allowed_col)
        .or(Some(departed))
}
...
fn inventory_transport_candidate_total_rank(
    st: &State,
    seg_idx: usize,
    relative_ops_len: usize,
    protect_len: usize,
) -> (usize, usize, usize, usize, Reverse<usize>, usize) {
    let dist = inventory_transport_candidate_score(st, seg_idx);
    let (row, col) = rc_of(st.head(), st.n);
    (
        relative_ops_len + dist * 4 + st.len().saturating_sub(protect_len) * 6,
        dist,
...
fn solve_inventory_stock(input: &Input, timer: &TimeKeeper) -> BeamState {
    ...
    let try_transport_and_place = |bs: &BeamState,
     ...
     -> Option<BeamState> {
        ...
        candidates.sort_unstable_by_key(|cand| {
            inventory_transport_candidate_total_rank(
                &cand.state,
                seg_idx,
                cand.ops.len(),
                target.len(),
        ...
        launch_pool.sort_unstable_by_key(|cand| {
            (
                ...
                inventory_transport_candidate_total_rank(
                    &cand.state,
                    seg_idx,
                    cand.ops.len().saturating_sub(bs.ops.len()),
                    target.len(),
                ),
            )
        });
    ...
    for seg_idx in 0..stock_cnt {
        ...
        if let Some(prepared) =
            inventory_prepare_build_start(&bs, input, &target, timer, 2 * seg_idx)
        {
            let prep_ops = &prepared.ops[bs.ops.len()..];
            ...
            for mut cand in grown {
                let mut merged = prep_ops.to_vec();
                merged.extend_from_slice(&cand.ops);
                cand.ops = merged;
    ...
    let tail_target = inventory_tail_target(input, stored_cnt, seg_len);
    {
        ...
        if let Some(prepared) =
            inventory_prepare_build_start(&bs, input, &tail_target, timer, 2 * stored_cnt)
        {
            let prep_ops = &prepared.ops[bs.ops.len()..];
            let grown = grow_to_target_prefix_candidates(
                input,
...
fn solve(input: &Input) -> Ops {
    ...
    if is_complete_exact(&best, input) {
        best = optimize_exact_suffix(input, best, &timer);
        best = optimize_exact_suffix_with_simple(input, best, &timer.start);
    } else if timer.exact_remaining_sec() > 0.35 {
        let alt = solve_base(input, &timer);
        if absolute_score_state(&alt.state, &input.d[..input.m], alt.ops.len())
            < absolute_score_state(&best.state, &input.d[..input.m], best.ops.len())
        {
            best = alt;
            if is_complete_exact(&best, input) {
                best = optimize_exact_suffix(input, best, &timer);
...
pub fn solve_from_parts(n: usize, m: usize, goal_colors: &[u8], food_cells: &[u8]) -> Vec<u8> {
    let mut d = [0_u8; MAX_LEN];
    for (dst, &src) in d.iter_mut().zip(goal_colors.iter()) {
        *dst = src;
    }
    let mut food = [0_u8; MAX_CELLS];
    for (dst, &src) in food.iter_mut().zip(food_cells.iter()) {
        *dst = src;
    }
    let input = Input {
        n,
