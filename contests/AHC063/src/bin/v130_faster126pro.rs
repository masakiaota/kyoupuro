...
fn step_impl(
    ...
    if ate != 0 {
        ...
        let tail = ns.pos[old_len - 1];
        ns.pos_occupancy.dec(tail);
        let tail_bias = u8::from(ns.pos[old_len - 2] == nh);
        ...
        return (ns, ate, bite_idx);
    }

    let bite_idx = {
        let bite = ns.pos_occupancy.count(nh) > 0;
        if bite {
            find_internal_bite_idx(&ns.pos)
        } else {
            None
