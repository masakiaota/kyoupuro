...
struct TimeProbe {
    ...
    over_checks: u64,
    first_over_ms: Option<u64>,
    max_overrun_ms: u64,
}

thread_local! {
    static TIME_PROBE: RefCell<TimeProbe> = RefCell::new(TimeProbe::default());
}

#[inline]
fn time_limit_ms() -> u64 {
    (TIME_LIMIT_SEC * 1000.0) as u64
}

fn probe_reset() {
    TIME_PROBE.with(|cell| {
        *cell.borrow_mut() = TimeProbe::default();
    });
}

fn probe_snapshot() -> TimeProbe {
    TIME_PROBE.with(|cell| cell.borrow().clone())
}
...
fn time_over(started: &Instant) -> bool {
    ...
    let over = elapsed.as_secs_f64() >= TIME_LIMIT_SEC;

    TIME_PROBE.with(|cell| {
        let mut probe = cell.borrow_mut();
        probe.checks += 1;

        if let Some(prev_ms) = probe.last_check_ms {
            let gap = elapsed_ms.saturating_sub(prev_ms);
            probe.gap_count += 1;
            probe.gap_sum_ms += gap;
            if gap > probe.gap_max_ms {
                probe.gap_max_ms = gap;
            }
        }
        probe.last_check_ms = Some(elapsed_ms);

        if over {
            probe.over_checks += 1;
            if probe.first_over_ms.is_none() {
                probe.first_over_ms = Some(elapsed_ms);
            }
            let overrun = elapsed_ms.saturating_sub(time_limit_ms());
            if overrun > probe.max_overrun_ms {
                probe.max_overrun_ms = overrun;
            }
        }
    });
...
fn main() {
    ...
    let ans = solve(&input);
    let probe = probe_snapshot();
    let gap_avg_ms = if probe.gap_count == 0 {
        0.0
    } else {
        probe.gap_sum_ms as f64 / probe.gap_count as f64
    };
    let first_over_ms = probe.first_over_ms.map(|x| x as i64).unwrap_or(-1);
    let first_overrun_ms = probe
        .first_over_ms
        .map(|x| x.saturating_sub(time_limit_ms()) as i64)
        .unwrap_or(-1);
    let last_check_ms = probe.last_check_ms.map(|x| x as i64).unwrap_or(-1);

    eprintln!(
        "time_probe checks={} gap_count={} gap_sum_ms={} gap_avg_ms={:.3} gap_max_ms={} over_checks={} first_over_ms={} first_overrun_ms={} max_overrun_ms={} last_check_ms={}",
        probe.checks,
        probe.gap_count,
        probe.gap_sum_ms,
        gap_avg_ms,
