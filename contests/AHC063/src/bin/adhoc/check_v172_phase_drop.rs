    ...
    fn phase_start_turn(input: &Input, phase_seg_idx: usize) -> usize {
        ...
        for seg_idx in 0..phase_seg_idx {
            bs = match inventory_run_segment_phase(&ctx, &bs, seg_idx) {
                ...
                None => panic!("phase {seg_idx} did not complete while reconstructing boundaries"),
            };
        }
        bs.ops.len()
    }
...
fn main() {
    ...
    if args.len() != 4 {
        eprintln!("usage: check_v172_phase_drop <input> <output> <seg_idx>");
        std::process::exit(1);
    }
