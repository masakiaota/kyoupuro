...
struct ProbeStats {
    ...
    ops_len: usize,
    remaining_food: usize,
    quick_safe_called: usize,
    quick_safe_hit: usize,
    quick_rescue_called: usize,
    quick_rescue_hit: usize,
    quick_exact_fallback_called: usize,
    quick_exact_fallback_hit: usize,
    quick_short_stage_inserted: usize,
    rescue_stage_called: usize,
    rescue_stage_hit: usize,
    rescue_stage_returned_total: usize,
    rescue_primary_bestfirst_called: usize,
    rescue_primary_bestfirst_hit: usize,
    rescue_primary_bestfirst_returned_total: usize,
    rescue_primary_exact_called: usize,
    rescue_primary_exact_hit: usize,
    rescue_primary_exact_returned_total: usize,
    rescue_fallback_bestfirst_called: usize,
    rescue_fallback_bestfirst_hit: usize,
    rescue_fallback_bestfirst_returned_total: usize,
    rescue_fallback_exact_called: usize,
    rescue_fallback_exact_hit: usize,
    rescue_fallback_exact_returned_total: usize,
}
...
impl Default for ProbeStats {
    fn default() -> Self {
        Self {
            ...
            quick_safe_called: 0,
            quick_safe_hit: 0,
            quick_rescue_called: 0,
            quick_rescue_hit: 0,
            quick_exact_fallback_called: 0,
            quick_exact_fallback_hit: 0,
            quick_short_stage_inserted: 0,
            rescue_stage_called: 0,
            rescue_stage_hit: 0,
            rescue_stage_returned_total: 0,
            rescue_primary_bestfirst_called: 0,
            rescue_primary_bestfirst_hit: 0,
            rescue_primary_bestfirst_returned_total: 0,
            rescue_primary_exact_called: 0,
            rescue_primary_exact_hit: 0,
            rescue_primary_exact_returned_total: 0,
            rescue_fallback_bestfirst_called: 0,
            rescue_fallback_bestfirst_hit: 0,
            rescue_fallback_bestfirst_returned_total: 0,
            rescue_fallback_exact_called: 0,
            rescue_fallback_exact_hit: 0,
            rescue_fallback_exact_returned_total: 0,
        }
    }
}

thread_local! {
    static PROBE_STATS: RefCell<ProbeStats> = RefCell::new(ProbeStats::default());
}

#[inline]
fn probe_reset() {
    PROBE_STATS.with(|stats| *stats.borrow_mut() = ProbeStats::default());
}

#[inline]
fn probe_update(f: impl FnOnce(&mut ProbeStats)) {
    PROBE_STATS.with(|stats| f(&mut stats.borrow_mut()));
}

#[inline]
fn probe_snapshot() -> ProbeStats {
    PROBE_STATS.with(|stats| stats.borrow().clone())
}
...
fn extend_fastlane_one(
    ...
    probe_update(|stats| stats.quick_safe_called += 1);
    if let Some(sol) = plan_color_goal_quick(bs, input, ell, target_color, safe_cfg, started) {
        probe_update(|stats| stats.quick_safe_hit += 1);
        return Some(sol);
    }
    ...
    probe_update(|stats| stats.quick_rescue_called += 1);
    if let Some(sol) = plan_color_goal_quick(bs, input, ell, target_color, rescue_cfg, started) {
        probe_update(|stats| stats.quick_rescue_hit += 1);
        return Some(sol);
    }

    probe_update(|stats| stats.quick_exact_fallback_called += 1);
    let sols =
    ...
    if out.is_some() {
        probe_update(|stats| stats.quick_exact_fallback_hit += 1);
    }
    out
}
...
fn rescue_stage(
    ...
    probe_update(|stats| stats.rescue_stage_called += 1);
    ...
    for &idx in &order {
        ...
        let mut sols = if endgame_mode {
            probe_update(|stats| stats.rescue_primary_exact_called += 1);
            let sols =
                collect_exact_solutions(bs, input, ell, target_color, MAX_TARGETS_RESCUE, started);
            if !sols.is_empty() {
                probe_update(|stats| {
                    stats.rescue_primary_exact_hit += 1;
                    stats.rescue_primary_exact_returned_total += sols.len();
                });
            }
            sols
        } else {
            probe_update(|stats| stats.rescue_primary_bestfirst_called += 1);
            let sols = stage_search_bestfirst(bs, input, ell, &BUDGETS_RESCUE, STAGE_BEAM, started);
            if !sols.is_empty() {
                probe_update(|stats| {
                    stats.rescue_primary_bestfirst_hit += 1;
                    stats.rescue_primary_bestfirst_returned_total += sols.len();
                });
            }
            sols
        };
        ...
        if sols.is_empty() && !time_over(started) {
            if endgame_mode {
                probe_update(|stats| stats.rescue_fallback_bestfirst_called += 1);
                ...
                if !sols.is_empty() {
                    probe_update(|stats| {
                        stats.rescue_fallback_bestfirst_hit += 1;
                        stats.rescue_fallback_bestfirst_returned_total += sols.len();
                    });
                }
            } else {
                probe_update(|stats| stats.rescue_fallback_exact_called += 1);
                ...
                if !sols.is_empty() {
                    probe_update(|stats| {
                        stats.rescue_fallback_exact_hit += 1;
                        stats.rescue_fallback_exact_returned_total += sols.len();
                    });
                }
            }
        }
    ...
    probe_update(|stats| {
        if !out.is_empty() {
            stats.rescue_stage_hit += 1;
        }
        stats.rescue_stage_returned_total += out.len();
    });
    out
}
...
fn solve_base(input: &Input, started: &Instant) -> BeamState {
    ...
    for ell in 5..input.m {
        ...
        if let Some(sol) = quick_short.clone() {
            probe_update(|stats| stats.quick_short_stage_inserted += 1);
        ...
        if new_map.is_empty() {
            stop_reason = if time_over(started) {
                "time_over"
            } else {
                "new_map_empty"
            };
...
fn solve(input: &Input) -> BeamState {
    ...
    probe_update(|stats| {
        stats.final_ell = best.state.len();
        stats.ops_len = best.ops.len();
        stats.remaining_food = remaining_food_count(&best.state);
    });
    best
}
...
fn main() {
    ...
    let case = env::var("PROBE_CASE").unwrap_or_else(|_| "unknown".to_string());
    let stats = probe_snapshot();
    eprintln!(
        "probe_case_summary case={} stop_reason={} final_ell={} ops_len={} remaining_food={} quick_safe_called={} quick_safe_hit={} quick_rescue_called={} quick_rescue_hit={} quick_exact_fallback_called={} quick_exact_fallback_hit={} quick_short_stage_inserted={} rescue_stage_called={} rescue_stage_hit={} rescue_stage_returned_total={} rescue_primary_bestfirst_called={} rescue_primary_bestfirst_hit={} rescue_primary_bestfirst_returned_total={} rescue_primary_exact_called={} rescue_primary_exact_hit={} rescue_primary_exact_returned_total={} rescue_fallback_bestfirst_called={} rescue_fallback_bestfirst_hit={} rescue_fallback_bestfirst_returned_total={} rescue_fallback_exact_called={} rescue_fallback_exact_hit={} rescue_fallback_exact_returned_total={}",
        case,
        stats.stop_reason,
        stats.final_ell,
        stats.ops_len,
        stats.remaining_food,
        stats.quick_safe_called,
        stats.quick_safe_hit,
        stats.quick_rescue_called,
        stats.quick_rescue_hit,
        stats.quick_exact_fallback_called,
        stats.quick_exact_fallback_hit,
        stats.quick_short_stage_inserted,
        stats.rescue_stage_called,
        stats.rescue_stage_hit,
        stats.rescue_stage_returned_total,
        stats.rescue_primary_bestfirst_called,
        stats.rescue_primary_bestfirst_hit,
        stats.rescue_primary_bestfirst_returned_total,
        stats.rescue_primary_exact_called,
        stats.rescue_primary_exact_hit,
        stats.rescue_primary_exact_returned_total,
        stats.rescue_fallback_bestfirst_called,
        stats.rescue_fallback_bestfirst_hit,
        stats.rescue_fallback_bestfirst_returned_total,
        stats.rescue_fallback_exact_called,
        stats.rescue_fallback_exact_hit,
        stats.rescue_fallback_exact_returned_total,
    );
}
