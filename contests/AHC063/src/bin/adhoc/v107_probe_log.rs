...
fn probe_state_line(tag: &str, idx: usize, st: &State, input: &Input, ell: usize, target_color: u8, ops_len: usize) {
    let head = st.head();
    let (hr, hc) = rc_of(head, st.n);
    let l = lcp(&st.colors, &input.d);
    let (nearest_target_dist, target_cnt) = nearest_food_dist(st, target_color);
    let mut legal = Vec::new();
    for dir in legal_dirs(st) {
        let nh = next_head_cell(st, dir).unwrap();
        legal.push(format!("{}:{}", DIRS[dir].2, st.food[nh as usize]));
    }
    let dist_text = if nearest_target_dist >= 1_000_000_000 {
        "-1".to_owned()
    } else {
        nearest_target_dist.to_string()
    };
    eprintln!(
        "{tag} idx={idx} ell={ell} lcp={l} len={} ops_len={} head=({}, {}) target_color={} target_cnt={} nearest_target_dist={} legal={}",
        st.colors.len(),
        ops_len,
        hr,
        hc,
...
fn solve(input: &Input) -> String {
    ...
    for ell in 5..input.m {
        if time_over(&started) {
            eprintln!("probe_stop reason=time_over ell={ell} beam_size={}", beam.len());
            break;
        }

        let target_color = input.d[ell];
        eprintln!(
            "probe_stage_start ell={ell} target_color={} beam_in={}",
            target_color,
            beam.len()
        );
        ...
        let mut stage_solutions_total = 0usize;
        let mut exact_solutions_total = 0usize;
        let mut bs_no_solution = 0usize;
        let mut bs_processed = 0usize;
        let mut rescue_added = 0usize;
        let mut rescue_called = false;
        ...
        for bs in &beam {
            ...
            if !time_over(&started) {
                ...
                stage_solutions_total += sols.len();
            }
            ...
            if sols.is_empty() && !time_over(&started) {
                ...
                for target in targets {
                    ...
                    exact_solutions_total += sols.len() - before;
                    if sols.len() >= STAGE_BEAM {
                        break;
                    }
                }
        ...
        eprintln!(
            "probe_stage_end ell={ell} beam_processed={} bs_no_solution={} stage_solutions_total={} exact_solutions_total={} rescue_called={} rescue_added={} new_map_size={}",
            bs_processed,
            bs_no_solution,
            stage_solutions_total,
            exact_solutions_total,
            rescue_called,
            rescue_added,
            new_map.len()
        );
        ...
        if new_map.is_empty() {
            eprintln!(
                "probe_stop reason=new_map_empty ell={ell} target_color={} beam_size={}",
                target_color,
                beam.len()
            );
            for (idx, bs) in beam.iter().enumerate() {
                probe_state_line("probe_stop_state", idx, &bs.state, input, ell, target_color, bs.ops.len());
            }
            break;
        }
        ...
        eprintln!("probe_stage_keep ell={ell} beam_out={}", new_beam.len());
        beam = new_beam;
    }
