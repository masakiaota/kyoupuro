// probe_v172_departure_case.rs

mod solver {
    #![allow(dead_code)]
    include!("../v172_safe_fallback.rs");
    ...
    pub fn probe(input_path: &str, output_path: &str, seg_idx: usize) {
        ...
        for done in 0..seg_idx {
            bs = match inventory_run_segment_phase(&ctx, &bs, done) {
                ...
                None => panic!("failed before seg_idx={seg_idx}, done={done}"),
            };
        }
        ...
        let safe_current = inventory_try_safe_current_build_fallback(
            &ctx,
            &bs,
            &prepared,
            &InventoryTarget {
        ...
        if let Some(safe_current) = safe_current {
            let sc_len = safe_current.ops.len().saturating_sub(bs.ops.len()).min(20);
            let sc_expected: String = safe_current.ops[bs.ops.len()..(bs.ops.len() + sc_len)]
                .iter()
                .map(|&d| DIR_CHARS[d as usize])
                .collect();
            println!("safe_current_prefix expected={}", sc_expected);
        } else {
            println!("safe_current=None");
        }
...
fn main() {
    ...
    if args.len() != 4 {
        eprintln!("usage: probe_v172_departure_case <input> <output> <seg_idx>");
        std::process::exit(1);
    }
