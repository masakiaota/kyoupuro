...
mod solver {
    ...
    fn placed_turn_boundaries(input: &Input) -> Vec<usize> {
        ...
        for seg_idx in 0..stock_cnt {
            bs = inventory_run_segment_phase(&ctx, &bs, seg_idx).unwrap();
            out.push(bs.ops.len());
        }
        out
    }
    ...
    pub fn analyze(input_path: &str, output_path: &str, phase_seg_idx: usize) {
        ...
        let phase_start = if phase_seg_idx == 0 {
            0
        } else {
            placed_turns[phase_seg_idx - 1]
        };
        let phase_end = placed_turns.get(phase_seg_idx).copied().unwrap_or(usize::MAX);
        ...
        for (turn, line) in output.lines().enumerate() {
            ...
            let in_phase = phase_start < turn + 1 && turn + 1 <= phase_end;
            if in_phase {
        ...
        println!("phase_start={} phase_end={}", phase_start, phase_end);
    }
}
...
fn main() {
    ...
    if args.len() != 4 {
        eprintln!("usage: check_v145_phase_drop <input> <output> <seg_idx>");
        std::process::exit(1);
    }
