...
mod v169 {
    #![allow(dead_code)]
    include!("../v169_static_branch_gates.rs");
...
fn main() {
    ...
    if args.len() != 2 {
        eprintln!("usage: probe_v167_v168_boundaries <input>");
        std::process::exit(1);
    }
    let input = &args[1];
    let (a, rem_a, tail_ok_a) = v167::phase_states(input);
    let (b, rem_b, tail_ok_b) = v169::phase_states(input);
    println!(
        "v167 {:?}",
    ...
    println!("v167 rem={:.6} tail_ok={}", rem_a, tail_ok_a);
    println!("v169 rem={:.6} tail_ok={}", rem_b, tail_ok_b);
