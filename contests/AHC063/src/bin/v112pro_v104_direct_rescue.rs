...
const DIRECT_FALLBACK_DEPTH: usize = 12;
const DIRECT_FALLBACK_NODES: usize = 3_000;
const DIRECT_RESCUE_DEPTH: usize = 20;
const DIRECT_RESCUE_NODES: usize = 20_000;
...
fn safe_capture_exact_solutions(
    ...
    while let Some(idx) = q.pop_front() {
        if nodes >= max_nodes || time_over(started) {
            break;
        }
        nodes += 1;
...
fn solve(input: &Input) -> String {
    ...
    for ell in 5..input.m {
        ...
        for bs in &beam {
            ...
            if sols.is_empty() && exact_prefix(&bs.state, &input.d, ell) && !time_over(&started) {
                let direct = safe_capture_exact_solutions(
                    bs,
                    input,
                    ell,
                    DIRECT_FALLBACK_DEPTH,
                    DIRECT_FALLBACK_NODES,
                    2,
                    &started,
                );
                sols.extend(direct);
