...
const INVENTORY_RESCUE_BITE_LIMIT: u8 = 2;
const INVENTORY_ORACLE_DEPTH_LIMIT: usize = 8;
const INVENTORY_ORACLE_NODE_LIMIT: usize = 12_000;
...
fn inventory_try_depart_empty4_static(
    ...
    while let Some(idx) = q.pop_front() {
        ...
        next_vec.sort_unstable_by_key(|&nxt| {
            let (r, c) = rc_of(nxt, n);
            (
                usize::from(st.food[nxt as usize] != 0),
                usize::MAX - c,
                r,
            )
        });
...
fn inventory_try_launch_direct(
    ...
    pq.push(Reverse(((root_residue, root_gap, 0usize, 0usize, 0usize), uid, 0usize)));
    uid += 1;
    ...
    while let Some(Reverse((_, _, idx))) = pq.pop() {
        ...
        let cur = nodes[idx].state.clone();
        if inventory_is_launch_ready(&cur, min_allowed_col) {
            let gap = goal_colors.len().saturating_sub(lcp_state(&cur, goal_colors));
            let key = (
                gap,
        ...
        for &dir_u8 in legal_dirs(&cur).as_slice() {
            ...
            if seen
                .get(&ns)
                .is_some_and(|&(best_depth, best_eat)| best_depth <= nd as u8 && best_eat <= next_eat)
            {
                continue;
            }
            seen.insert(ns.clone(), (nd as u8, next_eat));
            let residue = inventory_depart_bad_count(&ns, min_allowed_col);
            let gap = goal_colors.len().saturating_sub(lcp_state(&ns, goal_colors));
            let child = nodes.len();
            nodes.push(InventoryStateNode {
                state: ns.clone(),
                parent: Some(idx),
...
fn inventory_build_target_exact_legacy_parked(
    ...
    let built = if exact_prefix(&grown.state, target.colors, target.protect_len) {
        ...
        let gap = target.protect_len.saturating_sub(lcp_state(&grown.state, target.colors));
        if gap >= 6 {
...
fn inventory_try_direct_build_from_current(
    ...
    match inventory_build_target_exact(ctx, base, target, phases_left) {
...
#[inline]
fn inventory_harvest_entry_goal(n: usize, stock_cnt: usize) -> Option<Cell> {
...
fn inventory_ready_oracle_shallow_plan(
    ...
    if let Some(plan) =
        inventory_transport_oracle_direct_plan(st, protect_colors, protect_len, geom)
    {
        return Some(plan);
    }
...
fn inventory_ready_rescue_dfs(
    ...
    if try_commit_inventory_entry_and_validate_place(st, protect_colors, protect_len, geom)
        .is_some()
    {
        return true;
    }
    ...
    let mut cands: Vec<(usize, usize, usize, u8, State, u8, u8)> = Vec::with_capacity(4);
    for &dir_u8 in legal_dirs(st).as_slice() {
        let dir = dir_u8 as usize;
        let Some((sim, ate, bit)) =
            inventory_transport_step_state(st, dir, protect_colors, protect_len)
        else {
            continue;
        };
        if inventory_is_entry_cell(geom, sim.head()) {
            continue;
        }
        ...
        let next_bite = bite_count.saturating_add(u8::from(bit));
        if next_bite > INVENTORY_RESCUE_BITE_LIMIT {
            continue;
        }
        ...
        let order_key = goal_dist + next_eat as usize * 3 + next_bite as usize;
        cands.push((
            order_key,
            usize::from(ate != 0),
    ...
    cands.sort_unstable_by_key(|&(k1, k2, k3, _, _, _, _)| (k1, k2, k3));

    for (_, _, _, dir_u8, sim, next_eat, next_bite) in cands {
        path.push(dir_u8);
...
fn inventory_transport_to_goal_oracle_plan(
    ...
    for win in slice.windows(2) {
        ...
        let Some((ns, _, _)) = inventory_transport_step_state(&cur, dir, protect_colors, protect_len)
        else {
            return None;
        };
        cur = ns;
...
fn inventory_transport_to_goal_plan_with_limits(
    ...
    if let Some(plan) = inventory_transport_to_goal_oracle_plan(st, goal, protect_colors, protect_len)
    {
        return Some(plan);
    }
    ...
    while let Some(Reverse((_, _, idx))) = pq.pop() {
        ...
        let mut cands: Vec<((usize, usize, usize, usize, usize, usize), State, u8, u8, u8)> =
            Vec::with_capacity(4);
        for &dir_u8 in legal_dirs(&cur).as_slice() {
            let Some((ns, ate, bit)) =
                inventory_transport_step_state(&cur, dir_u8 as usize, protect_colors, protect_len)
...
fn choose_inventory_entry_ready_dir_loose(
    ...
    for &(avoid_food, allow_eat) in &[(true, false), (false, false), (false, true)] {
            ...
            bfs_next_dir_to_any_goal_with_blocked(st, goals, avoid_food, false, &blocked)
        {
            ...
            if (allow_eat || ate == 0) && !inventory_is_entry_cell(geom, ns.head()) {
                return Some(dir);
            }
        }
    }
    ...
    for &allow_eat in &[false, true] {
        ...
        for &dir_u8 in legal_dirs(st).as_slice() {
            ...
            let key = (
                ...
                4usize.saturating_sub(legal_dir_count(&ns)),
                ns.len().saturating_sub(protect_len),
                usize::from(ate != 0) + usize::from(bit),
            );
            if best.as_ref().map_or(true, |(best_key, _)| key < *best_key) {
                best = Some((key, dir));
            }
...
fn transport_to_entry_from_right(
    ...
    let plan = inventory_transport_plan(
        &bs.state,
        input,
        protect_colors,
        protect_len,
...
fn inventory_run_segment_phase(
    ...
    let split_built = if base.state.len() == 5 && seg_idx > 0 {
        if let Some(next_bs) = inventory_try_direct_build_from_current(ctx, base, &target, phases_left)
        {
            next_bs
        } else {
        ...
        if exact_prefix(&prepared.state, target.colors, target.protect_len) {
            ...
            match inventory_build_target_exact(ctx, &prepared, &target, phases_left) {
                ...
                _ => {
                    if base.state.len() == 5 {
                        match inventory_build_target_exact_legacy_parked(ctx, base, &target, phases_left) {
                            InventoryBuildOutcome::Built(next_bs) => next_bs,
                            InventoryBuildOutcome::GrowFailed => {
                                match inventory_try_transportable_build_from_parked(
                                    ctx,
...
fn inventory_build_tail_exact(
    ...
    let prepared = match inventory_prepare_build_start(ctx, base, target.colors, 1, 2 * ctx.stock_cnt)
    {
        Some(next_bs) => next_bs,
        None => {
...
fn inventory_finish_harvest(
    ...
    if ctx.stock_cnt > 0 {
        ...
        let goal = cell_of(ctx.input.n - 3, 2 * ctx.stock_cnt - 1, ctx.input.n);
        bs = match transport_to_cell_preserving_prefix(
            &bs,
            ctx.input,
            tail_colors,
            tail_colors.len(),
