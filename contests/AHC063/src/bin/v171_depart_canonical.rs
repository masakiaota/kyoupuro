...
#[derive(Clone)]
struct InventoryPhaseBudget {
    phase_start_remaining_sec: f64,
}

thread_local! {
    static ACTIVE_INVENTORY_PHASE_BUDGET: RefCell<Option<InventoryPhaseBudget>> = const { RefCell::new(None) };
}
...
#[inline]
fn push_inventory_phase_budget(
    timer: &TimeKeeper,
    _local_limit_sec: f64,
    _min_remaining_sec: f64,
) -> ScopedInventoryPhaseBudget {
    let next = InventoryPhaseBudget {
        phase_start_remaining_sec: timer.exact_remaining_sec(),
    };
    let prev = ACTIVE_INVENTORY_PHASE_BUDGET.with(|slot| {
        let prev = slot.borrow().clone();
        *slot.borrow_mut() = Some(next);
        prev
    });
    ScopedInventoryPhaseBudget { prev }
}

#[inline]
fn current_inventory_phase_left() -> Option<f64> {
    ACTIVE_INVENTORY_PHASE_BUDGET.with(|slot| {
        slot.borrow()
            .as_ref()
            .map(|budget| budget.phase_start_remaining_sec)
    })
}

#[inline]
fn inventory_exact_remaining_sec(timer: &TimeKeeper) -> f64 {
    current_inventory_phase_left().unwrap_or_else(|| timer.exact_remaining_sec())
}

#[inline]
fn inventory_time_left(started: &Instant) -> f64 {
    let global_left = current_time_left(started);
    if global_left < INVENTORY_EMERGENCY_LEFT_SEC {
        0.0
    } else {
        current_inventory_phase_left().unwrap_or(global_left)
    }
}

#[inline]
fn inventory_time_over(started: &Instant) -> bool {
    inventory_time_left(started) <= 0.0
}
...
struct ScopedInventoryPhaseBudget {
    prev: Option<InventoryPhaseBudget>,
}
...
impl Drop for ScopedInventoryPhaseBudget {
    fn drop(&mut self) {
        ACTIVE_INVENTORY_PHASE_BUDGET.with(|slot| {
            *slot.borrow_mut() = self.prev.take();
        });
...
const INVENTORY_EMERGENCY_LEFT_SEC: f64 = 0.01;
...
const INVENTORY_PHASE_ROLLBACK_MIN_SEC: f64 = 0.22;
const INVENTORY_PHASE_ROLLBACK_EARLY_ONLY_SEC: f64 = 0.5;
...
const INVENTORY_DEPART_CANONICAL_DEPTH_LIMIT: usize = 14;
const INVENTORY_DEPART_CANONICAL_NODE_LIMIT: usize = 12_000;
...
enum InventoryPhaseOutcome {
    Completed(BeamState),
    BuiltOnly(BeamState),
    PreparedOnly(BeamState),
    Failed,
}
...
fn inventory_build_time_limit(timer: &TimeKeeper, phases_left: usize) -> f64 {
    let free = (inventory_exact_remaining_sec(timer) - INVENTORY_MIN_LEFT_SEC).max(0.0);
...
fn inventory_depart_from_parked_search(
    ...
    while let Some(Reverse((_, depth, _, idx))) = pq.pop() {
        if expansions >= node_limit
            || inventory_time_over(&timer.start)
            || inventory_exact_remaining_sec(timer) < INVENTORY_MIN_LEFT_SEC
        {
            break;
        }
        expansions += 1;
...
#[inline(always)]
fn inventory_canonical_pose_rank(st: &State, min_allowed_col: usize) -> (usize, usize, usize, usize, usize, usize) {
    let n = st.n;
    let pref_c = (min_allowed_col + 1).min(n - 1);
    let mut above_bottom2 = 0usize;
    let mut right_spill = 0usize;
    let mut sum_row_dist = 0usize;
    let mut sum_col_dist = 0usize;
    let mut min_c = n;
    let mut max_c = 0usize;
    let mut min_r = n;
    let mut max_r = 0usize;
    for idx in 0..st.len() {
        let (r, c) = rc_of(st.pos[idx], n);
        if r + 2 < n {
            above_bottom2 += 1;
        }
        right_spill += c.saturating_sub(min_allowed_col + 2);
        sum_row_dist += (n - 1).saturating_sub(r);
        sum_col_dist += c.abs_diff(pref_c);
        min_c = min_c.min(c);
        max_c = max_c.max(c);
        min_r = min_r.min(r);
        max_r = max_r.max(r);
    }
    let (hr, hc) = rc_of(st.head(), n);
    let bbox_area = (max_r - min_r + 1) * (max_c - min_c + 1);
    (
        above_bottom2,
        right_spill,
        hr.abs_diff(n - 2) + hc.abs_diff(pref_c),
        bbox_area,
        sum_row_dist + sum_col_dist,
        4usize.saturating_sub(legal_dir_count(st)),
    )
}

#[inline(always)]
fn inventory_is_good_depart_pose(st: &State, min_allowed_col: usize) -> bool {
    let key = inventory_canonical_pose_rank(st, min_allowed_col);
    key.0 == 0 && key.1 == 0 && key.2 <= 2 && key.3 <= 6
}

fn inventory_canonicalize_launch_ready_len5(
    base: &BeamState,
    goal_colors: &[u8],
    timer: &TimeKeeper,
    min_allowed_col: usize,
) -> Option<BeamState> {
    if base.state.len() != 5 || !inventory_is_launch_ready(&base.state, min_allowed_col) {
        return None;
    }

    let mut nodes = Vec::with_capacity(INVENTORY_DEPART_CANONICAL_NODE_LIMIT.min(4096) + 8);
    let mut depths = Vec::with_capacity(INVENTORY_DEPART_CANONICAL_NODE_LIMIT.min(4096) + 8);
    nodes.push(InventoryStateNode {
        state: base.state.clone(),
        parent: None,
        mv: 0,
    ...
    let mut uid = 0usize;
    let mut pq = BinaryHeap::new();
    let root_key = inventory_canonical_pose_rank(&base.state, min_allowed_col);
    if inventory_is_good_depart_pose(&base.state, min_allowed_col) {
        return Some(base.clone());
    }
    pq.push(Reverse((root_key, 0usize, uid, 0usize)));
    uid += 1;

    let mut seen = FxHashMap::<State, u8>::default();
    seen.insert(base.state.clone(), 0u8);
    let mut best_idx = 0usize;
    let mut best_key = root_key;
    let mut expansions = 0usize;

    while let Some(Reverse((_, depth, _, idx))) = pq.pop() {
        if expansions >= INVENTORY_DEPART_CANONICAL_NODE_LIMIT
            || inventory_time_over(&timer.start)
            || inventory_exact_remaining_sec(timer) < INVENTORY_MIN_LEFT_SEC
        {
            break;
        ...
        let cur = nodes[idx].state.clone();
        let cur_key = inventory_canonical_pose_rank(&cur, min_allowed_col);
        if cur_key < best_key {
            best_key = cur_key;
            best_idx = idx;
        }
        if depth >= INVENTORY_DEPART_CANONICAL_DEPTH_LIMIT {
            continue;
        }

        let mut cands = Vec::with_capacity(4);
        for &dir_u8 in legal_dirs(&cur).as_slice() {
            let (ns, ate, bite_idx) = step(&cur, dir_u8 as usize);
            if ate != 0
                || bite_idx.is_some()
                || ns.len() != 5
                || !inventory_launch_prefix_ok(&ns, goal_colors)
            ...
            let nd = depth + 1;
            if seen.get(&ns).is_some_and(|&best_depth| best_depth <= nd as u8) {
                continue;
            }
            seen.insert(ns.clone(), nd as u8);
            cands.push((inventory_canonical_pose_rank(&ns, min_allowed_col), ns, dir_u8));
        }
        cands.sort_unstable_by_key(|(key, _, _)| *key);
        for (key, ns, dir_u8) in cands {
            let child = nodes.len();
            nodes.push(InventoryStateNode {
                state: ns,
                parent: Some(idx),
    ...
    if best_idx == 0 || !(best_key < root_key) {
        return Some(base.clone());
    }
    let mut ops = base.ops.clone();
    ops.extend_from_slice(&reconstruct_inventory_state_path(&nodes, best_idx));
    if ops.len() > MAX_TURNS {
        return None;
    }
    Some(BeamState {
        state: nodes[best_idx].state.clone(),
        ops,
    })
}
...
fn inventory_prepare_build_start(
    ...
    let needs_len5_normalize = departed.state.len() != 5;
    let normalized = if !needs_len5_normalize {
        departed
    } else {
        inventory_normalize_departed_len5(&departed, goal_colors, ctx.timer, min_allowed_col)
            .unwrap_or(departed)
    };
    if normalized.state.len() == 5
        && inventory_is_launch_ready(&normalized.state, min_allowed_col)
        && (needs_len5_normalize || !inventory_is_good_depart_pose(&normalized.state, min_allowed_col))
    {
        inventory_canonicalize_launch_ready_len5(
            &normalized,
            goal_colors,
            ctx.timer,
            min_allowed_col,
...
fn inventory_build_target_exact_legacy_current(
    ctx: &InventoryCtx<'_>,
    base: &BeamState,
    target: &InventoryTarget<'_>,
    phases_left: usize,
...
fn inventory_try_direct_build_from_current(
    ...
    let build_limit = inventory_build_time_limit(ctx.timer, phases_left)
        ...
        .min((inventory_exact_remaining_sec(ctx.timer) - INVENTORY_MIN_LEFT_SEC).max(0.0));
    if build_limit <= 0.0 {
        return None;
    }
    let grown = grow_to_target_prefix(
...
fn inventory_try_transportable_build_from_current(
    ctx: &InventoryCtx<'_>,
    base: &BeamState,
    target: &InventoryTarget<'_>,
    phases_left: usize,
...
fn inventory_candidate_respects_active_constraint(
    base: &BeamState,
    candidate: &BeamState,
) -> bool {
    if candidate.ops.len() < base.ops.len() {
        return false;
    }
    let suffix = &candidate.ops.as_slice()[base.ops.len()..];
    let mut st = base.state.clone();
    let mut dropped = DroppedBuf::new();
    for &dir_u8 in suffix {
        let (ns, _, bite_idx) = step_with_dropped(&st, dir_u8 as usize, &mut dropped);
        if bite_idx.is_some() && !dropped_respects_active_constraint(st.n, &dropped) {
            return false;
        }
        st = ns;
...
fn inventory_try_safe_parked_build_fallback(
    ctx: &InventoryCtx<'_>,
    base: &BeamState,
    target: &InventoryTarget<'_>,
    phases_left: usize,
    ...
    match inventory_build_target_exact_legacy_current(ctx, base, target, phases_left) {
        InventoryBuildOutcome::Built(next_bs)
            if inventory_candidate_respects_active_constraint(base, &next_bs) =>
        {
            return Some(next_bs);
        }
        _ => {}
    }
    inventory_try_transportable_build_from_current(ctx, base, target, phases_left, seg_idx)
        .filter(|candidate| inventory_candidate_respects_active_constraint(base, candidate))
}

fn inventory_push_unique_candidate(
    out: &mut Vec<BeamState>,
    seen: &mut FxHashSet<State>,
    candidate: Option<BeamState>,
) {
    let Some(candidate) = candidate else {
        return;
    };
    if seen.insert(candidate.state.clone()) {
        out.push(candidate);
    }
}

fn inventory_collect_transport_retry_candidates(
    ctx: &InventoryCtx<'_>,
    base: &BeamState,
    prepared_for_rescue: Option<&BeamState>,
    target: &InventoryTarget<'_>,
    phases_left: usize,
    seg_idx: usize,
    primary: &BeamState,
    ...
    seen.insert(primary.state.clone());

    let prepared_owned = match prepared_for_rescue.cloned() {
        Some(prepared) => Some(prepared),
        None if base.state.len() == 5 && seg_idx > 0 => inventory_prepare_build_start(
            ctx,
            base,
            target.colors,
            phases_left,
    ...
    if let Some(prepared) = prepared_owned.as_ref() {
        if exact_prefix(&prepared.state, target.colors, target.protect_len) {
            inventory_push_unique_candidate(&mut out, &mut seen, Some(prepared.clone()));
        }
        inventory_push_unique_candidate(
            &mut out,
            &mut seen,
        ...
        inventory_push_unique_candidate(
            &mut out,
            &mut seen,
            match inventory_build_target_exact_legacy_current(ctx, prepared, target, phases_left) {
                InventoryBuildOutcome::Built(next_bs) => Some(next_bs),
                _ => None,
            },
        );
        inventory_push_unique_candidate(
            &mut out,
            &mut seen,
            inventory_try_transportable_build_from_current(
                ctx,
                prepared,
                target,
                phases_left,
    ...
    inventory_push_unique_candidate(
        &mut out,
        &mut seen,
        inventory_try_safe_parked_build_fallback(ctx, base, target, phases_left, seg_idx),
    );
...
#[inline]
fn inventory_should_try_phase_rollback(ctx: &InventoryCtx<'_>, _phases_left: usize) -> bool {
    inventory_exact_remaining_sec(ctx.timer)
        >= INVENTORY_PHASE_ROLLBACK_EARLY_ONLY_SEC.max(
            INVENTORY_MIN_LEFT_SEC + INVENTORY_PHASE_ROLLBACK_MIN_SEC,
        )
}
    ...
    F: FnMut(&BeamState) -> bool,
{
    ...
    for idx in eval_list {
        if inventory_time_over(&timer.start)
            || inventory_exact_remaining_sec(timer) < INVENTORY_MIN_LEFT_SEC
        {
            break;
        }
...
fn inventory_transport_plan_with_limits(
    ...
    while let Some(Reverse((_, _, idx))) = pq.pop() {
        if expansions >= node_limit
            || inventory_time_over(started)
            || inventory_time_left(started) < INVENTORY_RESCUE_MIN_LEFT_SEC
        {
            break;
        }
        expansions += 1;
...
fn transport_to_cell_preserving_prefix(
    ...
    while st.head() != goal {
        if inventory_time_over(started) || inventory_time_left(started) < INVENTORY_MIN_LEFT_SEC {
            return None;
        }
        guard += 1;
        if guard > st.n * st.n * 80 {
...
fn harvest_inventory(bs: &BeamState, input: &Input, stock_cnt: usize) -> Option<BeamState> {
    harvest_inventory_impl(bs, input, stock_cnt, true)
}
...
fn inventory_finish_harvest_from_state(
    ...
    let out = harvest_inventory_impl(&start, input, stock_cnt, false)?;
...
fn inventory_run_segment_phase(
    ...
    let (split_built, prepared_for_rescue) = if base.state.len() == 5 && seg_idx > 0 {
        let prepared = match inventory_prepare_build_start(
            ...
            None => {
                eprintln!("inventory_fail launch seg={seg_idx}");
                return InventoryPhaseOutcome::Failed;
            }
        };

        if exact_prefix(&prepared.state, target.colors, target.protect_len) {
            (prepared.clone(), Some(prepared))
        } else if let Some(next_bs) =
            inventory_try_direct_build_from_current(ctx, &prepared, &target, phases_left)
        {
            (next_bs, Some(prepared))
        } else {
            match inventory_build_target_exact(ctx, &prepared, &target, phases_left) {
                InventoryBuildOutcome::Built(next_bs) => (next_bs, Some(prepared.clone())),
                InventoryBuildOutcome::GrowFailed => {
                    match inventory_build_target_exact_legacy_current(
                        ctx,
                        &prepared,
                        &target,
                        phases_left,
                        ...
                        InventoryBuildOutcome::Built(next_bs) => (next_bs, Some(prepared.clone())),
                        InventoryBuildOutcome::GrowFailed => {
                            match inventory_try_transportable_build_from_current(
                                ctx,
                                &prepared,
                                &target,
                                phases_left,
                                ...
                                Some(next_bs) => (next_bs, Some(prepared.clone())),
                                None => {
                                    if let Some(next_bs) = inventory_try_safe_parked_build_fallback(
                                        ctx,
                                        base,
                                        &target,
                                        phases_left,
                                        ...
                                        return InventoryPhaseOutcome::PreparedOnly(prepared);
                                    }
                                }
                            }
                        }
                            ...
                            match inventory_try_transportable_build_from_current(
                                ...
                                None => {
                                    if let Some(next_bs) = inventory_try_safe_parked_build_fallback(
                                        ...
                                        eprintln!("inventory_fail build_exact seg={seg_idx}");
                                        return InventoryPhaseOutcome::PreparedOnly(prepared);
                                    }
                                }
                            }
    ...
    let built_only_bs = split_built.clone();
    ...
    bs = match transport_to_entry_from_right(
        ...
        Some(next_bs) => next_bs,
        None => {
            if inventory_should_try_phase_rollback(ctx, phases_left) {
                let retry_candidates = inventory_collect_transport_retry_candidates(
                    ctx,
                    base,
                ...
                for alt_built in retry_candidates {
                    if let Some(next_bs) = transport_to_entry_from_right(
                        ...
                        recovered = Some(next_bs);
                        break;
                    }
                }
                if let Some(next_bs) = recovered {
                    next_bs
                } else {
                    eprintln!("inventory_fail transport seg={seg_idx}");
                    return InventoryPhaseOutcome::BuiltOnly(built_only_bs);
                }
            } else {
                eprintln!("inventory_fail transport seg={seg_idx}");
                return InventoryPhaseOutcome::BuiltOnly(built_only_bs);
    ...
    bs = match place_inventory_segment(&bs, seg_idx, ctx.input.n, target.colors, target.protect_len)
    {
        Some(next_bs) => next_bs,
        None => {
            eprintln!("inventory_fail place seg={seg_idx}");
            return InventoryPhaseOutcome::BuiltOnly(built_only_bs);
        }
    };
    eprintln!("inventory_ok placed seg={seg_idx}");
    InventoryPhaseOutcome::Completed(bs)
}
...
fn inventory_build_tail_exact(
    ...
    let prepared =
        match inventory_prepare_build_start(ctx, base, target.colors, 1, 2 * ctx.stock_cnt) {
            Some(next_bs) => next_bs,
            None => {
                eprintln!("inventory_fail launch_tail");
                return None;
            }
        };
    ...
    let next_bs = if exact_prefix(&prepared.state, target.colors, target.protect_len) {
        ...
        match inventory_build_target_exact(ctx, &prepared, &target, 1) {
            ...
            InventoryBuildOutcome::GrowFailed => {
                match inventory_build_target_exact_legacy_current(ctx, &prepared, &target, 1) {
                    InventoryBuildOutcome::Built(next_bs) => next_bs,
                    InventoryBuildOutcome::GrowFailed => {
                    ...
                    InventoryBuildOutcome::ExactFailed => {
                        eprintln!("inventory_fail build_tail_exact");
                        return None;
                    }
                }
            }
    ...
    eprintln!("inventory_ok tail_ready");
...
fn inventory_finish_harvest(
    ...
    let _phase_budget = push_inventory_phase_budget(ctx.timer, 0.0, 0.0);
    ...
    if ctx.stock_cnt > 0 {
        bs = match inventory_transport_harvest_entry(
            ...
            Some(next_bs) => next_bs,
            None => {
                eprintln!("inventory_fail transport_harvest_entry");
                return None;
            }
        };
    }
    ...
    if ctx.stock_cnt == 0 {
        eprintln!("inventory_ok no_stock_harvest");
        return Some(bs);
    }
    ...
    eprintln!("inventory_ok harvest_done");
    Some(next_bs)
}
