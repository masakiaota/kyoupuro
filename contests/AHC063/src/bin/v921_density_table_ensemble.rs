...
fn run_sequence(input: &CommonInput, order: &[SolverKind]) -> Candidate {
    ...
    let first = iter.next().unwrap_or(SolverKind::V139);
    ...
    for solver in iter {
        ...
        let eligible = match solver {
            ...
            SolverKind::V139 | SolverKind::V149 => cand.finished,
        };
        if eligible {
...
mod v139_impl {
    ...
    const SUFFIX_OPT_MIN_LEFT_SEC: f64 = 0.14;
...
mod v149_impl {
    ...
    const EMPTY_PATH_REMAINING_LIMIT: usize = 12;
    const EMPTY_PATH_MIN_LEFT_SEC: f64 = 0.10;
    const FASTLANE_MIN_LEFT_SEC: f64 = 0.22;
    const FAST_SAFE_DEPTH_LIMIT: u8 = 10;
    const FAST_SAFE_NODE_LIMIT: usize = 2_500;
    const FAST_RESCUE_DEPTH_LIMIT: u8 = 16;
    const FAST_RESCUE_NODE_LIMIT: usize = 9_000;
    ...
    const INVENTORY_MIN_LEFT_SEC: f64 = 0.14;
    ...
    fn inventory_try_launch_direct(
        ...
        while let Some(Reverse((_, _, idx))) = pq.pop() {
            ...
            let depth = depths[idx] as usize;
            if inventory_is_launch_ready(&nodes[idx].state, min_allowed_col) {
                let gap = goal_colors
                    .len()
                    .saturating_sub(lcp_state(&nodes[idx].state, goal_colors));
                let key = (
                    gap,
                    4usize.saturating_sub(legal_dir_count(&nodes[idx].state)),
            ...
            let dirs = legal_dirs(&nodes[idx].state);
            for &dir_u8 in dirs.as_slice() {
                let dir = dir_u8 as usize;
                let (ns, ate, bite_idx) = step(&nodes[idx].state, dir);
                if bite_idx.is_some()
                ...
                let mobility = 4usize.saturating_sub(legal_dir_count(&ns));
                let child = nodes.len();
                nodes.push(InventoryStateNode {
                    state: ns,
                    parent: Some(idx),
        ...
        F: FnMut(&BeamState) -> bool,
    {
        ...
        while let Some(idx) = q.pop_front() {
            ...
            let dirs = legal_dirs(&nodes[idx].state);
            let cur_n = nodes[idx].state.n;
            for &dir_u8 in dirs.as_slice() {
                let dir = dir_u8 as usize;
                let mut dropped = DroppedBuf::new();
                let (ns, _, bite_idx) = step_with_dropped(&nodes[idx].state, dir, &mut dropped);
                if bite_idx.is_some() && !dropped_respects_active_constraint(cur_n, &dropped) {
                    continue;
                }
                if ns.len() > goal_colors.len() || !prefix_ok(&ns, goal_colors, keep_prefix_len) {
                    continue;
