// v203_guarded_fulltime_minturn.rs
// メモ:
...
const FINAL_MIN_LEFT_SEC: f64 = 0.025;
const ANYTIME_MIN_LEFT_SEC: f64 = 0.11;
const BASE_SEARCH_MIN_REMAINING_SEC: f64 = 0.07;
const SUFFIX_SEARCH_MIN_REMAINING_SEC: f64 = 0.05;
const ANYTIME_SEARCH_MIN_REMAINING_SEC: f64 = 0.085;
const HEAVY_HELPER_MIN_LEFT_SEC: f64 = 0.018;
...
#[inline]
fn time_guard_exhausted(started: &Instant, min_left_sec: f64) -> bool {
    time_over(started) || time_left(started) < min_left_sec
}

#[inline]
fn time_guard_exhausted_opt(started: Option<&Instant>, min_left_sec: f64) -> bool {
    started.is_some_and(|s| time_guard_exhausted(s, min_left_sec))
}
...
fn collect_top_k_target_paths(
    ...
    if time_guard_exhausted_opt(started, min_left_sec) {
        return Vec::new();
    }
    ...
    for idx in 0..st.n * st.n {
        if (idx & 15) == 0 && time_guard_exhausted_opt(started, min_left_sec) {
            break;
        }
...
fn expand_bite_children(
    ...
    while q_head < q.len() {
        if time_guard_exhausted(started, min_left_sec) {
            break;
        }
...
fn simple_next_target_path_len(st: &State, input: &Input, next_ell: usize) -> Option<usize> {
    ...
    collect_top_k_target_paths(st, input.d[next_ell], 1, None, 0.0)
        .into_iter()
        .next()
    ...
    while ell < input.m {
        ...
        for parent in &beam {
            ...
            let mut children = expand_simple_children(
                ...
                candidate_width,
                started,
                min_left_sec.max(HEAVY_HELPER_MIN_LEFT_SEC),
            );
...
fn grow_to_target_prefix_impl(
    ...
    let min_remaining_sec =
        if config.stage_beam > STAGE_BEAM || config.suffix_stage_beam > SUFFIX_STAGE_BEAM {
            ANYTIME_SEARCH_MIN_REMAINING_SEC
        } else {
            match mode {
                GrowthMode::Base => BASE_SEARCH_MIN_REMAINING_SEC,
                GrowthMode::SuffixTurnFocused => SUFFIX_SEARCH_MIN_REMAINING_SEC,
            }
        };
    let _budget = push_search_budget(timer, local_time_limit_sec.max(0.0), min_remaining_sec);
    let _goal_hash = push_goal_prefix_hash(goal_colors);
    let started = Instant::now();
    let mut beam = vec![start_bs.clone()];
    let goal_len = goal_colors.len();
    let helper_min_left_sec = min_remaining_sec.max(HEAVY_HELPER_MIN_LEFT_SEC);
        ...
        GrowthMode::Base => {
            for ell in start_ell..goal_len {
                ...
                for bs in &beam {
                    ...
                    if !time_guard_exhausted(&started, helper_min_left_sec) {
...
fn schedule_anytime_chunk(timer: &TimeKeeper) -> f64 {
    let usable = (timer.exact_remaining_sec() - ANYTIME_SEARCH_MIN_REMAINING_SEC).max(0.0);
    if usable <= 0.0 {
        0.0
...
fn run_simple_suffix_pass(
    ...
    let min_left_sec = (local_time_limit_sec * 0.18)
        .clamp(SUFFIX_SEARCH_MIN_REMAINING_SEC, ANYTIME_SIMPLE_MIN_LEFT_SEC);
    let _budget = push_search_budget(
        timer,
        local_time_limit_sec.max(0.0),
        ANYTIME_SEARCH_MIN_REMAINING_SEC,
    );
