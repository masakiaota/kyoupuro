...
fn inventory_try_direct_build_from_current(
    ...
    let grown = grow_to_target_prefix(ctx.input, base.state.clone(), target.colors, ctx.timer, build_limit)?;
    if !exact_prefix(&grown.state, target.colors, target.protect_len) {
        return None;
    }
    append_incremental_beam(base, grown)
...
fn inventory_transport_plan_with_repair_limits(
    ...
    while let Some(Reverse((_, _, idx))) = pq.pop() {
        ...
        if let Some(done) =
            inventory_transport_finish_from_repair_node(&nodes, idx, protect_colors, protect_len, geom)
        {
            return Some(done);
        }
        ...
        for &dir_u8 in legal_dirs(&cur).as_slice() {
            ...
            if inventory_is_entry_cell(geom, ns.head()) {
                if let Some(finish) = inventory_transport_finish_kind(&ns, protect_colors, protect_len, geom)
                {
                    let child = nodes.len();
                    nodes.push(Node {
                        state: ns,
