...
fn inventory_build_target_exact(
    ...
    } else if exact_prefix(&grown.state, target.colors, target.protect_len) {
        append_incremental_beam(base, grown)
    } else {
        inventory_try_launch_rescue_build(
            base,
...
fn inventory_run_segment_phase(
    ...
    let mut bs = match inventory_build_target_exact(
        ctx,
        base,
        &target,
        ctx.stock_cnt - seg_idx + 1,
        seg_idx > 0 && base.state.len() == 5,
    ) {
...
fn inventory_build_tail_exact(
    ...
    let next_bs = match inventory_build_target_exact(
        ctx,
        base,
        &target,
        1,
        ctx.stock_cnt > 0 && base.state.len() == 5,
    ) {
        InventoryBuildOutcome::Built(next_bs) => next_bs,
        InventoryBuildOutcome::GrowFailed => {
...
fn inventory_finish_harvest(
    ...
    if ctx.stock_cnt > 0 {
        ...
        let goal = cell_of(ctx.input.n - 2, 2 * ctx.stock_cnt - 1, ctx.input.n);
