...
fn transport_to_cell_preserving_prefix(
    ...
    while st.head() != goal {
        ...
        *cnt += 1;
        ...
        let dir = choose_inventory_transport_dir(&st, goal, protect_colors, protect_len, input)?;
        let (ns, _, bite_idx) = step(&st, dir);
        if bite_idx.is_some() || !prefix_ok(&ns, protect_colors, protect_len) {
            return None;
        }
...
fn transport_to_entry_from_right(
    ...
    let mut seen = FxHashMap::<VisitKey, usize>::default();
    let mut guard = 0usize;
    let mut early_rescue_used = false;

    loop {
        if inventory_is_entry_cell(geom, st.head())
            && inventory_place_from_entry(&st, protect_colors, protect_len, geom, input.n).is_some()
        {
            return Some(BeamState {
                state: st.clone(),
                ops,
            ...
            try_commit_inventory_entry_and_validate_place(&st, protect_colors, protect_len, geom)
        {
            ...
            return Some(BeamState { state: ns, ops });
        }
        ...
        guard += 1;
        if guard > st.n * st.n * 100 {
            return None;
        }

        if let Some(dir) =
            inventory_ready_local_next_dir(&st, input, protect_colors, protect_len, geom, started)
        {
            let (ns, ate, bite_idx) = step(&st, dir);
            if bite_idx.is_none()
                && ate == 0
        ...
        } else if !early_rescue_used {
            early_rescue_used = true;
            eprintln!(
                "inventory_debug early_rescue_try seg={} ops={} head={:?}",
                seg_idx,
                ops.len(),
                rc_of(st.head(), st.n)
            );
            if let Some(rescue) = inventory_ready_oracle_shallow_plan(
                &st,
                input,
                protect_colors,
                protect_len,
                ...
                eprintln!(
                    "inventory_debug early_oracle_hit seg={} steps={}",
                    seg_idx,
                    rescue.ops.len()
                );
                if ops.len() + rescue.ops.len() > MAX_TURNS {
                    return None;
                }
                ops.extend_from_slice(&rescue.ops);
                st = rescue.state;
                continue;
            }
            if let Some(rescue) =
                inventory_ready_rescue_plan(&st, input, protect_colors, protect_len, geom, started)
            {
                eprintln!(
                    "inventory_debug early_rescue_hit seg={} steps={}",
                    seg_idx,
                    rescue.ops.len()
                );
                if ops.len() + rescue.ops.len() > MAX_TURNS {
            ...
            eprintln!("inventory_debug early_rescue_miss seg={}", seg_idx);
        }

        let key = make_visit_key(&st, inventory_pre_goal_hint(&st, geom), 0);
        let cnt = seen.entry(key).or_insert(0);
        *cnt += 1;
        let fast_dir = choose_inventory_entry_ready_dir_loose(
            &st,
            input,
            protect_colors,
            protect_len,
        ...
        if fast_dir.is_none() || *cnt > VISIT_REPEAT_LIMIT {
            if let Some(rescue) =
                inventory_ready_rescue_plan(&st, input, protect_colors, protect_len, geom, started)
            {
                if ops.len() + rescue.ops.len() > MAX_TURNS {
...
fn place_inventory_segment(
    ...
    let (st, seg_ops) = inventory_place_from_entry(&bs.state, protect_colors, protect_len, geom, n)?;
    let mut ops = bs.ops.clone();
...
fn solve_inventory_stock(input: &Input, timer: &TimeKeeper) -> BeamState {
    ...
    for seg_idx in 0..stock_cnt {
        ...
        let phases_left = stock_cnt - seg_idx + 1;
        {
            ...
            let build_limit = inventory_build_time_limit(timer, phases_left);
            let built = if seg_idx > 0 && bs.state.len() == 5 {
                let Some(grown) = grow_to_target_prefix(
                    input,
                    bs.state.clone(),
                    &target,
                    timer,
                    inventory_initial_build_limit(&bs.state, build_limit),
                ) else {
                    eprintln!("inventory_fail build seg={seg_idx}");
                    return bs;
                };
                if exact_prefix(&grown.state, &target, target.len()) {
                    append_incremental_beam(&bs, grown)
                } else {
                    let gap = target.len().saturating_sub(lcp_state(&grown.state, &target));
                    if gap >= 6 {
                        inventory_try_extra_build(&bs, input, &target, timer, build_limit)
                            .or_else(|| {
                        ...
                        inventory_try_greedy_launch_build(&bs, input, &target, timer, build_limit)
                            .or_else(|| {
                                inventory_try_launch_rescue_build(
                                    &bs,
                                    input,
            ...
            let Some(next_bs) = built else {
                eprintln!("inventory_fail build_exact seg={seg_idx}");
                return bs;
            };
            bs = next_bs;

            let Some(next_bs) = transport_to_entry_from_right(
                &bs,
                input,
                &target,
                target.len(),
            ...
            bs = next_bs;
            let Some(next_bs) =
                place_inventory_segment(&bs, seg_idx, input.n, &target, target.len())
            else {
    ...
    {
        let _constraint = push_movement_constraint(movement_constraint_with_min_col(2 * stock_cnt));
        let build_limit = inventory_build_time_limit(timer, 1);
        let built = if stock_cnt > 0 && bs.state.len() == 5 {
            let Some(grown) = grow_to_target_prefix(
                input,
                bs.state.clone(),
                &tail_target,
            ...
            if exact_prefix(&grown.state, &tail_target, tail_target.len()) {
                append_incremental_beam(&bs, grown)
            } else {
                let gap = tail_target
                    .len()
                    .saturating_sub(lcp_state(&grown.state, &tail_target));
                if gap >= 6 {
                    inventory_try_extra_build(&bs, input, &tail_target, timer, build_limit)
                        .or_else(|| {
                            inventory_try_greedy_launch_build(
                                &bs,
                                input,
                    ...
                    inventory_try_greedy_launch_build(&bs, input, &tail_target, timer, build_limit)
                        .or_else(|| {
                            inventory_try_launch_rescue_build(
                                &bs,
                                input,
    ...
    if stock_cnt > 0 {
        ...
        let goal = cell_of(input.n - 2, 2 * stock_cnt - 1, input.n);
        let Some(next_bs) = transport_to_cell_preserving_prefix(
            &bs,
            input,
            &tail_target,
            tail_target.len(),
    ...
    let Some(next_bs) = harvest_inventory(&bs, input, stock_cnt) else {
