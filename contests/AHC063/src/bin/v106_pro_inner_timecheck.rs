...
#[inline]
fn periodic_time_over(started: &Instant, tick: &mut usize) -> bool {
    *tick += 1;
    (*tick & TIME_CHECK_MASK) == 0 && time_over(started)
}
...
fn try_recover_exact(
    ...
    let mut recover_tick = 0usize;
    for ent in dropped.iter().take(need_cnt as usize) {
        if periodic_time_over(started, &mut recover_tick) {
            return None;
        }
    ...
    while let Some(Reverse((_, depth, _, idx))) = pq.pop() {
...

        {
            ...
            for dir1 in legal_dirs(&st) {
                if periodic_time_over(started, &mut inner_tick) {
                    time_stopped = true;
                    break;
                }
                ...
                if rs.colors.len() < ell {
                    let Some((rec_state, rec_ops)) =
                        try_recover_exact(&rs, input, ell, &dropped1, started)
                    else {
                        continue;
                    };
                    rs = rec_state;
        ...
        for dir in legal_dirs(&st) {
            ...
            if bite_idx.is_some() && ns.colors.len() < ell {
                ...
                let Some((rec_state, rec_ops)) = try_recover_exact(&ns, input, ell, &dropped, started)
                else {
                    continue;
                };
                ns = rec_state;
        ...
        if !exact_prefix(&st, &input.d, ell) {
            for dir in legal_dirs(&st) {
                ...
                if rs.colors.len() < ell {
                    let Some((rec_state, rec_ops)) =
                        try_recover_exact(&rs, input, ell, &dropped, started)
                    else {
                        continue;
                    };
                    rs = rec_state;
...
fn bfs_next_dir(
    ...
    let mut bfs_tick = 0usize;
    while let Some(cid) = q.pop_front() {
        if periodic_time_over(started, &mut bfs_tick) {
            return None;
        }
        if cid == goal {
...
fn navigate_to_goal_safe(bs: &BeamState, goal: u16, target: u16, started: &Instant) -> Option<BeamState> {
    ...
    while st.head() != goal {
        ...
        *cnt += 1;
        ...
        let dir = bfs_next_dir(&st, goal, target, true, true, started)?;
    ...
    while st.head() != goal || !restore_queue.is_empty() {
        ...
        *cnt += 1;
        ...
        let dir = if let Some(front) = restore_queue.front() {
            ...
            if let Some(dir) = bfs_next_dir(&st, goal, target, true, false, started) {
                dir
            } else {
                if time_over(started) {
                    return None;
                }
                bfs_next_dir(&st, goal, target, false, false, started)?
            }
        };
