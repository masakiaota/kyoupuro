...
#[inline]
fn apply_eat_no_bite_in_place(st: &mut State, nh: u16, need: u8) -> bool {
    if st.food[nh as usize] != need {
        return false;
    }
    if st.pos.len() >= 2 && nh == st.pos[1] {
        return false;
    }
    if st.pos[1..].contains(&nh) {
        return false;
    }

    let len = st.pos.len();
    st.pos.push(0);
    st.pos.copy_within(0..len, 1);
    st.pos[0] = nh;
    st.colors.push(need);
    st.food[nh as usize] = 0;
    true
}
...
fn extend_fastlane_one(
    ...
    sols.into_iter().min_by_key(|cand| cand.ops.len())
}
...
fn try_recover_exact(
    ...
    let need_cnt = need_cnt as usize;
    s.pos.reserve(need_cnt);
    s.colors.reserve(need_cnt);
    ...
    for ent in dropped.iter().take(need_cnt) {
        ...
        let nh = ent.cell;
        let dir = dir_between_cells(s.n, s.head(), nh)?;
        if !apply_eat_no_bite_in_place(&mut s, nh, need) {
            return None;
        }
        ops.push(DIRS[dir].2);
    }
