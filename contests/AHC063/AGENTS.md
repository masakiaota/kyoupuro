...
- `src/bin/*.rs` の top-level には提出候補を置く。各ファイルは単体で完結し、1 行目に `// <file_name>.rs` を置く(AtCoder上でも識別できるように)。
- `src/bin/adhoc/*.rs` には probe / bench / check などの補助 bin を置いてよい。
...
- `results/score_detail.csv` は `tools/in` の標準 100 ケース用詳細ログである。列順は `bin,total_avg,local_relative_score,case0000,...,case0099,max_elapsed,executed_at`。全ケース成功時のみ再計算して更新する。
...
- `src/bin/*.rs`
  - top-level は提出候補を置く場所である。
- `src/bin/adhoc/*.rs`
  - bench / probe / check など補助 bin を置く場所である。
- `scripts/run.sh`
  - 1 件の入力に対する手動実行を行う。
- `scripts/eval.sh`
  - 公式 scorer を使った並列評価本体である。
- `scripts/adhoc/`
  - 単発の分析、PoC、補助ベンチ用スクリプトを置く場所である。
- `scripts/lib/`
  - shell script から使う共通 helper を置く場所である。
- `notes/`
  - 問題固有の発見や性質を記録する場所である。
- `results/score_summary.csv`
  - score 要約の蓄積先である。
- `results/score_detail.csv`
  - `tools/in` の `0000.txt` から `0099.txt` を対象にした詳細ログである。各 run のケース別絶対スコアと `local_relative_score` を wide CSV で保持する。
...
## shell script の役割
- `scripts/run.sh`
  - Cargo 定義から解決した solver source をビルドし、stdin か 1 つの input file を手動確認する。
- `scripts/eval.sh`
