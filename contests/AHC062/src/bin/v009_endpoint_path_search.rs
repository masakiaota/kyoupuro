//! # v009
//! fixed-endpoint open path local search
...
fn apply_relocation(path: &mut [usize], l: usize, r: usize, p: usize, reversed: bool) {
    let seg_len = r - l + 1;
    let mut seg = path[l..=r].to_vec();
    if reversed {
        seg.reverse();
    }
    if p < l {
        path.copy_within(p + 1..l, p + 1 + seg_len);
        path[p + 1..p + 1 + seg_len].copy_from_slice(&seg);
    } else {
        path.copy_within(r + 1..=p, l);
        let q = p + 1 - seg_len;
...
fn relocate_delta_raw(
    ...
    if p < l {
        let start = p + 1;
        for idx in start..=r {
            old += idx as i64 * value[path[idx]];
        }
        for t in 0..seg_len {
            let cell = if reversed { path[r - t] } else { path[l + t] };
            new += (start + t) as i64 * value[cell];
        }
        let tail_len = l - start;
        for t in 0..tail_len {
            let cell = path[start + t];
            new += (start + seg_len + t) as i64 * value[cell];
        }
    } else {
        let start = l;
        for idx in l..=p {
        ...
        for t in 0..seg_len {
            let cell = if reversed { path[r - t] } else { path[l + t] };
            new += (start + tail_len + t) as i64 * value[cell];
        }
    }
...
fn relocate_delta_viol(
    ...
    let old_v = (!problem.is_adjacent(a, b)) as i32
        + (!problem.is_adjacent(c, d)) as i32
        + (!problem.is_adjacent(e, f)) as i32;

    let mut new_v = (!problem.is_adjacent(a, d)) as i32;
    if reversed {
        new_v += (!problem.is_adjacent(e, c)) as i32;
        new_v += (!problem.is_adjacent(b, f)) as i32;
    } else {
        new_v += (!problem.is_adjacent(e, b)) as i32;
        new_v += (!problem.is_adjacent(c, f)) as i32;
    }

    new_v - old_v
}

fn try_boundary_repair(
    problem: &Problem,
    state: &mut State,
    best: &mut Option<BestFeasible>,
    left_side: bool,
) -> bool {
    let target = if left_side {
        state.path[0]
    } else {
        state.path[problem.m - 1]
    };
    let p = if left_side { 0 } else { problem.m - 2 };
    let mut anchors = Vec::with_capacity(8);
    for pos in 1..problem.m - 1 {
        if problem.is_adjacent(target, state.path[pos]) {
            anchors.push(pos);
        }
    }
    if anchors.is_empty() {
    ...
    let mut best_choice: Option<(usize, usize, bool, i64)> = None;
    for &anchor in &anchors {
        for seg_len in 1..=96 {
            for offset in 0..seg_len {
                let l = anchor.saturating_sub(offset).max(1);
                let r = l + seg_len - 1;
                if r >= problem.m - 1 || !is_valid_relocate(problem.m, l, r, p) {
                    continue;
                }
                for &reversed in &[false, true] {
                    let boundary_cell = if left_side {
                        if reversed {
                            state.path[r]
                        } else {
                            state.path[l]
                    ...
                    if !problem.is_adjacent(target, boundary_cell) {
                        continue;
                    }
                    let delta_viol = relocate_delta_viol(problem, state, l, r, p, reversed);
                    if delta_viol >= 0 {
                        continue;
                    }
                    let delta_raw =
                        relocate_delta_raw(&state.path, &problem.value, l, r, p, reversed);
                    let cand = (l, r, reversed, delta_raw);
                    let better = best_choice.as_ref().is_none_or(|cur| delta_raw > cur.3);
                    if better {
                        best_choice = Some(cand);
                    }
                }
            }
        }
    ...
    let Some((l, r, reversed, _delta_raw)) = best_choice else {
        return false;
    };
    let mut new_path = state.path.clone();
    apply_relocation(&mut new_path, l, r, p, reversed);
    *state = State::from_path(problem, new_path);
    update_best(problem, state, best);
    true
}

fn try_two_break_reverse(
    problem: &Problem,
    state: &mut State,
    best: &mut Option<BestFeasible>,
) -> bool {
    if state.viol_cnt != 2 || state.breakpoints.len() != 2 {
        return false;
    }
    let mut bp = state.breakpoints.clone();
    bp.sort_unstable();
    let l = bp[0] + 1;
    let r = bp[1];
    if l >= r || r + 1 >= problem.m {
        return false;
    }
    let left = state.path[l - 1];
    let right = state.path[r + 1];
    let new_left = state.path[r];
    let new_right = state.path[l];
    if !problem.is_adjacent(left, new_left) || !problem.is_adjacent(new_right, right) {
        return false;
    }
    let mut new_path = state.path.clone();
    new_path[l..=r].reverse();
    *state = State::from_path(problem, new_path);
    update_best(problem, state, best);
    true
}
...
fn search_hotspot_window(
    ...
    let l = bp.saturating_sub(radius).max(bounds.left);
    let r = (bp + 1 + radius).min(bounds.right);
    if l >= problem.m || r >= problem.m || l > r {
        return None;
    }
    ...
    let exact_l = bp.saturating_sub(exact_half).max(bounds.left);
    let exact_r = (bp + 1 + exact_half).min(bounds.right);
    ...
    if repair_mode {
        let rest_insert_lo = pivot.saturating_sub(1);
        let rest_insert_hi = (pivot + 2).min(len);
        let step = if len > 160 { 2 } else { 1 };
        for seg_len in 1..=6 {
            let mut a = 0_usize;
            while a + seg_len <= len {
                let b = a + seg_len - 1;
                if !(b + 1 >= pivot && a <= pivot + 1) {
                    let rest_len = len - seg_len;
                    let insert_lo = rest_insert_lo.min(rest_len);
                    let insert_hi = rest_insert_hi.min(rest_len);
                    for insert_pos in insert_lo..=insert_hi {
                        let source_insert_pos = if insert_pos <= a {
                            insert_pos
                        } else {
                            insert_pos + seg_len
...
fn sample_rank_move(
    ...
    let mut best_p = rng.random_range(bounds.left..=bounds.right);
    let mut best_gap = rank_gap(problem, state, best_p).abs();
    for _ in 0..7 {
        let p = rng.random_range(bounds.left..=bounds.right);
    ...
    let l = best_p.saturating_sub(radius).max(bounds.left);
    let r = (best_p + radius).min(bounds.right);
    let exact_l = best_p.saturating_sub(4).max(bounds.left);
    let exact_r = (best_p + 4).min(bounds.right);
...
fn sample_random_move(
    ...
    let min_span = 18.min(len);
    let max_span = 56.min(len);
    let span = rng.random_range(min_span..=max_span);
    let l = rng.random_range(bounds.left..=bounds.right + 1 - span);
    let r = l + span - 1;
    let min_exact = 7.min(len);
    let max_exact = 10.min(len);
    let exact_len = rng.random_range(min_exact..=max_exact);
    let exact_l = rng.random_range(bounds.left..=bounds.right + 1 - exact_len);
    if let Some(mv) =
...
fn calibrate_scale(
    ...
    for _ in 0..96 {
        if let Some(mv) = sample_random_move(problem, state, 0, rng, bounds) {
...
fn legal_local_pass(
    ...
    let width = 8_usize.min(bounds.len());
    let stride = 4_usize;
    for offset in [0_usize, 2] {
        let mut l = bounds.left + offset;
        while l + width <= bounds.right + 1 && Instant::now() < deadline {
...
fn sweep_repair_pass(
    ...
    let width = width.min(bounds.len());
...
fn force_repair(
    ...
    while Instant::now() < deadline && state.viol_cnt > target_budget {
        ...
        let candidate = search_hotspot_window(problem, state, bp, lambda, radius, true, bounds);
...
fn pin_endpoints(problem: &Problem, path: &[usize]) -> Vec<usize> {
    let start_id = problem.ideal_path[0];
    let end_id = problem.ideal_path[problem.m - 1];
    let mut out = Vec::with_capacity(problem.m);
    out.push(start_id);
    for &id in path {
...
fn block_start(block: &[usize], reversed: bool) -> Option<usize> {
    if reversed {
        block.last().copied()
    } else {
        block.first().copied()
...
fn block_end(block: &[usize], reversed: bool) -> Option<usize> {
    if reversed {
        block.first().copied()
    } else {
        block.last().copied()
...
fn extend_block(out: &mut Vec<usize>, block: &[usize], reversed: bool) {
    if reversed {
...
fn build_one_cut_fixed_path(problem: &Problem, path: &[usize]) -> Option<Vec<usize>> {
    let start_id = problem.ideal_path[0];
    let end_id = problem.ideal_path[problem.m - 1];
    let interior: Vec<_> = path
        .iter()
        .copied()
    ...
    for k in 0..=len {
        ...
        for &(first_is_b, rev_a, rev_b) in &[
            ...
            let (first_block, first_rev, second_block, second_rev) = if first_is_b {
                (b, rev_b, a, rev_a)
            } else {
                (a, rev_a, b, rev_b)
            };
            let Some(first_cell) = block_start(first_block, first_rev) else {
                continue;
            };
            let Some(last_cell) = block_end(second_block, second_rev).or_else(|| {
                if second_block.is_empty() {
                    block_end(first_block, first_rev)
                } else {
                    None
                }
            }) else {
            ...
            if !problem.is_adjacent(start_id, first_cell) || !problem.is_adjacent(last_cell, end_id)
            {
                continue;
            }
            if !first_block.is_empty() && !second_block.is_empty() {
                let join_left = block_end(first_block, first_rev).unwrap();
                let join_right = block_start(second_block, second_rev).unwrap();
                if !problem.is_adjacent(join_left, join_right) {
                    continue;
                }
            }
            let mut out = Vec::with_capacity(problem.m);
            out.push(start_id);
            extend_block(&mut out, first_block, first_rev);
            extend_block(&mut out, second_block, second_rev);
            out.push(end_id);
            return Some(out);
        }
    }
...
fn build_two_cut_fixed_path(problem: &Problem, path: &[usize]) -> Option<Vec<usize>> {
    let start_id = problem.ideal_path[0];
    let end_id = problem.ideal_path[problem.m - 1];
    let interior: Vec<_> = path
        .iter()
        ...
        .collect();
    let len = interior.len();
    let mut cuts = vec![0_usize, len];
    for (idx, &id) in interior.iter().enumerate() {
        if problem.is_adjacent(start_id, id) || problem.is_adjacent(id, end_id) {
            cuts.push(idx);
            cuts.push(idx + 1);
        }
    }
    ...
    for &x in &cuts {
        for &y in &cuts {
            ...
            let segs = [&interior[..x], &interior[x..y], &interior[y..]];
            for perm in perms {
                for mask in 0..8_u8 {
                    let mut pieces: Vec<(&[usize], bool)> = Vec::with_capacity(3);
                    for &idx in &perm {
                        if segs[idx].is_empty() {
                            continue;
                        }
                        let reversed = ((mask >> idx) & 1) == 1;
                        pieces.push((segs[idx], reversed));
                    }
                    if pieces.is_empty() {
                        continue;
                    }
                    let first_cell = block_start(pieces[0].0, pieces[0].1).unwrap();
                    let last_cell =
                        block_end(pieces[pieces.len() - 1].0, pieces[pieces.len() - 1].1).unwrap();
                    if !problem.is_adjacent(start_id, first_cell)
                        || !problem.is_adjacent(last_cell, end_id)
                    {
                        continue;
                    }
                    let mut ok = true;
                    for w in pieces.windows(2) {
                        let left = block_end(w[0].0, w[0].1).unwrap();
                        let right = block_start(w[1].0, w[1].1).unwrap();
                        if !problem.is_adjacent(left, right) {
                            ok = false;
                            break;
                        }
                    }
                    ...
                    let mut out = Vec::with_capacity(problem.m);
                    out.push(start_id);
                    for &(block, reversed) in &pieces {
                        extend_block(&mut out, block, reversed);
                    }
                    out.push(end_id);
                    return Some(out);
                }
...
fn update_best(problem: &Problem, state: &State, best: &mut Option<BestFeasible>) {
    if state.viol_cnt == 0
        && best
            .as_ref()
            .is_none_or(|cur| state.raw_score > cur.raw_score)
    {
        let _ = problem;
        *best = Some(BestFeasible::from_state(state));
    }
}
...
fn run_search(
    ...
    bounds: SearchBounds,
    seed_salt: u64,
) -> (State, Option<BestFeasible>) {
    let start = Instant::now();
    let mut state = State::from_path(problem, initial_path);
    let mut best = if state.viol_cnt == 0 {
        Some(BestFeasible::from_state(&state))
    } else {
        None
    };

    let mut rng = Xoshiro256PlusPlus::seed_from_u64(seed_from_problem(problem) ^ seed_salt);
    let scale = calibrate_scale(problem, &state, &mut rng, bounds);
    let budgets = build_budgets(state.viol_cnt);
    ...
    let warmup_deadline = start + std::time::Duration::from_secs_f64(time_limit_sec.min(0.06));
    legal_local_pass(problem, &mut state, &mut best, warmup_deadline, bounds);
    if best.is_none() && state.viol_cnt > 0 {
        let mut pre_stats = PhaseStats::default();
        let pre_repair_deadline =
            start + std::time::Duration::from_secs_f64((time_limit_sec * 0.18).min(0.28));
        while best.is_none() && Instant::now() < pre_repair_deadline {
            let mut progressed = false;
            progressed |= try_two_break_reverse(problem, &mut state, &mut best);
            if best.is_some() {
                break;
            }
            if state.bad[0] == 1 {
                progressed |= try_boundary_repair(problem, &mut state, &mut best, true);
            }
            if state.bad[problem.m - 2] == 1 {
                progressed |= try_boundary_repair(problem, &mut state, &mut best, false);
            }
            if best.is_some() {
                break;
            }
            ...
            progressed |= state.viol_cnt < before;
            if best.is_some() {
                break;
            }
            if !sweep_repair_pass(
                problem,
                &mut state,
                &mut best,
                pre_repair_deadline,
    ...
    if debug_log {
        let sample_bp: Vec<_> = state.breakpoints.iter().copied().take(16).collect();
        eprintln!(
            "after_pre_repair viol={} sample_bp={:?} best_feasible={}",
            state.viol_cnt,
            sample_bp,
            best.is_some()
        );
    ...
    for phase in 0..PHASES {
        ...
        while Instant::now() < phase_end {
            ...
            let candidate = if roll < repair_weight {
                choose_breakpoint(&state, &mut rng, problem.m)
                    ...
                    .or_else(|| sample_rank_move(problem, &state, lambda, &mut rng, bounds))
            } else if roll < repair_weight + rank_weight {
                sample_rank_move(problem, &state, lambda, &mut rng, bounds)
                    .or_else(|| sample_random_move(problem, &state, lambda, &mut rng, bounds))
            } else {
                sample_random_move(problem, &state, lambda, &mut rng, bounds)
                    .or_else(|| sample_rank_move(problem, &state, lambda, &mut rng, bounds))
            };
            let Some(mv) = candidate else {
                continue;
        ...
        if debug_log {
            let best_score = best
                .as_ref()
                .map(|cur| (cur.raw_score + problem.m as i64 / 2) / problem.m as i64)
                .unwrap_or(-1);
            eprintln!(
...
fn main() {
    ...
    let fixed_attempt_sec = 0.22;
    let open_sec = (TIME_LIMIT_SEC - fixed_attempt_sec).max(0.40);

    let baseline_path = problem.build_baseline_path();
    let pinned_path = build_two_cut_fixed_path(&problem, &baseline_path)
        .or_else(|| build_one_cut_fixed_path(&problem, &baseline_path))
        .unwrap_or_else(|| pin_endpoints(&problem, &baseline_path));
    if std::env::var_os("AHC062_DEBUG").is_some() {
        let pinned_state = State::from_path(&problem, pinned_path.clone());
        let sample_bp: Vec<_> = pinned_state.breakpoints.iter().copied().take(16).collect();
        eprintln!(
            "pinned_init viol={} sample_bp={:?} start={:?} end={:?}",
            pinned_state.viol_cnt,
            sample_bp,
            problem.coord[problem.ideal_path[0]],
            problem.coord[problem.ideal_path[problem.m - 1]]
        );
    }
    let (fixed_state, fixed_best) = run_search(
        &problem,
        pinned_path.clone(),
        fixed_attempt_sec,
        SearchBounds::interior(problem.m),
        0xD1CE_F1F0,
    );
    let final_path = if let Some(best) = fixed_best {
        best.path
    } else if fixed_state.viol_cnt == 0 {
        fixed_state.path
    } else {
        let (open_state, open_best) = run_search(
            &problem,
            baseline_path.clone(),
            open_sec,
            SearchBounds::full(problem.m),
            0x0F0F_2026,
        );
        if let Some(best) = open_best {
            best.path
        } else if open_state.viol_cnt == 0 {
            open_state.path
        } else {
            baseline_path
        }
