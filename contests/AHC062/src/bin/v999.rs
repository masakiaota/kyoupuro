...
const TIME_LIMIT: f64 = 2.74;
const GUIDED_RATIO: f64 = 0.48;
const LNS_RATIO: f64 = 0.74;
const IMPROVE_RATIO: f64 = 0.90;
const HIGH_COUNTS: [usize; 3] = [128, 384, 1024];
...
const HIGH_CAND_LIMIT: usize = 18;
const LOW_CAND_LIMIT: usize = 10;
const NEG_DIVISOR: i64 = 4;
const LNS_HIGH_COUNT: usize = 1536;
const LNS_LOW_COUNT: usize = 384;
const LNS_ANCHOR_HIGH: usize = 14;
const LNS_ANCHOR_LOW: usize = 8;
const LNS_WINDOWS: [usize; 4] = [12, 11, 10, 9];
const PORTFOLIO_MIN_THREADS: usize = 2;
const CYCLE_TIME_LIMIT: f64 = 1.85;
const CYCLE_INITIAL_RATIO: f64 = 0.18;
const CYCLE_IMPROVE_RATIO: f64 = 0.88;
...
fn main() {
    ...
    let best =
        if std::thread::available_parallelism().map_or(1, |p| p.get()) >= PORTFOLIO_MIN_THREADS {
            let alt_weights = weights.clone();
            let handle = std::thread::spawn(move || solve_cycle_local(n, &alt_weights));
            let guided = solve_guided_lns(n, &weights);
            let cycle = handle.join().unwrap_or_else(|_| (Vec::new(), NEG_INF));
            if cycle.1 > guided.1 { cycle } else { guided }
        } else {
            solve_guided_lns(n, &weights)
        };

    print_route(&best.0, n);
}

fn print_route(route: &[usize], n: usize) {
    for &cell in route {
        println!("{} {}", cell / n, cell % n);
    }
}

fn solve_guided_lns(n: usize, weights: &[i64]) -> (Vec<usize>, i64) {
...
fn solve_cycle_local(n: usize, weights: &[i64]) -> (Vec<usize>, i64) {
    let neighbors = build_neighbors(n);
    let mut route = best_initial_cycle(n, weights);
    let mut score = relinearize_best_cut(&mut route, weights);

    let start = Instant::now();
    let time_limit = Duration::from_secs_f64(CYCLE_TIME_LIMIT);
    let initial_deadline = Duration::from_secs_f64(CYCLE_TIME_LIMIT * CYCLE_INITIAL_RATIO);
    let improve_deadline = Duration::from_secs_f64(CYCLE_TIME_LIMIT * CYCLE_IMPROVE_RATIO);
