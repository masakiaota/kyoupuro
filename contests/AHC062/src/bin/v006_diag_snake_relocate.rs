...
fn main() {
    ...
    while start.elapsed() < improve_deadline {
        ...
        let best_two_opt = best_two_opt_move(
            &route,
            &position,
            &neighbors,
            &prefix_weight,
...
fn best_initial_route(n: usize, weights: &[i64]) -> Vec<usize> {
    ...
    let mut row_path = row;
    let mut col_path = col;
    relinearize_best_cut(&mut row_path, weights);
    relinearize_best_cut(&mut col_path, weights);
    candidates.push(row_path);
    candidates.push(col_path);

    let diag = diag_snake_path(n);
    for &symmetry in &SYMMETRIES {
        let route = apply_symmetry_to_route(&diag, n, symmetry);
...
fn refine_around(
    ...
    for &anchor in anchors {
        for &window in &[9usize, 8usize] {
            ...
            let start_lo = anchor.saturating_sub(window + 1);
            let start_hi = anchor.saturating_add(1).min(len - 1);
            for start in start_lo..=start_hi {
                let width = (len - start).min(window);
                if width >= 2 {
                    optimize_window(route, weights, n, start, width, total_score);
                }
            }
        }
    }
