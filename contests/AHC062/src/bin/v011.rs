...
fn main() {
    ...
    let diag = std::env::var_os("V011_DIAG").is_some() || std::env::var_os("V010_DIAG").is_some();
...
fn sample_long_relocate(
    ...
    for _ in 0..LONG_RELOCATE_TRIES {
        ...
        let avg_pos = ((seg_sum / len as i64) - 1).clamp(0, (m - 1) as i64) as usize;
        let target_q = avg_pos.saturating_sub(len / 2).min(m - len);
        legal_qs.sort_unstable_by_key(|&q| q.abs_diff(target_q).min(max_span));
        ...
        if !low_cells.is_empty() {
            ...
            for &q in &legal_qs {
                    ...
                    evaluate_relocate(n, state, weights, start_bonus, end_bonus, l, r, q)
                {
                    ...
                    let key =
                        cand.delta_raw + 32_000 * cand.delta_endpoint_points - 4_000 * q as i64;
                    if key > best_key {
                        best_key = key;
                        best = Some(cand);
                    }
        ...
        if !high_cells.is_empty() {
            ...
            for &q in &legal_qs {
                    ...
                    evaluate_relocate(n, state, weights, start_bonus, end_bonus, l, r, q)
                {
                    ...
                    let key = cand.delta_raw + 32_000 * cand.delta_endpoint_points
