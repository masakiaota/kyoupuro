...
const ENDPOINT_K: usize = 32;
const ENDPOINT_BONUS_MAX: i64 = 220_000;
const START_TEMP_FALLBACK: f64 = 2_500_000.0;
...
impl State {
    ...
    fn rebuild(&mut self, weights: &[i64], start_bonus: &[i64], end_bonus: &[i64]) {
        ...
        self.endpoint_points =
            start_bonus[self.path[0]] + end_bonus[*self.path.last().unwrap_or(&self.path[0])];
    }
}
...
fn main() {
    ...
    let diag = std::env::var_os("V010_DIAG").is_some();
    ...
    let mut best_path = initial_path.clone();
    let mut best_raw_score = compute_raw_score(&initial_path, &weights);
    let mut state = State::new(initial_path, &weights, &start_bonus, &end_bonus);

    if state.raw_score > best_raw_score {
        best_raw_score = state.raw_score;
    ...
    while start.elapsed() < sa_deadline {
        ...
        let delta_obj = cand.delta_raw as f64 + bonus_weight * cand.delta_endpoint_points as f64;
    ...
    if final_state.raw_score > best_raw_score {
        best_raw_score = final_state.raw_score;
        ...
        for &symmetry in &SYMMETRIES {
            ...
            for candidate in &mut candidates {
                let score = compute_raw_score(candidate, weights)
                    + ENDPOINT_BONUS_MAX
                        * (start_bonus[candidate[0]]
                            + end_bonus[*candidate.last().unwrap_or(&candidate[0])]);
                if score > best_score {
                    best_score = score;
...
fn emit_diag(
    ...
    eprintln!(
        ...
        sa_state.raw_score, final_state.raw_score, best_raw_score
    );
    eprintln!(
...
fn estimate_start_temp(
    ...
    for _ in 0..192 {
        ...
        if let Some(cand) = candidate {
            samples.push(cand.delta_raw.unsigned_abs() as f64);
        }
    }
    if samples.is_empty() {
...
fn sample_relocate(
    ...
    for _ in 0..RELOCATE_TRIES {
        ...
        let target_q = avg_pos.saturating_sub(len / 2).min(m - len);
        let mut candidates = [
            target_q,
            l.saturating_sub(max_span),
            (l + max_span).min(m - len),
            0,
            m - len,
        ];
        candidates[1] = candidates[1].min(m - len);
        for &q in &candidates {
            if let Some(cand) =
                evaluate_relocate(n, state, weights, start_bonus, end_bonus, l, r, q)
            {
                ...
                let key = cand.delta_raw + 8_000 * cand.delta_endpoint_points;
                if key > best_key {
                    best_key = key;
                    best = Some(cand);
                }
        ...
        for _ in 0..4 {
            let jitter = rng.random_range(0..=(max_span.min(m - len)));
            let q = if rng.random::<bool>() {
                target_q.saturating_sub(jitter)
            } else {
                (target_q + jitter).min(m - len)
            };
            if let Some(cand) =
                evaluate_relocate(n, state, weights, start_bonus, end_bonus, l, r, q)
            {
...
fn sample_long_relocate(
    ...
    let mut best = None;
    let mut best_key = i64::MIN;
    let seg_hi = max_seg.min(m.saturating_sub(1));
    ...
    for _ in 0..LONG_RELOCATE_TRIES {
        ...
        let candidates = [
            target_q,
            target_q.saturating_sub(max_span),
            (target_q + max_span).min(m - len),
            l.saturating_sub(max_span),
            (l + max_span).min(m - len),
            0,
            m - len,
        ...
        for &q in &candidates {
                ...
                evaluate_relocate(n, state, weights, start_bonus, end_bonus, l, r, q)
            {
                ...
                let key = cand.delta_raw + 10_000 * cand.delta_endpoint_points;
                if key > best_key {
                    best_key = key;
                    best = Some(cand);
                }
...
fn sample_reverse(
    ...
    for _ in 0..8 {
        ...
        if let Some(cand) = evaluate_reverse(n, state, start_bonus, end_bonus, l, r) {
            let key = cand.delta_raw + 6_000 * cand.delta_endpoint_points;
            if key > best_key {
                best_key = key;
                best = Some(cand);
            }
...
fn sample_endpoint_relocate(
    ...
    for _ in 0..ENDPOINT_TRIES {
        if !low_cells.is_empty() {
            ...
            if let Some(cand) =
                evaluate_relocate(n, state, weights, start_bonus, end_bonus, l, r, 0)
            {
                let cand = Candidate {
                    class: MoveClass::Endpoint,
                    ..cand
                };
                let key = cand.delta_raw + 24_000 * cand.delta_endpoint_points;
                if key > best_key {
                    best_key = key;
                    best = Some(cand);
                }
        ...
        if !high_cells.is_empty() {
            ...
            let r = l + len - 1;
            if let Some(cand) =
                evaluate_relocate(n, state, weights, start_bonus, end_bonus, l, r, m - len)
            {
                let cand = Candidate {
                    class: MoveClass::Endpoint,
                    ..cand
