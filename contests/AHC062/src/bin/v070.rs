...
fn main() {
    ...
    for symmetry in SYMMETRIES {
        ...
        let mut route = build_banded_route(n, &transformed, &paths_by_pair);
        let mut score = compute_raw_score(&route, &transformed);
        let rev_score = compute_raw_score_reversed(&route, &transformed);
        if rev_score > score {
            route.reverse();
            score = rev_score;
        }
