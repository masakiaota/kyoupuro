...
const TIME_LIMIT: f64 = 2.78;
const GUIDED_RATIO: f64 = 0.60;
const IMPROVE_RATIO: f64 = 0.90;
const HIGH_COUNTS: [usize; 3] = [96, 224, 512];
const LOW_COUNTS: [usize; 2] = [64, 160];
const TARGET_OFFSETS: [isize; 17] = [
    0, -1, 1, -2, 2, -4, 4, -8, 8, -12, 12, -20, 20, -32, 32, -48, 48,
];
const MOVE_RADIUS: usize = 128;
...
fn run_guided_pass(
    ...
    for &count in &HIGH_COUNTS {
        for idx in 0..count.min(len) {
            ...
            if let Some(mv) =
                best_guided_move(route, weights, position, prefix_weight, n, cell, rank)
            {
            ...
            for (reversed_idx, &desired_start) in desired_starts.iter().enumerate() {
                ...
                for &delta in &TARGET_OFFSETS {
                    ...
                    if let Some(mv) = evaluate_guided_relocation(
                        ...
                        let improvement = (before - after) as i128;
                        let key = (mv.delta as i128) * 1_000_000 + improvement * 10_000
                            - (seg_sum as i128 - weights[cell] as i128) * 4
                            - dist as i128 * 25;
                        if key > best_key {
                            best_key = key;
                            best = Some(mv);
