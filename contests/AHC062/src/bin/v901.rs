...
const TIME_LIMIT: f64 = 1.70;
const STRIP_W: usize = 5;
const SIDE_BIAS: i64 = 5_000_000;
const MAX_SEG_LEN: usize = 8;
...
fn main() {
    ...
    let ideal_route = ideal_order_route(n, &weights);
    let guide_weights = build_guide_weights(&ideal_route, &weights);

    // v901: 常に合法経路を起点にしつつ、短冊交互の理想順へ寄せる。
    let mut route = best_legal_strip_route(n, &guide_weights);

    let start = Instant::now();
    let time_limit = Duration::from_secs_f64(TIME_LIMIT);
    let guide_mixed_limit = Duration::from_secs_f64(TIME_LIMIT * 0.72);
    let guide_window_limit = Duration::from_secs_f64(TIME_LIMIT * 0.93);
    ...
    run_legal_mixed_descent(
        ...
        n,
        &start,
        guide_mixed_limit.min(time_limit),
    );
    run_legal_window_descent(
        &mut route,
        &guide_weights,
        n,
        &start,
        guide_window_limit.min(time_limit),
        1,
    );

    // 最後に実スコア重みで軽く整える（構造を崩しすぎないよう window のみ）。
...
fn build_guide_weights(ideal_route: &[usize], weights: &[i64]) -> Vec<i64> {
    let mut pos = vec![0usize; ideal_route.len()];
    for (idx, &cell) in ideal_route.iter().enumerate() {
        pos[cell] = idx;
    }

    // 理想順の時刻を主目的にし、同時に実スコアも少しだけ見る。
    let mut guide = vec![0i64; ideal_route.len()];
    for cell in 0..ideal_route.len() {
        guide[cell] = pos[cell] as i64 * 1_000 + weights[cell];
    }
    guide
}
