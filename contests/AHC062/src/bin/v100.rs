...
fn main() {
    ...
    let mut route = ideal_order_route(&weights);
    let diag = std::env::var_os("V100_DIAG").is_some();
    ...
    let mixed_limit = Duration::from_secs_f64(TIME_LIMIT * 0.92);
    run_penalty_continuation(&mut route, &weights, &neighbors, n, &start, penalty_limit);
    if diag {
        let v = count_violations(&route, n);
        eprintln!("diag after_penalty violations={}", v);
    }
    ...
    if diag {
        let v = count_violations(&route, n);
        eprintln!("diag after_force violations={}", v);
    }
    if !is_valid_route(&route, n) {
        route = best_legal_snake_route(n, &weights);
        used_fallback = true;
    }
    ...
    if diag {
        let v = count_violations(&route, n);
        let raw = compute_raw_score(&route, &weights);
        eprintln!(
            "diag final violations={} raw_score={} used_fallback={}",
            v, raw, used_fallback
        );
    }
...
fn ideal_order_route(weights: &[i64]) -> Vec<usize> {
    let mut order: Vec<usize> = (0..weights.len()).collect();
    order.sort_unstable_by_key(|&cell| weights[cell]);
    order
}
...
fn count_violations(route: &[usize], n: usize) -> usize {
    let mut cnt = 0usize;
    for i in 0..route.len() - 1 {
        if !is_adj(route[i], route[i + 1], n) {
            cnt += 1;
...
fn best_legal_snake_route(n: usize, weights: &[i64]) -> Vec<usize> {
    let row = row_snake(n);
    ...
    for cand in [row_rev, col, col_rev] {
...
fn is_path_adjacent(route: &[usize], n: usize) -> bool {
    for i in 0..route.len() - 1 {
        if !is_adj(route[i], route[i + 1], n) {
            return false;
        }
