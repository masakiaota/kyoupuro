...
def output_subdir_for_normalized_input_dir(normalized_input_dir: str) -> Path:
    if Path(normalized_input_dir).is_absolute():
        digest = hashlib.sha256(normalized_input_dir.encode("utf-8")).hexdigest()[:16]
        return Path("_external") / digest

    parts = Path(normalized_input_dir).parts
    if not parts or any(part in ("", ".", "..") for part in parts):
        raise SystemExit(f"error: invalid normalized input dir: {normalized_input_dir}")
    return Path(*parts)
...
def format_avg_score(numerator: int, denominator: int) -> str:
    return format(
        (Decimal(numerator) / Decimal(denominator)).quantize(
...
def summarize(results: list[CaseResult]) -> tuple[str, int, int, int, int, int]:
    ...
    total_avg = format_avg_score(total_sum, len(success_results))
...
def main() -> int:
    ...
    output_dir = (
        ...
        / "out"
        / args.bin_name
        / output_subdir_for_normalized_input_dir(normalized_input_dir)
    )
