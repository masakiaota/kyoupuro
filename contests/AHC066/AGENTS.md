...
## 評価運用ルール
- `scripts/run.sh` は単発の手動実行専用であり、solver を既定で `--release --features local` 付きで build する。ローカルで実行確認や評価を行うときは常に `local` feature 付きにする。
- `scripts/eval.py` は評価パイプライン本体である。solver は既定で `--release --features local`、tools の score は通常の `--release` で build する。先頭入力で `run -> score` を 1 回ウォームアップしてから、本番の `run -> score` をケース単位で実行する。ウォームアップ結果は保存・集計しない。既定は `cpu//2 - 1` 並列で、最小値は 1 である。ローカルで評価するときは `--no-local` を使わず、time sensitiveにチューニングする際も `local` feature 付きで行う。
- ケース群は `tools/in` を通常比較、`tools/test2000` を終盤CV、`tools/val1` と `tools/val2` を追加の独立検証に使う。
...
- solver 内で時間制限や打ち切り判定を実装する場合、経過時間は必ず `main()` の開始直後を基準に測る。探索開始後や特定フェーズ開始後を基準にした局所的な time limit は使わない。
