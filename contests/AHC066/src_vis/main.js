...
function configureCaseStorage(projectKey) {
  ...
  if (Object.prototype.hasOwnProperty.call(saved, "evalSet") && typeof saved.evalSet === "string") {
    state.currentEvalSet = saved.evalSet;
  }
...
function saveCaseViewState() {
  ...
  writeSessionJson(state.caseStorageKey, {
    ...
    evalSet: state.currentEvalSet || "tools/in",
...
function currentEvalSetIndex() {
  return state.evalSets.indexOf(state.currentEvalSet);
}
...
function syncCasesForCurrentEvalSet() {
  state.cases = Array.isArray(state.casesByEvalSet[state.currentEvalSet])
    ? state.casesByEvalSet[state.currentEvalSet]
    : [];
}
...
function populateEvalSetOptions() {
  const previous = state.currentEvalSet;
  els.evalSet.textContent = "";

  for (const evalSet of state.evalSets) {
  ...
  if (previous && state.evalSets.includes(previous)) {
    state.currentEvalSet = previous;
  } else {
    state.currentEvalSet = state.evalSets[0] ?? "tools/in";
  }
  els.evalSet.value = state.currentEvalSet;
  syncCasesForCurrentEvalSet();
}
...
async function refreshVisualizerData({ reloadCase = false } = {}) {
  ...
  state.evalSets = Array.isArray(data.evalSets) ? data.evalSets : [];
  state.casesByEvalSet =
    data.casesByEvalSet && typeof data.casesByEvalSet === "object" ? data.casesByEvalSet : {};
  if (state.evalSets.length === 0 && Array.isArray(data.cases)) {
    state.evalSets = ["tools/in"];
    state.casesByEvalSet = { "tools/in": data.cases };
  }
  populateEvalSetOptions();
...
async function runSelectedRustBin() {
  ...
  state.running = true;
  updateRunButton();
  setRunStatus(`${state.currentBin} を実行中...`);
...
els.evalSet.addEventListener("change", () => {
  state.currentEvalSet = els.evalSet.value;
  syncCasesForCurrentEvalSet();
  populateCaseOptions(state.currentCase === CUSTOM_CASE_VALUE);
  updateCaseButtons();
  if (state.currentCase !== CUSTOM_CASE_VALUE) {
