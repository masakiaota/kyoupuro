...
description: AHC 形式の C++ solver から、日本語の再現可能な解説を作る。入力としてチャンピオン実装や提出候補コードが与えられたときに、Phase 1 で削除や簡素化により本質を抽出し、Phase 2 で japanese-tech-writing に従う解説を書き、subagent に解説だけから再実装と評価をさせて品質を検証する。
---
...
3. 対象 solver
4. `src/bin/v000_template.cpp`
5. 評価に使う `scripts/eval.py`, `scripts/run.sh`, `scripts/build_solver.sh`
