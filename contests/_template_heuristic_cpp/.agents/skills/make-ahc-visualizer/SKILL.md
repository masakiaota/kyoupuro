...
description: C++ solver を使う AHC プロジェクトで、tools/ 内の公式配布 ZIP を必要に応じて展開し、problem_description.txt と Rust 製の公式 tools/src をもとに、同梱テンプレートから case visualizer / eval viewer / WASM 接続を完成させる。
---
...
- `tools/Cargo.toml` が存在する
- `scripts/build_solver.sh` が存在する
