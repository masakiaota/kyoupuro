// variable_turn_rollback_beam.cpp
#include <bits/stdc++.h>
...
struct Action {
    ...
    static Action sample(uint8_t id, size_t step_count) {
        return {
            id,
            step_count,
            10 * static_cast<int64_t>(step_count) - static_cast<int64_t>(id),
            0xA0761D6478BD642FULL * (static_cast<uint64_t>(id) + step_count + 1),
        };
    }
...
public:
    ...
    vector<Action> enumerate_actions([[maybe_unused]] size_t turn) const {
        return {Action::sample(0, 1), Action::sample(1, 2)};
    }
...
vector<Action> rollback_beam_search(State state, size_t max_turn, size_t beam_width) {
    ...
    vector<vector<size_t>> beams_by_turn(max_turn + 1);
    beams_by_turn[0].push_back(0);

    vector<Selector> selectors;
    selectors.reserve(max_turn + 1);
    for (size_t turn = 0; turn <= max_turn; ++turn) {
        selectors.emplace_back(beam_width);
    }

    optional<size_t> best_leaf;
    for (size_t turn = 0; turn <= max_turn; ++turn) {
        for (const BeamCandidate& candidate : selectors[turn].take_sorted()) {
            nodes.push_back({
                candidate.parent,
                candidate.action,
                candidate.evaluator,
            ...
            beams_by_turn[turn].push_back(nodes.size() - 1);
        }

        if (beams_by_turn[turn].empty()) {
            continue;
        }

        const size_t local_best = *ranges::min_element(beams_by_turn[turn], {}, [&](size_t node) {
            return nodes[node].evaluator;
        });
        if (!best_leaf || nodes[local_best].evaluator < nodes[*best_leaf].evaluator) {
            best_leaf = local_best;
        }

        for (const TourEdge edge : build_tour_edges(beams_by_turn[turn], nodes)) {
            ...
            if (edge.type == TourEdgeType::Forward) {
                ...
                assert(node.turn == turn);
                if (state.is_terminal()) {
                    continue;
                }
                for (const Action action : state.enumerate_actions(turn)) {
                    assert(action.step() > 0);
                    const size_t next_turn = turn + action.step();
                    if (next_turn > max_turn) {
                        continue;
                    }
                    state.move_forward(action);
                    selectors[next_turn].push({
                        edge.node,
                        action,
                        state.evaluate(next_turn),
                        state.hash_key(),
                        next_turn,
                    });
    ...
    return reconstruct_actions(nodes, best_leaf.value_or(0));
}

int main() {
    [[maybe_unused]] const vector<Action> actions = rollback_beam_search(State{}, 5, 4);
    return 0;
}
