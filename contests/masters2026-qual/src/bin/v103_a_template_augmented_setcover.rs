// v103_a_template_augmented_setcover.rs
...
const TEMPLATE_LIBRARY_A: &[TemplateDef] = &[
    TemplateDef {
        m: 9,
        rules: &[
            (ACT_R, 6, ACT_R, 3),
...
fn add_template_candidates(env: &Env, best: &mut HashMap<BitSet, RobotSpec>) {
    for tpl in TEMPLATE_LIBRARY_A {
        if tpl.m == 0 || tpl.m > 4 * N_FIXED * N_FIXED || tpl.rules.len() != tpl.m {
            continue;
        }
        let mut a0 = Vec::<u8>::with_capacity(tpl.m);
        let mut b0 = Vec::<usize>::with_capacity(tpl.m);
        let mut a1 = Vec::<u8>::with_capacity(tpl.m);
        let mut b1 = Vec::<usize>::with_capacity(tpl.m);
        ...
        collect_from_automaton(env, tpl.m, &a0, &b0, &a1, &b1, best);
    }
}
...
fn main() {
    ...
    add_template_candidates(&env, &mut best);
