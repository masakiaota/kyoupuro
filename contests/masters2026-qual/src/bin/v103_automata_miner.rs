...
fn compute_fitness(aut: &Automaton, met: &EvalMetrics, maps: usize) -> i64 {
    let m = aut.m as i64;
    ...
    fn dfs(
        ...
        if i == m {
            ...
            if let Some(met) = evaluate_candidate(&aut, envs, cfg.quick_cover_floor) {
                let fit = compute_fitness(&aut, &met, envs.len());
                let cand = Cand {
                    aut,
                    metrics: met,
                    fitness: fit,
...
fn guided_expand(
    ...
    let mut rng = XorShift64::new(cfg.seed ^ 0xC2B2AE3D27D4EB4F);
    ...
    for m in (cfg.exact_m + 1)..=cfg.max_m {
        ...
        for _ in 0..cfg.trials_per_m {
            let p = &parents[rng.gen_usize(parents.len())];
...
fn write_library(path: &str, picked: &[Cand], maps: usize, cfg: &Config) -> io::Result<()> {
    ...
    writeln!(
        f,
        "# automata library generated by v103_automata_miner\n# problem={} input_dir={} stride={} max_seed={} exact_m={} max_m={} maps={}\n",
        cfg.problem, cfg.input_dir, cfg.stride, cfg.max_seed, cfg.exact_m, cfg.max_m, maps
    )?;
