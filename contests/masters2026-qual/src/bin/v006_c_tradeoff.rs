...
const DEFAULT_SEARCH_TIME_MS: u64 = 1100;
const DEFAULT_BASE_RANDOM_TRIALS: usize = 3200;
const DEFAULT_MAX_RANDOM_M: usize = 5;
const DEFAULT_FORCED_TOP: usize = 96;
const FULL_ROUTE_TRIALS: usize = 220;
...
fn main() {
    ...
    // C は K が重いので、より大きい状態数候補を厚く生成する。
    let max_random_m = if input.ak >= 900 && input.am <= 2 {
        6
    } else {
        DEFAULT_MAX_RANDOM_M
    };
    let base_trials = if input.ak >= 900 && input.am <= 2 {
        DEFAULT_BASE_RANDOM_TRIALS + 1000
    } else {
        DEFAULT_BASE_RANDOM_TRIALS
    };
    let route_trials = if input.ak >= 900 && input.am <= 2 {
        FULL_ROUTE_TRIALS + 120
    } else {
        FULL_ROUTE_TRIALS
    };
    ...
    if let Some(full) = build_full_route_candidate(&input, &env, route_trials) {
