...
const DEFAULT_SEARCH_TIME_MS: u64 = 980;
const INITIAL_NN_TOURS: usize = 14;
const EVAL_EXTRA_SHIFTS: usize = 16;
const OFFSET_TRIAL_LIMIT: usize = 192;
...
fn build_segment_plan_for_offset(
    ...
    while i < m {
        ...
        if has_wall(input, end_cell / input.n, end_cell % input.n, end_dir) {
            compressible = true;
        } else if allow_add_walls {
...
fn collect_offset_candidates(actions: &[u8], seed: u64) -> Vec<usize> {
    ...
    let mut scored = Vec::<(usize, usize)>::new(); // (score, offset)
    for &st in &starts {
        let mut run_len = 0usize;
        let mut p = st;
        while run_len < m && actions[p] == ACT_F {
            run_len += 1;
            p = (p + 1) % m;
        }
        // 長い直進runを優先しつつ、微差は offset で安定化
        scored.push((run_len * 2048 + (m - st), st));
    }
...
fn choose_best_plan(input: &Input, route: &RouteCandidate) -> Option<RobotPlan> {
    ...
    let allow_add_walls = input.aw * 8 <= input.am * 11;
