...
const DEFAULT_BASE_M3_TRIALS: usize = 3000;
...
fn add_sampled_candidates_m3(
    ...
    for _ in 0..trials {
        let m = 3usize;
        let a0v = vec![
            rng.gen_usize(3) as u8,
            rng.gen_usize(3) as u8,
            rng.gen_usize(3) as u8,
        ];
        let b0v = vec![rng.gen_usize(m), rng.gen_usize(m), rng.gen_usize(m)];
        let a1v = vec![rng.gen_usize(2) as u8, rng.gen_usize(2) as u8, rng.gen_usize(2) as u8];
        let b1v = vec![rng.gen_usize(m), rng.gen_usize(m), rng.gen_usize(m)];
        collect_from_automaton(env, m, &a0v, &b0v, &a1v, &b1v, best);
    }
}

fn add_sampled_candidates_m3_m4_timed(
    env: &Env,
    best: &mut HashMap<BitSet, RobotSpec>,
    rng: &mut XorShift64,
    deadline: Instant,
    ...
    while Instant::now() < deadline {
        let m = if rng.gen_usize(100) < 75 { 3usize } else { 4usize };
...
fn build_full_route_candidate(input: &Input, env: &Env) -> Option<RobotSpec> {
    let n = input.n;
    let cells = n * n;
...
fn local_search_add_remove(
    ...
    while Instant::now() < deadline {
        ...
        let old_m = m_sum;
        let old_v = input.ak * (old_k as i64 - 1) + input.am * old_m as i64;
        let new_v = input.ak * (new_k as i64 - 1) + input.am * new_m as i64;
        if (new_v, new_m, new_k) >= (old_v, old_m, old_k) {
            continue;
        }
...
fn pick_solution_timed(input: &Input, cands: &[RobotSpec], deadline: Instant, seed: u64) -> Vec<usize> {
    ...
    if Instant::now() < deadline {
        ...
        local_search_add_remove(input, cands, &cover_cells, &mut best_sel, deadline, &mut rng);
    }
    best_sel
}
...
fn main() {
    ...
    add_sampled_candidates_m3(&env, &mut best, &mut rng, DEFAULT_BASE_M3_TRIALS);
    let gen_deadline = start + Duration::from_millis(limit_ms.saturating_mul(45) / 100);
    if Instant::now() < gen_deadline {
        add_sampled_candidates_m3_m4_timed(&env, &mut best, &mut rng, gen_deadline);
    }
    if let Some(full) = build_full_route_candidate(&input, &env) {
        add_candidate(&mut best, full);
    }

    let mut cands: Vec<RobotSpec> = best.into_values().collect();
    cands.sort_by_key(|c| c.m);
    let selected = pick_solution_timed(&input, &cands, deadline, seed_from_input(&input));
