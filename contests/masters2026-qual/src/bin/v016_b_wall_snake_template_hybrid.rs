// v016_b_wall_snake_template_hybrid.rs
...
const TEMPLATE_ROUTE_INIT_LIMIT: usize = 6;
...
fn bitset_count_total(bits: &BitSet) -> u32 {
    bits.w.iter().map(|x| x.count_ones()).sum()
}

fn extract_order_from_route_cycle(input: &Input, route: &RouteCandidate) -> Option<Vec<usize>> {
...
fn collect_template_route_initial_orders(input: &Input, limit: usize) -> Vec<Vec<usize>> {
    if limit == 0 {
        return Vec::new();
    }
    ...
    let mut idxs: Vec<usize> = (0..cands.len()).collect();
    idxs.sort_by(|&i, &j| {
        let ci = bitset_count_total(&cands[i].cover);
        let cj = bitset_count_total(&cands[j].cover);
        cj.cmp(&ci)
            .then_with(|| cands[i].m.cmp(&cands[j].m))
            .then_with(|| i.cmp(&j))
    });
    ...
    for &idx in &idxs {
        ...
        if let Some(route) = template_choice_to_route_candidate(input, &cands[idx]) {
            if let Some(order) = extract_order_from_route_cycle(input, &route) {
                let mut h = 1469598103934665603u64;
                for &v in &order {
                    h ^= v as u64 + 1;
                    h = h.wrapping_mul(1099511628211);
...
fn best_route(input: &Input) -> Option<RouteCandidate> {
    ...
    let template_orders = collect_template_route_initial_orders(input, TEMPLATE_ROUTE_INIT_LIMIT);
    for ord in template_orders {
        initials.push(ord);
    }
...
fn minimize_auto_states(states: &[AutoState]) -> Vec<AutoState> {
    let n = states.len();
    if n <= 1 {
    ...
    let mut old_to_reach = vec![usize::MAX; n];
    let mut reach = Vec::<AutoState>::new();
    for i in 0..n {
        if vis[i] {
            old_to_reach[i] = reach.len();
            reach.push(states[i]);
        }
    }
    if reach.is_empty() {
    ...
    for st in &mut reach {
        st.b0 = if st.b0 < n && vis[st.b0] {
            old_to_reach[st.b0]
        } else {
            0
        };
        st.b1 = if st.b1 < n && vis[st.b1] {
            old_to_reach[st.b1]
        } else {
            0
        };
    ...
    let rn = reach.len();
    let mut cls = vec![0usize; rn];
    let mut by_out = HashMap::<(u8, u8), usize>::new();
    let mut cls_cnt = 0usize;
    for i in 0..rn {
        let key = (reach[i].a0, reach[i].a1);
        let id = *by_out.entry(key).or_insert_with(|| {
            let x = cls_cnt;
            cls_cnt += 1;
            x
        });
    ...
    loop {
        let mut nxt = vec![0usize; rn];
        let mut mp = HashMap::<(u8, u8, usize, usize), usize>::new();
        let mut nxt_cnt = 0usize;
        for i in 0..rn {
            let key = (reach[i].a0, reach[i].a1, cls[reach[i].b0], cls[reach[i].b1]);
            let id = *mp.entry(key).or_insert_with(|| {
                let x = nxt_cnt;
                nxt_cnt += 1;
                x
            });
    ...
    let mut rep = vec![usize::MAX; cls_cnt];
    for i in 0..rn {
        if rep[cls[i]] == usize::MAX {
            rep[cls[i]] = i;
        }
    }

    let start_cls = cls[0];
    let mut cls_order = Vec::<usize>::new();
    let mut q = VecDeque::<usize>::new();
    let mut cls_vis = vec![false; cls_cnt];
    cls_vis[start_cls] = true;
    q.push_back(start_cls);
    while let Some(c) = q.pop_front() {
        cls_order.push(c);
        let r = rep[c];
        let to0 = cls[reach[r].b0];
        let to1 = cls[reach[r].b1];
        if !cls_vis[to0] {
            cls_vis[to0] = true;
            q.push_back(to0);
        }
    ...
    let mut cls_new = vec![usize::MAX; cls_cnt];
    for (ni, &oc) in cls_order.iter().enumerate() {
        cls_new[oc] = ni;
    }
    ...
    for oc in 0..cls_cnt {
        ...
        out[ni] = AutoState {
            a0: reach[r].a0,
            b0: cls_new[cls[reach[r].b0]],
            a1: reach[r].a1,
            b1: cls_new[cls[reach[r].b1]],
        };
    }
    out
}
...
fn build_explicit_plan(input: &Input, route: &RouteCandidate) -> Option<RobotPlan> {
    ...
    let states = minimize_auto_states(&states);
    ...
    let plan = RobotPlan {
        ...
        wall_h_add,
        w_count: 0,
        value_v: compute_plan_value(input, m.min(M_LIMIT), 0),
    };
...
fn build_segment_plan_for_offset(
    ...
    states = minimize_auto_states(&states);
    if states.is_empty() || states.len() > M_LIMIT {
        return None;
    }
...
fn template_choice_to_route_candidate(input: &Input, c: &TemplateChoice) -> Option<RouteCandidate> {
    let m = c.m;
    if m == 0 || c.a0.len() != m || c.b0.len() != m || c.a1.len() != m || c.b1.len() != m {
        return None;
    }

    let total_states = N2 * 4 * m;
    let mut first = vec![-1i32; total_states];
    let mut cells = Vec::<usize>::new();
    let mut dirs = Vec::<usize>::new();
    let mut actions = Vec::<u8>::new();

    let mut cell = c.start_cell;
    let mut dir = c.start_dir;
    let mut s = 0usize;
    let cycle_start = loop {
        let sid = (cell * 4 + dir) * m + s;
        if first[sid] >= 0 {
            break first[sid] as usize;
        }
        first[sid] = cells.len() as i32;
        cells.push(cell);
        dirs.push(dir);

        let wall = has_wall(input, cell / input.n, cell % input.n, dir);
        let act = if wall { c.a1[s] } else { c.a0[s] };
        let ns = if wall { c.b1[s] } else { c.b0[s] };
        actions.push(act);

        match act {
            ACT_R | ACT_L => {
                dir = rotate_dir(dir, act);
            }
            ACT_F => {
                if !wall {
                    let i = cell / input.n;
    ...
    if cycle_start >= actions.len() {
        return None;
    }
    let route_actions = actions[cycle_start..].to_vec();
    if route_actions.is_empty() {
        return None;
    }

    let start_cell = cells[cycle_start];
    let start_dir = dirs[cycle_start];
    let mut cur_cell = start_cell;
    let mut cur_dir = start_dir;
    for &act in &route_actions {
        if act == ACT_F && has_wall(input, cur_cell / input.n, cur_cell % input.n, cur_dir) {
            return None;
        }
        let (nc, nd) = apply_action(input, cur_cell, cur_dir, act);
        cur_cell = nc;
        cur_dir = nd;
    }
    if cur_cell != start_cell || cur_dir != start_dir {
        return None;
    }
...
fn improve_template_single_with_wall(
    input: &Input,
    tp: &TemplatePlan,
    current_best: Option<RobotPlan>,
) -> Option<RobotPlan> {
    if tp.selected.len() != 1 {
        return current_best;
    }
    let idx = tp.selected[0];
    let choice = &tp.cands[idx];
    let Some(route) = template_choice_to_route_candidate(input, choice) else {
        return current_best;
    };
    let Some(plan) = choose_best_plan(input, &route) else {
        return current_best;
    };
    match current_best {
        None => Some(plan),
...
fn main() {
    ...
    let mut route_or_template1_best = route_best;
    if let Some(tp) = &template_best {
        route_or_template1_best =
            improve_template_single_with_wall(&input, tp, route_or_template1_best);
    }

    match (route_or_template1_best, template_best) {
        (Some(rp), Some(tp)) => {
