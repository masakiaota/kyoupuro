...
const DEFAULT_BASE_RANDOM_TRIALS: usize = 3400;
const DEFAULT_MAX_RANDOM_M: usize = 5;
const DEFAULT_FORCED_TOP: usize = 88;
const FULL_ROUTE_TRIALS: usize = 140;
...
fn main() {
    ...
    // B は AK が重いので、AM が小さいケースほど高状態候補を厚く生成する。
    let max_random_m = if input.am <= 3 { 6 } else { DEFAULT_MAX_RANDOM_M };
    let base_trials = if input.am <= 3 {
        DEFAULT_BASE_RANDOM_TRIALS + 1200
    } else {
        DEFAULT_BASE_RANDOM_TRIALS
    };
    let route_trials = if input.am <= 2 {
        FULL_ROUTE_TRIALS + 80
    } else {
        FULL_ROUTE_TRIALS
    };
