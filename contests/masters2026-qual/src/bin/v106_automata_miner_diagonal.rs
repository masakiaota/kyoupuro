...
impl Config {
    fn from_args() -> Self {
        ...
        for arg in std::env::args().skip(1) {
            ...
            } else if let Some(v) = arg.strip_prefix("--quick-diag2-floor=") {
                cfg.quick_diag2_floor = v.parse().unwrap_or(cfg.quick_diag2_floor);
            } else if let Some(v) = arg.strip_prefix("--diag2-weight=") {
                cfg.diag2_weight = v.parse().unwrap_or(cfg.diag2_weight);
...
fn diag2_count_from_cycle(cyc: &[usize], m: usize) -> i32 {
    let len = cyc.len();
    if len < 3 {
        return 0;
    }
    let mut cells = Vec::<usize>::with_capacity(len);
    for &x in cyc {
        let o = x / m;
        cells.push(o / 4);
    }
    ...
    for t in 0..len {
        let c0 = cells[t];
        let c2 = cells[(t + 2) % len];
        let r0 = (c0 / N_FIXED) as i32;
        let c0v = (c0 % N_FIXED) as i32;
        let r2 = (c2 / N_FIXED) as i32;
        let c2v = (c2 % N_FIXED) as i32;
        if (r0 - r2).abs() == 1 && (c0v - c2v).abs() == 1 {
            cnt += 1;
        }
    }
    cnt
...
fn analyze_on_env(env: &Env, aut: &Automaton) -> (i32, i32, i32, BitSet) {
    let m = aut.m;
    let total = ORIENTS * m;
    ...
    let mut comp_diag2 = Vec::<i32>::new();
    ...
    for st in 0..total {
        ...
        let cid = if mark[u] == 1 {
            ...
            comp_diag2.push(diag2_count_from_cycle(cyc, m));
            for &x in cyc {
                comp[x] = id;
            }
            id
    ...
    for o in 0..ORIENTS {
        ...
        let d2 = comp_diag2[cid];
        if c > best_cover
            || (c == best_cover && (d2 > best_diag2 || (d2 == best_diag2 && cyc < best_cycle)))
        {
            best_cover = c;
            best_diag2 = d2;
            best_cycle = cyc;
    ...
    (best_cover, best_cycle, best_diag2, best_bits)
}
...
fn evaluate_candidate(
    ...
    let (q_cover, _, q_diag2, _) = analyze_on_env(&envs[0], aut);
    if q_cover < quick_cover_floor {
        return None;
    }
    if q_diag2 < quick_diag2_floor {
        return None;
    }
    ...
    let mut total_diag2 = 0i32;
    let mut min_diag2 = i32::MAX;
    let mut fingerprint = BitSet::empty();

    for (idx, env) in envs.iter().enumerate() {
        let (cover, cyc, d2, bits) = analyze_on_env(env, aut);
        total_cover += cover;
        min_cover = min_cover.min(cover);
        total_cycle_len += cyc;
        total_diag2 += d2;
        min_diag2 = min_diag2.min(d2);
        if idx < 8 {
            for k in 0..BIT_WORDS {
...
fn compute_fitness(problem: char, aut: &Automaton, met: &EvalMetrics, maps: usize, diag2_weight: i64) -> i64 {
    let m = aut.m as i64;
    ...
    let total_diag2 = met.total_diag2 as i64;
    let min_diag2 = met.min_diag2 as i64;
    let proxy_diag2 = met.proxy_diag2 as i64;
    let base = total_cover * 10_000
        + min_cover * 3_000
        + proxy * 1_000
        + total_diag2 * diag2_weight
        + min_diag2 * (diag2_weight / 3)
        + proxy_diag2 * (diag2_weight / 5)
        - cyc;
    match problem {
...
fn exact_enumerate_m(
    ...
    fn dfs(
        ...
        if i == m {
            ...
            if let Some(met) =
                evaluate_candidate(&aut, envs, cfg.quick_cover_floor, cfg.quick_diag2_floor)
            {
                let fit = compute_fitness(cfg.problem, &aut, &met, envs.len(), cfg.diag2_weight);
                let cand = Cand {
                    aut,
                    metrics: met,
                    fitness: fit,
...
fn diagonal_seed_automata(m: usize) -> Vec<Automaton> {
    if m == 0 {
        return Vec::new();
    }
    let mut out = Vec::<Automaton>::new();
    for mode in 0..4 {
        let mut rules = vec![
            Rule {
                a0: ACT_F,
        ...
        for i in 0..m {
            let ni = (i + 1) % m;
            let parity = ((i + mode) & 1) == 0;
            let turn_a = if parity { ACT_R } else { ACT_L };
            let turn_b = if parity { ACT_L } else { ACT_R };
            rules[i].a0 = ACT_F;
            rules[i].b0 = ni as u8;
            if mode < 2 {
                rules[i].a1 = turn_a;
...
fn guided_expand(
    ...
    for m in (cfg.exact_m + 1)..=cfg.max_m {
        ...
        for aut in diagonal_seed_automata(m) {
            if let Some(met) =
                evaluate_candidate(&aut, envs, cfg.quick_cover_floor, cfg.quick_diag2_floor)
            {
                let fit = compute_fitness(cfg.problem, &aut, &met, envs.len(), cfg.diag2_weight);
        ...
        for tid in 0..threads {
            ...
            let quick_diag2_floor = cfg.quick_diag2_floor;
            let problem = cfg.problem;
            let diag2_weight = cfg.diag2_weight;
            ...
            handles.push(thread::spawn(move || {
                ...
                for _ in 0..trials {
                    ...
                    if let Some(met) = evaluate_candidate(
                        &aut,
                        &envs_local,
                        quick_cover_floor,
                        quick_diag2_floor,
                        ...
                        let fit =
                            compute_fitness(problem, &aut, &met, envs_local.len(), diag2_weight);
                        let cand = Cand {
                            aut,
                            metrics: met,
                            fitness: fit,
...
fn write_library(path: &str, picked: &[Cand], maps: usize, cfg: &Config) -> io::Result<()> {
    ...
    writeln!(
        f,
        "# automata library generated by v106_automata_miner_diagonal\n# problem={} input_dir={} stride={} max_seed={} exact_m={} max_m={} maps={} parallel={} diag2_weight={} quick_diag2_floor={}\n",
        cfg.problem,
        cfg.input_dir,
        cfg.stride,
        cfg.max_seed,
        ...
        cfg.parallel,
        cfg.diag2_weight,
        cfg.quick_diag2_floor
    )?;

    for (idx, c) in picked.iter().enumerate() {
        writeln!(
            f,
            "[candidate {}] m={} fitness={} avg_cover={:.3} min_cover={} avg_cycle={:.3} avg_diag2={:.3} min_diag2={} proxy_cover={} proxy_diag2={} origin={}",
            idx,
            c.aut.m,
            c.fitness,
            c.avg_cover(maps),
            c.metrics.min_cover,
            c.avg_cycle(maps),
            c.metrics.total_diag2 as f64 / maps as f64,
            c.metrics.min_diag2,
            c.metrics.proxy_cover,
            c.metrics.proxy_diag2,
            c.origin
        )?;
...
fn main() -> io::Result<()> {
    ...
    eprintln!(
        "v106_miner start: problem={} exact_m={} max_m={} trials_per_m={} parallel={} stride={} max_seed={} diag2_weight={} quick_diag2_floor={}",
        cfg.problem,
        cfg.exact_m,
        cfg.max_m,
        cfg.trials_per_m,
