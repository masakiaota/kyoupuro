// v105_c_wall_snake_cross_library_hybrid_fast.rs
...
const TEMPLATE_LIBRARY_C: &[TemplateDef] = &[
    TemplateDef {
        m: 14,
        rules: &[
            (ACT_L, 11, ACT_R, 9),
...
fn dedup_template_candidates_by_cover(cands: Vec<TemplateChoice>) -> Vec<TemplateChoice> {
    let mut best = HashMap::<BitSet, TemplateChoice>::new();
    for cand in cands {
        if let Some(cur) = best.get_mut(&cand.cover) {
            if cand.m < cur.m {
                *cur = cand;
            }
        } else {
...
fn build_template_cover_plan(input: &Input) -> Option<TemplatePlan> {
    ...
    cands = dedup_template_candidates_by_cover(cands);
