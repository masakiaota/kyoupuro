// probe_repack_fail.rs — v007 に repack 失敗の原因分類トレースを足した分析用 probe
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: 移動プランを「最初の採算成功」で決めず、attempt 上限 12 で採算を満たす
// プランを最大 3 つ収集し、repack 後盤面の断片化メトリック最小のプランを採用する。
// 採算は制約・盤面品質は目的と分けることで、深い探索の利益 (0077 +1.6%) を
// 質の低い移動の害 (0002 -4.0%) なしに取り込む。
...
const RELOC_TIME_BUDGET_RATIO: f64 = 0.65;
...
impl Solver {
    ...
    fn repack_blockers(
        ...
        for group_id in order {
            ...
            for state in &beam {
                ...
                // まず worst 以下 (C 無損失) を探し、無ければ worst+2 の 1 段だけ損失込みで許す。
                // 逃げ場不足による repack 失敗 (v501 実測 5/277 成功) を減らすため。
                for perimeter in (minimum_perimeter(P)..=worst_perimeter + 2).step_by(2) {
                    candidates =
            ...
            if next_beam.is_empty() {
                // 失敗の主原因を分類: 先頭 beam 状態の盤面で、この blocker の
                // 最大空き成分が P 未満なら行き場の総量不足、そうでなければ形状不足。
                local! {
                    let info = self.compute_free_info(&beam[0].occ, false);
                    let max_comp = info.sizes.iter().copied().max().unwrap_or(0);
                    if max_comp < P {
                        self.trace.count("repack_fail_component_short");
                    } else {
                        self.trace.count("repack_fail_shape_short");
                    }
                    self.trace.count_by("repack_fail_blocker_P", P as i64);
                }
                return None;
            }
    ...
    fn attempt_relocation(
        ...
        // 断片化メトリック 1 単位を金額へ換算する係数。メトリックは将来の配置可能性の
        // 損失を weight スケールで表しており、プラン間の net 差 (数万〜十数万) と
        // メトリック差 (数十) を同じ土俵に載せるための重みである。
        const METRIC_TO_MONEY: f64 = 800.0;
        let mut best_index = 0;
        let mut best_key = collected[0].1 - METRIC_TO_MONEY * collected[0].2;
        ...
        local! {
            ...
            if best_index != 0 {
                self.trace.count("reloc_plan_switched");
            }
            let (plan, _, _, cost, fee_loss) = &collected[best_index];
            self.trace
        ...
        collected.swap_remove(best_index).0
    }
        ...
        for turn in 0..self.M {
            ...
            if !fast_mode && reloc_worth_trying && q_value * optimistic_C >= 0.88 * base_threshold {
                ...
                move_plan = local_time!(self.trace, "relocation", {
                    self.attempt_relocation(&incoming, q_value, base_threshold, baseline)
                });
