// v034_no_move_growth_topology.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。

// 中心アイデア: v029 の growth 候補を、現行評価を悪化させずに
// 元の自由成分から切り離すセル数を減らす同一 fee 候補へ置き換える。
// 再移動は一切行わない。
...
impl Solver {
    ...
    /// 配置前に属した自由成分だけを対象に、配置によって最大残余成分から
    /// 切り離されるセル数を求める。無関係な自由成分は比較に混ぜない。
    fn growth_severed(&self, P: usize, candidate: &Placement, info: &FreeInfo) -> usize {
        debug_assert_eq!(candidate.explicit_cells.len(), P);
        let component_id = info.component[candidate.explicit_cells[0]];
        debug_assert!(component_id >= 0);
        let component_id = component_id as usize;
        debug_assert_eq!(info.sizes[component_id], candidate.component_size);

        let mut removed = vec![false; self.N * self.N];
        for &id in &candidate.explicit_cells {
            debug_assert_eq!(info.component[id], component_id as isize);
            debug_assert!(!removed[id]);
            removed[id] = true;
        }

        let mut visited = vec![false; self.N * self.N];
        let mut stack = Vec::with_capacity(info.sizes[component_id]);
        let mut largest_remaining = 0;
        let mut remaining_count = 0;
        const DIRS: [(isize, isize); 4] = [(-1, 0), (1, 0), (0, -1), (0, 1)];
        for &start in &info.cells[component_id] {
            ...
            while let Some(id) = stack.pop() {
                ...
                for (dr, dc) in DIRS {
                    ...
                    if !visited[next]
                        && !removed[next]
                        && info.component[next] == component_id as isize
                    {
                        visited[next] = true;
                        stack.push(next);
                    }
            ...
            largest_remaining = largest_remaining.max(size);
        }
        let remaining = info.sizes[component_id].saturating_sub(P);
        debug_assert_eq!(remaining_count, remaining);
        remaining.saturating_sub(largest_remaining)
    }
    ...
    fn growth_placement(
        ...
        // まず v029 と完全に同じ対象・順序・評価で baseline winner を決める。
        // topology の比較はその後だけに行い、候補生成や既存の探索を変えない。
        let mut evaluated: Vec<(usize, Placement, f64)> = Vec::new();
        let mut baseline_evaluated_index: Option<usize> = None;
        for (index, mut candidate) in candidates.into_iter().enumerate() {
            ...
            if baseline_evaluated_index
                .is_none_or(|current| candidate.final_score > evaluated[current].1.final_score)
            {
                baseline_evaluated_index = Some(evaluated.len());
            }
            evaluated.push((index, candidate, delta));
        }

        let baseline_evaluated_index = baseline_evaluated_index?;
        let baseline_perimeter = evaluated[baseline_evaluated_index].1.perimeter;
        let baseline_component_size = evaluated[baseline_evaluated_index].1.component_size;
        let baseline_fragment_delta = evaluated[baseline_evaluated_index].2;
        let baseline_severed = self.growth_severed(P, &evaluated[baseline_evaluated_index].1, info);
        let mut selected_evaluated_index = baseline_evaluated_index;
        let mut selected_severed = baseline_severed;
        let mut _same_fee_candidates = 0_i64;
        let mut _pareto_size = 0_i64;
        const FRAGMENT_EPS: f64 = 1e-9;
        for (evaluated_index, (_, candidate, fragment_delta)) in evaluated.iter().enumerate() {
            if evaluated_index == baseline_evaluated_index
                || candidate.perimeter != baseline_perimeter
                || candidate.component_size != baseline_component_size
            {
                continue;
            }
            _same_fee_candidates += 1;
            let severed = self.growth_severed(P, candidate, info);
            local! {
                self.trace.count("growth_topology_comparison");
            }
            if severed >= baseline_severed
                || *fragment_delta > baseline_fragment_delta + FRAGMENT_EPS
            {
                continue;
            }
            _pareto_size += 1;
            if severed < selected_severed
                || (severed == selected_severed
                    && candidate.final_score > evaluated[selected_evaluated_index].1.final_score)
            {
                selected_evaluated_index = evaluated_index;
                selected_severed = severed;
            }
        }
        local! {
            self.trace.count_by(
                "growth_topology_same_fee_candidate",
                _same_fee_candidates,
            );
            self.trace
                .count_by("growth_topology_pareto_size", _pareto_size);
        }

        if selected_evaluated_index != baseline_evaluated_index {
            let baseline = &evaluated[baseline_evaluated_index];
            let selected = &evaluated[selected_evaluated_index];
            debug_assert_eq!(selected.1.perimeter, baseline.1.perimeter);
            debug_assert_eq!(selected.1.component_size, baseline.1.component_size);
            debug_assert!(selected.2 <= baseline.2 + FRAGMENT_EPS);
            debug_assert!(selected_severed < baseline_severed);
            local! {
                self.trace.count("growth_topology_flip");
                self.trace.count_by(
                    "growth_topology_severed_before",
                    baseline_severed as i64,
                );
                self.trace.count_by(
                    "growth_topology_severed_after",
                    selected_severed as i64,
                );
                self.trace.count_by(
                    "growth_topology_severed_reduction",
                    (baseline_severed - selected_severed) as i64,
                );
                self.trace.count_by(
                    "growth_topology_fragment_before",
                    (1000.0 * baseline.2).round() as i64,
                );
                self.trace.count_by(
                    "growth_topology_fragment_after",
                    (1000.0 * selected.2).round() as i64,
                );
                self.trace.count_by(
                    "growth_topology_cheap_loss",
                    (baseline.1.cheap_score - selected.1.cheap_score).round() as i64,
                );
                let slack = baseline.1.perimeter - minimum_perimeter(P);
                if P >= 96 {
                    self.trace.count("growth_topology_flip_p96");
                } else if P >= 64 {
                    self.trace.count("growth_topology_flip_p64_95");
                } else {
                    self.trace.count("growth_topology_flip_p0_63");
                }
                if slack >= 8 {
                    self.trace.count("growth_topology_flip_slack8");
                } else if slack >= 4 {
                    self.trace.count("growth_topology_flip_slack4_6");
                } else {
                    self.trace.count("growth_topology_flip_slack0_2");
                }
                let baseline_is_box = _box_index.is_some_and(|index| index == baseline.0);
                let selected_is_box = _box_index.is_some_and(|index| index == selected.0);
                if baseline_is_box && !selected_is_box {
                    self.trace.count("growth_topology_box_to_seed");
                } else if !baseline_is_box && selected_is_box {
                    self.trace.count("growth_topology_seed_to_box");
                }
            }
        }

        let (selected_origin_index, selected_candidate, _) =
            evaluated.swap_remove(selected_evaluated_index);
        let best = Some((selected_origin_index, selected_candidate));
