...
// 中心アイデア: growth (自由形、slack 20〜40 になりがち) に落ちる前に、
// L_min+8/+10 の深い周長レベルの規則形状で受ける。形状生成の上限を L_min+12 へ、
// 通常探索の周長範囲を L_min+10 へ広げ、同じ受け入れをより良い C で実現する。
...
impl Solver {
    ...
    fn find_normal_placement(
        ...
        let min_L = minimum_perimeter(P);
        let max_extra = if fast_mode { 2 } else { 10 };
        ...
        for perimeter in (min_L..=min_L + max_extra).step_by(2) {
            ...
            local! {
                if let Some(candidate) = &best {
                    ...
                    if candidate.perimeter >= min_L + 8 {
                        self.trace.count("deep_level_placed");
                    }
                }
            }
            return best;
