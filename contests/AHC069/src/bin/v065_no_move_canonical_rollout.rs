// v065_no_move_canonical_rollout.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。

// 中心アイデア: 実配置はv047のまま固定し、rollout内の将来配置だけを一つの
// canonical near-rectangle形状族へ縮約する。節約時間は同じ22到着のscenario数へ回す。
...
/// 共通乱数のサンプル本数。保存済みv047盤面の補助benchでkernelが約2.2倍になったため、
/// 事前登録したfloor(3*時間比)を3..=9へ丸める規則から6本に確定した。
const ROLLOUT_SAMPLES: usize = 6;
...
/// rollout専用の一つのcanonical形状と、その回転からなる小さな形状族。
/// max_runを保持し、全長1..Nのrun tableを毎回構築しない。
#[derive(Clone, Default)]
struct RolloutShapeFamily {
    shapes: Vec<Shape>,
    max_run: usize,
}
...
struct Solver {
    ...
    rollout_shape_family: Vec<RolloutShapeFamily>,
...
/// 行ごとに左詰めしたnear-rectangleを作る。
/// `w=ceil(P/h)`とし、`h*w-P`マスを右下の角から一列ずつ削るため、
/// 各行は長さwまたはw-1の一つの区間になり、面積Pと連結性を保つ。
fn make_corner_trimmed_shape(P: usize, h: usize) -> Option<Shape> {
    if h == 0 || h > P {
        return None;
    }
    let w = P.div_ceil(h);
    ...
    let missing = h * w - P;
    debug_assert!(missing < h);
    let mut len = vec![w; h];
    for value in len.iter_mut().skip(h - missing) {
        *value -= 1;
    }
    if len.iter().any(|&value| value == 0) {
        return None;
    }
    let mut shape = Shape {
        h,
...
fn rotate_shape_clockwise(shape: &Shape, P: usize) -> Shape {
    let mut cells = Vec::with_capacity(P);
    for r in 0..shape.h {
        for c in shape.left[r]..(shape.left[r] + shape.len[r]) {
            cells.push((c, shape.h - 1 - r));
        }
    }

    let h = shape.w;
    let w = shape.h;
    let mut left = vec![usize::MAX; h];
    ...
    let mut len = vec![0; h];
    for r in 0..h {
        assert!(left[r] != usize::MAX);
        len[r] = right[r] - left[r] + 1;
        assert_eq!(
            len[r], count[r],
            "canonical rotation must remain row-convex"
        );
    }
    ...
    rotated.perimeter = shape_perimeter(&rotated, P);
    rotated
}

/// 各Pについて、次の辞書順で一つの基準形状を選ぶ。
/// (実周長, 外接長方形の余白, 縦横差, 長辺, 高さ)
/// 配置時には同じ形状の回転だけを許し、別の欠け方やL_min+2形状は使わない。
fn generate_rollout_shape_families(N: usize) -> Vec<RolloutShapeFamily> {
    let mut families = vec![RolloutShapeFamily::default(); MAX_P + 1];
    for (P, family) in families.iter_mut().enumerate().take(MAX_P + 1).skip(4) {
        let mut best: Option<((usize, usize, usize, usize, usize), Shape)> = None;
        for h in 1..=N.min(P) {
            let Some(shape) = make_corner_trimmed_shape(P, h) else {
                continue;
            };
            if shape.w > N {
                continue;
        ...
        let (_, base) = best.expect("every P must have a canonical near-rectangle");
        assert_eq!(base.perimeter, minimum_perimeter(P));
        ...
        for _ in 0..4 {
            ...
            if seen.insert(key) {
                family.max_run = family.max_run.max(*shape.len.iter().max().unwrap());
                family.shapes.push(shape.clone());
            }
            shape = rotate_shape_clockwise(&shape, P);
        }
        family.shapes.sort_by(|a, b| {
            a.h.cmp(&b.h)
                .then_with(|| a.w.cmp(&b.w))
                .then_with(|| a.left.cmp(&b.left))
                .then_with(|| a.len.cmp(&b.len))
        });
        assert!(!family.shapes.is_empty());
        assert!(family.shapes.len() <= 4);
    }
    families
}
...
impl Solver {
    fn new(N: usize, M: usize, grass_rows: Rows, timer: TimeKeeper) -> Self {
        ...
        let mut solver = Self {
            ...
            rollout_shape_family: generate_rollout_shape_families(N),
        ...
        local! {
            ...
            let rollout_kept = solver
                .rollout_shape_family
                .iter()
                .map(|family| family.shapes.len())
                .sum::<usize>();
            solver
                .trace
                .count_by("canonical_rollout_variants", rollout_kept as i64);
            solver.trace.count_by(
                "canonical_rollout_samples_per_comparison",
                ROLLOUT_SAMPLES as i64,
            );
        }
        solver
    }
    ...
    /// rolloutのcanonical形状が実際に使う最大行長までだけrun maskを作る。
    /// 実配置用のbuild_run_tableは全長を必要とするため変更しない。
    fn build_run_table_limited(&self, occ: &Rows, max_len: usize) -> RunTable {
        debug_assert!(max_len <= self.N);
        let mut runs = [[0_u64; MAX_N + 1]; MAX_N];
        if max_len == 0 {
            return runs;
        }
    ...
    /// v047のrollout配置。補助benchで新方式との速度と受入可否を照合する場合だけ残す。
    #[cfg(feature = "local")]
    fn quick_place_reference(&self, occ: &Rows, P: usize) -> Option<(Vec<usize>, f64)> {
        let runs = self.build_run_table(occ);
        let min_L = minimum_perimeter(P);
    ...
    /// rollout用のcanonical配置。
    /// 一つの最小周長near-rectangleとその回転だけを、必要な最大行長までのrun maskで走査する。
    /// 戻り値の第2要素は、検査したshape数をtraceするための値である。
    fn quick_place(&self, occ: &Rows, P: usize) -> (Option<(Vec<usize>, f64)>, usize) {
        let family = &self.rollout_shape_family[P];
        let runs = self.build_run_table_limited(occ, family.max_run);
        let mut tested = 0;
        for shape in &family.shapes {
            ...
            for x in 0..=(self.N - shape.h) {
                ...
                if ys != 0 {
                    ...
                    return (Some((cells, compactness(P, shape.perimeter))), tested);
                }
            }
        }
        (None, tested)
    ...
    fn rollout_one(
        ...
        for arrival in arrivals {
            ...
            let (quick, _tested) = self.quick_place(&occ, arrival.P);
            local! {
                self.trace.count("canonical_rollout_call");
                self.trace
                    .count_by("canonical_rollout_shape_test", _tested as i64);
            }
            if let Some((cells, C)) = quick {
                local! {
                    self.trace.count("canonical_rollout_success");
                }
                ...
                local! {
                    self.trace.count("canonical_rollout_reject");
                }
            }
        }
        score
    ...
    fn evaluate_candidates_rollout(
        ...
        for k in 0..ROLLOUT_SAMPLES {
            ...
            local! {
                self.trace.count("canonical_rollout_scenario");
            }
            for (i, cand) in cands.iter().enumerate() {
...
#[cfg(feature = "local")]
pub fn verify_canonical_rollout_shapes() {
    let families = generate_rollout_shape_families(MAX_N);
    for P in 4..=MAX_P {
        let family = &families[P];
        assert!(!family.shapes.is_empty());
        assert!(family.shapes.len() <= 4);
        for shape in &family.shapes {
            assert!(shape.h <= MAX_N && shape.w <= MAX_N);
            assert_eq!(shape.perimeter, minimum_perimeter(P));
            assert_eq!(shape.len.iter().sum::<usize>(), P);
            assert!(shape.len.iter().copied().max().unwrap() <= family.max_run);

            let mut occupied = vec![false; shape.h * shape.w];
            for r in 0..shape.h {
                assert!(shape.len[r] > 0);
                assert!(shape.left[r] + shape.len[r] <= shape.w);
                for c in shape.left[r]..(shape.left[r] + shape.len[r]) {
                    occupied[r * shape.w + c] = true;
                }
            }
            let start = occupied.iter().position(|&used| used).unwrap();
            ...
            while let Some(cell) = queue.pop() {
                ...
                for (dr, dc) in [(-1_isize, 0_isize), (1, 0), (0, -1), (0, 1)] {
                    let nr = r as isize + dr;
                    let nc = c as isize + dc;
                    if nr < 0 || nr >= shape.h as isize || nc < 0 || nc >= shape.w as isize {
                        continue;
                    }
                    let next = nr as usize * shape.w + nc as usize;
        ...
        assert_eq!(
            ...
            family
                .shapes
                .iter()
                .flat_map(|shape| shape.len.iter().copied())
                .max()
                .unwrap()
        );
    }
...
#[cfg(feature = "local")]
#[derive(Debug)]
pub struct CanonicalRolloutBench {
    pub queries: usize,
    pub repeats: usize,
    pub reference_accepts: usize,
    pub canonical_accepts: usize,
    pub accept_agreements: usize,
    pub both_accepts: usize,
    pub reference_only: usize,
    pub canonical_only: usize,
    pub both_accept_same_compactness: usize,
    pub reference_ms: f64,
    pub canonical_ms: f64,
    pub speed_ratio: f64,
    pub recommended_samples: usize,
}

/// 保存済みv047出力をreplayして得た到着直前盤面で、旧quick_placeとcanonical版を照合する。
/// solverは起動せず、同じquery列を交互の順序で反復してkernel時間だけを測る。
#[cfg(feature = "local")]
pub fn benchmark_canonical_rollout(
    input_path: &str,
    output_path: &str,
    repeats: usize,
) -> CanonicalRolloutBench {
    use std::hint::black_box;

    assert!(repeats > 0);
    let input = std::fs::read_to_string(input_path).expect("read benchmark input");
    let mut tokens = input.split_whitespace();
    let N: usize = tokens.next().unwrap().parse().unwrap();
    let M: usize = tokens.next().unwrap().parse().unwrap();
    let _R: f64 = tokens.next().unwrap().parse().unwrap();
    assert_eq!(N, MAX_N);
    ...
    let mut T = vec![0_usize; M];
    let mut P = vec![0_usize; M];
    for _ in 0..M {
        let id: usize = tokens.next().unwrap().parse().unwrap();
        S[id] = tokens.next().unwrap().parse().unwrap();
        T[id] = tokens.next().unwrap().parse().unwrap();
        P[id] = tokens.next().unwrap().parse().unwrap();
        let _V: i64 = tokens.next().unwrap().parse().unwrap();
    }
    assert!(tokens.next().is_none());

    let output = std::fs::read_to_string(output_path).expect("read saved v047 output");
    let mut output_tokens = output.split_whitespace();
    let mut placements: Vec<Option<Vec<usize>>> = vec![None; M];
    for id in 0..M {
        let move_count: usize = output_tokens.next().unwrap().parse().unwrap();
        assert_eq!(
            move_count, 0,
            "v047 benchmark output must have no relocation"
        );
        let decision = output_tokens.next().unwrap();
        match decision {
            "Yes" => {
                let mut cells = Vec::with_capacity(P[id]);
                for _ in 0..P[id] {
                    let r: usize = output_tokens.next().unwrap().parse().unwrap();
                    let c: usize = output_tokens.next().unwrap().parse().unwrap();
                    cells.push(r * N + c);
                }
                placements[id] = Some(cells);
            }
            "No" => {}
            _ => panic!("unexpected decision token: {decision}"),
        }
    }
    assert!(output_tokens.next().is_none());

    let solver = Solver::new(N, M, grass_rows, TimeKeeper::new(1.0e9));
    let mut occ = [0_u64; MAX_N];
    let mut accepted_cells: Vec<Vec<usize>> = vec![Vec::new(); M];
    let mut departures: BinaryHeap<Reverse<(usize, usize)>> = BinaryHeap::new();
    let mut queries = Vec::with_capacity(M);
    for id in 0..M {
        while let Some(&Reverse((departure, group_id))) = departures.peek() {
            if departure >= S[id] {
                break;
            }
            departures.pop();
            for &cell in &accepted_cells[group_id] {
                occ[cell / N] &= !(1_u64 << (cell % N));
            }
        }
        queries.push((occ, P[id]));
        if let Some(cells) = placements[id].take() {
            assert_eq!(cells.len(), P[id]);
            let mut unique = HashSet::new();
            for &cell in &cells {
                let r = cell / N;
                let c = cell % N;
                assert!(((grass_rows[r] >> c) & 1) != 0);
                assert!(((occ[r] >> c) & 1) == 0);
                assert!(unique.insert(cell));
                occ[r] |= 1_u64 << c;
            }
            accepted_cells[id] = cells;
            departures.push(Reverse((T[id], id)));
        }
    }

    let mut reference_accepts = 0;
    let mut canonical_accepts = 0;
    let mut accept_agreements = 0;
    let mut both_accept_same_compactness = 0;
    let verify_placement =
        |board: &Rows, group_size: usize, placement: &Option<(Vec<usize>, f64)>| {
            let Some((cells, C)) = placement else {
                return;
            };
            assert_eq!(cells.len(), group_size);
            ...
            while let Some(cell) = stack.pop() {
                ...
                for (dr, dc) in [(-1_isize, 0_isize), (1, 0), (0, -1), (0, 1)] {
                    ...
                    if unique.contains(&next) && reached.insert(next) {
                        stack.push(next);
                    }
                }
            }
            assert_eq!(reached.len(), group_size);
        };
    for &(board, group_size) in &queries {
        let reference = solver.quick_place_reference(&board, group_size);
        let (canonical, _) = solver.quick_place(&board, group_size);
        verify_placement(&board, group_size, &reference);
        verify_placement(&board, group_size, &canonical);
        reference_accepts += usize::from(reference.is_some());
        canonical_accepts += usize::from(canonical.is_some());
        accept_agreements += usize::from(reference.is_some() == canonical.is_some());
        if let (Some((reference_cells, reference_C)), Some((canonical_cells, canonical_C))) =
            (&reference, &canonical)
        {
            assert_eq!(reference_cells.len(), group_size);
            assert_eq!(canonical_cells.len(), group_size);
            both_accept_same_compactness += usize::from((reference_C - canonical_C).abs() < 1e-12);
        }
        black_box(reference);
        black_box(canonical);
    }
    let both_accepts =
        (accept_agreements + reference_accepts + canonical_accepts - queries.len()) / 2;
    let reference_only = reference_accepts - both_accepts;
    let canonical_only = canonical_accepts - both_accepts;

    let mut reference_seconds = 0.0;
    let mut canonical_seconds = 0.0;
    for repeat in 0..repeats {
        let measure_reference = || {
            ...
            for &(board, group_size) in &queries {
                checksum += usize::from(
                    solver
                        .quick_place_reference(black_box(&board), black_box(group_size))
                        .is_some(),
                );
            }
            black_box(checksum);
        ...
        let measure_canonical = || {
            let start = Instant::now();
            let mut checksum = 0_usize;
            for &(board, group_size) in &queries {
                checksum += usize::from(
                    solver
                        .quick_place(black_box(&board), black_box(group_size))
                        .0
                        .is_some(),
                );
            }
        ...
        if repeat % 2 == 0 {
            reference_seconds += measure_reference();
            canonical_seconds += measure_canonical();
        } else {
            canonical_seconds += measure_canonical();
            reference_seconds += measure_reference();
        }
    }
    let speed_ratio = reference_seconds / canonical_seconds;
    let recommended_samples = ((3.0 * speed_ratio).floor() as usize).clamp(3, 9);
    CanonicalRolloutBench {
        queries: queries.len(),
        repeats,
        reference_accepts,
        ...
        reference_only,
        canonical_only,
        both_accept_same_compactness,
        reference_ms: reference_seconds * 1_000.0,
        canonical_ms: canonical_seconds * 1_000.0,
        speed_ratio,
        recommended_samples,
    }
}
