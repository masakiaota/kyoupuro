// v089_repack_parent_arena.rs
#![allow(non_snake_case)]
// 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: v088 の探索意味を保ち、repack beam の深い path clone と
// state ごとの scratch allocation だけを固定長 arena・再利用 workspace へ置き換える。
...
const MAX_REPACK_BLOCKERS: usize = 4;
const REPACK_BEAM_WIDTH: usize = 9;
const REPACK_BRANCHES: usize = 4;
...
impl FreeInfo {
    fn with_capacity(N: usize, keep_cells: bool) -> Self {
...
#[derive(Clone, Copy)]
struct RepackBeamState {
    occ: Rows,
    path: [usize; MAX_REPACK_BLOCKERS],
    depth: u8,
    score: f64,
}

struct RepackWorkspace {
    beam: Vec<RepackBeamState>,
    next_beam: Vec<RepackBeamState>,
    /// 各 child がこの arena の現在段だけを所有し、過去段は固定長 path で共有する。
    placement_arena: Vec<(usize, Placement)>,
    info: FreeInfo,
    weights: Box<WeightData>,
    runs: Box<RunTable>,
    candidates: Vec<Placement>,
    clearance_candidates: Vec<(f64, Placement)>,
    #[cfg(feature = "local")]
    trace_child_states: usize,
    #[cfg(feature = "local")]
    trace_path_refs: usize,
    #[cfg(feature = "local")]
    trace_deferred_materializations: usize,
}

impl RepackWorkspace {
    fn new(N: usize) -> Self {
        Self {
            beam: Vec::with_capacity(REPACK_BEAM_WIDTH * REPACK_BRANCHES),
            next_beam: Vec::with_capacity(REPACK_BEAM_WIDTH * REPACK_BRANCHES),
            placement_arena: Vec::with_capacity(
                MAX_REPACK_BLOCKERS * REPACK_BEAM_WIDTH * REPACK_BRANCHES,
            ),
            info: FreeInfo::with_capacity(N, true),
            weights: Box::new(WeightData {
                prefix: [[0.0; MAX_N + 1]; MAX_N],
                cell: [0.0; MAX_N * MAX_N],
            }),
            runs: Box::new([[0_u64; MAX_N + 1]; MAX_N]),
            candidates: Vec::with_capacity(REPACK_BRANCHES),
            clearance_candidates: Vec::with_capacity(SIZE_CLEARANCE_TOP_COUNT),
            #[cfg(feature = "local")]
            trace_child_states: 0,
            #[cfg(feature = "local")]
            trace_path_refs: 0,
            #[cfg(feature = "local")]
            trace_deferred_materializations: 0,
        }
    }
}
...
struct Solver {
    ...
    /// repack は再入しないため、一時的に take して全 call 間で scratch を再利用する。
    repack_workspace: Option<Box<RepackWorkspace>>,
    ...
    #[cfg(feature = "local")]
    runtime_repack_workspace_calls: Cell<usize>,
    #[cfg(feature = "local")]
    runtime_repack_child_states: Cell<usize>,
    #[cfg(feature = "local")]
    runtime_repack_path_refs: Cell<usize>,
    #[cfg(feature = "local")]
    runtime_repack_deferred_materializations: Cell<usize>,
    #[cfg(feature = "local")]
    trace: TraceStats,
}
...
impl Solver {
    fn new(
        ...
        let mut solver = Self {
            ...
            repack_workspace: Some(Box::new(RepackWorkspace::new(N))),
            theta_cum: Vec::new(),
            theta_sd: 1_732.0,
            ...
            #[cfg(feature = "local")]
            runtime_repack_workspace_calls: Cell::new(0),
            #[cfg(feature = "local")]
            runtime_repack_child_states: Cell::new(0),
            #[cfg(feature = "local")]
            runtime_repack_path_refs: Cell::new(0),
            #[cfg(feature = "local")]
            runtime_repack_deferred_materializations: Cell::new(0),
            #[cfg(feature = "local")]
    ...
    fn compute_free_info(&self, occ: &Rows, keep_cells: bool) -> FreeInfo {
        let mut info = FreeInfo::with_capacity(self.N, keep_cells);
        self.populate_free_info(occ, keep_cells, &mut info);
        info
    }

    fn compute_free_info_into(&self, occ: &Rows, keep_cells: bool, info: &mut FreeInfo) {
        // Solver 内で参照され得る id は 0..N*N だけなので、tail の初期化を省く。
        info.component[..self.N * self.N].fill(-1);
        info.sizes.clear();
        info.cell_pool.clear();
        info.cell_offsets.clear();
        if keep_cells {
            info.cell_offsets.push(0);
        }
        info.free_count = 0;
        info.dead_ends = 0;
        info.metric = 0.0;
        self.populate_free_info(occ, keep_cells, info);
    }

    /// `info` が空でcomponentだけ-1になっている前提の共通本体。
    fn populate_free_info(&self, occ: &Rows, keep_cells: bool, info: &mut FreeInfo) {
    ...
    fn build_weight_data(
        ...
        self.build_weight_data_into(occ, info, incoming_T, theta, use_owner_time, &mut data);
        data
    }

    fn build_weight_data_into(
        &self,
        occ: &Rows,
        info: &FreeInfo,
        incoming_T: usize,
    ...
    fn build_run_table(&self, occ: &Rows) -> RunTable {
        ...
        self.build_run_table_into(occ, &mut runs);
        runs
    }

    fn build_run_table_into(&self, occ: &Rows, runs: &mut RunTable) {
        for r in 0..self.N {
    ...
    /// repack childでは規則形状をセルVecへ展開せず、同じ占有maskだけを直接反映する。
    #[inline]
    fn apply_placement_to_rows(&self, rows: &mut Rows, placement: &Placement, P: usize) {
        if !placement.explicit_cells.is_empty() {
            for &cell in &placement.explicit_cells {
                rows[cell / self.N] |= Self::bit_at(cell % self.N);
            }
            return;
        }
        let shape = &self.shapes_by_p[P][placement.shape_index];
        for rr in 0..shape.h {
            let begin = placement.y + unsafe { *shape.left.get_unchecked(rr) };
            let len = unsafe { *shape.len.get_unchecked(rr) };
            let mask = ((1_u64 << len) - 1) << begin;
            unsafe {
                *rows.get_unchecked_mut(placement.x + rr) |= mask;
            }
        }
    }
    ...
    fn scan_regular_level_into(
        ...
        shape_limit: usize,
        add_clearance_candidates: bool,
        top: &mut Vec<Placement>,
        clearance_top: &mut Vec<(f64, Placement)>,
    ) {
        top.clear();
        clearance_top.clear();
            ...
            for x in 0..=(self.N - shape.h) {
                ...
                while ys != 0 {
                    ...
                    if add_clearance_candidates {
                        Self::insert_top_by_cheap(top, candidate.clone(), top_count);
                        ...
                        // repackではclearance候補を作らないため、空Vecを含むPlacementのcloneも省く。
                        Self::insert_top_by_cheap(top, candidate, top_count);
                    }
                }
            }
        }
        for (_, candidate) in clearance_top.drain(..) {
    ...
    fn repack_blockers(
        ...
        local! {
            ...
            self.runtime_repack_workspace_calls
                .set(self.runtime_repack_workspace_calls.get() + 1);
        }
        let mut workspace = self
            .repack_workspace
            .take()
            .expect("repack workspace is not reentrant");
        local! {
            workspace.trace_child_states = 0;
            workspace.trace_path_refs = 0;
            workspace.trace_deferred_materializations = 0;
        }
        let result =
            self.repack_blockers_with_workspace(blocker_ids, target_cells, workspace.as_mut());
        local! {
            self.runtime_repack_child_states.set(
                self.runtime_repack_child_states.get() + workspace.trace_child_states,
            );
            self.runtime_repack_path_refs
                .set(self.runtime_repack_path_refs.get() + workspace.trace_path_refs);
            self.runtime_repack_deferred_materializations.set(
                self.runtime_repack_deferred_materializations.get()
                    + workspace.trace_deferred_materializations,
            );
        }
        self.repack_workspace = Some(workspace);
        result
    }

    fn repack_blockers_with_workspace(
        &mut self,
        blocker_ids: &[usize],
        target_cells: &[usize],
        workspace: &mut RepackWorkspace,
    ) -> Option<(Vec<(usize, Placement)>, f64, f64)> {
        debug_assert!(blocker_ids.len() <= MAX_REPACK_BLOCKERS);
        ...
        let mut order = [0_usize; MAX_REPACK_BLOCKERS];
        order[..blocker_ids.len()].copy_from_slice(blocker_ids);
        order[..blocker_ids.len()].sort_by(|&a, &b| {
        ...
        workspace.next_beam.clear();
        workspace.placement_arena.clear();
        workspace.candidates.clear();
        workspace.clearance_candidates.clear();
        workspace.beam.push(RepackBeamState {
            occ: base,
            path: [0; MAX_REPACK_BLOCKERS],
            depth: 0,
            score: 0.0,
        });

        for &group_id in &order[..blocker_ids.len()] {
            let P = self.groups[group_id].P;
            let worst_perimeter = self.groups[group_id].worst_perimeter;
            workspace.next_beam.clear();
            for state_index in 0..workspace.beam.len() {
                if self.timer.reached(REPACK_LIMIT_RATIO) {
                    break;
                }
                // 過去PlacementのセルVecはarenaで共有し、Rowsはchild生成時だけ値コピーする。
                let state = &workspace.beam[state_index];
                self.compute_free_info_into(&state.occ, true, &mut workspace.info);
                if !workspace.info.sizes.iter().any(|&size| size >= P) {
                    continue;
                }
                self.build_weight_data_into(
                    &state.occ,
                    &workspace.info,
                    0,
                    5_000.0,
                    false,
                    workspace.weights.as_mut(),
                );
                self.build_run_table_into(&state.occ, workspace.runs.as_mut());
                ...
                for perimeter in (minimum_perimeter(P)..=worst_perimeter + 6).step_by(2) {
                    self.scan_regular_level_into(
                        P,
                        workspace.runs.as_ref(),
                        workspace.weights.as_ref(),
                        perimeter,
                        REPACK_BRANCHES,
                        8,
                        false,
                        &mut workspace.candidates,
                        &mut workspace.clearance_candidates,
                    );
                ...
                if workspace.candidates.is_empty()
                    ...
                    && let Some(candidate) = self.growth_placement(
                        P,
                        &state.occ,
                        &workspace.info,
                        workspace.weights.as_ref(),
                ...
                #[cfg(feature = "local")]
                let generated_before = workspace.next_beam.len();
                #[cfg(feature = "local")]
                let mut deferred_materializations = 0;
                let candidates = &mut workspace.candidates;
                let placement_arena = &mut workspace.placement_arena;
                let next_beam = &mut workspace.next_beam;
                for candidate in candidates.drain(..) {
                    #[cfg(feature = "local")]
                    if candidate.explicit_cells.is_empty() {
                        deferred_materializations += 1;
                    }
                    let mut child_occ = state.occ;
                    self.apply_placement_to_rows(&mut child_occ, &candidate, P);
                    let candidate_score = candidate.cheap_score;
                    let placement_index = placement_arena.len();
                    placement_arena.push((group_id, candidate));
                    let mut path = state.path;
                    let depth = state.depth as usize;
                    debug_assert!(depth < MAX_REPACK_BLOCKERS);
                    path[depth] = placement_index;
                    next_beam.push(RepackBeamState {
                        occ: child_occ,
                        path,
                        depth: state.depth + 1,
                ...
                local! {
                    let generated = workspace.next_beam.len() - generated_before;
                    workspace.trace_child_states += generated;
                    workspace.trace_path_refs += generated * state.depth as usize;
                    workspace.trace_deferred_materializations += deferred_materializations;
                }
            }
            if workspace.next_beam.is_empty() {
                return None;
            }
            workspace
                .next_beam
                .sort_by(|a, b| b.score.total_cmp(&a.score));
            workspace.next_beam.truncate(REPACK_BEAM_WIDTH);
            std::mem::swap(&mut workspace.beam, &mut workspace.next_beam);
        }

        if workspace.beam.is_empty() {
            return None;
        }
        let mut best_index = 0;
        let mut best_score = -1e100;
        let mut best_metric = 0.0;
        for (i, state) in workspace.beam.iter().enumerate() {
        ...
        let best_state = workspace.beam[best_index];
        let mut chosen = Vec::with_capacity(best_state.depth as usize);
        for &placement_index in &best_state.path[..best_state.depth as usize] {
            let (group_id, mut placement) = {
                let entry = &mut workspace.placement_arena[placement_index];
                (entry.0, std::mem::take(&mut entry.1))
            };
            if placement.explicit_cells.is_empty() {
                placement.explicit_cells = self.materialize(&placement, self.groups[group_id].P);
            }
            chosen.push((group_id, placement));
        }
    ...
    fn run<R: BufRead, W: Write>(
        ...
        local! {
            ...
            self.trace.count_by(
                "runtime_repack_workspace_call",
                self.runtime_repack_workspace_calls.get() as i64,
            );
            self.trace.count_by(
                "runtime_repack_child",
                self.runtime_repack_child_states.get() as i64,
            );
            self.trace.count_by(
                "runtime_repack_path_ref",
                self.runtime_repack_path_refs.get() as i64,
            );
            self.trace.count_by(
                "runtime_repack_deferred_materialize",
                self.runtime_repack_deferred_materializations.get() as i64,
            );
            self.trace
