...
// 中心アイデア: 通常配置が不在または悪形 (slack>=4) のとき、既存グループの移動で
// 最小周長近傍の配置を作り、悪形配置との純利益比較 (移動費 + blocker の C 低下損失込み)
// で移動プランを採用する。v501 では relocation は受け入れ不成立時の救済に限られていた。
...
impl Solver {
    ...
    /// blocker たちを target を塞いだ盤面へ置き直す。戻り値は (配置列, fee_loss 合計)。
    /// fee_loss は worst_perimeter を超える形へ逃げた blocker の退去時利用料の減少分で、
    /// 呼び出し側が移動費と合わせて採算判定に使う。
    fn repack_blockers(
        &mut self,
        blocker_ids: &[usize],
        target_cells: &[usize],
    ) -> Option<(Vec<(usize, Placement)>, f64)> {
        local! {
    ...
    fn attempt_relocation(
        ...
        // attempt 上限は 6 に留める。rank 下位の target で成功する移動は採算ぎりぎりで
        // 盤面も悪くしやすく、上限 12 では 0002 で -4% の悪化を実測した。
        for (attempt, option) in options.into_iter().enumerate() {
            if attempt >= 6 || self.timer.reached(REPACK_LIMIT_RATIO) {
                break;
            }
            let target_cells = self.materialize(&option.placement, incoming.P);
            let Some((repacked, fee_loss)) = self.repack_blockers(&option.blockers, &target_cells)
            else {
                continue;
            };
