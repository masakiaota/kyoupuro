// v006_shape_move_combo.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: 採用済み 2 部品の統合。v005_move_rescue (悪形配置を移動による良形配置で
// 置き換える主経路化) に、v005_shape_diversity (既存14形状を保つ周長ごと上限20の
// 形状上位集合) を取り込む。両者は独立の損失源に効くため加算的な改善を狙う。
