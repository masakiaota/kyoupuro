// v082_continuous_topology_portfolio.rs
#![allow(non_snake_case)]
// 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: 入力時のterrain二分岐を廃止し、単一状態上でpacking候補と
// topology保存region候補を共通rolloutへ載せ、探索量だけを動的riskで連続配分する。
...
struct Placement {
    ...
    /// 0: 既存候補、1: size/clearance、2: box侵食、3: topology region。
    strategy_tag: u8,
...
struct Solver {
    ...
    /// 入力地形から得る連続的なtopologyリスク。routeには使わず探索配分の一項にする。
    static_topology_risk: f64,
    /// fractional schedulingにより、riskに比例した長期頻度でregion候補を生成する。
    topology_search_credit: f64,
    /// 現在の自由成分・dead-end・占有圧まで含めた毎ターンのrisk。
    current_topology_risk: f64,
    /// packing候補の周長・断片化または不在から得る、別候補を調べる価値。
    current_packing_weakness: f64,
...
impl Solver {
    fn new(
        ...
        let mut solver = Self {
            ...
            topology_search_credit: 0.0,
            current_topology_risk: static_topology_risk,
            current_packing_weakness: 0.0,
            timer,
    ...
    fn find_normal_placements(
        ...
        self.refine_topology_risk(&info);
        self.current_packing_weakness = 0.0;
        if !info.sizes.iter().any(|&size| size >= P) {
            self.current_packing_weakness = 1.0;
            return None;
        }
        ...
        for perimeter in (min_L..=min_L + max_extra).step_by(2) {
            ...
            let slack_signal = ((perimeter.saturating_sub(min_L)) as f64 / 8.0).clamp(0.0, 1.0);
            let fragment_signal = (evaluated[current_index].1.max(0.0) / 24.0).clamp(0.0, 1.0);
            self.current_packing_weakness =
                (0.45 * slack_signal + 0.55 * fragment_signal).clamp(0.0, 1.0);
            if fast_mode {
        ...
        let Some(growth) = self.growth_placement(P, &occ, &info, &weights, usize::MAX, 44) else {
            self.current_packing_weakness = 1.0;
            return None;
        };
        ...
        let best_slack = choices[0].perimeter.saturating_sub(min_L);
        self.current_packing_weakness = (0.35 + best_slack as f64 / 14.0).clamp(0.0, 1.0);
        Some(choices)
    }
    ...
    /// terrainを分類する値ではなく、topology候補へ探索時間を何割配るかを決める連続量。
    /// 自由成分の分裂とdead-endは配置・退去で変わるため、同じ入力でも毎ターン変化する。
    fn refine_topology_risk(&mut self, info: &FreeInfo) {
        let largest = info.sizes.iter().copied().max().unwrap_or(0);
        let fragmented = if info.free_count == 0 {
            1.0
        } else {
            1.0 - largest as f64 / info.free_count as f64
        };
        let fragmentation_signal = (4.0 * fragmented).clamp(0.0, 1.0);
        let dead_end_signal = if info.free_count == 0 {
            1.0
        } else {
            (12.0 * info.dead_ends as f64 / info.free_count as f64).clamp(0.0, 1.0)
        };
        let pressure =
            (self.occupied_count() as f64 / self.effective_capacity.max(1.0)).clamp(0.0, 1.0);
        self.current_topology_risk = (0.45 * self.static_topology_risk
            + 0.25 * fragmentation_signal
            + 0.15 * dead_end_signal
            + 0.15 * pressure)
            .clamp(0.0, 1.0);
    }

    /// riskをhard gateにせずfractional creditへ加算し、長期の探索頻度を連続化する。
    /// packing不能時だけは候補不在という直接的なvalue-of-informationを優先する。
    fn request_topology_proposals(&mut self, P: usize, packing_exists: bool) -> bool {
        let size_need = ((P as f64 - 4.0) / 146.0).clamp(0.0, 1.0);
        let mass = (0.035
            + 0.24 * self.current_topology_risk
            + 0.18 * self.current_packing_weakness
            + 0.08 * size_need)
            .clamp(0.035, 0.535);
        self.topology_search_credit += mass;
        if !packing_exists {
            self.topology_search_credit = self.topology_search_credit.min(1.0);
            return true;
        }
        if self.topology_search_credit >= 1.0 {
            self.topology_search_credit -= 1.0;
            true
        } else {
            false
        }
    ...
    fn run<R: BufRead, W: Write>(
        ...
        for turn in 0..self.M {
            ...
            let pressure =
                (self.occupied_count() as f64 / self.effective_capacity.max(1.0)).clamp(0.0, 1.0);
            self.current_topology_risk =
                (0.70 * self.static_topology_risk + 0.30 * pressure).clamp(0.0, 1.0);
            self.current_packing_weakness = 0.0;
            ...
            if passed_price_prefilter {
                ...
                let packing_choices = local_time!(self.trace, "normal_search", {
                    self.find_normal_placements(P, T, theta, fast_mode)
                });
                let packing_exists = packing_choices.as_ref().is_some_and(|v| !v.is_empty());
                let mut choices = packing_choices.unwrap_or_default();
                if !fast_mode && self.request_topology_proposals(P, packing_exists) {
                    let size_need = ((P as f64 - 4.0) / 146.0).clamp(0.0, 1.0);
                    let effort = (0.35
                        + 1.10 * self.current_topology_risk
                        + 0.70 * self.current_packing_weakness
                        + 0.25 * size_need)
                        .clamp(0.35, 2.40);
                    let result_limit = (1.0 + effort).round() as usize;
                    let value_ratio = if base_threshold > 0.0 {
                        q_value * optimistic_C / base_threshold
                    } else {
                        2.0
                    };
                    let priority = (0.48
                        + 0.17 * (1.20 * value_ratio.max(0.08).ln()).tanh()
                        + 0.20 * self.current_topology_risk * size_need)
                        .clamp(0.06, 0.94);
                    let proposals = local_time!(self.trace, "topology_proposal", {
                        topology_advisor.propose_from_primary(
                            self,
                            P,
                            T,
                            priority,
                    ...
                    local! {
                        self.trace.count("topology_proposal_search");
                    }
                    let mut _offered = 0_i64;
                    for candidate in proposals {
                        if !choices
                            .iter()
                            .any(|current| current.explicit_cells == candidate.explicit_cells)
                        {
                            choices.push(candidate);
                            _offered += 1;
                        }
                    ...
                    local! {
                        self.trace.count_by("topology_proposal_offered", _offered);
                        if _offered > 0 {
                            self.trace.count("topology_proposal_turn");
                            if self.current_topology_risk < 1.0 / 3.0 {
                                self.trace.count_by("topology_offered_low", _offered);
                            } else if self.current_topology_risk < 2.0 / 3.0 {
                                self.trace.count_by("topology_offered_mid", _offered);
                            } else {
                                self.trace.count_by("topology_offered_high", _offered);
                            }
                        }
                    }
                }
                ...
                if !choices.is_empty() {
                    // packing候補を先に格納しているため、全候補が価格落ちした場合の
                    // relocation基準はv081のpacking候補を保つ。packing不在時だけregionを使う。
                    let rejected_baseline_placement = choices[0].clone();
                    let mut eligible = Vec::with_capacity(choices.len());
                    for candidate in choices {
                        let actual_threshold = base_threshold
                            * self.component_threshold_factor(candidate.component_size);
                        let quality = q_value * compactness(P, candidate.perimeter);
                        let mut candidate_accepted =
                            base_threshold == 0.0 || quality >= actual_threshold;
                        if candidate_accepted && base_threshold > 0.0 {
                            let margin = quality / actual_threshold.max(1e-300);
                            let duration_ratio = duration as f64 / theta;
                            let slack = candidate.perimeter.saturating_sub(minimum_perimeter(P));
                    ...
                    accepted = !eligible.is_empty();
                    let mut placement = if accepted {
                        let has_packing =
                            eligible.iter().any(|candidate| candidate.strategy_tag != 3);
                        let has_topology =
                            eligible.iter().any(|candidate| candidate.strategy_tag == 3);
                        let winner = if eligible.len() >= 2 {
                            if has_packing && has_topology {
                                local! {
                                    self.trace.count("cross_source_rollout");
                                }
                            }
                            self.select_normal_by_rollout(&eligible, id, theta, base_threshold)
                        } else {
                            0
                        };
                        local! {
                            match eligible[winner].strategy_tag {
                                1 => self.trace.count("size_clearance_selected"),
                                2 => self.trace.count("box_erosion_selected"),
                                3 => {
                                    self.trace.count("topology_selected");
                                    self.trace.count_by(
                                        "topology_selected_risk_milli_sum",
                                        (self.current_topology_risk * 1_000.0).round() as i64,
                                    );
                                    if self.current_topology_risk < 1.0 / 3.0 {
                                        self.trace.count("topology_selected_low");
                                    } else if self.current_topology_risk < 2.0 / 3.0 {
                                        self.trace.count("topology_selected_mid");
                                    } else {
                                        self.trace.count("topology_selected_high");
                                    }
                                    if has_packing {
                                        self.trace.count("topology_rollout_win");
                                    }
                                }
                                _ => {}
                            }
                            if eligible[winner].strategy_tag != 3 {
                                self.trace.count("packing_selected");
                                if has_topology {
                                    self.trace.count("packing_rollout_win");
                                }
                            }
                        }
                        eligible.swap_remove(winner)
                    } else {
                        rejected_baseline_placement
                    };
                    ...
                    if accepted {
                        ...
                        let selected = normal.as_ref().expect("accepted normal placement exists");
                        let final_perimeter = selected.perimeter;
                        let final_quality = q_value * compactness(P, final_perimeter);
                        let actual_threshold = base_threshold
                            * self.component_threshold_factor(selected.component_size);
            ...
            local! {
                self.trace.count("topology_risk_turn");
                self.trace.count_by(
                    "topology_risk_milli_sum",
                    (self.current_topology_risk * 1_000.0).round() as i64,
                );
                if self.current_topology_risk < 1.0 / 3.0 {
                    self.trace.count("topology_risk_low");
                } else if self.current_topology_risk < 2.0 / 3.0 {
                    self.trace.count("topology_risk_mid");
                } else {
                    self.trace.count("topology_risk_high");
                }
            }
            local! {
                if !passed_price_prefilter {
...
mod rough {
    ...
    impl Solver {
        pub(super) fn new(N: usize, M: usize, R_milli: i64, board: Vec<String>) -> Self {
        ...
        /// primary solverだけを正本とし、その現在盤面を候補生成用graph viewへ写す。
        /// rough側は独自の受否・履歴・出力を持たないため、二つの世界線は生じない。
        fn sync_from_primary(&mut self, primary: &super::Solver, dynamic_risk: f64) {
            ...
            self.terrain_roughness = dynamic_risk.clamp(0.0, 1.0);
        }

        /// topology側の直接配置だけを提案する。受否と最終比較はprimaryの共通rolloutが行う。
        pub(super) fn propose_from_primary(
            &mut self,
            ...
            self.sync_from_primary(primary, primary.current_topology_risk);
            let fs = self.build_free_state();
            // effortは連続だが探索回数は整数なので、丸めた二段階のanytime深度に落とす。
            // これはterrain routeではなく、同一候補生成器へ与える計算量だけを変える。
            let mode = if effort >= 1.35 { 1 } else { 0 };
            self.find_regions_with_state(
                P,
                result_limit.clamp(1, 4),
                mode,
                Some(target_T),
                priority,
                &fs,
            ...
            .filter_map(|region| {
                ...
                Some(super::Placement {
                    perimeter: region.L,
                    cheap_score: -(region.layout as f64),
                    final_score: -(region.layout as f64),
                    component_size: fs.comp_size[component as usize],
                    strategy_tag: 3,
        ...
        fn add_compact_template_candidates(
            ...
            let mut best: BinaryHeap<(i64, usize, usize, usize)> = BinaryHeap::with_capacity(64);
            let mut extra: [BinaryHeap<(i64, usize, usize, usize)>; 2] =
                std::array::from_fn(|_| BinaryHeap::with_capacity(16));
        ...
        fn grow_region(
            ...
            let mut heap: BinaryHeap<Reverse<HeapNode>> = BinaryHeap::with_capacity(16_384);
        ...
        fn find_regions_with_state(
            ...
            if box_limit > 0 {
                let mut heap: BinaryHeap<(i64, usize, usize, usize, usize)> =
            ...
            eligible.sort_by_key(|&component| fs.comp_size[component]);
...
fn main() -> io::Result<()> {
    ...
    let _clearance = mean_static_clearance(&board);
    ...
    let mut topology_advisor = Box::new(rough::Solver::new(N, M, R_milli, board));
    ...
    let mut solver = Solver::new(N, M, R_milli, grass_rows, timer, static_topology_risk);
    local! {
        solver.trace.count("unified_route");
        solver.trace.count_by(
            "input_clearance_milli_sum",
            (_clearance * 1_000.0).round() as i64,
        );
        solver.trace.count_by(
