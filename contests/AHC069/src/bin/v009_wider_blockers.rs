// v009_wider_blockers.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: target 列挙時の blocker_limit を 4→6 に上げる。逃げ先制約の撤廃
// (v008) で repack がほぼ通るようになった今、大きな空間を要する target を blocker 数
// 上限で捨てるのが適用機会の主な制限になっているはず。
