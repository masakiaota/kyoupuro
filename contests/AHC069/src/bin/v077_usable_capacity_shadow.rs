// v077_usable_capacity_shadow.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
                          // 中心アイデア: v053 の探索・配置・posterior rollout・relocationを固定し、
                          // admission の占有誤差だけへ、将来groupが収まりにくい小空き成分の
                          // `P`分布fit確率で割り引いた期待未利用容量を加える。
...
impl Solver {
    ...
    /// 空き連結成分のうち、既知の `P` 分布では次のgroupを収容できない期待面積を返す。
    /// 単なる空き面積でなく将来利用可能性を占有誤差へ反映するためのshadow occupancyである。
    fn dynamic_fragment_waste(&self) -> f64 {
        let info = self.compute_free_info(&self.occupied_rows, false);
        info.sizes
            .iter()
            .filter(|&&size| size < MAX_P)
            .map(|&size| (size as f64) * (1.0 - self.fit_probability(size)))
            .sum()
    }
    ...
    fn base_dynamic_threshold(&mut self, S: usize, duration: usize, P: usize, theta: f64) -> f64 {
        ...
        let fragment_waste = self.dynamic_fragment_waste();
        local! {
            self.trace.count("dynamic_fragment_waste_turn");
            if fragment_waste > 0.0 {
                self.trace.count("dynamic_fragment_waste_positive");
            }
            self.trace.count_by(
                "dynamic_fragment_waste_milli_sum",
                (1_000.0 * fragment_waste).round() as i64,
            );
        }
        let error = ((self.occupied_count() as f64) + fragment_waste + 0.5 * (P as f64)
            - target_occupancy)
