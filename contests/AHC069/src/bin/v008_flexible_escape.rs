// v008_flexible_escape.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: blocker の逃げ先の品質制約を撤廃する。repack 失敗の 79〜99% は
// 「最大空き成分 >= P なのに worst+2 以下の周長で置ける形がない」形状不足 (Q4 probe 実測)
// なので、規則形状は worst+6 まで段階探索し、growth は周長上限なしで許し、
// C 低下損失 (fee_loss) 込みの採算に採否を完全に任せる。
...
// v008 で repack 1 回のコストが上がり dry-run max 1527ms と予算超過が出たため、
// relocation 系の締め切りを v007 より前へ戻して安全余裕を確保する。
const TARGET_SCAN_LIMIT_RATIO: f64 = 172.0 / 190.0;
const GROWTH_LIMIT_RATIO: f64 = 174.0 / 190.0;
const REPACK_LIMIT_RATIO: f64 = 176.0 / 190.0;
...
const RELOC_TIME_BUDGET_RATIO: f64 = 0.60;
