// v061_prefix_map_rollout.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: v059 の未補正 MAP を、未到着 group の prefix 尤度を含む7次近似MAPへ
// 置換する。rollout本数・到着数・乱数列は固定し、得た単一thetaから仮想到着ごとの
// l[i]を個別サンプルする経路もv059のまま保つ。
...
struct Solver {
    ...
    /// prefix補正MAPの逐次Newtonを、直前の解から始めるために保持するbeta=H/theta。
    prefix_beta_hat: f64,
    expected_p: f64,
    compactness_bar: f64,
...
impl Solver {
    ...
    /// 変数変換のヤコビアンは掛けず、v059と同じtheta密度のMAPを求める。
    fn prefix_map_eval(
        beta: f64,
        n: usize,
        Y: u64,
    ...
    /// 最初の32groupでは、文書の計算量どおり12回だけ目的関数を評価する。
    ...
    fn evaluate_candidates_rollout(
        ...
        for k in 0..ROLLOUT_SAMPLES {
            ...
            // prefix補正MAPを全rolloutで共有する。各本のP・滞在・qの乱数列と、
            // 候補間の共通乱数というv059の比較条件はそのまま保つ。
            let theta_k = theta;
            local! {
