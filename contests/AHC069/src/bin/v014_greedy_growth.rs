// v012_rollout_choice.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: 移動プランの「選択」と「最終採否」を、断片化メトリックの代理評価でなく
// 将来到着の共通乱数ロールアウトによる直接評価で行う。候補 = 各プラン適用後盤面 +
// baseline (悪形受け入れ後 or 棄却=現状)。総合値 = 即時実額 (V×C − 移動費 − fee_loss)
// + ロールアウト将来項。機会費用はロールアウトが直接測るため、最終比較にシャドー価格は
// 使わない (収集フィルタとしては残す)。生成規則既知と θ 推定を評価に初めて本格利用する。
    ...
    fn growth_placement(
        ...
        if let Some(candidate) = &best {
            local! {
                ...
                self.trace.count_by(
                    "growth_slack_sum",
                    (candidate.perimeter - minimum_perimeter(P)) as i64,
                );
            }
        }
        best
