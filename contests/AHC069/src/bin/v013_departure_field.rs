// v013_departure_field.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: 退去時刻グラデーションの大域場。幾何勾配場 field=1−(x+y)/(2(N−1)) と
// グループの滞在分布位置 f(dur)=1−exp(−dur/θ) の一致ボーナスを通常配置の weight に加え、
// 「奥=長期滞在、手前=短期滞在」の大域構造を作って退去の空間的まとまりを生む。
// v002 系の局所的な退去時刻近接評価 (隣接セルのみ) を大域へ拡張する。
...
impl Solver {
    ...
    fn initialize_empty_board_cache(&mut self) {
        ...
        *self.permanent_weights =
            self.build_weight_data(&empty_occ, &info, 0, 5_000.0, false, None);
    ...
    fn build_weight_data(
        ...
        theta: f64,
        use_owner_time: bool,
        field_target: Option<f64>,
    ) -> WeightData {
        let mut data = WeightData {
            prefix: [[0.0; MAX_N + 1]; MAX_N],
            cell: [0.0; MAX_N * MAX_N],
        };
        // 退去時刻グラデーション場: 奥 (左上) = 長期滞在、手前 (右下) = 短期滞在に寄せる。
        // field[cell] = 1 − (x+y)/(2(N−1)) が滞在分布位置 f(dur) = 1 − exp(−dur/θ) に
        // 一致する場所を優遇する。
        const FIELD_WEIGHT: f64 = 2.5;
        let field_scale = 1.0 / (2.0 * ((self.N - 1) as f64));
        const DIRS: [(isize, isize); 4] = [(-1, 0), (1, 0), (0, -1), (0, 1)];
        for r in 0..self.N {
            for c in 0..self.N {
                let id = r * self.N + c;
                let mut weight = 0.0;
                if self.is_free(occ, r, c) {
                    if let Some(target) = field_target {
                        let field = 1.0 - ((r + c) as f64) * field_scale;
                        weight -= FIELD_WEIGHT * (field - target).abs();
                    }
                    for (dr, dc) in DIRS {
                        let nr = (r as isize) + dr;
                        let nc = (c as isize) + dc;
    ...
    fn find_normal_placement(
        ...
        // 滞在時間の分布位置。長期滞在ほど 1 (奥)、短期ほど 0 (手前) に寄せる。
        let field_target = 1.0 - (-(((incoming_T - incoming_S) as f64) / theta.max(1.0))).exp();
        local! {
            self.trace.count("field_bonus_turns");
        }
        let weights =
            self.build_weight_data(&occ, &info, incoming_T, theta, true, Some(field_target));
    ...
    fn repack_blockers(
        ...
        for group_id in order {
            ...
            for state in &beam {
                ...
                let weights = self.build_weight_data(&state.occ, &info, 0, 5_000.0, false, None);
        ...
        for turn in 0..self.M {
            ...
            if passed_price_prefilter {
                ...
                normal = local_time!(self.trace, "normal_search", {
                    self.find_normal_placement(P, S, T, theta, fast_mode)
                });
