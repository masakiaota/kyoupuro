// v032_no_move_cross_perimeter.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。

// 中心アイデア: 最初に置ける周長 L の候補と admission は変えず、L の候補が価格を
// 通るときだけ L+2 の候補も短期ロールアウトへ加える。再移動は一切行わない。
...
impl Solver {
    ...
    fn find_normal_placements(
        ...
        for perimeter in (min_L..=min_L + max_extra).step_by(2) {
            ...
            // 候補0が従来の価格判定を通る場合だけ、周長を2増やして盤面保存を
            // 優先できる余地をロールアウトへ渡す。候補0が落ちる場合の救済には使わない。
            let base_component_size = choices[0].component_size;
            let base_quality = q_value * compactness(P, perimeter);
            let base_actual_threshold =
                base_threshold * self.component_threshold_factor(base_component_size);
            let base_price_pass = base_threshold == 0.0 || base_quality >= base_actual_threshold;
            if base_price_pass
                && !fast_mode
                && !self.timer.reached(GROWTH_SA_LIMIT_RATIO)
                && perimeter + 2 <= min_L + 8
            {
                local! {
                    self.trace.count("cross_perimeter_turn");
                }
                let cross_perimeter = perimeter + 2;
                let cross_candidates =
                    self.scan_regular_level(P, &runs, &weights, cross_perimeter, 20, usize::MAX);
                local! {
                    self.trace
                        .count_by("cross_perimeter_scan", cross_candidates.len() as i64);
                }
                let mut best_cross: Option<Placement> = None;
                for mut candidate in cross_candidates {
                    let cells = self.materialize(&candidate, P);
                    let component_id = info.component[cells[0]];
                    candidate.component_size = if component_id >= 0 {
                        info.sizes[component_id as usize]
                    ...
                    if base_threshold != 0.0 && quality < actual_threshold {
                        continue;
                    }
                    local! {
                        self.trace.count("cross_perimeter_price_pass");
                    }
                    if best_cross
                        .as_ref()
                        .is_none_or(|current| candidate.final_score > current.final_score)
                    {
                        best_cross = Some(candidate);
                    }
                }
                if let Some(candidate) = best_cross {
                    if !choices
                        .iter()
                        .any(|choice| choice.explicit_cells == candidate.explicit_cells)
                    {
                        local! {
                            self.trace.count("cross_perimeter_candidate_added");
                            let fee_loss = (incoming_V as f64
                                * (compactness(P, perimeter)
                                    - compactness(P, candidate.perimeter)))
                                .round() as i64;
                            self.trace.count_by(
                                "cross_perimeter_immediate_fee_loss",
                                fee_loss,
                            );
                        }
                        choices.push(candidate);
        ...
        for turn in 0..self.M {
            ...
            if passed_price_prefilter {
                ...
                if let Some(mut choices) = normal_choices {
                    ...
                    let _cross_perimeter_index = choices
                        .iter()
                        .position(|placement| placement.perimeter != current_perimeter);
                    ...
                    local! {
                        ...
                        if accepted && _cross_perimeter_index.is_some() && choices.len() >= 2 {
                            self.trace.count("cross_perimeter_rollout_comparison");
                            if _cross_perimeter_index == Some(winner) {
                                self.trace.count("cross_perimeter_rollout_chosen");
                            }
                        }
                    }
                    ...
                    let selected_quality = q_value * compactness(P, placement.perimeter);
                    debug_assert!(base_threshold == 0.0 || selected_quality >= selected_threshold);
