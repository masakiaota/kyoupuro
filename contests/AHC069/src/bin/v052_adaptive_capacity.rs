...
struct Solver {
    ...
    /// 静的式による packing 比と usable セル数 (適応の基準点)。
    static_capacity_ratio: f64,
    usable_cells: f64,
    /// 幾何棄却 (価格通過後に配置不能) 時の占有率 EMA。詰め限界の直接観測。
    reject_occ_ema: f64,
    capacity_obs: usize,
...
impl Solver {
    ...
    fn initialize_static_capacity(&mut self) {
        ...
        self.static_capacity_ratio = packing_efficiency;
        self.usable_cells = (usable as f64).max(1.0);
    }

    /// 幾何棄却時の占有率を詰め限界の観測として取り込み、実効容量をケース内適応させる。
    /// θ (入力の性質) と違い、「この盤面での自分の詰め能力」という未知量を到着ごとに推定する。
    /// 低占有時の幾何棄却は断片化のシグナルであって容量限界ではない (Q2: 絶対容量不足は
    /// ほぼゼロ) ため、静的比の近傍以上で起きた棄却だけを限界の観測として使う。
    fn observe_packing_limit(&mut self) {
        let occ_rate = (self.occupied_count() as f64) / self.usable_cells;
        if occ_rate < self.static_capacity_ratio - 0.06 {
            local! {
                self.trace.count("capacity_obs_skipped_low_occ");
            }
            return;
        }
        const LAMBDA: f64 = 0.15;
        if self.capacity_obs == 0 {
            self.reject_occ_ema = occ_rate;
        } else {
            self.reject_occ_ema = self.reject_occ_ema * (1.0 - LAMBDA) + occ_rate * LAMBDA;
        }
        self.capacity_obs += 1;
        // 観測が少ないうちは静的式を信頼し、貯まるほど実測へ寄せる。margin は
        // 幾何棄却が限界の少し下でも起きることの補正。
        const MARGIN: f64 = 0.015;
        let weight = ((self.capacity_obs as f64) / 20.0).min(0.7);
        let adaptive = (self.reject_occ_ema + MARGIN).clamp(0.72, 0.93);
        let ratio = self.static_capacity_ratio * (1.0 - weight) + adaptive * weight;
        self.effective_capacity = (ratio * self.usable_cells).max(1.0);
        local! {
            self.trace.count("capacity_obs");
        }
    }
    ...
    fn run<R: BufRead, W: Write>(
        ...
        for turn in 0..self.M {
            ...
            if move_plan.ok {
                ...
                if accepted {
                    ...
                    // 幾何棄却 (価格を通ったのに配置候補ゼロ) は「この盤面で実際に
                    // 詰められる限界」の直接観測。占有率の EMA で実効容量を適応させる。
                    if passed_price_prefilter && normal.is_none() {
                        self.observe_packing_limit();
                    }
                    local! {
        ...
        local! {
            ...
            self.trace.count_by(
                "adaptive_ratio_final_permille",
                (self.effective_capacity / self.usable_cells * 1000.0).round() as i64,
            );
            self.trace.count_by(
                "static_ratio_permille",
                (self.static_capacity_ratio * 1000.0).round() as i64,
            );
            self.trace.summary();
        }
        Ok(())
