// v076_simple_terminal_weight.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: v053 の道中価格校正を保ち、既存boundary factorの終端側を除いた後、
// 到着groupの既知区間に対する平均W_endをadmission閾値へ一度だけ掛ける。
...
impl Solver {
    ...
    /// 時刻0より前に到着がない序盤効果だけを表す。終端効果は含めない。
    fn start_load_factor(time: f64, theta: f64) -> f64 {
        let clipped = time.clamp(0.0, HORIZON as f64);
        (1.0 - (-clipped / theta).exp()).max(0.0)
    }

    /// `[S,S+duration)` の各整数時刻における
    /// W_end(t)=1-exp(-(HORIZON-t)/theta)を正確に平均する。
    ...
    fn local_bid_at(&mut self, time: f64, theta: f64, offered_area: f64) -> f64 {
        local! {
            self.trace.count("start_only_bid_eval");
        }
        let local_offer = offered_area * Self::start_load_factor(time, theta);
    ...
    fn base_dynamic_threshold(&mut self, S: usize, duration: usize, P: usize, theta: f64) -> f64 {
        ...
        let threshold = average_bid * (D / theta).powf(0.1) * terminal_weight;

        // occupancy feedbackにも旧終端係数を残さず、W_endの明示適用を上の一回に限定する。
        let current_offer = offered_area * Self::start_load_factor(S as f64, theta);
        ...
        local! {
            self.trace.count("simple_terminal_weight_turn");
            self.trace.count("start_only_target_turn");
            self.trace.count_by(
                "simple_terminal_weight_ppm_sum",
                (terminal_weight * 1_000_000.0).round() as i64,
            );
            if terminal_weight < 0.999_5 {
                self.trace.count("simple_terminal_discount_turn");
            }
            if average_bid > 0.0 && terminal_weight < 0.999_5 {
                self.trace.count("simple_terminal_price_discounted_turn");
            }
        }
