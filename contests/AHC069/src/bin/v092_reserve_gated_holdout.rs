// v092_reserve_gated_holdout.rs
#![allow(non_snake_case)]
// 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: v091 の追加holdoutを第一層として保ち、第一層が進捗枠へ追いついた間だけ
// 大きな実測slackを持つsmooth caseへ第二層のfresh posterior scenarioを配る。
...
/// 基準deep枠を消費済みのときだけ使う、余剰時間側のpaired holdout。
const SPARE_ROLLOUT_SESSION_LIMIT: usize = 32;
const SPARE_ROLLOUT_SAMPLES: usize = 9;
const SPARE_ROLLOUT_SEED_OFFSET: usize = 17;
const SPARE_ROLLOUT_PACE_START_RATIO: f64 = 10.0 / 190.0;
const SPARE_ROLLOUT_PACE_SPAN_RATIO: f64 = 170.0 / 190.0;
const SPARE_ROLLOUT_PACE_END_RATIO: f64 = 180.0 / 190.0;
const SPARE_ROLLOUT_MIN_SLACK_RATIO: f64 = 1.0 / 190.0;
/// v091の第一層を置換せず、そのpaced枠へ追いついた間だけ使うslack保全型の第二層。
const RESERVE_ROLLOUT_SESSION_LIMIT: usize = 96;
const RESERVE_ROLLOUT_SEED_OFFSET: usize = 29;
const RESERVE_ROLLOUT_MIN_SLACK_RATIO: f64 = 30.0 / 190.0;
const REAL_HARD_LIMIT_RATIO: f64 = 184.0 / 190.0;
...
struct TimeKeeper {
    start: Instant,
    time_limit_sec: f64,
    /// 基準経路以外だけに使った実時間。基準方策の時刻判定から除外する。
    excluded_sec: f64,
}
...
impl TimeKeeper {
    ...
    fn reached(&self, ratio: f64) -> bool {
        let real_elapsed = self.real_elapsed_sec();
        (real_elapsed - self.excluded_sec).max(0.0) >= self.time_limit_sec * ratio
            || real_elapsed >= self.time_limit_sec * REAL_HARD_LIMIT_RATIO
    }

    #[inline]
    fn elapsed_sec(&self) -> f64 {
        (self.real_elapsed_sec() - self.excluded_sec).max(0.0)
    }
...
struct Solver {
    ...
    /// 基準deepのpaced枠を消費した後、実測slack内で行った追加holdout数。
    spare_rollout_sessions: usize,
    spare_rollout_spent_sec: f64,
    /// 第一層が進捗枠へ追いつき、なお大きなslackが残るときだけ行う第二層。
    reserve_rollout_sessions: usize,
    reserve_rollout_spent_sec: f64,
...
impl Solver {
    fn new(
        ...
        let mut solver = Self {
            ...
            spare_rollout_sessions: 0,
            spare_rollout_spent_sec: 0.0,
            reserve_rollout_sessions: 0,
            reserve_rollout_spent_sec: 0.0,
    ...
    #[inline]
    fn spare_rollout_pace_sec(&self, incoming_id: usize) -> f64 {
        let progress = (incoming_id + 1) as f64 / self.M.max(1) as f64;
        let ratio = (SPARE_ROLLOUT_PACE_START_RATIO
            + SPARE_ROLLOUT_PACE_SPAN_RATIO * progress)
            .min(SPARE_ROLLOUT_PACE_END_RATIO);
        self.timer.budget_sec(ratio)
    }
    ...
    fn evaluate_candidates_rollout(
            ...
            && !self.timer.reached(DEEP_ROLLOUT_TIME_RATIO)
        {
            ...
            let scale = totals[best].abs().max(1.0);
            let paced_limit =
                Self::paced_session_limit(DEEP_ROLLOUT_SESSION_LIMIT, incoming_id, self.M);
        ...
        // 基準deepのpaced枠を先に使い切った後だけ、同じ上位2候補をfresh scenarioで
        // 再確認する。追加時間は基準時計から除外するが、実時間pace/hard limitは守る。
        if allow_deep
            && cands.len() >= 2
            && self.M - 1 - incoming_id >= DEEP_ROLLOUT_ARRIVALS
        {
            ...
            let scale = totals[best]
                .abs()
                .max(totals[second].abs())
                .max(1.0);
            let gap = (totals[best] - totals[second]).abs();
            let base_paced_limit =
                Self::paced_session_limit(DEEP_ROLLOUT_SESSION_LIMIT, incoming_id, self.M);
            let spare_paced_limit =
                Self::paced_session_limit(SPARE_ROLLOUT_SESSION_LIMIT, incoming_id, self.M);
            let reserve_paced_limit =
                Self::paced_session_limit(RESERVE_ROLLOUT_SESSION_LIMIT, incoming_id, self.M);
            let real_elapsed = self.timer.real_elapsed_sec();
            let pace_deadline = self
                .spare_rollout_pace_sec(incoming_id)
                .min(self.timer.budget_sec(REAL_HARD_LIMIT_RATIO));
            let spare_minimum_slack = self.timer.budget_sec(SPARE_ROLLOUT_MIN_SLACK_RATIO);
            let reserve_minimum_slack =
                self.timer.budget_sec(RESERVE_ROLLOUT_MIN_SLACK_RATIO);
            let base_ready = self.deep_rollout_sessions >= base_paced_limit;
            let gap_ready = gap <= DEEP_ROLLOUT_GAP_RATIO * scale;
            let spare_session_ready = self.spare_rollout_sessions < spare_paced_limit;
            // 第二層は第一層を遅らせない。現在進捗の第一層枠へ追いついた比較だけを使う。
            let reserve_session_ready = !spare_session_ready
                && self.reserve_rollout_sessions < reserve_paced_limit;
            let spare_slack_ready = real_elapsed + spare_minimum_slack <= pace_deadline;
            let reserve_slack_ready = real_elapsed + reserve_minimum_slack <= pace_deadline;
            local! {
                if base_ready && gap_ready {
                    self.trace.count("spare_rollout_candidate");
                    if spare_session_ready {
                        if !spare_slack_ready {
                            self.trace.count("spare_rollout_slack_reject");
                        }
                    } else {
                        self.trace.count("spare_rollout_session_cap");
                        self.trace.count("reserve_rollout_candidate");
                        if !reserve_session_ready {
                            self.trace.count("reserve_rollout_session_cap");
                        } else if !reserve_slack_ready {
                            self.trace.count("reserve_rollout_slack_reject");
                        }
                    }
                }
            }
            let use_spare = base_ready
                && gap_ready
                && spare_session_ready
                && spare_slack_ready;
            let use_reserve = base_ready
                && gap_ready
                && reserve_session_ready
                && reserve_slack_ready;
            if use_spare || use_reserve {
                debug_assert!((cands[best].immediate - cands[second].immediate).abs() <= 1e-7);
                let is_reserve = use_reserve;
                if is_reserve {
                    self.reserve_rollout_sessions += 1;
                } else {
                    self.spare_rollout_sessions += 1;
                }
                local! {
                    if is_reserve {
                        self.trace.count("reserve_rollout_session");
                        self.trace.count_by(
                            "reserve_rollout_slack_us_sum",
                            ((pace_deadline - real_elapsed) * 1_000_000.0).round() as i64,
                        );
                        self.trace.count_by(
                            "reserve_rollout_gap_ppm_sum",
                            (1_000_000.0 * gap / scale).round() as i64,
                        );
                    } else {
                        self.trace.count("spare_rollout_session");
                        self.trace.count_by(
                            "spare_rollout_slack_us_sum",
                            ((pace_deadline - real_elapsed) * 1_000_000.0).round() as i64,
                        );
                        self.trace.count_by(
                            "spare_rollout_gap_ppm_sum",
                            (1_000_000.0 * gap / scale).round() as i64,
                        );
                    }
                }
                ...
                let _before = best;
                let mut fresh_best = 0.0;
                let mut fresh_second = 0.0;
                let mut completed = 0;
                let mut _budget_stopped = false;
                // 第一層はv091どおりpace deadlineまで使える。第二層だけは確保したslackを
                // batch途中にも侵食しないため、保全境界をscenarioごとに確認する。
                let batch_deadline = if is_reserve {
                    pace_deadline - reserve_minimum_slack
                } else {
                    pace_deadline
                };
                for k in 0..SPARE_ROLLOUT_SAMPLES {
                    if self.timer.real_elapsed_sec() >= batch_deadline {
                        _budget_stopped = true;
                        break;
                    }
                    let sample_index = if is_reserve {
                        RESERVE_ROLLOUT_SEED_OFFSET + k
                    } else {
                        SPARE_ROLLOUT_SEED_OFFSET + k
                    };
                    let seed = (incoming_id as u64) * 1_000_003
                        + (sample_index as u64) * 7_919
                        + 1;
                    let mut theta_rng = XorShift64::new(seed ^ 0xA5A5_5A5A_1234_5678);
                    let theta_k = self.sample_theta(theta_rng.next_f64());
                    let arrivals = self.make_future_arrivals(
                    ...
                    fresh_best += cands[best].immediate
                        + self.rollout_one(
                            &cands[best],
                            &dep_base,
                            &arrivals,
                    ...
                    fresh_second += cands[second].immediate
                        + self.rollout_one(
                            &cands[second],
                            &dep_base,
                            &arrivals,
                    ...
                    local! {
                        if is_reserve {
                            self.trace.count("reserve_rollout_scenario");
                            self.trace
                                .count_by("reserve_terminal_value_evaluation", 2);
                        } else {
                            self.trace.count("spare_rollout_scenario");
                            self.trace
                                .count_by("spare_terminal_value_evaluation", 2);
                        }
                    }
                }
                let extra_elapsed = extra_start.elapsed().as_secs_f64();
                if is_reserve {
                    self.reserve_rollout_spent_sec += extra_elapsed;
                } else {
                    self.spare_rollout_spent_sec += extra_elapsed;
                }
                self.timer.exclude_elapsed(extra_elapsed);
                if completed == SPARE_ROLLOUT_SAMPLES {
                    local! {
                        if is_reserve {
                            self.trace.count("reserve_rollout_complete");
                        } else {
                            self.trace.count("spare_rollout_complete");
                        }
                    }
                    let after = if fresh_second > fresh_best {
                        second
                    } else {
                        best
                    };
                    local! {
                        if after != _before {
                            if is_reserve {
                                self.trace.count("reserve_rollout_flip");
                            } else {
                                self.trace.count("spare_rollout_flip");
                            }
                        }
                    }
                    best = after;
                } else {
                    // 不完全なholdoutは標本数が実行速度に依存するため、判断には使わない。
                    local! {
                        if is_reserve {
                            self.trace.count("reserve_rollout_incomplete");
                        } else {
                            self.trace.count("spare_rollout_incomplete");
                        }
                    }
                }
                local! {
                    if _budget_stopped {
                        if is_reserve {
                            self.trace.count("reserve_rollout_budget_stop");
                        } else {
                            self.trace.count("spare_rollout_budget_stop");
                        }
                    }
                }
            }
    ...
    fn select_normal_by_rollout(
        ...
        debug_assert!(choices.iter().all(|placement| {
            placement.perimeter == choices[0].perimeter
                && placement.component_size == choices[0].component_size
        }));
        let incoming_S = self.groups[incoming_id].S;
        let incoming_T = self.groups[incoming_id].T;
        let incoming_P = self.groups[incoming_id].P;
    ...
    fn run<R: BufRead, W: Write>(
        ...
        local! {
            ...
            self.trace.count_by(
                "spare_rollout_extra_time_us",
                (self.spare_rollout_spent_sec * 1_000_000.0).round() as i64,
            );
            self.trace.count_by(
                "spare_rollout_virtual_excluded_us",
                (self.spare_rollout_spent_sec * 1_000_000.0).round() as i64,
            );
            self.trace.count_by(
                "reserve_rollout_extra_time_us",
                (self.reserve_rollout_spent_sec * 1_000_000.0).round() as i64,
            );
            self.trace.count_by(
                "reserve_rollout_virtual_excluded_us",
                (self.reserve_rollout_spent_sec * 1_000_000.0).round() as i64,
            );
            self.trace.count_by(
                "policy_virtual_excluded_us",
                (self.timer.excluded_sec * 1_000_000.0).round() as i64,
            );
            if self.timer.real_elapsed_sec()
                >= self.timer.budget_sec(REAL_HARD_LIMIT_RATIO)
            {
                self.trace.count("spare_rollout_real_hard_reached");
            }
            self.trace.count_by(
