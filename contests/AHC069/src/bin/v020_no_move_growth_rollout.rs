// v020_no_move_growth_rollout.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: v019 のgrowth SA後に同条件の完成候補を最大3件残し、既存の
// 短期rolloutで将来の配置余地を比較する。regular配置を含む他の方針は変更しない。
...
impl Solver {
    ...
    fn growth_placement(
        ...
        // 元のgrowth候補を残したまま、有望な少数だけをSAで置き換える。
        // これにより時間打ち切り時も少なくともv018相当の完成候補が残る。
        let mut sa_order: Vec<usize> = (0..candidates.len()).collect();
        sa_order.sort_by(|&a, &b| {
            candidates[a]
                .perimeter
        ...
        sa_order.truncate(GROWTH_SA_CANDIDATES);
        for index in sa_order {
        ...
        for mut candidate in candidates {
            ...
            evaluated.push((candidate, delta));
        }

        // 候補0はv019と同じ走査順・比較式で選ぶ。以降は即時feeとadmissionを
        // 同一に保つため、同じperimeter・component_sizeの候補だけに限定する。
        let current_index = current_index?;
        let current_perimeter = evaluated[current_index].0.perimeter;
        let current_component_size = evaluated[current_index].0.component_size;
        let mut cheap_index = current_index;
        let mut fragment_index = current_index;
        for i in 0..evaluated.len() {
            if evaluated[i].0.perimeter != current_perimeter
                || evaluated[i].0.component_size != current_component_size
            {
                continue;
            }
        ...
        local! {
            ...
            self.trace.count_by(
                "growth_slack_sum",
                (current_perimeter - minimum_perimeter(P)) as i64,
            );
        }
        Some(choices)
    }
        ...
        for turn in 0..self.M {
            ...
            if passed_price_prefilter {
                ...
                let normal_choices = local_time!(self.trace, "normal_search", {
                    self.find_normal_placements(P, S, T, fast_mode)
                });
                if let Some(mut choices) = normal_choices {
                    ...
                    let from_growth = choices[0].shape_index == usize::MAX;
                    if from_growth {
                        // growth候補は同じ即時fee (perimeter) とadmission閾値
                        // (component_size) を持つため、rolloutだけが選択差になる。
                        debug_assert!(choices.iter().all(|placement| {
                            placement.shape_index == usize::MAX
                                && placement.perimeter == current_perimeter
                                && placement.component_size == current_component_size
                        }));
                    }
                    ...
                    let compared_by_rollout = accepted && choices.len() >= 2;
                    let winner = if compared_by_rollout {
                        self.select_normal_by_rollout(&choices, id, theta, base_threshold)
                    } else {
                        0
                    };
                    local! {
                        if from_growth && compared_by_rollout {
                            self.trace.count("growth_rollout_session");
                            self.trace
                                .count_by("growth_rollout_candidate_sum", choices.len() as i64);
                            if winner != 0 {
                                self.trace.count("growth_rollout_flip");
                            }
                        }
                    }
