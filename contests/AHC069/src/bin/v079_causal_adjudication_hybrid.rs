// v079_causal_adjudication_hybrid.rs
#![allow(non_snake_case)]
// 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: v078 の二専門家構成に、近接した rollout 上位2候補の中央値再判定と、
// 短命・低余剰の受け入れを保留する因果 veto を加える。低 R の relocation 予算も拡張する。
        ...
        if cands.len() >= 2 && !self.timer.reached(0.82) {
            ...
            if totals[best] - totals[second] <= 0.025 * scale {
                ...
                totals[best] += cands[best].immediate
                    + self.rollout_one(&cands[best], &dep_base, &arrivals, threshold);
                totals[second] += cands[second].immediate
                    + self.rollout_one(&cands[second], &dep_base, &arrivals, threshold);
