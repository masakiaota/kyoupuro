...
impl Solver {
    ...
    fn evaluate_candidates_rollout(
        ...
        for k in 0..ROLLOUT_SAMPLES {
            ...
            // 決定的 3 シナリオ: θ_k を事後分位点 (低負荷 10% / 中央 50% / 高負荷 90%)
            // に固定する。v053 のランダムサンプルと同じ多様性を、サンプリング分散なしで得る。
            const SCENARIO_U: [f64; 3] = [0.1, 0.5, 0.9];
            let theta_k = self.sample_theta(SCENARIO_U[k % SCENARIO_U.len()]);
            local! {
                if k == ROLLOUT_SAMPLES - 1 {
                    let width = (self.sample_theta(0.9) - self.sample_theta(0.1)).abs();
                    self.trace.count_by(
                        "theta_scenario_width_permille",
                        ((width / theta.max(1.0)) * 1000.0).round() as i64,
                    );
                }
            }
