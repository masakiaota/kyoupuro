// v080_terminal_rollout_hybrid.rs
#![allow(non_snake_case)]
// 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: v079 の二専門家構成へ、拒否を候補に含む終端容量価値付き rollout、
// terrain-aware な候補生成、rough 側の共有探索状態と回数 quota を統合する。
...
impl Solver {
    ...
    fn lognormal_tail_first_moment(q_min: f64) -> f64 {
        const SIGMA: f64 = 0.8 * std::f64::consts::LN_2;
        const INV_SQRT_2: f64 = 0.707_106_781_186_547_6;
    ...
    fn expected_truncated_duration09(theta: f64, remaining_time: usize) -> f64 {
        ...
        for k in 0..8 {
            ...
            let duration = (remaining_time as f64).min(-theta * (-u).ln_1p() + 1.0);
    ...
    fn rollout_one(
        ...
        if add_terminal_value {
            let remaining_groups =
                self.M as isize - 1 - incoming_id as isize - arrivals.len() as isize;
            if remaining_groups > 0 {
                ...
                let probe_at = HORIZON.min(terminal_at + (mean_gap.round() as usize).max(1));

                // 22到着後の次の期待到着時刻まで、既知退去とrollout内退去を反映する。
                let mut probe = occ;
                let mut probe_base_i = base_i;
                let mut probe_future = future_deps;
                loop {
                    let next_base = dep_base.get(probe_base_i).map(|&(t, _)| t);
                    let next_future = probe_future
                        .iter()
                        .enumerate()
                        .filter(|(_, (t, _))| *t < probe_at)
                        .min_by_key(|(_, (t, _))| *t)
                        .map(|(i, (t, _))| (*t, i));
                    match (next_base, next_future) {
                        (Some(tb), Some((tf, fi))) if tb < probe_at || tf < probe_at => {
                            if tb < probe_at && (tf >= probe_at || tb <= tf) {
                                let gid = dep_base[probe_base_i].1;
                                probe_base_i += 1;
                                self.release_cells_for(&mut probe, gid, &cand.overrides);
                            } else {
                                let (_, cells) = probe_future.swap_remove(fi);
                                for cell in cells {
                                    probe[cell / self.N] &= !Self::bit_at(cell % self.N);
                                }
                            }
                        }
                        (Some(tb), None) if tb < probe_at => {
                            let gid = dep_base[probe_base_i].1;
                            probe_base_i += 1;
                            self.release_cells_for(&mut probe, gid, &cand.overrides);
                        }
    ...
    fn evaluate_candidates_rollout(
        ...
        for k in 0..ROLLOUT_SAMPLES {
            ...
            for (i, cand) in cands.iter().enumerate() {
                ...
                local! {
                    if add_terminal_value {
                        self.trace.count("terminal_value_evaluation");
                    }
                }
            }
        }
        ...
        if cands.len() >= 2 && !self.timer.reached(0.82) {
            ...
            if totals[best] - totals[second] <= 0.025 * scale {
                ...
                local! {
                    if add_terminal_value {
                        self.trace.count_by("terminal_value_evaluation", 2);
                    }
                }
    ...
    fn select_normal_by_rollout(
        ...
        debug_assert!(choices.len() >= 2 || include_reject);
        ...
        let mut cands = Vec::with_capacity(choices.len() + usize::from(include_reject));
        ...
        if include_reject {
            ...
            local! {
                self.trace.count("terminal_rollout_session");
            }
        }
        ...
        for turn in 0..self.M {
            ...
            if passed_price_prefilter {
                ...
                if let Some(mut choices) = normal_choices {
                    ...
                    let best_perimeter = choices
                        .iter()
                        .map(|placement| placement.perimeter)
                        .min()
                        .expect("non-empty placement choices");
                    let best_margin = if base_threshold > 0.0 {
                        q_value * compactness(P, best_perimeter) / actual_threshold.max(1e-300)
                    } else {
                        1e100
                    };
                    let duration_ratio = (duration as f64) / theta;
                    let best_slack = best_perimeter - minimum_perimeter(P);
                    let reject_rollout = accepted
                        && base_threshold > 0.0
                        && !self.timer.reached(0.80)
                        && (1.0..=1.13).contains(&best_margin)
                        && duration_ratio < 2.0
                        && best_slack <= 14;
                    local! {
                        if reject_rollout {
                            self.trace.count("reject_rollout_offered");
                        }
                    }

                    let mut winner = if accepted && (choices.len() >= 2 || reject_rollout) {
                    ...
                    let rollout_rejected = reject_rollout && winner == choices.len();
                    if rollout_rejected {
                        accepted = false;
                        winner = 0;
                        local! {
                            self.trace.count("reject_rollout_selected");
                        }
                    } else if reject_rollout {
                        local! {
                            self.trace.count("reject_rollout_accept_selected");
                        }
                    }
                    local! {
                    ...
                    if accepted && base_threshold > 0.0 && !reject_rollout {
...
mod rough {
    ...
    struct FixedHeap<T: Copy + Ord, const CAP: usize> {
        data: [Option<T>; CAP],
        len: usize,
    }

    impl<T: Copy + Ord, const CAP: usize> FixedHeap<T, CAP> {
        const fn new() -> Self {
            Self {
                data: [None; CAP],
                len: 0,
        ...
        fn push(&mut self, value: T) {
            ...
            while at > 0 {
                let parent = (at - 1) / 2;
                if self.data[parent].unwrap() >= value {
                    break;
                }
                self.data[at] = self.data[parent];
                at = parent;
            }
            self.data[at] = Some(value);
        }
        ...
        fn pop(&mut self) -> Option<T> {
            ...
            let last = self.data[self.len].take().unwrap();
            let mut at = 0;
            while 2 * at + 1 < self.len {
                let mut child = 2 * at + 1;
                if child + 1 < self.len && self.data[child + 1].unwrap() > self.data[child].unwrap()
                {
                    child += 1;
                }
                if self.data[child].unwrap() <= last {
                    break;
                }
                self.data[at] = self.data[child];
                at = child;
            }
            self.data[at] = Some(last);
            result
        }

        fn drain_to(&mut self, output: &mut Vec<T>) {
    ...
    impl RegionHashSet {
        ...
        fn insert(&mut self, key: u64) -> bool {
            let mut position = splitmix64(key) as usize & 511;
            for _ in 0..512 {
    ...
    pub(super) struct Solver {
        ...
        pair_touch_comp: Vec<Vec<usize>>,
        pair_touch_group: Vec<u8>,
        pair_active_ids: Vec<usize>,
        pair_active_pos: Vec<isize>,
        #[cfg(feature = "local")]
        trace: TraceStats,
    }
    ...
    impl Solver {
        pub(super) fn new(
            ...
            let mut solver = Self {
                ...
                pair_touch_comp: (0..M).map(|_| Vec::new()).collect(),
                pair_touch_group: Vec::new(),
                pair_active_ids: vec![0; M],
                pair_active_pos: vec![-1; M],
                #[cfg(feature = "local")]
                trace: TraceStats::default(),
            };
        ...
        fn placement_priority(&self, group: &Group, q_req: f64) -> f64 {
            let ratio = group.raw_q / q_req.max(0.12);
        ...
        fn add_compact_template_candidates(
            ...
            let mut best: FixedHeap<(i64, usize, usize, usize), 64> = FixedHeap::new();
            let mut extra: [FixedHeap<(i64, usize, usize, usize), 16>; 2] =
                [FixedHeap::new(), FixedHeap::new()];
            ...
            best.drain_to(&mut hits);
            extra[0].drain_to(&mut hits);
            extra[1].drain_to(&mut hits);
        ...
        fn grow_region(
            ...
            let mut heap: FixedHeap<Reverse<HeapNode>, 16384> = FixedHeap::new();
        ...
        fn polish_region(
            ...
            let mut additions = [0_usize; MAX_C];
            for _ in 0..max_iterations {
                ...
                let mut best_add = 0;
                for &w in &additions[..addition_count] {
        ...
        fn peel_box_region(
            ...
            let mut best_component = [0_usize; MAX_C];
            let mut best_component_len = 0;
            let mut queue = [0_usize; MAX_C];
            for x in x0..x0 + h {
                for y in y0..y0 + w {
                    ...
                    if tail > best_component_len {
                        best_component[..tail].copy_from_slice(&queue[..tail]);
                        best_component_len = tail;
                    }
                }
            }
            if best_component_len < P {
                return None;
            }
            let full_size = best_component_len;
            let mut alive = [false; MAX_C];
            let mut seen = [0_i32; MAX_C];
            let mut bfs_queue = [0_usize; MAX_C];
            for (index, &z) in best_component[..full_size].iter().enumerate() {
                alive[index] = true;
                self.local_index[z] = index as isize;
            }
            ...
            for &z in &best_component[..full_size] {
                for &direction in &[0_usize, 2] {
                    let v = self.nb[z][direction];
                    if v >= 0 && self.local_index[v as usize] >= 0 {
                        internal_edges += 1;
                    }
                }
            }
            let mut current_L = (4 * full_size - 2 * internal_edges) as i32;
            let mut candidates = [(0_i64, 0_usize, 0_i32); MAX_C];
            while alive_count > P {
                let mut sum_x = 0_i64;
                let mut sum_y = 0_i64;
                for (index, &z) in best_component[..full_size].iter().enumerate() {
                ...
                for (index, &z) in best_component[..full_size].iter().enumerate() {
                    ...
                    let score = (2 * degree - 4) as i64 * 1_000_000_000_000
                        ...
                        - (self.terrain_roughness
                            ...
                            * self.separator_field[z] as f64)
                            .round() as i64;
                    candidates[candidate_count] = (score, index, degree);
                    candidate_count += 1;
                }
                candidates[..candidate_count]
                    .sort_unstable_by_key(|&(score, index, _)| (score, index));
                let mut removed = false;
                for &(_, remove, degree) in &candidates[..candidate_count] {
                    alive[remove] = false;
                    let mut connected = true;
                    if degree >= 2 {
                        let root = alive[..full_size].iter().position(|&value| value);
                        seen_stamp += 1;
                        let mut head = 0;
                        let mut tail = 0;
                        let mut got = 0;
            ...
            let cells: Vec<usize> = best_component[..full_size]
                .iter()
                .enumerate()
        ...
        fn find_peel_regions_with_state(
            ...
            let mut heap: FixedHeap<(i64, usize, usize, usize, usize), 32> = FixedHeap::new();
        ...
        fn find_regions_with_state(
            ...
            if box_limit > 0 {
                let mut heap: FixedHeap<(i64, usize, usize, usize, usize), 64> = FixedHeap::new();
        ...
        fn same_region(region: &Region, group: &Group) -> bool {
            ...
            let mut marked = [false; MAX_C];
            for &z in &region.cells {
                marked[z] = true;
            }
            group.cells.iter().all(|&z| marked[z])
        }
        ...
        fn movable_candidates(
            ...
            let mut best = [(0.0_f64, 0_usize); 16];
            ...
            for group_id in 0..self.M {
                ...
                if best_count < limit {
                    ...
                    for index in 1..best_count {
                        if best[index].0 > best[worst].0
                            || (best[index].0 == best[worst].0 && best[index].1 > best[worst].1)
                        {
                            worst = index;
                        }
                    }
                    if item.0 < best[worst].0 || (item.0 == best[worst].0 && item.1 < best[worst].1)
                    {
                        best[worst] = item;
                    }
                }
            }
            best[..best_count].sort_by(|a, b| {
                a.0.partial_cmp(&b.0)
                    .unwrap_or(Ordering::Equal)
                    .then(a.1.cmp(&b.1))
            });
        ...
        fn movable_pair_candidates(
            ...
            self.pair_active_pos.fill(-1);
            let mut active_count = 0;
            for group_id in 0..self.M {
                if self.groups[group_id].active {
                    self.pair_active_pos[group_id] = active_count as isize;
                    self.pair_active_ids[active_count] = group_id;
                    active_count += 1;
                }
            }
            let pair_slots = active_count * active_count;
            if self.pair_touch_group.len() < pair_slots {
                self.pair_touch_group.resize(pair_slots, 0);
            }
            self.pair_touch_group[..pair_slots].fill(0);
            let mut seen_component = [-1_isize; MAX_C];
            for aa in 0..active_count {
                self.pair_touch_comp[aa].clear();
                    ...
                    for &w in &self.nb[z] {
                        ...
                        if self.occ[w] == -1 {
                            ...
                            if component >= 0 && seen_component[component as usize] != aa as isize {
                                ...
                                self.pair_touch_comp[aa].push(component as usize);
                            }
                        } else if self.occ[w] >= 0 {
                            let bb = self.pair_active_pos[self.occ[w] as usize];
                            if bb >= 0 && bb as usize != aa {
                                self.pair_touch_group[aa * active_count + bb as usize] = 1;
                            }
                        }
                    }
                }
                self.pair_touch_comp[aa].sort_unstable();
            }
            let mut best = [(0.0_f64, 0_usize, 0_usize); 16];
            let mut best_count = 0;
            for aa in 0..active_count {
                for bb in aa + 1..active_count {
                    let mut linked = self.pair_touch_group[aa * active_count + bb] != 0
                        || self.pair_touch_group[bb * active_count + aa] != 0;
                    let touching_a = &self.pair_touch_comp[aa];
                    let touching_b = &self.pair_touch_comp[bb];
                    let mut ia = 0;
                    let mut ib = 0;
                    ...
                    while ia < touching_a.len() || ib < touching_b.len() {
                        let component;
                        if ib == touching_b.len()
                            || (ia < touching_a.len() && touching_a[ia] < touching_b[ib])
                        {
                            component = touching_a[ia];
                            ia += 1;
                        } else if ia == touching_a.len() || touching_b[ib] < touching_a[ia] {
                            component = touching_b[ib];
                            ib += 1;
                        } else {
                            component = touching_a[ia];
                            ia += 1;
                    ...
                    let score = loss
                        ...
                        + 0.00002
                            * ((self.groups[group_a].P + self.groups[group_b].P) as f64
                                - incoming_P as f64)
                                .abs();
                    let item = (score, group_a, group_b);
                    if best_count < limit {
                        best[best_count] = item;
                        best_count += 1;
                    } else {
                        ...
                        for index in 1..best_count {
                            if best[index].0 > best[worst].0
                                || (best[index].0 == best[worst].0
                                    && (best[index].1, best[index].2)
                                        > (best[worst].1, best[worst].2))
                            {
                                worst = index;
                            }
                        }
                        if item.0 < best[worst].0
                            || (item.0 == best[worst].0
                                && (item.1, item.2) < (best[worst].1, best[worst].2))
                        {
                            best[worst] = item;
                        }
                    }
            ...
            best[..best_count].iter().map(|&(_, a, b)| (a, b)).collect()
        }
        ...
        fn consider_move_plan(
            ...
            let fee_quality = net_gain as f64 / current.denominator;
        ...
        pub(super) fn run<R: BufRead, W: Write>(
            ...
            for index in 0..self.M {
                ...
                let duration = T - S;
                self.groups[index].denominator = P as f64 * (duration as f64).powf(0.9);
                self.groups[index].raw_q = V as f64 / self.groups[index].denominator;
                ...
                let denominator = self.groups[index].denominator;
                let raw_q = self.groups[index].raw_q;
