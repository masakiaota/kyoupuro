...
// 中心アイデア: v053 の既存黒字 relocation 経路を保ったまま、短期採算ゲートで落ちる
// 再配置案を別枠で少数だけ既存 rollout へ渡し、将来価値で短期赤字を回収できるか試す。
...
impl Solver {
    ...
    fn collect_relocation_targets(
        ...
        blocker_limit: usize,
        baseline: f64,
    ) -> (Vec<TargetOption>, Vec<TargetOption>) {
        ...
        let mut deferred_options = Vec::new();
        #[cfg(feature = "local")]
        let mut deferred_target_count = 0_i64;
                ...
                for x in 0..=(self.N - shape.h) {
                    ...
                    while ys != 0 {
                        ...
                        let cost: i64 = blockers
                            ...
                            .sum();
                        // 従来の採算ゲートを通るtargetと、短期赤字を将来価値で回収するための
                        // 別枠targetを分ける。後者もgross surplusが正のものに限定する。
                        let is_deferred = surplus <= baseline + 1.12 * (cost as f64);
                        ...
                        let target_options = if is_deferred {
                            local! {
                                deferred_target_count += 1;
                            }
                            &mut deferred_options
                        } else {
                            &mut options
                        };
        ...
        deferred_options.sort_by(|a, b| a.rank.total_cmp(&b.rank));
        local! {
            self.trace
                .count_by("deferred_target_option", deferred_target_count);
            self.trace
                .count_by("relocation_option", options.len() as i64);
            self.trace
                .count_by("deferred_target_kept", deferred_options.len() as i64);
        }
    ...
    fn attempt_relocation(
        ...
        let blocker_limit = 4;
        let (options, deferred_options) = self.collect_relocation_targets(
            incoming,
            q_value,
            base_threshold,
            blocker_limit,
        ...
        let baseline_immediate = baseline_normal
            .as_ref()
            .map_or(0.0, |(_, C)| (incoming.V as f64) * C);
        ...
        let mut deferred_collected: Vec<(MovePlan, f64, f64, i64, f64)> = Vec::new();
        for (attempt, option) in options.into_iter().enumerate() {
            ...
            let threshold =
                base_threshold * self.component_threshold_factor(option.placement.component_size);
            let surplus = scale * (q_value * C - threshold);
            let net = surplus - (cost as f64) - fee_loss;
            if net <= baseline + 0.12 * (cost as f64) {
                let immediate = (incoming.V as f64) * C - (cost as f64) - fee_loss;
                if immediate < baseline_immediate && deferred_collected.len() < 2 {
                    local! {
                        self.trace.count("deferred_plan_collected");
                        self.trace.count("deferred_from_final_gate");
                        self.trace.count_by(
                            "deferred_shortfall_sum",
                            (baseline_immediate - immediate).round() as i64,
                        );
                    }
                    let mut incoming_placement = option.placement;
                    incoming_placement.explicit_cells = target_cells;
                    deferred_collected.push((
                        MovePlan {
                            ok: true,
        ...
        // 従来のtarget採算ゲートで落ちた案も少数だけrepackする。既存の黒字案最大3件は
        // そのまま残し、実際の即時実額がbaselineより低い案だけを別枠最大2件へ入れる。
        const DEFERRED_ATTEMPT_LIMIT: usize = 6;
        for (attempt, option) in deferred_options.into_iter().enumerate() {
            if attempt >= DEFERRED_ATTEMPT_LIMIT
                || deferred_collected.len() >= 2
            ...
            let C = compactness(incoming.P, option.placement.perimeter);
            let immediate = (incoming.V as f64) * C - (cost as f64) - fee_loss;
            if immediate >= baseline_immediate {
                local! {
                    self.trace.count("deferred_actual_nonloss");
                }
                continue;
            }
            let threshold =
            ...
            local! {
                self.trace.count("deferred_plan_collected");
                self.trace.count("deferred_from_target_gate");
                self.trace.count_by(
                    "deferred_shortfall_sum",
                    (baseline_immediate - immediate).round() as i64,
                );
        ...
        collected.extend(deferred_collected);
        ...
        local! {
            ...
            let winner_C = compactness(incoming.P, plan.incoming.perimeter);
            let winner_immediate =
                (incoming.V as f64) * winner_C - (*cost as f64) - *fee_loss;
            if winner_immediate < baseline_immediate {
                self.trace.count("deferred_rollout_win");
                self.trace.count_by(
                    "deferred_winner_shortfall_sum",
                    (baseline_immediate - winner_immediate).round() as i64,
                );
            }
            self.trace
