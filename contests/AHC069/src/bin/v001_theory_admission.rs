...
const FEASIBLE_CANDIDATE_LIMIT: usize = 64;
...
fn contact_score(
    ...
    for &(dx, dy) in &template.cells {
        ...
        for (sx, sy) in [(-1_i32, 0_i32), (1, 0), (0, -1), (0, 1)] {
            ...
            if nx < 0 || ny < 0 || nx >= N as i32 || ny >= N as i32 {
                score += 1;
            } else if (blocked_rows[nx as usize] & (1_u64 << (ny as usize))) != 0 {
                score += 1;
            }
        }
    }
...
fn find_placement(
    ...
    for component in components {
        ...
        let mut feasible_in_component = 0;
        for template in templates {
            ...
            for top in 0..=N - template.height {
                for left in 0..=N - template.width {
                    ...
                    feasible_in_component += 1;
                    feasible_total += 1;
                    let score = contact_score(N, blocked_rows, template, top, left);
                    if best
                        .as_ref()
                    ...
                    if feasible_in_component >= FEASIBLE_CANDIDATE_LIMIT {
                        return PlacementSearch {
                            cells: best.map(|(_, cells)| cells),
                            checked,
...
fn main() {
    ...
    let _ = R; // v001 は移動を行わないため、移動費係数は意思決定に使わない。
    ...
    for turn in 0..M {
        ...
        let _ = (search.checked, search.feasible);
