// v015_quick_repack.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: blocker の repack 冒頭で軽量な逐次 first-fit を試し、全員を置けて
// 既存採算も満たす場合だけ即採用する。配置不能または fee_loss が採算上限を超える場合は、
// 初期盤面から v014 と同じ beam repack を実行し、既存探索の範囲を狭めない。
...
impl Solver {
    ...
    fn try_quick_repack(
        ...
        order: &[usize],
        base: &Rows,
    ) -> Result<(Vec<(usize, Placement)>, Rows, f64), usize> {
        let mut occ = *base;
        let mut placements = Vec::with_capacity(order.len());
        let mut fee_loss = 0.0;
        for (depth, &group_id) in order.iter().enumerate() {
            let group = &self.groups[group_id];
            let Some((cells, perimeter)) = self.quick_place_with_perimeter(&occ, group.P) else {
                return Err(depth);
            };
            for &cell in &cells {
                occ[cell / self.N] |= Self::bit_at(cell % self.N);
            }
            if perimeter > group.worst_perimeter {
                fee_loss += (group.V as f64)
                    * (compactness(group.P, group.worst_perimeter)
                        - compactness(group.P, perimeter));
            }
            placements.push((
                group_id,
                Placement {
    ...
    fn repack_blockers(
        ...
        local! {
            self.trace.count("repack_quick_attempt");
        }
        let quick = local_time!(self.trace, "repack_quick", {
            self.try_quick_repack(&order, &base)
        });
        match quick {
            Ok((placements, occ, fee_loss)) => {
                local! {
                    self.trace.count("repack_quick_success");
                }
                // quick が既存の最終採算を満たすときだけ確定する。不採算なら、同じ base から
                // 既存ビームをそのまま走らせ、より低損失な配置を探す。
                if fee_loss < max_quick_fee_loss {
                    let metric = self.fragment_metric(&occ);
                    local! {
                        self.trace.count("fragment_evaluated");
                        self.trace.count("repack_quick_used");
                        self.trace.count("repack_success");
                        if fee_loss > 0.0 {
                            self.trace.count("repack_used_perimeter_over");
                        }
                    }
                    return Some((placements, fee_loss, metric));
                }
                local! {
                    self.trace.count("repack_quick_economic_reject");
                    self.trace.count("repack_beam_fallback");
                }
            }
            Err(_depth) => {
                local! {
                    self.trace.count_by("repack_quick_fail_depth_sum", _depth as i64);
                    self.trace.count("repack_beam_fallback");
                }
            }
        }
    ...
    /// L_min と L_min+2 の先頭数形状を bitboard で走査し、最初に入る位置へ置く。
    /// growth はしない。repack と rollout で同じ軽量探索を共有する。
    fn quick_place_with_perimeter(&self, occ: &Rows, P: usize) -> Option<(Vec<usize>, usize)> {
        let runs = self.build_run_table(occ);
        let min_L = minimum_perimeter(P);
    ...
    /// ロールアウト用の簡易配置。戻り値は (セル列, C)。
    fn quick_place(&self, occ: &Rows, P: usize) -> Option<(Vec<usize>, f64)> {
        self.quick_place_with_perimeter(occ, P)
            .map(|(cells, perimeter)| (cells, compactness(P, perimeter)))
    }
    ...
    fn attempt_relocation(
        ...
        for (attempt, option) in options.into_iter().enumerate() {
            ...
            // blocker の移動費は配置形状によらないため、quick を確定できる fee_loss 上限を
            // repack 前に求められる。
            let cost: i64 = option
                .blockers
                .iter()
            ...
            let max_quick_fee_loss = surplus - baseline - 1.12 * (cost as f64);
            let repack_result = local_time!(self.trace, "repack_total", {
                self.repack_blockers(&option.blockers, &target_cells, max_quick_fee_loss)
            });
            let Some((repacked, fee_loss, final_metric)) = repack_result else {
                continue;
            };
            ...
            if passed_price_prefilter {
                ...
                if let Some(placement) = &normal {
                    ...
                    if accepted {
                        ...
                        net_normal = scale * (quality - actual_threshold);
                    } else {
                        local! {
