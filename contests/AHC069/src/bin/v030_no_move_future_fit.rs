// v030_no_move_future_fit.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。

// 中心アイデア: v029 を保ち、長寿命の通常配置候補を、現役groupの
// 将来退去を反映した3時点の compact shape 配置余地で比較する。再移動は行わない。
...
const FUTURE_FIT_P: [usize; 3] = [48, 80, 120];
const FUTURE_FIT_WEIGHT: [f64; 3] = [1.0, 3.0, 7.0];
const FUTURE_FIT_SHAPES_PER_P: usize = 6;
const FUTURE_FIT_REPRESENTATIVES: usize = FUTURE_FIT_P.len() * FUTURE_FIT_SHAPES_PER_P;
const FUTURE_FIT_FRACTION: [f64; 3] = [0.2, 0.5, 0.8];
const FUTURE_FIT_MIN_DURATION: usize = 5_000;
const SLOT_PENALTY_EPS: f64 = 1e-7;
...
#[derive(Clone, Copy)]
struct FutureFitRepresentative {
    P: usize,
    shape_index: usize,
    weight: f64,
}
...
#[derive(Clone, Copy)]
struct FutureFitBaseline {
    valid_starts: Rows,
    count: usize,
}

#[derive(Clone, Copy)]
struct FutureFitEvaluation {
    weighted_retention: f64,
}
...
struct Solver {
    ...
    future_fit_representatives: [FutureFitRepresentative; FUTURE_FIT_REPRESENTATIVES],
...
impl Solver {
    fn new(N: usize, M: usize, grass_rows: Rows, timer: TimeKeeper) -> Self {
        ...
        let shapes_by_p = generate_shapes(N);
        let mut future_fit_representatives = Vec::with_capacity(FUTURE_FIT_REPRESENTATIVES);
        for (scale, &P) in FUTURE_FIT_P.iter().enumerate() {
            let min_L = minimum_perimeter(P);
            let shape_indices: Vec<usize> = shapes_by_p[P]
                .iter()
                .enumerate()
                .filter_map(|(index, shape)| (shape.perimeter == min_L).then_some(index))
                .take(FUTURE_FIT_SHAPES_PER_P)
                .collect();
            assert_eq!(
                shape_indices.len(),
                FUTURE_FIT_SHAPES_PER_P,
                "P={P} must have six minimum-perimeter representative shapes"
            );
            for shape_index in shape_indices {
                future_fit_representatives.push(FutureFitRepresentative {
                    P,
                    shape_index,
                    weight: FUTURE_FIT_WEIGHT[scale],
                });
            }
        }
        let future_fit_representatives: [FutureFitRepresentative; FUTURE_FIT_REPRESENTATIVES] =
            future_fit_representatives
                .try_into()
                .unwrap_or_else(|_| unreachable!("representative count was asserted"));
        let mut solver = Self {
            N,
            M,
            grass_rows,
    ...
    /// `mask` の各bitを右へ `[0, len)` だけ広げる。shapeと交差する
    /// 開始列をbit演算でまとめて除外するために使う。
    #[inline]
    fn future_fit_right_dilate(mask: u64, len: usize) -> u64 {
        debug_assert!(len >= 1);
        let highest_bit = (usize::BITS - 1 - len.leading_zeros()) as usize;
        let mut result = mask;
        let mut span = 1;
        for bit in (0..highest_bit).rev() {
            result |= result >> span;
            span *= 2;
            if len & (1_usize << bit) != 0 {
                result |= mask >> span;
                span += 1;
            }
        }
    ...
    fn build_future_fit_baselines_for_runs(
        &self,
        runs: &RunTable,
    ) -> [FutureFitBaseline; FUTURE_FIT_REPRESENTATIVES] {
        let mut baselines = [FutureFitBaseline {
            valid_starts: [0; MAX_N],
            count: 0,
        }; FUTURE_FIT_REPRESENTATIVES];
        for (index, representative) in self.future_fit_representatives.iter().enumerate() {
            let shape = &self.shapes_by_p[representative.P][representative.shape_index];
            debug_assert_eq!(shape.perimeter, minimum_perimeter(representative.P));
            ...
            for x in 0..=(self.N - shape.h) {
                ...
                baselines[index].valid_starts[x] = ys;
                baselines[index].count += ys.count_ones() as usize;
            }
        }
        baselines
    }

    /// 現在確定している退去だけを反映し、incomingは未配置のまま、
    /// 寿命の20/50/80%時点における代表shapeの合法開始位置を作る。
    fn build_future_fit_snapshots(
        &self,
        incoming_S: usize,
        incoming_T: usize,
    ) -> [[FutureFitBaseline; FUTURE_FIT_REPRESENTATIVES]; FUTURE_FIT_FRACTION.len()] {
        let duration = incoming_T - incoming_S;
        let empty = [FutureFitBaseline {
            valid_starts: [0; MAX_N],
            count: 0,
        }; FUTURE_FIT_REPRESENTATIVES];
        let mut snapshots = [empty; FUTURE_FIT_FRACTION.len()];
        for (snapshot_index, &fraction) in FUTURE_FIT_FRACTION.iter().enumerate() {
            let tau = incoming_S + ((duration as f64) * fraction).round() as usize;
            let mut occ = self.occupied_rows;
            for group in &self.groups {
                if group.active && group.T < tau {
                    self.release_cells_for(&mut occ, group.id);
                }
            }
            let runs = self.build_run_table(&occ);
            snapshots[snapshot_index] = self.build_future_fit_baselines_for_runs(&runs);
        }
        snapshots
    }

    /// 各snapshotで、候補を置く前に合法だった代表shapeの開始位置が
    /// 候補後にどれだけ残るかを測る。snapshotは等重み、Pは1:3:7で重み付けする。
    fn evaluate_future_fit(
        &self,
        candidate: &Placement,
        snapshots: &[[FutureFitBaseline; FUTURE_FIT_REPRESENTATIVES]; FUTURE_FIT_FRACTION.len()],
    ) -> Option<FutureFitEvaluation> {
        let mut candidate_rows = [0_u64; MAX_N];
        for &cell in &candidate.explicit_cells {
            candidate_rows[cell / self.N] |= Self::bit_at(cell % self.N);
        }

        let mut weighted_sum = 0.0;
        let mut valid_weight_sum = 0.0;
        for baselines in snapshots {
            for (index, representative) in self.future_fit_representatives.iter().enumerate() {
                ...
                let shape = &self.shapes_by_p[representative.P][representative.shape_index];
                let mut after = baseline.count;
                for x in 0..=(self.N - shape.h) {
                    let valid_starts = baseline.valid_starts[x];
                    if valid_starts == 0 {
                        continue;
                    }
                    let mut intersecting_starts = 0_u64;
                    for rr in 0..shape.h {
                        let row_mask = candidate_rows[x + rr];
                        if row_mask != 0 {
                            let shifted = row_mask >> shape.left[rr];
                            intersecting_starts |=
                                Self::future_fit_right_dilate(shifted, shape.len[rr]);
                        }
                    }
                    after -= (valid_starts & intersecting_starts).count_ones() as usize;
                }
                weighted_sum += representative.weight * (after as f64) / (baseline.count as f64);
                valid_weight_sum += representative.weight;
            }
        }
        (valid_weight_sum > 0.0).then_some(FutureFitEvaluation {
            weighted_retention: weighted_sum / valid_weight_sum,
        })
    }
    ...
    fn find_normal_placements(
        ...
        for perimeter in (min_L..=min_L + max_extra).step_by(2) {
            ...
            let future_fit = if incoming_T - incoming_S >= FUTURE_FIT_MIN_DURATION {
                local! {
                    self.trace.count("future_fit_turn");
                    self.trace.count_by(
                        "future_fit_snapshot",
                        FUTURE_FIT_FRACTION.len() as i64,
                    );
                }
                Some(local_time!(self.trace, "future_fit", {
                    let snapshots = self.build_future_fit_snapshots(incoming_S, incoming_T);
                    let current = &evaluated[current_index].0;
                    let mut best: Option<(usize, FutureFitEvaluation)> = None;
                    let mut current_evaluation = None;
                    let mut evaluated_count = 0_i64;
                    for (index, (candidate, _)) in evaluated.iter().enumerate() {
                        let Some(fit) = self.evaluate_future_fit(candidate, &snapshots) else {
                            continue;
                        };
                        evaluated_count += 1;
                        if index == current_index {
                            current_evaluation = Some(fit);
                        }
                        if candidate.component_size != current.component_size
                            || candidate.slot_penalty > current.slot_penalty + SLOT_PENALTY_EPS
                        {
                            continue;
                        }
                        if best.as_ref().is_none_or(|(best_index, best_fit)| {
                            fit.weighted_retention > best_fit.weighted_retention
                                || (fit.weighted_retention == best_fit.weighted_retention
                                    && candidate.final_score > evaluated[*best_index].0.final_score)
                        }) {
                            best = Some((index, fit));
                        }
                    }
                    (best, current_evaluation, evaluated_count)
                }))
            } else {
                None
            };
            local! {
                if let Some((_, _, evaluated_count)) = &future_fit {
                    self.trace
                        .count_by("future_fit_evaluated", *evaluated_count);
                }
            }
            ...
            if let Some((Some((fit_index, fit)), Some(current_fit), _)) = future_fit {
                let mut candidate = evaluated[fit_index].0.clone();
                if !choices
                    .iter()
                    .any(|choice| choice.explicit_cells == candidate.explicit_cells)
                {
                    assert_eq!(
                        candidate.perimeter, choices[0].perimeter,
                        "future-fit candidate must stay on the first feasible perimeter"
                    );
                    assert_eq!(
                        candidate.component_size, choices[0].component_size,
                        "future-fit candidate must keep admission component size"
                    );
                    assert!(
                        candidate.slot_penalty <= choices[0].slot_penalty + SLOT_PENALTY_EPS,
                        "future-fit candidate must not worsen slot penalty"
                    );
                    assert!(
                        fit.weighted_retention + f64::EPSILON >= current_fit.weighted_retention,
                        "future-fit candidate must retain at least candidate0 future inventory"
                    );
                    candidate.future_fit_candidate = true;
                    local! {
                        self.trace.count("future_fit_candidate_added");
                        self.trace.count_by(
                            "future_fit_retention_before_million",
                            (current_fit.weighted_retention * 1_000_000.0).round() as i64,
                        );
                        self.trace.count_by(
                            "future_fit_retention_after_million",
                            (fit.weighted_retention * 1_000_000.0).round() as i64,
                        );
                    }
                    choices.push(candidate);
                }
    ...
    fn select_normal_by_rollout(
        ...
        let _future_fit_comparison = choices
            .iter()
            .any(|placement| placement.future_fit_candidate);
        local! {
            if _future_fit_comparison {
                self.trace.count("future_fit_rollout_comparison");
            }
        }
        ...
        local! {
            ...
            if choices[winner].future_fit_candidate {
                self.trace.count("future_fit_rollout_chosen");
            }
        }
        winner
    }
