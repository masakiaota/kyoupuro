// v067_posterior_long_stay_veto.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。

// 中心アイデア: v053 の意思決定を保ったまま、推定滞在時間 theta の 2 倍以上滞在する
// group を形状探索前に一律拒否し、長期占有より盤面の回転を優先する。
...
/// 推定平均滞在時間に対する倍率だけで決める hard veto の境界。
const LONG_STAY_VETO_THETA_MULTIPLE: f64 = 2.0;
...
#[inline]
fn should_veto_long_stay(duration: usize, theta: f64) -> bool {
    debug_assert!(theta.is_finite() && theta > 0.0);
    (duration as f64) >= LONG_STAY_VETO_THETA_MULTIPLE * theta
}
...
impl Solver {
    ...
    fn run<R: BufRead, W: Write>(
        ...
        for turn in 0..self.M {
            ...
            let long_stay_veto = should_veto_long_stay(duration, theta);
            ...
            local! {
                self.trace.count("long_stay_veto_considered");
                if long_stay_veto {
                    self.trace.count("long_stay_veto_executed");
                    self.trace
                        .count_by("long_stay_veto_P_sum", P as i64);
                    self.trace
                        .count_by("long_stay_veto_D_sum", duration as i64);
                    self.trace.count_by(
                        "long_stay_veto_cell_time_sum",
                        (P as i64) * (duration as i64),
                    );
                    self.trace.count_by("long_stay_veto_V_sum", V);
                    self.trace.count_by(
                        "long_stay_veto_duration_ratio_milli_sum",
                        ((duration as f64 / theta) * 1_000.0).round() as i64,
                    );

                    // 棄却対象に高品質groupがどれだけ含まれたかを、v053 の即時価格に対する
                    // 最良形状での品質比で三帯に分ける。意思決定には使わない観測量である。
                    let quality_ratio = if base_threshold > 0.0 {
                        q_value * optimistic_C / base_threshold
                    } else {
                        f64::INFINITY
                    };
                    if quality_ratio < 0.5 {
                        self.trace.count("long_stay_veto_quality_below_half");
                    } else if quality_ratio < 1.0 {
                        self.trace.count("long_stay_veto_quality_below_price");
                    } else {
                        self.trace.count("long_stay_veto_quality_at_least_price");
                    }
                }
            }
            ...
            let passed_price_prefilter = !long_stay_veto
            ...
            local! {
                if !long_stay_veto && !passed_price_prefilter {
            ...
            local! {
                if long_stay_veto {
                    assert!(!accepted, "long-stay veto reached an accepted path");
                    assert!(normal.is_none(), "long-stay veto entered normal search");
                    assert!(!move_plan.ok, "long-stay veto entered relocation");
                }
            }
            ...
            if move_plan.ok {
                ...
                if accepted {
                    ...
                    local! {
                        ...
                        if long_stay_veto {
                            self.trace.count("long_stay_veto_rejected");
...
/// solver を動かさず、hard veto の境界と理論上の対象規模だけを検査する。
pub fn verify_long_stay_veto() {
    assert!(!should_veto_long_stay(3_999, 2_000.0));
    assert!(should_veto_long_stay(4_000, 2_000.0));
    assert!(should_veto_long_stay(8_000, 4_000.0));
    assert!(!should_veto_long_stay(7_999, 4_000.0));

    // D/theta が Exp(1) なら、対象group比は e^-2、対象セル時間比は 3e^-2 である。
    let group_fraction = (-LONG_STAY_VETO_THETA_MULTIPLE).exp();
    let cell_time_fraction = (LONG_STAY_VETO_THETA_MULTIPLE + 1.0) * group_fraction;
    assert!((group_fraction - 0.135_335_283).abs() < 1e-9);
    assert!((cell_time_fraction - 0.406_005_849).abs() < 1e-9);
}
