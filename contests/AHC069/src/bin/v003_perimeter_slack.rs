...
fn generate_templates(P: usize, N: usize) -> Vec<Vec<Template>> {
    let minimum_L = minimum_perimeter(P);
    let mut unique_by_slack: Vec<HashSet<Vec<(usize, usize)>>> =
        (0..4).map(|_| HashSet::new()).collect();

    // h 行に floor(P/h) 個ずつ置き、余りを隣接列に連続して付ける。
    // 最小周長から 6 までの偶数差だけを周長レベル別に残す。
    for height in 1..=P.min(N) {
        ...
        for start in starts {
            ...
            let L = perimeter(&cells);
            if L < minimum_L || L > minimum_L + 6 || (L - minimum_L) % 2 != 0 {
                continue;
            }
            let slack_index = (L - minimum_L) / 2;
            for kind in 0..8 {
                ...
                if variant_height <= N && variant_width <= N {
                    unique_by_slack[slack_index].insert(variant);
                }
            }
        }
    }
...
fn main() {
    ...
    let _ = R; // v003 は移動を行わないため、移動費係数は意思決定に使わない。
    ...
    let mut templates_by_P: Vec<Option<Vec<Vec<Template>>>> = (0..=MAX_P).map(|_| None).collect();
    ...
    for turn in 0..M {
        ...
        let minimum_L = minimum_perimeter(P);
        let ideal_compactness = 4.0 * (P as f64).sqrt() / (minimum_L as f64);
        let ideal_fee = ((V as f64) * ideal_compactness).round() as i64;
        let density = (ideal_fee as f64) / ((P * D) as f64);
        ...
        if was_missing {
            ...
            local! {
                let levels = templates_by_P[P].as_ref().unwrap();
                trace.count("template_size_generated");
                trace.count_by(
                    "template_variants_generated",
                    levels.iter().map(Vec::len).sum::<usize>() as i64,
                );
                trace.count_by(
                    "slack_template_variants_generated",
                    levels.iter().skip(1).map(Vec::len).sum::<usize>() as i64,
                );
            }
        }
        let deadline_sec =
            placement_budget_start_sec + placement_budget_sec * ((turn + 1) as f64 / M as f64);
        let mut chosen_cells = None;
        let mut chosen_slack = 0;
        let mut placement_was_unprofitable = false;
        for slack_index in 0..4 {
            if slack_index > 0 {
                local! {
                    trace.count("slack_attempt");
                }
            }
            let templates = &templates_by_P[P].as_ref().unwrap()[slack_index];
            ...
            let Some(cells) = search.cells else {
                continue;
            };
            let actual_L = minimum_L + 2 * slack_index;
            let actual_compactness = 4.0 * (P as f64).sqrt() / (actual_L as f64);
            let actual_fee = ((V as f64) * actual_compactness).round() as i64;
            let actual_density = (actual_fee as f64) / ((P * D) as f64);
            if actual_density < threshold {
                placement_was_unprofitable = true;
                local! {
                    trace.count("post_placement_price_reject");
                }
                break;
            }
            chosen_cells = Some(cells);
            chosen_slack = slack_index;
            if slack_index > 0 {
                local! {
                    trace.count("slack_placed");
                    trace.count_by("actual_fee_loss", ideal_fee - actual_fee);
                    match slack_index {
                        1 => trace.count("slack_placed_2"),
                        2 => trace.count("slack_placed_4"),
                        3 => trace.count("slack_placed_6"),
                        _ => unreachable!(),
                    }
                }
            }
        ...
        let _ = (chosen_slack, placement_was_unprofitable);

        if let Some(cells) = chosen_cells {
            ...
            local! {
                trace.count("placed");
                if chosen_slack == 0 {
                    trace.count("minimum_perimeter_placed");
                }
            }
        } else {
            writeln!(out, "No").unwrap();
            local! {
                if placement_was_unprofitable {
                    trace.count("slack_price_reject");
                } else {
