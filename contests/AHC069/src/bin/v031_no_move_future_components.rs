// v031_no_move_future_components.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。

// 中心アイデア: 長期滞在する通常配置では既知の退去時刻を使って将来盤面を3点観測し、
// 大物を置ける自由連結成分の容量を保つ候補を既存ロールアウトへ1つ追加する。
// 再移動は一切行わない。
...
struct Placement {
    ...
    slot_penalty: f64,
    slot_count: usize,
    /// 将来の自由連結成分容量を保つ評価から追加された候補か。
    future_component_candidate: bool,
...
impl Default for Placement {
    fn default() -> Self {
        Self {
            ...
            slot_penalty: 0.0,
            slot_count: 0,
            future_component_candidate: false,
...
impl Solver {
    ...
    /// 現在の active group のうち、各観測時刻より前に退去するものを解放した盤面。
    /// incoming はまだ盤面にないので、呼び出し側が候補ごとのセルを重ねる。
    fn future_component_snapshot_bases(&self, S: usize, D: usize) -> [Rows; 3] {
        let taus = [S + D / 5, S + D / 2, S + 4 * D / 5];
        let mut snapshots = [self.occupied_rows; 3];
        for (snapshot, &tau) in snapshots.iter_mut().zip(taus.iter()) {
            for group in &self.groups {
                if !group.active || group.T >= tau {
                    continue;
                }
                for &cell in &group.cells {
                    snapshot[cell / self.N] &= !Self::bit_at(cell % self.N);
                }
            }
        }
        snapshots
    ...
    /// P=64,96,120 を置ける開始位置数の粗い上界として、自由連結成分ごとに
    /// max(size-P+1, 0) を足す。短時間に多数回呼ぶため heap allocation はしない。
    fn future_component_capacity_proxy(&self, occ: &Rows) -> [usize; 3] {
        const TARGETS: [usize; 3] = [64, 96, 120];
        const DIRS: [(isize, isize); 4] = [(-1, 0), (1, 0), (0, -1), (0, 1)];
        let mut capacities = [0_usize; 3];
            ...
            for c in 0..self.N {
                ...
                while head < tail {
                    ...
                    for (dr, dc) in DIRS {
                        ...
                        if visited[next] || !self.is_free(occ, nr as usize, nc as usize) {
                            continue;
                        }
                        visited[next] = true;
                        queue[tail] = next;
                ...
                for (index, &target) in TARGETS.iter().enumerate() {
                    capacities[index] += tail.saturating_sub(target - 1);
                }
            }
        }
        capacities
    ...
    fn future_component_capacities(
        &self,
        snapshot_bases: &[Rows; 3],
        cells: &[usize],
    ) -> [[usize; 3]; 3] {
        let mut result = [[0_usize; 3]; 3];
        for snapshot_index in 0..3 {
            let mut occ = snapshot_bases[snapshot_index];
            for &cell in cells {
                debug_assert_eq!(occ[cell / self.N] & Self::bit_at(cell % self.N), 0);
                occ[cell / self.N] |= Self::bit_at(cell % self.N);
            }
            result[snapshot_index] = self.future_component_capacity_proxy(&occ);
        }
        result
    }

    fn future_component_retention(baseline: &[[usize; 3]; 3], candidate: &[[usize; 3]; 3]) -> f64 {
        const TARGET_WEIGHTS: [f64; 3] = [1.0, 3.0, 7.0];
        let mut score = 0.0;
        for snapshot_index in 0..3 {
            for target_index in 0..3 {
                let base = baseline[snapshot_index][target_index];
                if base > 0 {
                    score += TARGET_WEIGHTS[target_index]
                        * (candidate[snapshot_index][target_index] as f64)
                        / (base as f64);
                }
            }
        }
    ...
    fn weighted_future_component_capacity(capacities: &[[usize; 3]; 3]) -> usize {
        const TARGET_WEIGHTS: [usize; 3] = [1, 3, 7];
        capacities
            .iter()
            .map(|capacity| {
                capacity
    ...
    fn find_normal_placements(
        ...
        for perimeter in (min_L..=min_L + max_extra).step_by(2) {
            ...
            // 長期滞在だけ、既知の退去後にも大物用の自由連結成分容量を残す候補を
            // 1つ追加する。候補0と同じ成分・同じslot制約なので admission は不変である。
            let duration = incoming_T - incoming_S;
            if duration >= 5_000 {
                debug_assert!(!fast_mode);
                let snapshot_bases = self.future_component_snapshot_bases(incoming_S, duration);
                // 正規化の基準は incoming をまだ置いていない将来盤面である。
                let normalization_capacities =
                    self.future_component_capacities(&snapshot_bases, &[]);
                let current_capacities = self.future_component_capacities(
                    &snapshot_bases,
                    &evaluated[current_index].0.explicit_cells,
                );
                let baseline_slot_penalty = evaluated[current_index].0.slot_penalty;
                let current_retention = Self::future_component_retention(
                    &normalization_capacities,
                    &current_capacities,
                );
                // 候補0で初期化することで、将来容量が悪化する別候補は追加しない。
                let mut best_future = (current_index, current_retention, current_capacities);

                local! {
                    self.trace.count("future_component_turn");
                    self.trace.count_by("future_component_snapshot", 3);
                }
                for (index, (candidate, _)) in evaluated.iter().enumerate() {
                    let capacities = if index == current_index {
                        current_capacities
                    } else {
                        self.future_component_capacities(&snapshot_bases, &candidate.explicit_cells)
                    };
                    local! {
                        self.trace.count("future_component_evaluated");
                    }
                    if candidate.component_size != component_size
                        || candidate.slot_penalty > baseline_slot_penalty + 1e-9
                    {
                        continue;
                    }
                    let retention =
                        Self::future_component_retention(&normalization_capacities, &capacities);
                    if retention > best_future.1 {
                        best_future = (index, retention, capacities);
                    }
                }
                let (index, selected_retention, selected_capacities) = best_future;
                debug_assert!(selected_retention + 1e-12 >= current_retention);
                let _current_capacity =
                    Self::weighted_future_component_capacity(&current_capacities);
                let _selected_capacity =
                    Self::weighted_future_component_capacity(&selected_capacities);
                local! {
                    self.trace.count_by(
                        "future_component_capacity_before",
                        _current_capacity as i64,
                    );
                    self.trace.count_by(
                        "future_component_capacity_after",
                        _selected_capacity as i64,
                    );
                }
                if let Some(choice) = choices
                    .iter_mut()
                    .find(|choice| choice.explicit_cells == evaluated[index].0.explicit_cells)
                {
                    choice.future_component_candidate = true;
                } else {
                    let mut candidate = evaluated[index].0.clone();
                    debug_assert_eq!(candidate.perimeter, choices[0].perimeter);
                    debug_assert_eq!(candidate.component_size, choices[0].component_size);
                    debug_assert!(candidate.slot_penalty <= choices[0].slot_penalty + 1e-9);
                    debug_assert!(
                        !choices
                            .iter()
                            .any(|choice| { choice.explicit_cells == candidate.explicit_cells })
                    );
                    candidate.future_component_candidate = true;
                    choices.push(candidate);
                    local! {
                        self.trace.count("future_component_candidate_added");
                    }
                }
            }
    ...
    fn select_normal_by_rollout(
        ...
        let _future_component_index = choices
            .iter()
            .position(|placement| placement.future_component_candidate);
        local! {
            ...
            if let Some(index) = _future_component_index {
                self.trace.count("future_component_rollout_comparison");
                if winner == index {
                    self.trace.count("future_component_rollout_chosen");
                }
            }
        }
        winner
