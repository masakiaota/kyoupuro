// v042_no_move_admission_rollout.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。

// 中心アイデア: near-threshold の受理だけを accept/reject の共通乱数 rollout で
// 比較し、解析shadowの未観測区間を戻して3本全てreject有利な場合だけvetoする。
// 再移動は一切行わない。
...
const NEAR_VETO_LIMIT_RATIO: f64 = 0.84;
const NEAR_VETO_MARGIN_MAX: f64 = 1.10;
...
impl TraceStats {
    ...
    fn max_by(&mut self, key: &'static str, value: i64) {
        self.counts
            .entry(key)
...
impl Solver {
    ...
    /// base threshold と部分shadowで同じGauss 3点積分を使い、式のdriftを防ぐ。
    fn average_local_bid(&mut self, S: f64, span: f64, theta: f64, offered_area: f64) -> f64 {
        ...
        W0 * self.local_bid_at(S + X0 * span, theta, offered_area)
            + W1 * self.local_bid_at(S + X1 * span, theta, offered_area)
            + W2 * self.local_bid_at(S + X2 * span, theta, offered_area)
    }

    fn base_dynamic_threshold(
        &mut self,
        S: usize,
        duration: usize,
        P: usize,
        ...
        let average_bid = self.average_local_bid(S as f64, D, theta, offered_area);
        ...
        ThresholdTerms {
            base_threshold: threshold * multiplier,
            multiplier,
            offered_area,
        }
    }

    fn partial_shadow_price(
        &mut self,
        S: usize,
        h: usize,
        P: usize,
        ...
        let h = h as f64;
        let average_bid = self.average_local_bid(S as f64, h, theta, offered_area);
        (P as f64) * h * theta.powf(-0.1) * multiplier * component_factor * average_bid
    }
    ...
    /// 元のadmissionが通ったnear-threshold配置だけをaccept/rejectで比較する。
    /// 平均や多数決ではなく、3 sample全てでZが厳密に負の場合だけvetoする。
    #[allow(clippy::too_many_arguments)]
    fn should_veto_near_threshold_accept(
        &mut self,
        placement: &Placement,
        incoming_id: usize,
        theta: f64,
        ...
        assert_eq!(ROLLOUT_SAMPLES, 3);
        let S = self.groups[incoming_id].S;
        let T = self.groups[incoming_id].T;
        let D = T - S;
        let P = self.groups[incoming_id].P;
        let cells = self.materialize(placement, P);
        assert!(self.explicit_candidate_is_valid(&cells, P, &self.occupied_rows));
        assert_eq!(self.perimeter_of_cells(&cells), placement.perimeter);

        // h=Dなら部分shadowは元のfull shadowへ戻る。式の共有だけでなく数値でも照合する。
        let full_from_partial =
            self.partial_shadow_price(S, D, P, theta, multiplier, component_factor, offered_area);
        let identity_scale = lambda_full.abs().max(full_from_partial.abs()).max(1.0);
        let identity_error = (full_from_partial - lambda_full).abs() / identity_scale;
        assert!(
            identity_error <= 1e-10,
            "partial/full shadow mismatch: partial={full_from_partial}, full={lambda_full}, relative_error={identity_error}"
        );

        let mut accept_board = self.occupied_rows;
        for &cell in &cells {
            accept_board[cell / self.N] |= Self::bit_at(cell % self.N);
        }
        let accept_cand = RolloutCandidate {
            board: accept_board,
            immediate: G,
            incoming_dep: Some((T, cells)),
        };
        let reject_cand = RolloutCandidate {
        ...
        let mut negative_samples = 0_usize;
        let mut _empty_samples = 0_i64;
        let mut _h_sum = 0_i64;
        let mut _future_accept_sum = 0.0;
        let mut _future_reject_sum = 0.0;
        let mut _future_delta_sum = 0.0;
        let mut _lambda_h_sum = 0.0;
        let mut _z_sum = 0.0;
        for k in 0..ROLLOUT_SAMPLES {
            let seed = (incoming_id as u64) * 1_000_003 + (k as u64) * 7_919 + 1;
            let arrivals = self.make_future_arrivals(incoming_id, S, theta, seed);
            if arrivals.is_empty() {
                _empty_samples += 1;
            }
            let h = arrivals
                .last()
                .map(|arrival| arrival.at.saturating_sub(S).min(D))
                .unwrap_or(0);
            let future_accept =
                self.rollout_one(&accept_cand, &dep_base, &arrivals, base_threshold);
            let future_reject =
                self.rollout_one(&reject_cand, &dep_base, &arrivals, base_threshold);
            let lambda_h = self.partial_shadow_price(
                S,
                h,
                P,
                theta,
            ...
            let z = G - lambda_full + future_accept - future_reject + lambda_h;
            assert!(
                z.is_finite(),
                "near-threshold rollout produced non-finite Z"
            );
            if z < 0.0 {
                negative_samples += 1;
            }
            _h_sum += h as i64;
            _future_accept_sum += future_accept;
            _future_reject_sum += future_reject;
            _future_delta_sum += future_accept - future_reject;
            _lambda_h_sum += lambda_h;
            _z_sum += z;
        }
        let veto = negative_samples == ROLLOUT_SAMPLES;
        if veto {
            assert_eq!(negative_samples, 3);
        }
        local! {
            self.trace.count("near_veto_session");
            self.trace.count_by("near_veto_candidate_sum", 2);
            self.trace
                .count_by("near_veto_sample", ROLLOUT_SAMPLES as i64);
            self.trace
                .count_by("near_veto_negative_sample", negative_samples as i64);
            self.trace
                .count_by("near_veto_empty_arrival_sample", _empty_samples);
            self.trace.count_by("near_veto_h_sum", _h_sum);
            self.trace.count_by("near_veto_G", G.round() as i64);
            self.trace
                .count_by("near_veto_lambda_full", lambda_full.round() as i64);
            self.trace.count_by(
                "near_veto_future_accept",
                _future_accept_sum.round() as i64,
            );
            self.trace.count_by(
                "near_veto_future_reject",
                _future_reject_sum.round() as i64,
            );
            self.trace.count_by(
                "near_veto_future_delta",
                _future_delta_sum.round() as i64,
            );
            self.trace
                .count_by("near_veto_lambda_h", _lambda_h_sum.round() as i64);
            self.trace.count_by("near_veto_z", _z_sum.round() as i64);
            self.trace.count("near_veto_identity_check");
            self.trace.max_by(
                "near_veto_identity_error_e15_max",
                (identity_error * 1e15).round() as i64,
            );
            self.trace.count_by("near_veto_reject_to_accept", 0);
            if veto {
                self.trace.count("near_veto_executed");
                self.trace.count_by(
                    "near_veto_executed_negative_sample",
                    negative_samples as i64,
                );
                self.trace
                    .count_by("near_veto_rejected_G", G.round() as i64);
            } else {
                self.trace.count("near_veto_kept");
            }
        }
        veto
    }
    ...
    fn run<R: BufRead, W: Write>(
        ...
        for turn in 0..self.M {
            ...
            let threshold_terms = self.base_dynamic_threshold(S, duration, P, theta);
            let base_threshold = threshold_terms.base_threshold;
            ...
            if passed_price_prefilter {
                ...
                if let Some(mut choices) = normal_choices {
                    ...
                    // 元のadmissionが通ったnear-threshold配置だけをveto候補にする。
                    // placement選択と通常rolloutを先に確定し、rejectからacceptへは変えない。
                    if accepted && base_threshold > 0.0 {
                        let placement = normal.as_ref().expect("normal placement selected");
                        let C = compactness(P, placement.perimeter);
                        let G = (V as f64) * C;
                        let lambda_full =
                            (P as f64) * (duration as f64).powf(0.9) * actual_threshold;
                        assert!(G.is_finite() && lambda_full.is_finite() && lambda_full > 0.0);
                        let margin = G / lambda_full;
                        if (1.0..=NEAR_VETO_MARGIN_MAX).contains(&margin) {
                            local! {
                                self.trace.count("near_veto_margin_eligible");
                                self.trace.count_by(
                                    "near_veto_margin_milli_sum",
                                    (margin * 1_000.0).round() as i64,
                                );
                            }
                            if self.timer.reached(NEAR_VETO_LIMIT_RATIO) {
                                local! {
                                    self.trace.count("near_veto_time_skip");
                                }
                            } else {
                                let veto = local_time!(self.trace, "near_veto_rollout", {
                                    self.should_veto_near_threshold_accept(
                                        placement,
                                        id,
                                        theta,
                                        base_threshold,
                                        threshold_terms.multiplier,
                                        threshold_terms.offered_area,
                                        component_factor,
                                        G,
                                        lambda_full,
                                    )
