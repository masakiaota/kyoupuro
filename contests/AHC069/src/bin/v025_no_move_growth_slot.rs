// v025_no_move_growth_slot.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: v024を保ち、regular全不成立後のgrowth候補にもslot保全候補を追加する。
// 従来勝者と同一周長・同一成分サイズの範囲で、大型slotの占有遅延を最小化する。
...
impl Solver {
    ...
    fn growth_placement(
        ...
        let mut evaluated: Vec<Placement> = Vec::with_capacity(candidates.len());
        ...
        for mut candidate in candidates {
            ...
            if current_index
                .is_none_or(|index| candidate.final_score > evaluated[index].final_score)
            {
                current_index = Some(evaluated.len());
            }
            evaluated.push(candidate);
        }
        let current_index = current_index.expect("best perimeter guarantees a growth candidate");
        let mut choices = vec![evaluated[current_index].clone()];

        // calendar構築は従来候補が実際にadmissionを通り、LNSの時間枠内に収まる場合だけ行う。
        let actual_threshold =
            base_threshold * self.component_threshold_factor(choices[0].component_size);
        let quality = q_value * compactness(P, choices[0].perimeter);
        let passed_admission = base_threshold == 0.0 || quality >= actual_threshold;
        let has_slot_alternative = evaluated.iter().enumerate().any(|(index, candidate)| {
            index != current_index
                && candidate.perimeter == choices[0].perimeter
                && candidate.component_size == choices[0].component_size
                && candidate.explicit_cells != choices[0].explicit_cells
        });
        if !is_large_target
            && !fast_mode
            && passed_admission
            && has_slot_alternative
            && !self.timer.reached(GROWTH_LNS_LIMIT_RATIO)
        {
            local! {
                self.trace.count("growth_slot_alternative_turn");
                self.trace.count("growth_slot_calendar_turn");
            }
            let calendar = self.build_slot_calendar(incoming_id, incoming_S, theta);
            if !calendar.slots.is_empty() {
                let current_delay =
                    self.growth_slot_delay(&choices[0].explicit_cells, incoming_T, &calendar);
                choices[0].slot_delay = current_delay;
                choices[0].slot_penalty = Self::slot_penalty(&calendar, current_delay);
                choices[0].slot_count = calendar.slots.len();

                let mut slot_index = current_index;
                let mut slot_delay = current_delay;
                for (index, candidate) in evaluated.iter().enumerate() {
                    if candidate.perimeter != choices[0].perimeter
                        || candidate.component_size != choices[0].component_size
                    {
                        continue;
                    }
                    let delay =
                        self.growth_slot_delay(&candidate.explicit_cells, incoming_T, &calendar);
                    if delay < slot_delay
                        || (delay == slot_delay
                            && candidate.final_score > evaluated[slot_index].final_score)
                    {
                        slot_index = index;
                        slot_delay = delay;
                    }
                }
                let mut slot_candidate = evaluated[slot_index].clone();
                if slot_delay < current_delay
                    && slot_candidate.explicit_cells != choices[0].explicit_cells
                {
                    debug_assert_eq!(slot_candidate.perimeter, choices[0].perimeter);
                    debug_assert_eq!(slot_candidate.component_size, choices[0].component_size);
                    slot_candidate.slot_delay = slot_delay;
                    slot_candidate.slot_penalty = Self::slot_penalty(&calendar, slot_delay);
                    slot_candidate.slot_count = calendar.slots.len();
                    choices.push(slot_candidate);
                    local! {
                        self.trace.count("slot_candidate_added");
                        self.trace.count("growth_slot_candidate_added");
                    }
                }
            }
        }
        ...
        local! {
            ...
            self.trace.count_by(
                "growth_slack_sum",
                (choices[0].perimeter - minimum_perimeter(P)) as i64,
            );
        }
        Some(choices)
    }
    ...
    fn find_normal_placements(
        ...
        let growth = self.growth_placement(
            incoming_id,
            P,
            incoming_V,
            incoming_S,
        ...
        for turn in 0..self.M {
            ...
            if passed_price_prefilter {
                ...
                if let Some(mut choices) = normal_choices {
                    ...
                    let _is_growth = choices[0].shape_index == usize::MAX;
                    debug_assert!(
                        choices
                            .iter()
                    ...
                    local! {
                        if _slot_active && accepted && choices.len() >= 2 {
                            ...
                            if _is_growth {
                                self.trace.count("growth_slot_comparison");
                                self.trace.count_by(
                                    "growth_slot_delay_before",
                                    choices[0].slot_delay as i64,
                                );
                                self.trace.count_by(
                                    "growth_slot_delay_after",
                                    choices[winner].slot_delay as i64,
                                );
                                if choices[winner].slot_delay < choices[0].slot_delay {
                                    self.trace.count("growth_slot_preservation_flip");
                                }
                            }
                        }
                    }
