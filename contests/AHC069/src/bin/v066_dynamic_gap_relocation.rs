// v066_dynamic_gap_relocation.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: 動的な小空き成分を移動先として既存groupを詰め、旧領域を
// 最大空き成分へ返す。v053の既存配置・relocation・posterior rolloutは維持する。
...
// ---- 動的小空きへの能動的再配置 ----
const GAP_RELOC_BUDGET_SHARE: f64 = 0.20;
const GAP_RELOC_PAIR_LIMIT: usize = 8;
const GAP_RELOC_CANDIDATE_LIMIT: usize = 3;
...
struct Solver {
    ...
    reloc_spent_sec: f64,
    /// gap移動は既存relocation予算の一部だけを使い、incoming救済の探索を残す。
    gap_reloc_spent_sec: f64,
    /// gap移動後に同じgroupを既存relocationで再移動しないためのターン記録。
    turn_moved: Vec<usize>,
    current_turn: usize,
    timer: TimeKeeper,
    #[cfg(feature = "local")]
    trace: TraceStats,
...
impl Solver {
    fn new(N: usize, M: usize, R_milli: i64, grass_rows: Rows, timer: TimeKeeper) -> Self {
        ...
        let mut solver = Self {
            ...
            c_max_table: (0..=MAX_P)
                ...
                .collect(),
            reloc_spent_sec: 0.0,
            gap_reloc_spent_sec: 0.0,
            turn_moved: vec![usize::MAX; M],
    ...
    fn remove_expired(&mut self, current_S: usize) -> usize {
        let mut expired = 0;
    ...
    fn group_touches_component(
        ...
        const DIRS: [(isize, isize); 4] = [(-1, 0), (1, 0), (0, -1), (0, 1)];
        self.groups[group_id].cells.iter().any(|&id| {
            let r = id / self.N;
            let c = id % self.N;
            DIRS.iter().any(|&(dr, dc)| {
                let nr = (r as isize) + dr;
                ...
                nr >= 0
                    ...
                    && nc >= 0
                    && nc < self.N as isize
                    && info.component[(nr as usize) * self.N + (nc as usize)]
                        == component_id as isize
            })
        })
    }

    /// 指定した自由成分だけを移動先とし、現在のworst perimeter以下で最も局所重みの
    /// 高い規則形状を返す。最初に見つかる周長レベルで止めるためcompactnessを落とさない。
    fn find_regular_in_component(
        &self,
        P: usize,
        component_id: usize,
        info: &FreeInfo,
        ...
        for perimeter in (min_perimeter..=max_perimeter).step_by(2) {
            let mut best: Option<Placement> = None;
            for (shape_index, shape) in self.shapes_by_p[P].iter().enumerate() {
                if shape.perimeter != perimeter {
                    continue;
                ...
                for x in 0..=(self.N - shape.h) {
                    ...
                    while ys != 0 {
                        ...
                        if info.component[first] != component_id as isize {
                            continue;
                        }
                        ...
                        for rr in 0..shape.h {
                            ...
                            let end = begin + shape.len[rr];
                            cheap_score +=
                                weights.prefix[x + rr][end] - weights.prefix[x + rr][begin];
                        }
                        if best
                            .as_ref()
                            .is_none_or(|current| cheap_score > current.cheap_score)
                        {
                            best = Some(Placement {
                                ...
                                perimeter,
                                cheap_score,
                                component_size: info.sizes[component_id],
                                ..Placement::default()
                            });
                        }
                    }
    ...
    /// 小空き成分を移動先として先に決め、旧領域が最大空き成分を拡大するgroupだけを
    /// posterior rolloutでbaselineと比較する。1ターンで採る移動は最大1件である。
    #[allow(clippy::too_many_arguments)]
    fn try_gap_relocation(
        &mut self,
        incoming_id: usize,
        incoming_S: usize,
        incoming_T: usize,
        ...
        local! {
            self.trace.count("gap_reloc_trigger");
        }
        ...
        if self.gap_reloc_spent_sec
            >= self.timer.budget_sec(RELOC_TIME_BUDGET_RATIO) * GAP_RELOC_BUDGET_SHARE
        {
            local! {
                self.trace.count("gap_reloc_budget_hit");
            }
            return None;
        }

        let start = self.timer.elapsed_sec();
        let result = (|| -> Option<GapMove> {
            let occ = self.occupied_rows;
            let info = self.compute_free_info(&occ, true);
            if info.sizes.len() < 2 {
                return None;
            }
            let max_component = info
                .sizes
                .iter()
                .enumerate()
                .max_by_key(|&(_, &size)| size)
                .map(|(component_id, _)| component_id)?;
            let old_max_size = info.sizes[max_component];
            let mut gaps: Vec<(usize, usize)> = info
                .sizes
                .iter()
                .enumerate()
                .filter_map(|(component_id, &size)| {
                    (component_id != max_component && (4..MAX_P).contains(&size))
                        .then_some((component_id, size))
                })
                .collect();
            gaps.sort_unstable_by_key(|&(component_id, size)| (size, component_id));
            local! {
                self.trace.count_by("gap_reloc_gap_count", gaps.len() as i64);
            }
            if gaps.is_empty() {
                return None;
            }

            // (gapの余り, 移動費, 大きいgroup優先, gap_id, group_id)。隙間をよく埋め、
            // 旧領域を最大成分へ返せる安いgroupだけを高価な形状探索へ渡す。
            let mut pairs: Vec<(usize, i64, Reverse<usize>, usize, usize)> = Vec::new();
            for &(gap_id, gap_size) in &gaps {
                for group_id in 0..incoming_id {
                    let group = &self.groups[group_id];
                    if !group.active
                        || group.move_count >= 4
                        || self.turn_moved[group_id] == self.current_turn
                        || group.P > gap_size
                        || !self.group_touches_component(group_id, &info, max_component)
                    {
                        continue;
                    }
                    pairs.push((
                        gap_size - group.P,
                        self.movement_cost(group_id),
                        Reverse(group.P),
                        gap_id,
                        group_id,
                    ));
            ...
            pairs.sort_unstable();
            pairs.truncate(GAP_RELOC_PAIR_LIMIT);
            local! {
                self.trace
                    .count_by("gap_reloc_pair_generated", pairs.len() as i64);
            }
            if pairs.is_empty() {
                return None;
            }
            ...
            for &(_, move_cost, _, gap_id, group_id) in &pairs {
                local! {
                    self.trace.count("gap_reloc_pair_checked");
                }
                let group = &self.groups[group_id];
                let weights = self.build_weight_data(&occ, &info, group.T, theta, true);
                let Some(mut placement) = self.find_regular_in_component(
                    group.P,
                    gap_id,
                    &info,
                    &runs,
                    &weights,
                    group.worst_perimeter,
                ) else {
                    continue;
                };
                let cells = self.materialize(&placement, group.P);
                placement.explicit_cells = cells.clone();
                local! {
                    self.trace.count("gap_reloc_fit");
                    self.trace.count_by("gap_reloc_invariant_check", 4);
                }
                assert!(self.explicit_candidate_is_valid(&cells, group.P, &occ));
                assert!(
                    cells
                        .iter()
                        .all(|&cell| info.component[cell] == gap_id as isize)
                );
                assert!(placement.perimeter <= group.worst_perimeter);

                let mut moved_board = occ;
                for &cell in &group.cells {
                    moved_board[cell / self.N] &= !Self::bit_at(cell % self.N);
                }
                for &cell in &cells {
                    moved_board[cell / self.N] |= Self::bit_at(cell % self.N);
                }
                let moved_info = self.compute_free_info(&moved_board, false);
                assert_eq!(moved_info.free_count, info.free_count);
                let new_max_size = moved_info.sizes.iter().copied().max().unwrap_or(0);
                if new_max_size <= old_max_size {
                    continue;
                }
                local! {
                    self.trace.count("gap_reloc_topology_candidate");
                }
                candidates.push(GapMove {
                    group_id,
                    placement,
                    gap_size: info.sizes[gap_id],
                    max_component_gain: new_max_size - old_max_size,
                    move_cost,
                });
            }
            if candidates.is_empty() {
            ...
            candidates.sort_by(|a, b| {
                b.max_component_gain
                    .cmp(&a.max_component_gain)
                    .then_with(|| a.move_cost.cmp(&b.move_cost))
                    .then_with(|| {
                        (a.gap_size - self.groups[a.group_id].P)
                            .cmp(&(b.gap_size - self.groups[b.group_id].P))
                    })
                    .then_with(|| a.group_id.cmp(&b.group_id))
            });
            candidates.truncate(GAP_RELOC_CANDIDATE_LIMIT);

            let mut rollout_candidates = Vec::with_capacity(candidates.len() + 1);
            for candidate in &candidates {
                let mut board = occ;
                for &cell in &self.groups[candidate.group_id].cells {
                    board[cell / self.N] &= !Self::bit_at(cell % self.N);
                }
                for &cell in &candidate.placement.explicit_cells {
                    board[cell / self.N] |= Self::bit_at(cell % self.N);
                }
                rollout_candidates.push(RolloutCandidate {
                    board,
                    immediate: -(candidate.move_cost as f64),
                    incoming_dep: None,
                    overrides: vec![(
                        candidate.group_id,
                        candidate.placement.explicit_cells.clone(),
                    )],
                });
            }
            let baseline_index = rollout_candidates.len();
            rollout_candidates.push(RolloutCandidate {
                board: occ,
                immediate: 0.0,
                incoming_dep: None,
            ...
            let incoming_duration = incoming_T - incoming_S;
            let incoming_q =
                (incoming_V as f64) / ((incoming_P as f64) * (incoming_duration as f64).powf(0.9));
            let first = FutureArrival {
                at: incoming_S,
                dur: incoming_duration,
                P: incoming_P,
                q: incoming_q,
                value: incoming_V as f64,
            };
            local! {
                self.trace.count("gap_reloc_rollout");
            }
            let winner = self.evaluate_candidates_rollout_with(
                &rollout_candidates,
                incoming_id,
                incoming_S,
                theta,
            ...
            if winner == baseline_index {
                local! {
                    self.trace.count("gap_reloc_rollout_reject");
                }
                return None;
            }
            let chosen = candidates.swap_remove(winner);
            local! {
                self.trace.count("gap_reloc_success");
                self.trace.count_by(
                    "gap_reloc_gain_sum",
                    chosen.max_component_gain as i64,
                );
                self.trace
                    .count_by("gap_reloc_move_cost", chosen.move_cost);
                self.trace.count_by(
                    "gap_reloc_remaining_gap_sum",
                    (chosen.gap_size - self.groups[chosen.group_id].P) as i64,
                );
            }
            Some(chosen)
        })();
        let spent = self.timer.elapsed_sec() - start;
        self.gap_reloc_spent_sec += spent;
        self.reloc_spent_sec += spent;
        local! {
            self.trace
                .count_by("gap_reloc_spent_micros", (spent * 1_000_000.0) as i64);
        }
        result
    }

    fn commit_gap_move(&mut self, gap_move: &GapMove) {
        let group_id = gap_move.group_id;
        assert_ne!(self.turn_moved[group_id], self.current_turn);
        self.clear_group_from_board(group_id);
        let cells = gap_move.placement.explicit_cells.clone();
        {
            let group = &mut self.groups[group_id];
            assert!(gap_move.placement.perimeter <= group.worst_perimeter);
                    ...
                    while ys != 0 {
                        ...
                        for rr in 0..shape.h {
                            ...
                            for c in begin..end {
                                ...
                                if !self.groups[owner].active
                                    || self.groups[owner].move_count >= 4
                                    || self.turn_moved[owner] == self.current_turn
                                {
                                    invalid = true;
                                    break;
                                }
    ...
    fn evaluate_candidates_rollout(
        ...
        self.evaluate_candidates_rollout_with(cands, incoming_id, now, theta, threshold, None)
    }

    /// gap移動の比較では、これから処理するincomingを確定した先頭到着として加える。
    /// θサンプルとそれ以降の到着列はv053のposterior rolloutをそのまま使う。
    fn evaluate_candidates_rollout_with(
        &mut self,
        cands: &[RolloutCandidate],
        incoming_id: usize,
        now: usize,
        theta: f64,
        threshold: f64,
        extra_first: Option<FutureArrival>,
    ) -> usize {
        ...
        for k in 0..ROLLOUT_SAMPLES {
            ...
            let mut arrivals = self.make_future_arrivals(incoming_id, now, theta_k, seed);
            if let Some(first) = extra_first {
    ...
    fn print_move_block<W: Write>(
        ...
        gap_move: Option<&GapMove>,
    ) -> io::Result<()> {
        let moved: &[(usize, Placement)] = match plan.filter(|plan| plan.ok) {
            Some(plan) => &plan.moved,
            None => &[],
        };
        let total = moved.len() + usize::from(gap_move.is_some());
        if total == 0 {
            writeln!(writer, "0")?;
            writer.flush()?;
            return Ok(());
        ...
        if let Some(gap_move) = gap_move {
            debug_assert!(
                moved
                    .iter()
                    .all(|(group_id, _)| *group_id != gap_move.group_id)
            );
            writeln!(writer, "{}", gap_move.group_id)?;
            for &cell in &gap_move.placement.explicit_cells {
                writeln!(writer, "{} {}", cell / self.N, cell % self.N)?;
            }
        }
        for (group_id, placement) in moved {
    ...
    fn run<R: BufRead, W: Write>(
        ...
        local! {
            self.trace.count_by("gap_reloc_invariant_violation", 0);
        }
        for turn in 0..self.M {
            ...
            let expired_now = self.remove_expired(S);
            ...
            // 新しい隙間が生じた退去ターンだけ、隙間を移動先とする整理を先に試す。
            // 採用した移動はここで盤面へ反映し、以後の通常配置とrelocationは整理後盤面を見る。
            let mut gap_move = None;
            if expired_now > 0 && !fast_mode {
                gap_move = local_time!(self.trace, "gap_relocation", {
                    self.try_gap_relocation(id, S, T, P, V, theta, base_threshold)
                });
                if let Some(selected) = &gap_move {
                    self.commit_gap_move(selected);
                    local! {
                        self.trace.count("gap_reloc_output");
                    }
                }
            }
            ...
            if move_plan.ok {
                ...
                self.print_move_block(writer, Some(&move_plan), gap_move.as_ref())?;
                ...
                self.print_move_block(writer, None, gap_move.as_ref())?;
                if accepted {
