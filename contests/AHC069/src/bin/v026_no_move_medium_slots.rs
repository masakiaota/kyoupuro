// v026_no_move_medium_slots.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: v024の大型slotを先に同じ規則で確保した後、空いた枠へ
// 非重複の中型slotを追加し、64<=P<=95の長期・高価値区画にも将来容量を残す。
...
struct Placement {
    ...
    slot_penalty: f64,
    slot_count: usize,
    medium_slot_delay: usize,
    medium_slot_count: usize,
...
struct SlotCalendar {
    ...
    large_arrival_rate: f64,
    medium_arrival_rate: f64,
    large_K: usize,
    medium_K: usize,
    medium_count: usize,
}
...
impl Solver {
    ...
    #[inline]
    fn slot_order(a: &ReservedSlot, b: &ReservedSlot) -> std::cmp::Ordering {
        a.ready
            .cmp(&b.ready)
    ...
    #[inline]
    fn slots_overlap(a: &ReservedSlot, b: &ReservedSlot) -> bool {
        a.x < b.x + b.h && b.x < a.x + a.h && a.y < b.y + b.w && b.y < a.y + a.w
    }

    /// 共通ready盤面から、指定された矩形群のmax readyを横・縦のseparable scanで列挙する。
    fn scan_slot_pool(
        &self,
        ready: &[usize; MAX_N * MAX_N],
        dims: &[(usize, usize)],
        class: SlotClass,
    ) -> Vec<ReservedSlot> {
    ...
    fn build_slot_calendar(&mut self, incoming_id: usize, S: usize, theta: f64) -> SlotCalendar {
        ...
        if remaining == 0 || remaining_time == 0 {
            local! {
                ...
                self.trace.count_by("medium_slot_quota_sum", 0);
                self.trace.count_by("medium_slot_inventory_sum", 0);
            }
            return SlotCalendar {
                slots: Vec::new(),
                large_arrival_rate: 0.0,
                medium_arrival_rate: 0.0,
                large_K: 0,
                medium_K: 0,
                medium_count: 0,
            };
        ...
        let large_arrival_rate =
            remaining_rate * (1.0 - self.p_cdf[95]) * 0.5 * (-6_000.0 / theta.max(1.0)).exp();
        let large_K = (large_arrival_rate * theta).ceil().clamp(1.0, 6.0) as usize;
        let medium_arrival_rate = remaining_rate
            * (self.p_cdf[95] - self.p_cdf[63])
            * 0.5
            * (-6_000.0 / theta.max(1.0)).exp();
        let medium_K = (medium_arrival_rate * theta).ceil().clamp(1.0, 6.0) as usize;

        const INVALID: usize = usize::MAX / 4;
        const LARGE_DIMS: [(usize, usize); 9] = [
            (10, 10),
            (10, 12),
            (12, 10),
            (11, 11),
        ...
        const MEDIUM_DIMS: [(usize, usize); 5] = [(8, 8), (8, 10), (10, 8), (9, 9), (10, 10)];
        ...
        let mut large_pool = self.scan_slot_pool(&ready, &LARGE_DIMS, SlotClass::Large);
        if large_pool.len() > POOL_LIMIT {
            large_pool.select_nth_unstable_by(POOL_LIMIT, Self::slot_order);
            large_pool.truncate(POOL_LIMIT);
        }
        large_pool.sort_unstable_by(Self::slot_order);
        let mut slots: Vec<ReservedSlot> = Vec::with_capacity(6);
        for candidate in large_pool {
                ...
                .all(|selected| !Self::slots_overlap(selected, &candidate))
            {
                ...
                if slots.len() == large_K {
                    break;
                }
            }
        }

        let large_count = slots.len();
        let medium_limit = medium_K.min(6 - large_count);
        if medium_limit > 0 {
            let mut medium_pool = self.scan_slot_pool(&ready, &MEDIUM_DIMS, SlotClass::Medium);
            if medium_pool.len() > POOL_LIMIT {
                medium_pool.select_nth_unstable_by(POOL_LIMIT, Self::slot_order);
                medium_pool.truncate(POOL_LIMIT);
            }
            medium_pool.sort_unstable_by(Self::slot_order);
            for candidate in medium_pool {
                ...
                {
                    slots.push(candidate);
                    if slots.len() == large_count + medium_limit {
                        break;
                    }
                }
            }
        }
        let medium_count = slots.len() - large_count;
        assert!(slots.len() <= 6);
        for i in 0..slots.len() {
            for j in (i + 1)..slots.len() {
                assert!(!Self::slots_overlap(&slots[i], &slots[j]));
            }
        }
        local! {
            self.trace
            ...
            self.trace
                .count_by("medium_slot_quota_sum", medium_K as i64);
            self.trace
                .count_by("medium_slot_inventory_sum", medium_count as i64);
        }
        SlotCalendar {
            slots,
            large_arrival_rate,
    ...
    fn regular_slot_effect(
        ...
        let mut large_delay = 0;
        let mut medium_delay = 0;
        for slot in &calendar.slots {
            if !self.regular_intersects_slot(shape, x, y, slot) {
                continue;
            }
            let delay = incoming_T.saturating_sub(slot.ready);
            match slot.class {
                SlotClass::Large => large_delay += delay,
                SlotClass::Medium => medium_delay += delay,
            }
        }
        // 414k/222kは各targetを悪形に追いやる1件あたりの観測平均損失である。
        let mut effect = SlotEffect {
            delay: large_delay,
            penalty: (large_delay as f64) * calendar.large_arrival_rate * 414_000.0
                / (calendar.large_K.max(1) as f64),
            medium_delay,
        };
        // 中型target自身にはMediumを明け渡す必要がない。物理delayはtrace用に保持するが、
        // penaltyと候補タイブレーク用delayからは除き、Largeの保護だけを残す。
        if !exempt_medium {
            effect.delay += medium_delay;
            effect.penalty += (medium_delay as f64) * calendar.medium_arrival_rate * 222_000.0
                / (calendar.medium_K.max(1) as f64);
        }
        effect
    }

    /// baselineのtop集合とは独立に、componentごとのslot penalty最小候補を全位置から拾う。
        ...
        for (shape_index, shape) in self.shapes_by_p[P].iter().enumerate() {
            ...
            for x in 0..=(self.N - shape.h) {
                ...
                while ys != 0 {
                    ...
                    let effect =
                        self.regular_slot_effect(shape, x, y, incoming_T, calendar, exempt_medium);
                    if best[component].as_ref().is_none_or(|current| {
                        effect.penalty < current.slot_penalty
                            || (effect.penalty == current.slot_penalty
                                && (effect.delay < current.slot_delay
                                    || (effect.delay == current.slot_delay
                        ...
                        best[component] = Some(Placement {
                            ...
                            slot_delay: effect.delay,
                            slot_penalty: effect.penalty,
                            slot_count: calendar.slots.len(),
                            medium_slot_delay: effect.medium_delay,
                            medium_slot_count: calendar.medium_count,
                            ..Placement::default()
                        });
                    }
                }
        ...
        for perimeter in (min_L..=min_L + max_extra).step_by(2) {
            ...
            for mut candidate in candidates {
                    ...
                    .filter(|calendar| !calendar.slots.is_empty())
                {
                    ...
                    let effect = self.regular_slot_effect(
                        shape,
                        candidate.x,
                        candidate.y,
                        incoming_T,
                    ...
                    candidate.slot_delay = effect.delay;
                    candidate.slot_penalty = effect.penalty;
                    candidate.slot_count = calendar.slots.len();
                    candidate.medium_slot_delay = effect.medium_delay;
                    candidate.medium_slot_count = calendar.medium_count;
                }
                ...
                .filter(|calendar| !calendar.slots.is_empty())
            {
                ...
                if let Some(mut candidate) = slot_best[current_component].clone() {
                    ...
                    {
                        let _improves_medium = !is_medium_target
                            && candidate.medium_slot_delay < choices[0].medium_slot_delay;
                        choices.push(candidate);
                        local! {
                            self.trace.count("slot_candidate_added");
                            if _improves_medium {
                                self.trace.count("medium_slot_candidate_added");
                            }
                        }
                    }
                }
                ...
                .is_none_or(|calendar| calendar.slots.is_empty())
            {
                debug_assert!(choices.iter().all(|placement| {
                    placement.slot_delay == 0
                        ...
                        && placement.medium_slot_delay == 0
                        && placement.medium_slot_count == 0
                }));
            }
            if is_large_target {
                local! {
    ...
    fn run<R: BufRead, W: Write>(
        ...
        for turn in 0..self.M {
            ...
            let is_medium_target = (64..=95).contains(&P) && duration >= 6_000 && q_value >= 1.0;
            ...
            if passed_price_prefilter {
                ...
                if let Some(mut choices) = normal_choices {
                    ...
                    let _medium_slot_active = choices[0].medium_slot_count > 0 && !is_medium_target;
                    debug_assert!(
                        choices
                            .iter()
                    ...
                    debug_assert!(choices.iter().all(|placement| {
                        placement.medium_slot_count == choices[0].medium_slot_count
                    }));
                    ...
                    local! {
                        ...
                        if _medium_slot_active && accepted && choices.len() >= 2 {
                            self.trace.count("medium_slot_comparison");
                            self.trace.count_by(
                                "medium_slot_delay_before",
                                choices[0].medium_slot_delay as i64,
                            );
                            self.trace.count_by(
                                "medium_slot_delay_after",
                                choices[winner].medium_slot_delay as i64,
                            );
                            if choices[winner].medium_slot_delay
                                < choices[0].medium_slot_delay
                            {
                                self.trace.count("medium_slot_preservation_flip");
                            }
                        }
                    }
