// v021_no_move_departure_affinity.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: 移動なし・growth SAのv019を保ち、配置の局所重みだけを
// 隣接区画との退去時刻親和性へ戻す。他の探索・判定・時間定数は変更しない。
...
impl Solver {
    ...
    fn build_weight_data(
        ...
        let mut _departure_affinity_earlier_edges = 0_i64;
        let mut _departure_affinity_later_edges = 0_i64;
        const DIRS: [(isize, isize); 4] = [(-1, 0), (1, 0), (0, -1), (0, 1)];
        for r in 0..self.N {
            for c in 0..self.N {
                let id = r * self.N + c;
                    ...
                    for (dr, dc) in DIRS {
                        ...
                        if nr < 0 || nr >= self.N as isize || nc < 0 || nc >= self.N as isize {
                            ...
                            if !self.is_grass(nr, nc) {
                                ...
                                let owner = usize::try_from(self.owner_cell[nr * self.N + nc])
                                    .expect("occupied cell must have an owner");
                                debug_assert!(owner < self.M && self.groups[owner].active);
                                let z = (self.groups[owner].T as f64 - incoming_T as f64)
                                    / theta.max(1.0);
                                if z >= 0.0 {
                                    // 隣接区画が同時刻以降に退去するほど強くまとめる。
                                    weight += 8.0 + 8.0 * (-z.min(7.0)).exp();
                                    _departure_affinity_later_edges += 1;
                                } else {
                                    weight += 2.0 + 8.0 * z.max(-7.0).exp();
                                    _departure_affinity_earlier_edges += 1;
                                }
                            }
                        }
                    }
        ...
        local! {
            self.trace
                .count_by("departure_affinity_earlier_edges", _departure_affinity_earlier_edges);
            self.trace
                .count_by("departure_affinity_later_edges", _departure_affinity_later_edges);
        }
        data
    }
    ...
    fn find_normal_placements(
        ...
        let weights = self.build_weight_data(&occ, &info, incoming_T, theta);
