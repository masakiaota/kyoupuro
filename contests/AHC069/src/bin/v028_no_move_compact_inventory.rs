// v028_no_move_compact_inventory.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: v024を保ち、通常配置の既存top候補から、P=48,80,120の
// 最小周長shapeが将来も置ける位置を多尺度で残す候補をrollout比較へ1つ追加する。
...
const COMPACT_FIT_P: [usize; 3] = [48, 80, 120];
const COMPACT_FIT_WEIGHT: [f64; 3] = [1.0, 3.0, 7.0];
const COMPACT_FIT_SHAPES_PER_P: usize = 6;
const COMPACT_FIT_REPRESENTATIVES: usize = COMPACT_FIT_P.len() * COMPACT_FIT_SHAPES_PER_P;
...
#[derive(Clone, Copy)]
struct CompactFitRepresentative {
    P: usize,
    shape_index: usize,
    weight: f64,
}
...
#[derive(Clone, Copy)]
struct CompactFitBaseline {
    valid_starts: Rows,
    count: usize,
}

#[derive(Clone, Copy)]
struct CompactFitEvaluation {
    weighted_retention: f64,
    #[cfg_attr(not(feature = "local"), allow(dead_code))]
    counts_by_p: [usize; COMPACT_FIT_P.len()],
}
...
struct Solver {
    ...
    compact_fit_representatives: [CompactFitRepresentative; COMPACT_FIT_REPRESENTATIVES],
...
impl Solver {
    fn new(N: usize, M: usize, grass_rows: Rows, timer: TimeKeeper) -> Self {
        ...
        let mut compact_fit_representatives = Vec::with_capacity(COMPACT_FIT_REPRESENTATIVES);
        for (scale, &P) in COMPACT_FIT_P.iter().enumerate() {
            let min_L = minimum_perimeter(P);
            let shape_indices: Vec<usize> = shapes_by_p[P]
                .iter()
                .enumerate()
                .filter_map(|(index, shape)| (shape.perimeter == min_L).then_some(index))
                .take(COMPACT_FIT_SHAPES_PER_P)
                .collect();
            assert_eq!(
                shape_indices.len(),
                COMPACT_FIT_SHAPES_PER_P,
                "P={P} must have six minimum-perimeter representative shapes"
            );
            for shape_index in shape_indices {
                compact_fit_representatives.push(CompactFitRepresentative {
                    P,
                    shape_index,
                    weight: COMPACT_FIT_WEIGHT[scale],
                });
            }
        }
        let compact_fit_representatives: [CompactFitRepresentative; COMPACT_FIT_REPRESENTATIVES] =
            compact_fit_representatives
                .try_into()
    ...
    /// `mask` の各bitを右へ `[0, len)` だけ広げる。shapeの1行が候補セルと
    /// 交差する開始yを、セル単位の走査なしでまとめて得るために使う。
    #[inline]
    fn compact_fit_right_dilate(mask: u64, len: usize) -> u64 {
        debug_assert!(len >= 1);
        let highest_bit = (usize::BITS - 1 - len.leading_zeros()) as usize;
        let mut result = mask;
        let mut span = 1;
        // 長さの2進表現を上位から読み、現在の区間を倍化して必要なら1つ足す。
    ...
    /// 現盤面で置ける18代表shapeの開始位置を、shapeごと・開始行ごとのbitmaskにする。
    fn build_compact_fit_baselines(
        &self,
        runs: &RunTable,
    ) -> [CompactFitBaseline; COMPACT_FIT_REPRESENTATIVES] {
        let mut baselines = [CompactFitBaseline {
            valid_starts: [0; MAX_N],
            count: 0,
        }; COMPACT_FIT_REPRESENTATIVES];
        for (index, representative) in self.compact_fit_representatives.iter().enumerate() {
    ...
    /// 現盤面で合法だった開始位置から、candidateのセルと交差するものだけを除く。
    /// weighted_retentionは候補抽出専用で、Placementの既存評価値は変更しない。
    fn evaluate_compact_fit(
        &self,
        candidate: &Placement,
        baselines: &[CompactFitBaseline; COMPACT_FIT_REPRESENTATIVES],
    ) -> Option<CompactFitEvaluation> {
        let mut candidate_rows = [0_u64; MAX_N];
        for &cell in &candidate.explicit_cells {
            candidate_rows[cell / self.N] |= Self::bit_at(cell % self.N);
        }

        let mut counts_by_p = [0_usize; COMPACT_FIT_P.len()];
            ...
            for x in 0..=(self.N - shape.h) {
                ...
                for rr in 0..shape.h {
                    ...
                    if row_mask != 0 {
                        ...
                        intersecting_starts |=
                            Self::compact_fit_right_dilate(shifted, shape.len[rr]);
                    }
                }
                after -= (valid_starts & intersecting_starts).count_ones() as usize;
            }
            let scale = index / COMPACT_FIT_SHAPES_PER_P;
            debug_assert_eq!(representative.P, COMPACT_FIT_P[scale]);
            counts_by_p[scale] += after;
            weighted_sum += representative.weight * (after as f64) / (baseline.count as f64);
            valid_weight_sum += representative.weight;
        }
        (valid_weight_sum > 0.0).then_some(CompactFitEvaluation {
    ...
    fn find_normal_placements(
        ...
        for perimeter in (min_L..=min_L + max_extra).step_by(2) {
            ...
            local! {
                self.trace.count("compact_fit_turn");
            }
            let (compact_fit_best, current_fit, _fit_before_by_p, _fit_evaluated) =
                local_time!(self.trace, "compact_fit", {
                    let baselines = self.build_compact_fit_baselines(&runs);
                    let mut before_by_p = [0_usize; COMPACT_FIT_P.len()];
                    for (index, baseline) in baselines.iter().enumerate() {
                        before_by_p[index / COMPACT_FIT_SHAPES_PER_P] += baseline.count;
                    }
                    let current = &evaluated[current_index].0;
                    let mut best: Option<(usize, CompactFitEvaluation)> = None;
                    let mut current_evaluation = None;
                    let mut evaluated_count = 0;
                    for (index, (candidate, _)) in evaluated.iter().enumerate() {
                        let Some(fit) = self.evaluate_compact_fit(candidate, &baselines) else {
                            continue;
                        };
                        evaluated_count += 1;
                    ...
                    (best, current_evaluation, before_by_p, evaluated_count)
                });
            local! {
                self.trace
                    .count_by("compact_fit_evaluated", _fit_evaluated as i64);
            }
            ...
            if let (Some((fit_index, fit)), Some(current_fit)) = (compact_fit_best, current_fit) {
                    ...
                    .any(|choice| choice.explicit_cells == candidate.explicit_cells)
                {
                    assert!(
                        ...
                        "compact-fit candidate must not worsen slot penalty"
                    );
                    assert!(
                        fit.weighted_retention + f64::EPSILON >= current_fit.weighted_retention,
                        "compact-fit candidate must retain at least candidate0 inventory"
                    );
                    candidate.compact_fit_candidate = true;
                    local! {
                        self.trace.count("compact_fit_candidate_added");
                        self.trace.count_by(
                            "fit48_before",
                            _fit_before_by_p[0] as i64,
                        );
                        self.trace
                            .count_by("fit48_after", fit.counts_by_p[0] as i64);
                        self.trace.count_by(
                            "fit80_before",
                            _fit_before_by_p[1] as i64,
                        );
                        self.trace
                            .count_by("fit80_after", fit.counts_by_p[1] as i64);
                        self.trace.count_by(
                            "fit120_before",
                            _fit_before_by_p[2] as i64,
                        );
                        self.trace
                            .count_by("fit120_after", fit.counts_by_p[2] as i64);
                        self.trace.count_by(
                            "compact_fit_slot_penalty_before",
                            choices[0].slot_penalty.round() as i64,
                        );
                        self.trace.count_by(
                            "compact_fit_slot_penalty_after",
                            candidate.slot_penalty.round() as i64,
                        );
                    }
                    choices.push(candidate);
                }
    ...
    fn select_normal_by_rollout(
        ...
        let _compact_fit_comparison = choices
            .iter()
            .any(|placement| placement.compact_fit_candidate);
        local! {
            if _compact_fit_comparison {
                self.trace.count("compact_fit_rollout_comparison");
            }
        }
        ...
        local! {
            ...
            if choices[winner].compact_fit_candidate {
                self.trace.count("compact_fit_rollout_chosen");
            }
        }
        winner
    }
