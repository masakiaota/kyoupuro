// v041_no_move_swap_rollout.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。

// 中心アイデア: v040 の biased swap が厳密に周長を短縮したときだけ、baseline と
// improved を64到着×3本の共通乱数ロールアウトで比較し、後続受理fee込みで選ぶ。
// 再移動は一切行わない。
...
const BIASED_SWAP_START_RATIO: f64 = 0.84;
const BIASED_SWAP_LIMIT_RATIO: f64 = 0.88;
const BIASED_SWAP_ITERATIONS: usize = 512;
const BIASED_SWAP_MIN_RECOVERABLE_FEE: f64 = 100_000.0;
...
/// biased swap の候補比較専用。通常配置用22到着×3本とは独立に長期化する。
const SWAP_ROLLOUT_ARRIVALS: usize = 64;
const SWAP_ROLLOUT_SAMPLES: usize = 3;
...
impl Solver {
    ...
    fn improve_final_growth_by_biased_swap(&mut self, initial: Placement, V: i64) -> Placement {
        ...
        if time_limit_hit {
            // 64到着 rollout の予算を保証するため、biased 探索を完走できなければ
            // 途中の best-so-far は使わず v040 以前の baseline を保つ。
            initial
        } else if best_perimeter < initial_perimeter {
    ...
    /// biased swap 比較専用の将来到着列。生成則と seed は通常 rollout と揃え、
    /// 件数だけを64へ延ばす。通常配置用22件の経路には手を入れない。
    fn make_swap_future_arrivals(
        &self,
        incoming_id: usize,
        now: usize,
        theta: f64,
        ...
        let remaining = (self.M - 1 - incoming_id) as f64;
        let remaining_time = (HORIZON - now) as f64;
        let mut arrivals = Vec::with_capacity(SWAP_ROLLOUT_ARRIVALS);
        if remaining < 1.0 || remaining_time < 1.0 {
            return arrivals;
        }
        let mean_gap = remaining_time / remaining;
        let mut t = now as f64;
        for _ in 0..SWAP_ROLLOUT_ARRIVALS {
            t += (-rng.next_f64().ln()) * mean_gap;
            if t >= HORIZON as f64 {
                break;
            }
    ...
    /// biased swap 比較専用の長期 rollout。既存 active、incoming、rollout 内で
    /// 受理した未来 group の退去を、共通到着列に対して候補ごとに同じ規則で処理する。
    fn swap_rollout_one(
        &self,
        cand: &RolloutCandidate,
        dep_base: &[(usize, usize)],
        ...
        let mut occ = cand.board;
        let mut base_i = 0;
        let mut future_deps: Vec<(usize, Vec<usize>)> =
            Vec::with_capacity(SWAP_ROLLOUT_ARRIVALS + 1);
        if let Some((t, cells)) = &cand.incoming_dep {
            future_deps.push((*t, cells.clone()));
        }
        let mut score = 0.0;
    ...
    /// biased swap の2盤面だけを、64到着×3本の専用共通乱数 rollout で比較する。
    /// index 0 を baseline とし、厳密に上回った場合だけ improved(index 1)を返す。
    fn evaluate_swap_candidates_rollout(
        &self,
        cands: &[RolloutCandidate],
        incoming_id: usize,
        now: usize,
        ...
        let mut totals = [
            cands[0].immediate * (SWAP_ROLLOUT_SAMPLES as f64),
            cands[1].immediate * (SWAP_ROLLOUT_SAMPLES as f64),
        ];
        for k in 0..SWAP_ROLLOUT_SAMPLES {
            let seed = (incoming_id as u64) * 1_000_003 + (k as u64) * 7_919 + 1;
            let arrivals = self.make_swap_future_arrivals(incoming_id, now, theta, seed);
            for (i, cand) in cands.iter().enumerate() {
                totals[i] += self.swap_rollout_one(cand, &dep_base, &arrivals, threshold);
            }
        }
        if totals[1] > totals[0] {
            1
        } else {
            // 同点では盤面を変えず baseline を維持する。
            0
        }
    }

    /// v040 の baseline と、完走した biased swap の厳密短縮案を通貨尺度で比較する。
    fn select_growth_swap_by_rollout(
        &mut self,
        baseline: Placement,
        improved: Placement,
        incoming_id: usize,
        ...
        let group = &self.groups[incoming_id];
        let P = group.P;
        let V = group.V;
        assert_eq!(baseline.shape_index, usize::MAX);
        assert_eq!(improved.shape_index, usize::MAX);
        assert_eq!(baseline.component_size, improved.component_size);
        assert!(improved.perimeter < baseline.perimeter);

        let baseline_cells = self.materialize(&baseline, P);
        let improved_cells = self.materialize(&improved, P);
        assert!(self.explicit_candidate_is_valid(&baseline_cells, P, &self.occupied_rows));
        assert!(self.explicit_candidate_is_valid(&improved_cells, P, &self.occupied_rows));
        assert_eq!(self.perimeter_of_cells(&baseline_cells), baseline.perimeter);
        assert_eq!(self.perimeter_of_cells(&improved_cells), improved.perimeter);

        let mut cands = Vec::with_capacity(2);
        for (placement, cells) in [(&baseline, baseline_cells), (&improved, improved_cells)] {
            ...
            cands.push(RolloutCandidate {
                board,
                // slot calendarは候補比較へ入れず、実際に得る利用料だけを数える。
                immediate: (V as f64) * compactness(P, placement.perimeter),
                incoming_dep: Some((group.T, cells)),
            });
        }

        let winner = local_time!(self.trace, "swap_rollout", {
            self.evaluate_swap_candidates_rollout(
                &cands,
                incoming_id,
                group.S,
                theta,
        ...
        let proposed_reduction = baseline.perimeter - improved.perimeter;
        let proposed_fee_gain =
            (V as f64) * (compactness(P, improved.perimeter) - compactness(P, baseline.perimeter));
        let _selected_reduction = if winner == 1 { proposed_reduction } else { 0 };
        let _selected_fee_gain = if winner == 1 { proposed_fee_gain } else { 0.0 };
        local! {
            self.trace.count("swap_rollout_comparison");
            self.trace.count_by("swap_rollout_candidate_sum", 2);
            self.trace.count_by(
                "swap_rollout_proposed_perimeter_reduction",
                proposed_reduction as i64,
            );
            self.trace.count_by(
                "swap_rollout_selected_perimeter_reduction",
                _selected_reduction as i64,
            );
            self.trace.count_by(
                "swap_rollout_proposed_fee_gain",
                proposed_fee_gain.round() as i64,
            );
            self.trace.count_by(
                "swap_rollout_selected_fee_gain",
                _selected_fee_gain.round() as i64,
            );
            if winner == 1 {
                self.trace.count("swap_rollout_improved");
            } else {
                self.trace.count("swap_rollout_baseline");
            }
        }
        if winner == 1 { improved } else { baseline }
    }
            ...
            if passed_price_prefilter {
                ...
                if let Some(mut choices) = normal_choices {
                    ...
                    if accepted && placement.shape_index == usize::MAX {
                        ...
                        let improved = local_time!(self.trace, "biased_swap", {
                            self.improve_final_growth_by_biased_swap(baseline.clone(), V)
                        });
                        placement = if improved.perimeter < baseline.perimeter {
                            self.select_growth_swap_by_rollout(
                                baseline,
                                improved,
                                id,
                                theta,
