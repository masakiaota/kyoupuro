// v004_component_aware_choice.rs
...
// 成分別の割引価格は推定値なので、新たに救済する候補だけ 2% の余裕を要求する。
const COMPONENT_RESCUE_SURPLUS: f64 = 1.02;
    ...
    for (P, target_shapes) in shapes_by_p.iter_mut().enumerate().take(MAX_P + 1).skip(4) {
        ...
        while begin < generated.len() {
            ...
            if count <= CAP_PER_LEVEL {
                ...
                chosen.extend(0..first);
                let remain = CAP_PER_LEVEL - first;
                for k in 0..remain {
                ...
                chosen.dedup();
                for idx in chosen {
                    kept.push(generated[begin + idx].clone());
                }
            }
            begin = end;
        }
...
impl Solver {
    ...
    fn find_normal_placement(
        ...
        // v501 は空間スコア最良の一候補を返した後で価格判定する。
        // 最良候補が不採算な場合だけ探索を広げ、成分別価格を満たす候補を救済する。
        ...
        for perimeter in (min_L..=min_L + max_extra).step_by(2) {
            ...
            let mut best_any: Option<Placement> = None;
            let mut best_rescue: Option<Placement> = None;
            for mut candidate in candidates {
                ...
                if self.placement_meets_component_rescue_price(
                    P,
                    &candidate,
                    q_value,
                    base_threshold,
                    ...
                    .is_none_or(|current| candidate.final_score > current.final_score)
                {
                    best_rescue = Some(candidate.clone());
                }
                if best_any
                    .as_ref()
                    .is_none_or(|current| candidate.final_score > current.final_score)
            ...
            let best_any = best_any.expect("nonempty candidate level has a best placement");
            if baseline.is_none() {
                baseline = Some(best_any);
            }
            let original_is_profitable = self.placement_meets_component_price(
                P,
                baseline.as_ref().expect("baseline was set above"),
                q_value,
                base_threshold,
            );
            if original_is_profitable {
                return baseline;
            }
            if let Some(best) = best_rescue {
                let original = baseline.as_ref().expect("baseline was set above");
                debug_assert!(!self.placement_meets_component_price(
                    P,
                    original,
                    q_value,
                    base_threshold,
                ...
                local! {
                    self.trace.count("component_price_rescue");
                    let rescue_threshold = base_threshold
                        * self.component_threshold_factor(best.component_size);
                    let margin = q_value * compactness(P, best.perimeter)
                        / rescue_threshold.max(f64::MIN_POSITIVE)
                        - 1.0;
                    if margin < 0.01 {
                        self.trace.count("component_price_margin_lt_1pct");
                    } else if margin < 0.02 {
                        self.trace.count("component_price_margin_1_2pct");
                    } else if margin < 0.05 {
                        self.trace.count("component_price_margin_2_5pct");
                    } else if margin < 0.10 {
                        self.trace.count("component_price_margin_5_10pct");
                    } else {
                        self.trace.count("component_price_margin_ge_10pct");
                    }
                    match best.component_size.saturating_sub(P) {
                        0..=3 => self.trace.count("component_price_remainder_0_3"),
                        4..=15 => self.trace.count("component_price_remainder_4_15"),
                        16..=31 => self.trace.count("component_price_remainder_16_31"),
                        _ => self.trace.count("component_price_remainder_ge_32"),
                    }
                    if best.perimeter == original.perimeter {
                        self.trace.count("component_price_same_level");
                    } else {
                        self.trace.count("component_price_later_level");
                    }
                }
                return Some(best);
            }

            // 1 turn が時刻閾値をまたいだら、既存候補を返して通常の後段判定へ委ねる。
            if self.timer.reached(FAST_MODE_RATIO) {
                break;
            }
        }

        // 採算候補がなければ v501 が最初に選んだ候補を維持し、
        // 後段の棄却・局所再配置条件を変えない。
        if baseline.is_some() {
            return baseline;
        }
    ...
    fn placement_meets_component_price(
        &self,
        P: usize,
        placement: &Placement,
        q_value: f64,
        ...
        base_threshold == 0.0
            || q_value * compactness(P, placement.perimeter)
                >= base_threshold * self.component_threshold_factor(placement.component_size)
    }

    fn placement_meets_component_rescue_price(
        &self,
        P: usize,
        placement: &Placement,
        q_value: f64,
        ...
        base_threshold == 0.0
            || q_value * compactness(P, placement.perimeter)
                >= COMPONENT_RESCUE_SURPLUS
                    * base_threshold
                    * self.component_threshold_factor(placement.component_size)
    }
        ...
        for turn in 0..self.M {
            ...
            if passed_price_prefilter {
                ...
                normal = local_time!(self.trace, "normal_search", {
                    self.find_normal_placement(P, T, theta, fast_mode, q_value, base_threshold)
                });
