// v072_anytime_holdout_rollout.rs
#![allow(non_snake_case)]
// 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: v071 の基準winnerを先に確定し、進捗別目標より実時間が前にある場合だけ、
// 通常配置top20の未使用候補をfresh共通乱数でscreeningする。最良challengerは別seedの
// paired holdoutで再確認し、追加処理時間を基準時計から除外して既存探索を置換しない。
...
// ---- 通常配置top20のanytime holdout rollout ----
// local時のPROGRAM_TIME_LIMIT_SEC=1.52秒に対し、進捗比例の1.32秒まで仕事を供給する。
// judge側でも同じ割合で伸縮させ、秒数を直接埋め込まない。
const ANYTIME_TARGET_RATIO: f64 = 132.0 / 152.0;
// 追加処理を基準時計から除外しても、全経路はlocal時1.48秒で軽量経路へ切り替える。
const ANYTIME_HARD_LIMIT_RATIO: f64 = 148.0 / 152.0;
const ANYTIME_SCREEN_LIMIT: usize = 12;
const ANYTIME_SCREEN_SEED_SALT: u64 = 0x6A09_E667_F3BC_C909;
const ANYTIME_HOLDOUT_SEED_SALT: u64 = 0xBB67_AE85_84CA_A73B;
...
// ---- v069 の進捗予測による追加探索 ----
// C++原案の秒数閾値を、V053と同じ1.90秒基準に対する比率で表す。
// local feature でも PROGRAM_TIME_LIMIT_SEC と同じ比率で縮む。
const ADAPTIVE_MIN_TURN: usize = 400;
const ADAPTIVE_LEVEL2_ELAPSED_RATIO: f64 = 80.0 / 100.0;
const ADAPTIVE_LEVEL1_ELAPSED_RATIO: f64 = 82.0 / 100.0;
const ADAPTIVE_LEVEL2_TOTAL_RATIO: f64 = 75.0 / 190.0;
const ADAPTIVE_LEVEL2_RELOCATION_RATIO: f64 = 38.0 / 190.0;
const ADAPTIVE_LEVEL1_TOTAL_RATIO: f64 = 108.0 / 190.0;
const ADAPTIVE_LEVEL1_RELOCATION_RATIO: f64 = 65.0 / 190.0;
...
struct TimeKeeper {
    start: Instant,
    time_limit_sec: f64,
    /// anytime処理だけに費やした実時間。v071の各締切では差し引く。
    excluded_extra_sec: f64,
}
...
impl TimeKeeper {
    fn new(time_limit_sec: f64) -> Self {
        Self {
            start: Instant::now(),
            time_limit_sec,
            excluded_extra_sec: 0.0,
        }
    }

    #[inline]
    fn reached(&self, ratio: f64) -> bool {
        self.hard_reached() || self.elapsed_sec() >= self.time_limit_sec * ratio
    }

    /// v071の探索量を決める仮想経過時間。anytime処理中だけ時計を止める。
    #[inline]
    fn elapsed_sec(&self) -> f64 {
        (self.real_elapsed_sec() - self.excluded_extra_sec).max(0.0)
    }

    #[inline]
    fn real_elapsed_sec(&self) -> f64 {
        self.start.elapsed().as_secs_f64()
    }

    #[inline]
    fn exclude_extra_sec(&mut self, elapsed_sec: f64) {
        self.excluded_extra_sec += elapsed_sec;
    }

    #[inline]
    fn hard_reached(&self) -> bool {
        self.real_elapsed_sec() >= self.time_limit_sec * ANYTIME_HARD_LIMIT_RATIO
    }
    ...
    fn elapsed_ms(&self) -> f64 {
        self.elapsed_sec() * 1000.0
    }

    #[cfg(feature = "local")]
    #[inline]
    fn real_elapsed_ms(&self) -> f64 {
        self.real_elapsed_sec() * 1000.0
    }
}
...
struct Placement {
    ...
    /// V053のtop 20外から追加した候補か。採否と機構確認のtraceにだけ使う。
...
struct NormalPlacementSet {
    /// v071がrolloutへ渡す代表候補。内容と順序を固定する。
    choices: Vec<Placement>,
    /// 既に局所評価したtop20のうち、代表候補に含まれない同条件候補。
    anytime_pool: Vec<Placement>,
}
...
struct Solver {
    ...
    reloc_spent_sec: f64,
    /// 現在の到着番号と、完走予測から選んだ追加探索レベル (0/1/2)。
    current_turn: usize,
    current_extra_level: u8,
    timer: TimeKeeper,
    #[cfg(feature = "local")]
    trace: TraceStats,
}
...
impl Solver {
    ...
    /// 現在までのCPUを完走時へ線形外挿し、V053の探索量に安全な余剰がある場合だけ
    /// 追加探索を許す。序盤は初期化・初期盤面の偏りが強いため予測に使わない。
    fn adaptive_search_level(&self) -> u8 {
        if self.current_turn < ADAPTIVE_MIN_TURN {
            return 0;
        }
        let progress = (((self.current_turn + 1) as f64) / (self.M as f64)).max(0.40);
        let projected_total = self.timer.elapsed_sec() / progress;
        let projected_relocation = self.reloc_spent_sec / progress;
        if !self.timer.reached(ADAPTIVE_LEVEL2_ELAPSED_RATIO)
            && projected_total < self.timer.budget_sec(ADAPTIVE_LEVEL2_TOTAL_RATIO)
            && projected_relocation < self.timer.budget_sec(ADAPTIVE_LEVEL2_RELOCATION_RATIO)
        {
            return 2;
        }
        if !self.timer.reached(ADAPTIVE_LEVEL1_ELAPSED_RATIO)
            && projected_total < self.timer.budget_sec(ADAPTIVE_LEVEL1_TOTAL_RATIO)
            && projected_relocation < self.timer.budget_sec(ADAPTIVE_LEVEL1_RELOCATION_RATIO)
        {
            return 1;
        }
        0
    ...
    /// V053のtop候補と、その真部分集合である広いtop候補を一走査で集める。
    /// これにより追加探索でもV053の20候補を削らず、走査の二重実行も避ける。
    fn scan_regular_level_dual(
        &self,
        P: usize,
        runs: &RunTable,
        weights: &WeightData,
        ...
        let mut base_top = Vec::new();
        let mut extended_top = Vec::new();
        let mut used_shapes = 0;
        for (shape_index, shape) in self.shapes_by_p[P].iter().enumerate() {
            if shape.perimeter != perimeter {
                continue;
            ...
            for x in 0..=(self.N - shape.h) {
                ...
                while ys != 0 {
                    ...
                    Self::insert_top_by_cheap(&mut base_top, candidate.clone(), base_count);
                    Self::insert_top_by_cheap(&mut extended_top, candidate, extended_count);
                }
            }
        }
        (base_top, extended_top)
    }
    ...
    /// 共有辺数 d を最優先する成長キー。d が同じ候補だけを関節点損失で比較し、
    /// その後は v069 と同じ ring/manhattan/attraction の順に戻す。
    #[inline]
    fn growth_key(
        &self,
        d: i64,
        ...
        let original_tail = 100_000 * (ring as i64) + 1_000 * (manhattan as i64) - attraction;
        (-d, cut_loss[id], original_tail, id)
    }
    ...
    fn push_growth_cell(
        ...
        cut_loss: &[usize],
        selected: &[bool],
        frontier: &mut BinaryHeap<Reverse<(i64, usize, i64, usize)>>,
    ) {
    ...
    fn growth_placement(
        ...
        // seed_limit=44 は通常配置、20 は relocation/repack でのみ使う。C++版と同じく
        // 通常配置だけで低リンクを計算し、repack の探索順・時間配分は v069 のまま保つ。
        let use_cutloss = seed_limit >= 44;
        let computed_cut_loss = if use_cutloss {
            local! {
                self.trace.count("cutloss_turn");
            }
            let free = info
                .component
                .iter()
                ...
                .collect::<Vec<_>>();
            let loss = free_cut_loss(self.N, &free);
            local! {
                if loss.iter().any(|&value| value > 0) {
                    self.trace.count("cutloss_positive_turn");
                }
            }
            Some(loss)
        } else {
        ...
        // repack 側は既存のゼロ損失順を使う。繰り返し呼ばれる経路で Vec を確保しない。
        let zero_cut_loss = [0_usize; MAX_N * MAX_N];
        let cut_loss = computed_cut_loss
            .as_deref()
            .unwrap_or(&zero_cut_loss[..self.N * self.N]);
        ...
        for component_id in 0..info.sizes.len() {
            ...
            let mut best1 = None;
            let mut best2 = None;
            let mut weight_best1 = None;
            let mut weight_best2 = None;
            for &id in list {
                let weight_better = |candidate: usize, current: Option<usize>| {
                    current.is_none_or(|current| weights.cell[candidate] > weights.cell[current])
                };
                if weight_better(id, weight_best1) {
                    weight_best2 = weight_best1;
                    weight_best1 = Some(id);
                } else if weight_better(id, weight_best2) {
                    weight_best2 = Some(id);
                }
                let cut_better = |candidate: usize, current: Option<usize>| {
                    let Some(current) = current else {
                        return true;
                    };
                    if cut_loss[candidate] != cut_loss[current] {
                        return cut_loss[candidate] < cut_loss[current];
                    }
                    if weights.cell[candidate] != weights.cell[current] {
                        return weights.cell[candidate] > weights.cell[current];
                    }
                    candidate < current
                };
                let better1 = if use_cutloss {
                    cut_better(id, best1)
                } else {
                    weight_better(id, best1)
                };
                if better1 {
                    best2 = best1;
                    best1 = Some(id);
                } else {
                    let better2 = if use_cutloss {
                        cut_better(id, best2)
                    } else {
                        weight_better(id, best2)
                    };
                    if better2 {
                        best2 = Some(id);
                    }
            ...
            local! {
                if use_cutloss && (best1 != weight_best1 || best2 != weight_best2) {
                    self.trace.count("cutloss_seed_changed");
                }
            }
        ...
        for seed in seeds {
            ...
            region.push(seed);
            local! {
                if use_cutloss && cut_loss[seed] > 0 {
                    self.trace.count("cutloss_positive_selected");
                }
            }
            for (dr, dc) in DIRS {
            ...
            while region.len() < P {
                let Some(Reverse((negative_d, _, _, id))) = frontier.pop() else {
                    break;
                };
                let d_recorded = -negative_d;
                if selected[id] {
                    continue;
                }
    ...
    fn find_normal_placements(
        ...
        for perimeter in (min_L..=min_L + max_extra).step_by(2) {
            let (candidates, extended_candidates) = if !fast_mode && self.current_extra_level > 0 {
                let extended_count = if self.current_extra_level == 2 {
                    40
                } else {
                    28
                };
                local! {
                    self.trace.count("adaptive_normal_search");
                }
                self.scan_regular_level_dual(
                    P,
                    &runs,
                    &weights,
                    perimeter,
                ...
                (
                    self.scan_regular_level(
                        P,
                        &runs,
                        &weights,
                        perimeter,
                        if fast_mode { 7 } else { 20 },
            ...
            if fast_mode {
                return Some(NormalPlacementSet {
                    choices: vec![evaluated.swap_remove(current_index).0],
                    anytime_pool: Vec::new(),
                });
            }
            ...
            // 追加top候補は、V053の代表3候補より局所scoreまたは断片化が厳密に良いものだけ
            // posterior rolloutへ渡す。V053候補そのものは常にchoicesに残る。
            if !extended_candidates.is_empty() {
                let mut extended_evaluated = Vec::with_capacity(extended_candidates.len());
                for mut candidate in extended_candidates {
                    let cells = self.materialize(&candidate, P);
                    let component_id = info.component[cells[0]];
                    candidate.component_size = if component_id >= 0 {
                        info.sizes[component_id as usize]
                    ...
                    candidate.final_score = candidate.cheap_score - 1.4 * fragment_delta;
                    candidate.explicit_cells = cells;
                    extended_evaluated.push((candidate, fragment_delta));
                }

                let mut best_final: Option<usize> = None;
                let mut best_fragment: Option<usize> = None;
                for index in 0..extended_evaluated.len() {
                    if extended_evaluated[index].0.component_size != component_size {
                        continue;
                    }
                    if best_final.is_none_or(|best| {
                        extended_evaluated[index].0.final_score
                            > extended_evaluated[best].0.final_score
                    }) {
                        best_final = Some(index);
                    }
                    if best_fragment
                        .is_none_or(|best| extended_evaluated[index].1 < extended_evaluated[best].1)
                    {
                        best_fragment = Some(index);
                    }
                }

                for index in [best_final, best_fragment].into_iter().flatten() {
                    let is_better_final = extended_evaluated[index].0.final_score
                        > evaluated[current_index].0.final_score + 1e-12;
                    let is_better_fragment =
                        extended_evaluated[index].1 < evaluated[fragment_index].1 - 1e-12;
                    if !is_better_final && !is_better_fragment {
                        continue;
                    }
                    let mut candidate = extended_evaluated[index].0.clone();
                    if choices
                        .iter()
                        .any(|choice: &Placement| choice.explicit_cells == candidate.explicit_cells)
                    {
                    ...
                    candidate.extended_search = true;
                    choices.push(candidate);
                    local! {
                        self.trace.count("adaptive_normal_candidate");
                    }
                }
            }
            // v071ではここで捨てていた局所評価済みtop20を、余剰時間用の仕事として保持する。
            // admissionと即時利用料を固定するため、代表候補と同じ周長・componentだけに限る。
            let pool_start = Instant::now();
            let mut anytime_pool = Vec::new();
            for (candidate, _) in &evaluated {
                if candidate.component_size != component_size
                    || choices
                        .iter()
                        .any(|choice| choice.explicit_cells == candidate.explicit_cells)
                    || anytime_pool
                        .iter()
                        .any(|pool: &Placement| pool.explicit_cells == candidate.explicit_cells)
                {
                    continue;
                }
                debug_assert_eq!(candidate.perimeter, choices[0].perimeter);
                anytime_pool.push(candidate.clone());
            }
            anytime_pool.sort_by(|a, b| {
                b.final_score
                    .total_cmp(&a.final_score)
                    .then_with(|| b.cheap_score.total_cmp(&a.cheap_score))
                    .then_with(|| a.explicit_cells.cmp(&b.explicit_cells))
            });
            let pool_elapsed_sec = pool_start.elapsed().as_secs_f64();
            self.timer.exclude_extra_sec(pool_elapsed_sec);
            local! {
                if !anytime_pool.is_empty() {
                    self.trace.count("anytime_pool_session");
                    self.trace
                        .count_by("anytime_pool_candidate", anytime_pool.len() as i64);
                }
                self.trace.count_by(
                    "anytime_extra_time_us",
                    (pool_elapsed_sec * 1_000_000.0).round() as i64,
                );
            }
            return Some(NormalPlacementSet {
                choices,
        ...
        self.growth_placement(P, &occ, &info, &weights, usize::MAX, 44)
            .map(|placement| NormalPlacementSet {
                choices: vec![placement],
                anytime_pool: Vec::new(),
            })
    }
    ...
    fn evaluate_candidates_rollout(
        ...
        self.evaluate_candidates_rollout_seeded(cands, incoming_id, now, theta, threshold, 0)
    }

    /// 基準rolloutとは独立した乱数列を使う版。salt=0ならv071と完全に同じ列になる。
    fn evaluate_candidates_rollout_seeded(
        &mut self,
        cands: &[RolloutCandidate],
        incoming_id: usize,
        now: usize,
        ...
        for k in 0..ROLLOUT_SAMPLES {
            let base_seed = (incoming_id as u64) * 1_000_003 + (k as u64) * 7_919 + 1;
            let seed = base_seed ^ seed_salt;
    ...
    /// v071のwinnerを基準に、余剰時間だけで未使用top20候補をscreeningし、
    /// 最良challengerを独立seedのpaired holdoutで再確認する。
    fn improve_normal_with_anytime(
        &mut self,
        baseline: Placement,
        pool: &[Placement],
        incoming_id: usize,
        ...
        if self.timer.hard_reached() {
            local! {
                self.trace.count("anytime_hard_stop");
            }
            return baseline;
        }

        let progress = ((self.current_turn + 1) as f64) / (self.M as f64);
        let target_elapsed = self.timer.budget_sec(ANYTIME_TARGET_RATIO) * progress;
        if self.timer.real_elapsed_sec() >= target_elapsed {
            local! {
                self.trace.count("anytime_target_skip");
            }
            return baseline;
        }

        let extra_start = Instant::now();
        let screen_count = pool.len().min(ANYTIME_SCREEN_LIMIT);
        let incoming = &self.groups[incoming_id];
        let incoming_S = incoming.S;
        let incoming_T = incoming.T;
        let incoming_P = incoming.P;
        let incoming_V = incoming.V;

        // Releaseビルドでも、料金とadmissionを変えないための条件を必ず検証する。
        let mut seen_cells = HashSet::with_capacity(screen_count + 1);
        assert!(seen_cells.insert(baseline.explicit_cells.clone()));
        for candidate in pool.iter().take(screen_count) {
            assert_eq!(candidate.perimeter, baseline.perimeter);
            assert_eq!(candidate.component_size, baseline.component_size);
            assert!(seen_cells.insert(candidate.explicit_cells.clone()));
        }

        let mut screening = Vec::with_capacity(screen_count + 1);
        screening
            .push(self.normal_rollout_candidate(&baseline, incoming_P, incoming_T, incoming_V));
        for candidate in pool.iter().take(screen_count) {
            screening
                .push(self.normal_rollout_candidate(candidate, incoming_P, incoming_T, incoming_V));
        }
        let screen_winner = local_time!(self.trace, "anytime_screen_rollout", {
            self.evaluate_candidates_rollout_seeded(
                &screening,
                incoming_id,
                incoming_S,
                theta,
                base_threshold,
                ANYTIME_SCREEN_SEED_SALT,
            )
        });
        local! {
            self.trace.count("anytime_screen_session");
            self.trace
                .count_by("anytime_screen_candidate", screen_count as i64);
            self.trace.count_by(
                "anytime_screen_total_candidate",
                (screen_count + 1) as i64,
            );
            self.trace
                .count_by("anytime_invariant_checked", screen_count as i64);
            if screen_winner != 0 {
                self.trace.count("anytime_screen_challenger");
            }
        }
        ...
        if screen_winner != 0 {
            if self.timer.hard_reached() {
                ...
                let challenger = &pool[screen_winner - 1];
                let holdout = [
                    self.normal_rollout_candidate(&baseline, incoming_P, incoming_T, incoming_V),
                    self.normal_rollout_candidate(challenger, incoming_P, incoming_T, incoming_V),
                ];
                let holdout_winner = local_time!(self.trace, "anytime_holdout_rollout", {
                    self.evaluate_candidates_rollout_seeded(
                        &holdout,
                        incoming_id,
                        incoming_S,
                        theta,
                        base_threshold,
                        ANYTIME_HOLDOUT_SEED_SALT,
                    )
                });
                local! {
                    self.trace.count("anytime_holdout_session");
                    if holdout_winner == 1 {
                        self.trace.count("anytime_selected");
                    }
                }
                if holdout_winner == 1 {
                    selected = Some(challenger.clone());
                }
            }
        }

        let extra_elapsed_sec = extra_start.elapsed().as_secs_f64();
        self.timer.exclude_extra_sec(extra_elapsed_sec);
        local! {
            self.trace.count_by(
                "anytime_extra_time_us",
                (extra_elapsed_sec * 1_000_000.0).round() as i64,
            );
        }
        selected.unwrap_or(baseline)
    }
    ...
    fn attempt_relocation(
        ...
        let mut collected_attempt = Vec::new();
        let attempt_limit = match self.current_extra_level {
            2 => 22,
            1 => 17,
            _ => 12,
        };
        for (attempt, option) in options.into_iter().enumerate() {
            if attempt >= attempt_limit
                || collected.len() >= 3
                || self.timer.reached(REPACK_LIMIT_RATIO)
            {
                break;
            }
            if attempt >= 12 {
                // 通常の12試行を使い切った後は、追加探索が残り時間を侵食しないよう
                // level選択より短い0.82時点で必ず止める。
                if self.timer.reached(ADAPTIVE_LEVEL1_ELAPSED_RATIO) {
                    break;
                }
                local! {
                    self.trace.count("adaptive_repack_attempt");
                }
            }
            ...
            if attempt >= 12 && !collected.is_empty() {
                let best_net = collected
                    .iter()
                    .map(|(_, net, _, _, _)| *net)
                    .fold(-1e100_f64, f64::max);
                let best_metric = collected
                    .iter()
                    .map(|(_, _, metric, _, _)| *metric)
                    .fold(1e100_f64, f64::min);
                // 既存3案のどれよりも純利益・断片化の両方で悪い案を増やしても、
                // rolloutの候補数だけが増える。少なくとも一方を厳密に改善する案だけ残す。
                if net <= best_net + 1e-12 && final_metric >= best_metric - 1e-12 {
                    continue;
                }
            }
            collected.push((
            ...
            collected_attempt.push(attempt);
            if attempt >= 12 {
                local! {
                    self.trace.count("adaptive_plan_collected");
                }
            }
        }
        ...
        if collected_attempt[winner] >= 12 {
            local! {
                self.trace.count("adaptive_relocation_selected");
            }
        }
        local! {
    ...
    fn run<R: BufRead, W: Write>(
        ...
        for turn in 0..self.M {
            ...
            self.current_extra_level = self.adaptive_search_level();
            local! {
                match self.current_extra_level {
                    2 => self.trace.count("adaptive_level2_turn"),
                    1 => self.trace.count("adaptive_level1_turn"),
                    _ => {}
                }
            }
            ...
            if passed_price_prefilter {
                ...
                if let Some(placement_set) = normal_choices {
                    let mut choices = placement_set.choices;
                    let anytime_pool = placement_set.anytime_pool;
                    ...
                    if accepted && placement.extended_search {
                        local! {
                            self.trace.count("adaptive_normal_selected");
                        }
                    }
                    if accepted && !anytime_pool.is_empty() {
                        placement = self.improve_normal_with_anytime(
                            placement,
                            &anytime_pool,
                            id,
                            theta,
        ...
        local! {
            self.trace
                .add_time_ms("program_elapsed", self.timer.real_elapsed_ms());
            self.trace
                .add_time_ms("program_virtual_elapsed", self.timer.elapsed_ms());
            self.trace.add_time_ms(
                "anytime_excluded_time",
                self.timer.excluded_extra_sec * 1000.0,
            );
            self.trace.summary();
        }
        Ok(())
