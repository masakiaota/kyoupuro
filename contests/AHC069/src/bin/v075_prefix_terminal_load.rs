// v075_prefix_terminal_load.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: v074 の期待絶対需要と単一終端係数を保ち、theta推定だけを未到着groupの
// prefix尤度を含む7次近似MAPへ置換する。単一MAPを通常判断とrolloutで共有する。
...
const PREFIX_H: f64 = HORIZON as f64;
...
struct Solver {
    ...
    /// 7次prefix補正MAPで使う Y_n = sum(D[i]-1)。
    duration_minus_one_sum: u64,
    duration_count: usize,
    /// 逐次Newtonを直前の解から始めるために保持する beta=H/theta。
    prefix_beta_hat: f64,
    expected_p: f64,
    compactness_bar: f64,
...
impl Solver {
    ...
    /// u_k(beta,a)=integral_0^a z^k beta exp(-beta z) dz をu_0..u_max_kまで返す。
    /// beta*aが小さい領域は、漸化式の差し引きで桁落ちしない級数を使う。
    ...
    /// theta側の事後密度へbeta=H/thetaを代入した目的関数と、そのbeta微分。
    /// prefix補正Q_theta(S)^(M-n)は推定にだけ使い、終端係数として再乗算しない。
    fn prefix_map_eval(
        beta: f64,
        n: usize,
        Y: u64,
    ...
    /// 33group目以降は前回MAPから2回だけ保護付きNewton更新する。
    ...
    fn theta_prefix_map(&mut self, S: usize) -> f64 {
        let (beta, _guarded_steps) = if self.duration_count <= 32 {
            local! {
                self.trace.count("theta_prefix_interval_turn");
            }
            (self.prefix_interval_map(S), 0)
        } else {
            local! {
                self.trace.count("theta_prefix_newton_turn");
            }
            self.prefix_newton_map(S)
        };
    ...
    /// `[S, S+duration)` の各整数時刻について
    /// `1-exp(-(HORIZON-t)/theta)` を正確に平均する。
    /// これは終端で減る期待需要の相対量であり、admission閾値へ一度だけ掛ける。
    fn average_terminal_demand_weight(S: usize, duration: usize, theta: f64) -> f64 {
        debug_assert!(duration > 0);
        let T = S + duration;
        debug_assert!(T <= HORIZON);

        // t=S..T-1 に対する exp(-(HORIZON-t)/theta) の等比級数。
        // exp_m1を使い、duration/thetaが小さい場合も桁落ちさせない。
        let first_remaining = HORIZON - T + 1;
        let first_tail = (-(first_remaining as f64) / theta).exp();
        let numerator = -(-(duration as f64) / theta).exp_m1();
        let denominator = -(-1.0 / theta).exp_m1();
        let average_tail = first_tail * numerator / (denominator * (duration as f64));
        (1.0 - average_tail).clamp(0.0, 1.0)
    }
    ...
    fn base_dynamic_threshold(
        ...
        let remaining_groups = self.M - incoming_id - 1;
        let remaining_time = HORIZON - S;
        let arrival_rate = if remaining_groups == 0 || remaining_time == 0 {
            0.0
        } else {
            (remaining_groups as f64) / (remaining_time as f64)
        };

        // 絶対需要の未知な実現値は使わず、オンラインで既知の残りgroup数と
        // posterior thetaから、道中に同時滞在する期待セル数を一度だけ求める。
        let offered_area = arrival_rate * self.expected_p * theta;
        let mid_bid = if offered_area <= self.effective_capacity {
            0.0
        } else {
            let fraction = self.effective_capacity / offered_area;
            self.q_threshold_for_fraction(fraction) * self.compactness_bar
        };

        let D = duration as f64;
        let terminal_weight = Self::average_terminal_demand_weight(S, duration, theta);
        // (D/theta)^0.1 は V/(P D^0.9) をセル時間密度へ直す単位変換であり、
        // 終端補正ではない。終端係数の乗算はこの一箇所だけに限定する。
        let threshold = mid_bid * (D / theta).powf(0.1) * terminal_weight;

        // 盤面occupancyのfeedbackは残すが、目標値へ終端係数を再適用しない。
        let target_occupancy = self.effective_capacity.min(offered_area);
        ...
        local! {
            self.trace.count("expected_load_turn");
            self.trace
                .count_by("expected_load_cells_sum", offered_area.round() as i64);
            self.trace.count_by(
                "terminal_weight_ppm_sum",
                (terminal_weight * 1_000_000.0).round() as i64,
            );
            if remaining_groups == 0 {
                self.trace.count("expected_load_no_future");
                debug_assert_eq!(mid_bid, 0.0);
            }
            if offered_area <= self.effective_capacity {
                self.trace.count("expected_load_under_capacity");
            }
            if terminal_weight < 0.999_5 {
                self.trace.count("terminal_discount_turn");
            }
            if mid_bid > 0.0 && terminal_weight < 0.999_5 {
                self.trace.count("terminal_price_discounted_turn");
            }
        }
    ...
    fn evaluate_candidates_rollout(
        ...
        for k in 0..ROLLOUT_SAMPLES {
            ...
            // prefix補正MAPを全rolloutで共有する。P・滞在・qの乱数列と、
            // 候補間の共通乱数は維持する。
            let theta_k = theta;
            local! {
                self.trace.count("theta_prefix_map_rollout");
            }
            let arrivals = self.make_future_arrivals(incoming_id, now, theta_k, seed);
            local! {
                self.trace
                    .count_by("rollout_l_sample", arrivals.len() as i64);
            }
            for (i, cand) in cands.iter().enumerate() {
    ...
    fn run<R: BufRead, W: Write>(
        ...
        for turn in 0..self.M {
            ...
            let duration = T - S;
            self.duration_minus_one_sum += (duration - 1) as u64;
            self.duration_count += 1;
            let _raw_theta = ((self.duration_minus_one_sum as f64) / (self.duration_count as f64))
                .clamp(2_000.0, 8_000.0);
            let theta = self.theta_prefix_map(S);
            local! {
                self.trace.count("theta_prefix_map_turn");
                self.trace
                    .count_by("theta_prefix_map_value_milli", (theta * 1_000.0).round() as i64);
                let delta_milli = ((theta - _raw_theta) * 1_000.0).round() as i64;
                self.trace
            ...
            let base_threshold = self.base_dynamic_threshold(id, S, duration, P, theta);
