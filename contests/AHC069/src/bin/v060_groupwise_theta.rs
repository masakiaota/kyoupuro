// v060_groupwise_theta.rs
#![allow(non_snake_case)]
// 中心アイデア: v059 の O(1) 十分統計 n, Y=sum(D[i]-1) は維持し、
// 各 rollout の仮想到着 group ごとに連続事後から異なる theta を正確にサンプルする。
// 通常判断の MAP、rollout 本数・到着数、候補間の共通到着列は固定する。
...
/// 現在の観測 prefix が定める連続事後分布の O(1) サンプリング状態。
/// theta ではなく z=log(theta) 上で棄却すると、提案分布を区間一様にできる。
#[derive(Clone, Copy)]
struct ThetaPosterior {
    n: usize,
    Y: f64,
    log_density_max: f64,
}
...
impl ThetaPosterior {
    fn new(n: usize, Y: u64) -> Self {
        assert!(n > 0, "theta posterior requires at least one observation");
        let Y = Y as f64;
        // z 上の密度は exp(-(n-1)z - Y exp(-z)) に比例する。
        // その最頻点を切断区間へ射影して棄却上界に使う。
    ...
    /// log(theta) 一様提案による棄却法。返値は切断された
    /// p(theta|D) ∝ theta^-n exp(-Y/theta) に近似なしで従う。
    #[inline]
    fn sample(self, rng: &mut XorShift64) -> (f64, usize) {
        let mut proposals = 0;
        loop {
            proposals += 1;
            let z = LOG_THETA_MIN + LOG_THETA_WIDTH * rng.next_f64();
            // exp/log の端点丸めだけを区間へ戻す（連続分布の測度は変わらない）。
            let theta = z.exp().clamp(THETA_MIN, THETA_MAX);
            let log_density = -((self.n - 1) as f64) * z - self.Y / theta;
            if rng.next_f64().ln() <= log_density - self.log_density_max {
...
struct Solver {
    ...
    /// 文書の軽量推定で使う Y_n = sum(D[i]-1)。整数のままO(1)更新する。
...
impl Solver {
    ...
    /// prefix補正を省いた連続事後分布のtheta側MAP。
    /// nとY_nの更新・点推定はいずれもO(1)で、数値積分や格子評価を行わない。
    fn theta_map(&self) -> f64 {
        if self.duration_count == 0 {
            return 5_000.0;
        }
        ((self.duration_minus_one_sum as f64) / (self.duration_count as f64))
    ...
    /// 滞在は仮想到着ごとに現在のtheta事後から独立に引いた指数分布、
    /// P は生成分布、q = 2^gauss(0,0.8)。theta用乱数を分離し、v059の
    /// 到着時刻・P・滞在用一様乱数・qの乱数列は変えない。
    fn make_future_arrivals(
        ...
        let mut rng = XorShift64::new(seed);
        let mut theta_rng = XorShift64::new(seed ^ 0xD1B5_4A32_D192_ED03);
        let posterior = ThetaPosterior::new(self.duration_count, self.duration_minus_one_sum);
        ...
        let mut _sample_count = 0_i64;
        let mut _proposal_count = 0_i64;
        let mut _adjacent_change_count = 0_i64;
        let mut _abs_deviation_milli = 0_i64;
        let mut previous_theta = None;
        for _ in 0..ROLLOUT_ARRIVALS {
            ...
            let P = lo;
            let (theta, proposals) = posterior.sample(&mut theta_rng);
            assert!((THETA_MIN..=THETA_MAX).contains(&theta));
            _sample_count += 1;
            _proposal_count += proposals as i64;
            _abs_deviation_milli += ((theta - theta_map).abs() * 1_000.0).round() as i64;
            if let Some(previous) = previous_theta {
                if theta != previous {
                    _adjacent_change_count += 1;
                }
            }
            previous_theta = Some(theta);
            let dur =
        ...
        local! {
            self.trace.count_by("theta_exact_group_sample", _sample_count);
            self.trace.count_by("theta_exact_proposal", _proposal_count);
            self.trace
                .count_by("theta_exact_adjacent_change", _adjacent_change_count);
            self.trace
                .count_by("theta_exact_abs_deviation_milli", _abs_deviation_milli);
        }
        arrivals
    }
    ...
    fn evaluate_candidates_rollout(
        ...
        for k in 0..ROLLOUT_SAMPLES {
            ...
            // 各仮想到着のthetaは独立に引くが、生成済み到着列は候補間で共有する。
            local! {
                self.trace.count("theta_map_rollout");
            }
    ...
    fn run<R: BufRead, W: Write>(
        ...
        for turn in 0..self.M {
            ...
            let theta = self.theta_map();
            local! {
                self.trace.count("theta_map_turn");
                self.trace
                    .count_by("theta_map_value_milli", (theta * 1_000.0).round() as i64);
            }
