// v064_no_move_contact_sync.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。

// 中心アイデア: v047の時間積分接触を主項として保ち、既存groupとの共有辺だけへ
// 小さな退出同期bonusを加える。再移動は一切行わない。
...
/// 現行の時間積分接触係数10.0に対し、完全同期時でも最大10%だけ補助する。
...
impl Solver {
    ...
    fn build_weight_data(
        ...
        let mut _departure_sync_evaluated_edges = 0_i64;
        let mut _departure_sync_bonus = 0.0_f64;
        const DIRS: [(isize, isize); 4] = [(-1, 0), (1, 0), (0, -1), (0, 1)];
        for r in 0..self.N {
            for c in 0..self.N {
                let id = r * self.N + c;
                let mut contact_time = 0_usize;
                let mut departure_sync_bonus = 0.0_f64;
                if self.is_free(occ, r, c) {
                    for (dr, dc) in DIRS {
                        let nr = (r as isize) + dr;
                        let nc = (c as isize) + dc;
                        ...
                        if !self.is_grass(nr, nc) {
                                ...
                                && self.groups[owner as usize].active
                            {
                                ...
                                let owner_T = self.groups[owner as usize].T;
                                let overlap = owner_T.min(incoming_T).saturating_sub(incoming_S);
                                contact_time += overlap;
                                _integrated_contact_edges += 1;

                                // overlapを主項として残し、主項が同点になる遅い退出同士だけを
                                // 退出時刻差で緩く区別する。thetaでケースの時間尺度へ正規化する。
                                let overlap_ratio = (overlap as f64) / (incoming_D.max(1) as f64);
                                let proximity =
                                    (-(incoming_T.abs_diff(owner_T) as f64) / theta.max(1.0)).exp();
                                let bonus = DEPARTURE_SYNC_BONUS_WEIGHT * overlap_ratio * proximity;
                                departure_sync_bonus += bonus;
                                _departure_sync_bonus += bonus;
                                _departure_sync_evaluated_edges += 1;
                            }
                        }
                    }
        ...
        local! {
            ...
            self.trace.count_by(
                "departure_sync_evaluated_edges",
                _departure_sync_evaluated_edges,
            );
            self.trace.count_by(
                "departure_sync_bonus_micros",
                (_departure_sync_bonus * 1_000_000.0).round() as i64,
            );
        }
        data
    }
    ...
    #[cfg(feature = "local")]
    fn selected_departure_sync_metrics(
        &self,
        cells: &[usize],
        incoming_S: usize,
        incoming_T: usize,
        ...
        for &id in cells {
            ...
            for (dr, dc) in [(-1_isize, 0_isize), (1, 0), (0, -1), (0, 1)] {
                ...
                if owner < 0 || (owner as usize) >= self.M {
                    continue;
                }
                let group = &self.groups[owner as usize];
                if !group.active {
                    continue;
                }
                let overlap = group.T.min(incoming_T).saturating_sub(incoming_S);
                let overlap_ratio = (overlap as f64) / (incoming_D.max(1) as f64);
                let proximity = (-(incoming_T.abs_diff(group.T) as f64) / theta.max(1.0)).exp();
                bonus += DEPARTURE_SYNC_BONUS_WEIGHT * overlap_ratio * proximity;
                edges += 1;
            }
        }
        (edges, (bonus * 1_000_000.0).round() as i64)
    }
    ...
    fn find_normal_placements(
        ...
        let weights = self.build_weight_data(
            &occ,
            &info,
            incoming_S,
            incoming_T,
            incoming_T - incoming_S,
            theta,
        );
    ...
    fn run<R: BufRead, W: Write>(
        ...
        for turn in 0..self.M {
            ...
            if accepted {
                ...
                local! {
                    let (edges, bonus_micros) = self.selected_departure_sync_metrics(
                        &placement.explicit_cells,
                        S,
                        T,
                        duration,
                        theta,
                    ...
                    self.trace
                        .count_by("departure_sync_selected_edges", edges);
                    self.trace.count_by(
                        "departure_sync_selected_bonus_micros",
                        bonus_micros,
                    );
                }
