// v022_no_move_growth_lns.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: v019のgrowth SAを保ち、その後に利用料損失が大きい大面積候補だけを
// 複数セルの侵食・再成長LNSで追加改善する。growth以外の探索・判定は変更しない。
...
impl Solver {
    ...
    fn find_normal_placements(
        ...
        self.growth_placement(P, incoming_V, &occ, &info, &weights, usize::MAX, 44)
        ...
        for turn in 0..self.M {
            ...
            if passed_price_prefilter {
                ...
                let normal_choices = local_time!(self.trace, "normal_search", {
                    self.find_normal_placements(P, V, S, T, fast_mode)
                });
