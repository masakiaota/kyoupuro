// v039_no_move_temporal_cutloss.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。

// 中心アイデア: v035 の static cut-loss growth を基準候補として先に確定し、
// 高価値な悪形だけは退去時系列を加重した cut-loss で追加 growth 候補を作る。
// 再移動は一切行わない。
...
/// temporal 候補は基準 growth の探索を完了した後、この時刻までに着手できる場合だけ作る。
const TEMPORAL_CUTLOSS_LIMIT_RATIO: f64 = 0.90;
const TEMPORAL_SNAPSHOT_NUMERATORS: [usize; 4] = [0, 2, 5, 8];
const TEMPORAL_SNAPSHOT_WEIGHTS: [usize; 4] = [2, 3, 3, 2];
...
/// 現在の占有盤面から snapshot までに退去する active 区画を除き、将来自由セルを返す。
/// 現在非activeの履歴セルは occupied_rows に存在しないので、activeだけを対象にする。
fn future_free_cells(
    N: usize,
    grass_rows: &Rows,
    occupied_rows: &Rows,
    groups: &[Group],
    ...
    let mut future_occupied = *occupied_rows;
    for group in groups {
        if !group.active || group.T > snapshot {
            continue;
        }
        for &cell in &group.cells {
            future_occupied[cell / N] &= !(1_u64 << (cell % N));
        }
    }
    let mut free = vec![false; N * N];
    for r in 0..N {
        for c in 0..N {
            free[r * N + c] =
                ((grass_rows[r] >> c) & 1) != 0 && ((future_occupied[r] >> c) & 1) == 0;
        }
    }
    free
}

/// future-free 上で region を塞いだとき、region が属していた元成分の残余セルのうち、
/// 最大残余成分へ残れないセル数を厳密に数える。
fn region_severed_loss(N: usize, free: &[bool], region: &[usize]) -> usize {
    assert!(!region.is_empty());
    assert_eq!(free.len(), N * N);
    let mut removed = vec![false; N * N];
    for &cell in region {
        assert!(cell < N * N && free[cell] && !removed[cell]);
        removed[cell] = true;
    }
    ...
    while let Some(v) = stack.pop() {
        ...
        .flatten()
        {
            if free[next] && !original[next] {
                original[next] = true;
                stack.push(next);
            }
        }
    }
    assert!(region.iter().all(|&cell| original[cell]));

    let residual_total = original_size - region.len();
    let mut seen = removed;
    let mut largest_residual = 0_usize;
    for residual_start in 0..N * N {
        if !original[residual_start] || seen[residual_start] {
            continue;
        }
        seen[residual_start] = true;
        stack.push(residual_start);
        ...
        while let Some(v) = stack.pop() {
            ...
            .flatten()
            {
                if original[next] && !seen[next] {
                    seen[next] = true;
                    stack.push(next);
                }
            }
        }
        largest_residual = largest_residual.max(size);
    }
    residual_total - largest_residual
}

fn persistent_region_severed(N: usize, snapshots: &[Vec<bool>], region: &[usize]) -> usize {
    assert_eq!(snapshots.len(), TEMPORAL_SNAPSHOT_WEIGHTS.len());
    snapshots
        .iter()
        .zip(TEMPORAL_SNAPSHOT_WEIGHTS)
        .map(|(free, weight)| weight * region_severed_loss(N, free, region))
        .sum()
}
...
#[cfg(feature = "local")]
fn verify_temporal_helpers() {
    fn naive_future_free(
        N: usize,
        grass_rows: &Rows,
        occupied_rows: &Rows,
        groups: &[Group],
        ...
        for cell in 0..N * N {
            let r = cell / N;
            let c = cell % N;
            let grass = ((grass_rows[r] >> c) & 1) != 0;
            let occupied = ((occupied_rows[r] >> c) & 1) != 0;
            let released = groups
                .iter()
                .any(|group| group.active && group.T <= snapshot && group.cells.contains(&cell));
            result[cell] = grass && (!occupied || released);
        }
        result
    }

    fn reachable(N: usize, free: &[bool], start: usize, removed: &[bool]) -> Vec<bool> {
        let mut seen = vec![false; N * N];
        if !free[start] || removed[start] {
            return seen;
        }
        let mut stack = vec![start];
        seen[start] = true;
        while let Some(v) = stack.pop() {
            ...
            .flatten()
            {
                if free[next] && !removed[next] && !seen[next] {
                    seen[next] = true;
                    stack.push(next);
                }
            }
    ...
    fn naive_severed(N: usize, free: &[bool], region: &[usize]) -> usize {
        let no_removal = vec![false; N * N];
        let original = reachable(N, free, region[0], &no_removal);
        assert!(region.iter().all(|&cell| original[cell]));
        let original_size = original.iter().filter(|&&inside| inside).count();
        let mut removed = vec![false; N * N];
        for &cell in region {
            removed[cell] = true;
        }
        ...
        for start in 0..N * N {
            if original[start] && !removed[start] {
                let component = reachable(N, free, start, &removed);
                largest = largest.max(
                    component
                        .iter()
                        .zip(&original)
                        .filter(|(seen, inside)| **seen && **inside)
                        .count(),
                );
            }
        }
        original_size - region.len() - largest
    }

    const TEST_N: usize = 6;
    let mut grass_rows = [0_u64; MAX_N];
    for row in grass_rows.iter_mut().take(TEST_N) {
        *row = (1_u64 << TEST_N) - 1;
    }
    grass_rows[1] &= !(1_u64 << 4);
    grass_rows[4] &= !(1_u64 << 1);
    let groups = vec![
        Group {
            T: 4,
            active: true,
            cells: vec![1, 2, TEST_N + 2],
            ..Group::default()
        },
        Group {
            T: 9,
            active: true,
            cells: vec![3 * TEST_N + 3, 3 * TEST_N + 4],
            ..Group::default()
        },
        Group {
            T: 2,
            active: false,
            cells: vec![5 * TEST_N + 5],
            ..Group::default()
        },
    ];
    let mut occupied_rows = [0_u64; MAX_N];
    for group in &groups {
        if group.active {
            for &cell in &group.cells {
                occupied_rows[cell / TEST_N] |= 1_u64 << (cell % TEST_N);
            }
        }
    }
    for snapshot in [0, 4, 6, 9, 12] {
        assert_eq!(
            future_free_cells(TEST_N, &grass_rows, &occupied_rows, &groups, snapshot,),
            naive_future_free(TEST_N, &grass_rows, &occupied_rows, &groups, snapshot,)
        );
    }

    for case_id in 0..12_usize {
        let mut free = vec![false; TEST_N * TEST_N];
        for r in 0..TEST_N {
            for c in 0..TEST_N {
                let value = (r * 19 + c * 23 + r * c * 5 + case_id * 11) % 13;
                free[r * TEST_N + c] = value >= 3;
            }
        }
        let Some(seed) = free.iter().position(|&is_free| is_free) else {
            continue;
        };
        let original = reachable(TEST_N, &free, seed, &vec![false; TEST_N * TEST_N]);
        let mut region = Vec::new();
        let mut queued = vec![false; TEST_N * TEST_N];
        let mut queue = vec![seed];
        queued[seed] = true;
        let mut head = 0;
        let target = (1 + case_id % 4).min(original.iter().filter(|&&x| x).count());
        while head < queue.len() && region.len() < target {
            let v = queue[head];
            head += 1;
            region.push(v);
            let r = v / TEST_N;
            let c = v % TEST_N;
            for next in [
                (r > 0).then_some(v.wrapping_sub(TEST_N)),
                (r + 1 < TEST_N).then_some(v + TEST_N),
                (c > 0).then_some(v.wrapping_sub(1)),
                (c + 1 < TEST_N).then_some(v + 1),
            ]
            .into_iter()
            .flatten()
            {
                if original[next] && !queued[next] {
                    queued[next] = true;
                    queue.push(next);
                }
            }
        ...
        assert_eq!(
            region_severed_loss(TEST_N, &free, &region),
            naive_severed(TEST_N, &free, &region)
        );
    }
}
...
impl Solver {
    ...
    /// incoming の滞在中4時点で、そこまでに退去する区画を除いた盤面と、
    /// 各時点の1セル cut-loss を 2:3:3:2 で合成した整数 field を作る。
    fn temporal_cutloss_field(
        &mut self,
        incoming_S: usize,
        incoming_T: usize,
    ) -> (Vec<Vec<bool>>, Vec<usize>) {
        let D = incoming_T - incoming_S;
        let mut snapshots = Vec::with_capacity(TEMPORAL_SNAPSHOT_NUMERATORS.len());
        let mut field = vec![0_usize; self.N * self.N];
        for (snapshot_index, (&numerator, &weight)) in TEMPORAL_SNAPSHOT_NUMERATORS
            .iter()
            .zip(TEMPORAL_SNAPSHOT_WEIGHTS.iter())
            .enumerate()
        {
            let snapshot = incoming_S + D * numerator / 10;
            let free = future_free_cells(
                self.N,
                &self.grass_rows,
                &self.occupied_rows,
                &self.groups,
            ...
            if snapshot_index == 0 {
                local! {
                    let current_free: Vec<bool> = (0..self.N * self.N)
                        .map(|cell| {
                            let r = cell / self.N;
                            let c = cell % self.N;
                            self.is_free(&self.occupied_rows, r, c)
                        })
                        .collect();
                    assert_eq!(free, current_free, "snapshot0 must equal the current free board");
                    self.trace.count("temporal_snapshot0_consistent");
                }
            }
            let snapshot_cut_loss = free_cut_loss(self.N, &free);
            for cell in 0..self.N * self.N {
                field[cell] += weight * snapshot_cut_loss[cell];
            }
            snapshots.push(free);
            local! {
                self.trace.count("temporal_cutloss_snapshot");
            }
        }
        local! {
            self.trace.count_by(
                "temporal_cutloss_field_positive",
                field.iter().filter(|&&loss| loss > 0).count() as i64,
            );
            self.trace.count_by(
                "temporal_cutloss_field_sum",
                field.iter().sum::<usize>() as i64,
            );
        }
        (snapshots, field)
    }

    /// baseline と同じ seed 集合のうち、baseline が属する現在自由成分の seed だけから
    /// temporal field を d に次ぐ tie-break として使う。SA/LNS はこの追加候補へ適用しない。
    fn temporal_growth_candidates(
        &mut self,
        P: usize,
        seeds: &[usize],
        baseline_component: isize,
        baseline_perimeter: usize,
        info: &FreeInfo,
        weights: &WeightData,
        temporal_field: &[usize],
    ) -> Vec<Placement> {
        const DIRS: [(isize, isize); 4] = [(-1, 0), (1, 0), (0, -1), (0, 1)];
        let mut candidates = Vec::new();
        for &seed in seeds {
            if info.component[seed] != baseline_component {
                continue;
            }
            assert!(baseline_component >= 0);
            assert!(info.sizes[baseline_component as usize] >= P);
            ...
            if perimeter > baseline_perimeter {
                continue;
            }
            ...
            candidates.push(Placement {
                perimeter,
                cheap_score,
                component_size: info.sizes[baseline_component as usize],
                explicit_cells: region,
                ..Placement::default()
            });
            local! {
                self.trace.count("temporal_cutloss_candidate");
            }
        }
        candidates
    }
    ...
    fn growth_placement(
        ...
        let Some((_baseline_index, baseline)) = best else {
            return None;
        };
        local! {
            self.trace.count("growth_placement_success");
            let _slack = baseline.perimeter - minimum_perimeter(P);
            self.trace.count_by(
                "growth_slack_sum",
                _slack as i64,
            );
            ...
            if _box_index.is_some_and(|box_index| box_index == _baseline_index) {
                self.trace.count("box_candidate_selected");
            }
        }

        // ここまでの演算順と候補群は v035 と同一であり、temporal 機構は baseline を
        // 完全に確定してからだけ動かす。gate 不成立なら追加計算なしでそのまま返す。
        let recoverable_fee =
            (V as f64) * (compactness(P, min_L) - compactness(P, baseline.perimeter));
        let temporal_eligible = P >= 64
            && recoverable_fee >= 100_000.0
            && !self.timer.reached(TEMPORAL_CUTLOSS_LIMIT_RATIO);
        if !temporal_eligible {
            return Some(baseline);
        }

        let _temporal_started = Instant::now();
        local! {
            self.trace.count("temporal_cutloss_eligible");
            self.trace.count_by(
                "temporal_cutloss_recoverable_fee",
                recoverable_fee.round() as i64,
            );
            if P >= 96 {
                self.trace.count("temporal_cutloss_eligible_p96");
            } else {
                self.trace.count("temporal_cutloss_eligible_p64_95");
            }
        }
        let baseline_component = info.component[baseline.explicit_cells[0]];
        assert!(baseline_component >= 0);
        assert!(
            baseline
                .explicit_cells
                .iter()
                .all(|&cell| info.component[cell] == baseline_component)
        );
        assert_eq!(
            baseline.component_size,
            info.sizes[baseline_component as usize]
        );
        let baseline_perimeter = baseline.perimeter;
        let baseline_component_size = baseline.component_size;
        let baseline_final_score = baseline.final_score;
        let baseline_cells = baseline.explicit_cells.clone();

        let (snapshots, temporal_field) = self.temporal_cutloss_field(incoming_S, incoming_T);
        let baseline_severed =
            persistent_region_severed(self.N, &snapshots, &baseline.explicit_cells);
        let temporal_candidates = self.temporal_growth_candidates(
            P,
            &seeds,
            baseline_component,
            baseline.perimeter,
        ...
        let baseline_fee = (V as f64) * compactness(P, baseline.perimeter);
        let baseline_admission_quality = compactness(P, baseline.perimeter);
        let mut temporal_best: Option<(Placement, usize)> = None;
        for mut candidate in temporal_candidates {
            ...
            assert!(candidate.perimeter <= baseline.perimeter);

            let candidate_fee = (V as f64) * compactness(P, candidate.perimeter);
            let candidate_admission_quality = compactness(P, candidate.perimeter);
            if candidate.perimeter == baseline.perimeter {
                assert_eq!(candidate_fee.to_bits(), baseline_fee.to_bits());
                assert_eq!(
                    candidate_admission_quality.to_bits(),
                    baseline_admission_quality.to_bits()
                );
            } else {
                assert!(candidate_fee >= baseline_fee);
                assert!(candidate_admission_quality >= baseline_admission_quality);
            }
            assert_eq!(
                self.component_threshold_factor(candidate.component_size)
                    .to_bits(),
                self.component_threshold_factor(baseline.component_size)
                    .to_bits()
            );
            ...
            candidate.final_score = candidate.cheap_score - 1.4 * delta;
            let severed = persistent_region_severed(self.N, &snapshots, &candidate.explicit_cells);
            if temporal_best
                .as_ref()
                .is_none_or(|(current, current_severed)| {
                    candidate.perimeter < current.perimeter
                        || (candidate.perimeter == current.perimeter
                            && if candidate.perimeter == baseline.perimeter {
                                severed < *current_severed
                                    || (severed == *current_severed
            ...
            {
                temporal_best = Some((candidate, severed));
            }
        }

        let mut selected = baseline.clone();
        let mut _selected_severed = baseline_severed;
        if let Some((candidate, candidate_severed)) = temporal_best {
            let should_flip = candidate.perimeter < selected.perimeter
                || (candidate.perimeter == selected.perimeter
                    && (candidate_severed < _selected_severed
                        || (candidate_severed == _selected_severed
                            && candidate.final_score > selected.final_score)));
            if should_flip {
                selected = candidate;
                _selected_severed = candidate_severed;
                local! {
                    self.trace.count("temporal_cutloss_flip");
                }
            }
        }

        assert_eq!(selected.component_size, baseline_component_size);
        assert!(selected.perimeter <= baseline_perimeter);
        assert_eq!(baseline.perimeter, baseline_perimeter);
        assert_eq!(baseline.component_size, baseline_component_size);
        assert_eq!(
            baseline.final_score.to_bits(),
            baseline_final_score.to_bits()
        );
        assert_eq!(baseline.explicit_cells, baseline_cells);
        local! {
            self.trace.count("temporal_baseline_consistent");
            self.trace
                .count_by("temporal_L_before", baseline_perimeter as i64);
            self.trace
                .count_by("temporal_L_after", selected.perimeter as i64);
            self.trace.count_by(
                "temporal_persistent_severed_before",
                baseline_severed as i64,
            );
            self.trace.count_by(
                "temporal_persistent_severed_after",
                _selected_severed as i64,
            );
            self.trace.count_by(
                "temporal_persistent_severed_reduction",
                baseline_severed as i64 - _selected_severed as i64,
            );
            self.trace.add_time_ms(
                "temporal_cutloss",
                _temporal_started.elapsed().as_secs_f64() * 1000.0,
            );
        }
        Some(selected)
    }
