// v063_no_move_size_gradient.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。

// 中心アイデア: compactさを決める周長選択は変えず、同一周長候補の位置だけを
// Pに応じて左上から右下へ緩く誘導する。再移動は一切行わない。
...
/// 位置項は既存の境界接触数個分に留め、hard zoneにしない。
const SIZE_POSITION_BIAS_PER_SQRT_P: f64 = 12.0;
...
struct Solver {
    ...
    /// 池を除く草地を左上から右下へ並べた面積順位。
    grass_area_rank: Vec<f64>,
    grass_rank_prefix: [[f64; MAX_N + 1]; MAX_N],
    /// Pごとの、期待占有面積で較正した対角方向の目標順位。
...
impl Solver {
    fn new(N: usize, M: usize, grass_rows: Rows, timer: TimeKeeper) -> Self {
        ...
        let grass_area_rank = Self::build_grass_area_rank(N, &grass_rows);
        let grass_rank_prefix = Self::build_grass_rank_prefix(N, &grass_area_rank);
        let mut solver = Self {
            N,
            M,
            grass_rows,
        ...
        local! {
            solver.trace.count_by(
                "size_position_target_p4_milli",
                (solver.p_area_target[4] * 1_000.0).round() as i64,
            );
            solver.trace.count_by(
                "size_position_target_p64_milli",
                (solver.p_area_target[64] * 1_000.0).round() as i64,
            );
            solver.trace.count_by(
                "size_position_target_p150_milli",
                (solver.p_area_target[150] * 1_000.0).round() as i64,
            );
            let kept = solver
                .shapes_by_p
                .iter()
    ...
    fn build_grass_area_rank(N: usize, grass_rows: &Rows) -> Vec<f64> {
        ...
        let mut rank = vec![0.0; N * N];
        let denominator = cells.len().max(1) as f64;
        for (index, cell) in cells.into_iter().enumerate() {
            rank[cell] = (index as f64 + 0.5) / denominator;
        }
        rank
    }

    fn build_grass_rank_prefix(N: usize, grass_area_rank: &[f64]) -> [[f64; MAX_N + 1]; MAX_N] {
        let mut prefix = [[0.0; MAX_N + 1]; MAX_N];
        for r in 0..N {
            for c in 0..N {
                prefix[r][c + 1] = prefix[r][c] + grass_area_rank[r * N + c];
            }
        }
        prefix
    }
    ...
    fn initialize_p_distribution(&mut self) {
        ...
        // 各Pのtargetを、そのPが占める期待面積massの区間中央に置く。
        // DはPと独立なので、到着面積Pで重み付けすれば期待active面積にも対応する。
        let mut cumulative_area = 0.0;
        for P in 4..=MAX_P {
            let area_mass = (P as f64) * self.p_probability[P];
            self.p_area_target[P] =
                (cumulative_area + 0.5 * area_mass) / self.expected_p.max(1e-12);
            cumulative_area += area_mass;
        }
    }

    fn position_metrics(&self, P: usize, cells: &[usize]) -> (f64, f64, f64) {
        debug_assert_eq!(cells.len(), P);
        let rank = cells
            .iter()
            .map(|&cell| self.grass_area_rank[cell])
            .sum::<f64>()
            / (P as f64);
        let error = (rank - self.p_area_target[P]).abs();
        let penalty = SIZE_POSITION_BIAS_PER_SQRT_P * (P as f64).sqrt() * error * error;
        (rank, error, penalty)
    }

    fn regular_position_metrics(
        &self,
        P: usize,
        shape: &Shape,
        x: usize,
        ...
        for rr in 0..shape.h {
            ...
            rank_sum += self.grass_rank_prefix[r][end] - self.grass_rank_prefix[r][begin];
        }
        let rank = rank_sum / (P as f64);
        let error = (rank - self.p_area_target[P]).abs();
        let penalty = SIZE_POSITION_BIAS_PER_SQRT_P * (P as f64).sqrt() * error * error;
        (rank, error, penalty)
    }
        ...
        for (shape_index, shape) in self.shapes_by_p[P].iter().enumerate() {
            ...
            for x in 0..=(self.N - shape.h) {
                ...
                while ys != 0 {
                    ...
                    let (position_rank, position_error, position_penalty) =
                        self.regular_position_metrics(P, shape, x, y);
                    cheap_score -= position_penalty;
    ...
    fn growth_placement(
        ...
        for (index, mut candidate) in candidates.into_iter().enumerate() {
            ...
            let (position_rank, position_error, position_penalty) =
                self.position_metrics(P, &candidate.explicit_cells);
            candidate.position_rank = position_rank;
            candidate.position_error = position_error;
            candidate.position_penalty = position_penalty;
            candidate.final_score = candidate.cheap_score - 1.4 * delta - position_penalty;
            evaluated.push((index, candidate));
        }
        let mut _unbiased_index = 0;
        for i in 1..evaluated.len() {
            let score = evaluated[i].1.final_score + evaluated[i].1.position_penalty;
            let best_score = evaluated[_unbiased_index].1.final_score
                + evaluated[_unbiased_index].1.position_penalty;
            if score > best_score {
                _unbiased_index = i;
            }
        }
        let baseline_perimeter = evaluated[_unbiased_index].1.perimeter;
        let mut best_index = _unbiased_index;
        for i in 0..evaluated.len() {
            if evaluated[i].1.perimeter == baseline_perimeter
                && evaluated[i].1.final_score > evaluated[best_index].1.final_score
            {
                best_index = i;
            }
        }
        local! {
            self.trace.count("size_position_growth_compare");
            self.trace.count_by(
                "size_position_growth_error_before_micros",
                (evaluated[_unbiased_index].1.position_error * 1_000_000.0).round() as i64,
            );
            self.trace.count_by(
                "size_position_growth_error_after_micros",
                (evaluated[best_index].1.position_error * 1_000_000.0).round() as i64,
            );
            assert_eq!(
                evaluated[_unbiased_index].1.perimeter,
                evaluated[best_index].1.perimeter,
            );
            if evaluated[_unbiased_index].0 != evaluated[best_index].0 {
                self.trace.count("size_position_growth_flip");
            }
        }
        let best = Some(evaluated.swap_remove(best_index));
    ...
    fn find_normal_placements(
        ...
        for perimeter in (min_L..=min_L + max_extra).step_by(2) {
            ...
            let mut _unbiased_index = 0;
            for i in 1..evaluated.len() {
                let score = evaluated[i].0.final_score + evaluated[i].0.position_penalty;
                let best_score = evaluated[_unbiased_index].0.final_score
                    + evaluated[_unbiased_index].0.position_penalty;
                if score > best_score {
                    _unbiased_index = i;
                }
            }
            local! {
                self.trace.count("size_position_regular_compare");
                self.trace.count_by(
                    "size_position_regular_error_before_micros",
                    (evaluated[_unbiased_index].0.position_error * 1_000_000.0).round() as i64,
                );
                self.trace.count_by(
                    "size_position_regular_error_after_micros",
                    (evaluated[current_index].0.position_error * 1_000_000.0).round() as i64,
                );
                assert_eq!(
                    evaluated[_unbiased_index].0.perimeter,
                    evaluated[current_index].0.perimeter,
                );
                if evaluated[_unbiased_index].0.explicit_cells
                    != evaluated[current_index].0.explicit_cells
                    || evaluated[_unbiased_index].0.x != evaluated[current_index].0.x
                    || evaluated[_unbiased_index].0.y != evaluated[current_index].0.y
                {
                    self.trace.count("size_position_regular_flip");
                }
            }
            if fast_mode {
    ...
    fn commit_normal_placement(&mut self, group_id: usize, placement: &Placement) {
        ...
        let (_position_rank, _position_error, _position_penalty) = self.position_metrics(P, &cells);
        local! {
            self.trace.count("size_position_selected");
            self.trace.count_by(
                "size_position_selected_error_micros",
                (_position_error * 1_000_000.0).round() as i64,
            );
            self.trace.count_by(
                "size_position_selected_penalty_milli",
                (_position_penalty * 1_000.0).round() as i64,
            );
            let (count_key, rank_key) = if P <= 31 {
                (
                    "size_position_small_count",
                    "size_position_small_rank_milli",
                )
            } else if P <= 95 {
                (
                    "size_position_mid_count",
                    "size_position_mid_rank_milli",
                )
            } else {
                (
                    "size_position_large_count",
                    "size_position_large_rank_milli",
                )
            };
            self.trace.count(count_key);
            self.trace
                .count_by(rank_key, (_position_rank * 1_000.0).round() as i64);
        }
        {
    ...
    fn run<R: BufRead, W: Write>(
        ...
        local! {
            ...
            for key in [
                "size_position_regular_compare",
                "size_position_regular_flip",
                "size_position_growth_compare",
                "size_position_growth_flip",
                "size_position_selected",
                "size_position_small_count",
                "size_position_small_rank_milli",
                "size_position_mid_count",
                "size_position_mid_rank_milli",
...
#[cfg(feature = "local")]
pub fn verify_size_position_bias() {
    ...
    // 草地rankは左上から右下へ厳密に増え、池を順位母数へ含めない。
    let mut grass = full_grass(7);
    grass[3] &= !Solver::bit_at(3);
    let rank = Solver::build_grass_area_rank(7, &grass);
    let mut ordered = Vec::new();
    for r in 0..7 {
        for c in 0..7 {
            if ((grass[r] >> c) & 1) != 0 {
                ordered.push((r + c, r, c, rank[r * 7 + c]));
            }
        }
    }
    ordered.sort_by_key(|&(diagonal, r, c, _)| (diagonal, r, c));
    assert!(ordered.windows(2).all(|pair| pair[0].3 < pair[1].3));
    assert_eq!(rank[3 * 7 + 3], 0.0);

    let solver = Solver::new(13, 1, full_grass(13), TimeKeeper::new(60.0));
    for r in 0..13 {
        for c in 0..13 {
            assert!(
                (solver.grass_rank_prefix[r][c + 1]
                    - solver.grass_rank_prefix[r][c]
                    - solver.grass_area_rank[r * 13 + c])
                    .abs()
                    < 1e-12
            );
        }
    ...
    // targetは期待面積massの中点と一致し、Pとともに単調増加する。
    let mut cumulative_area = 0.0;
    for P in 4..=MAX_P {
        let mass = (P as f64) * solver.p_probability[P];
        let expected = (cumulative_area + 0.5 * mass) / solver.expected_p;
        assert!((solver.p_area_target[P] - expected).abs() < 1e-12);
        if P > 4 {
            assert!(solver.p_area_target[P] > solver.p_area_target[P - 1]);
        }
        cumulative_area += mass;
    }
    assert!(solver.p_area_target[4] < solver.p_area_target[64]);
    assert!(solver.p_area_target[64] < solver.p_area_target[150]);

    // 小型は左上、大型は右下の同面積候補を好む。
    let small_left = vec![0, 1, 13, 14];
    let small_right = vec![11 * 13 + 11, 11 * 13 + 12, 12 * 13 + 11, 12 * 13 + 12];
    let (_, small_left_error, small_left_penalty) = solver.position_metrics(4, &small_left);
    let (_, small_right_error, small_right_penalty) = solver.position_metrics(4, &small_right);
    assert!(small_left_error < small_right_error);
    assert!(small_left_penalty < small_right_penalty);

    let mut by_rank: Vec<usize> = (0..13 * 13).collect();
    by_rank.sort_by(|&a, &b| solver.grass_area_rank[a].total_cmp(&solver.grass_area_rank[b]));
    let large_left = by_rank[..150].to_vec();
    let large_right = by_rank[19..].to_vec();
    let (_, large_left_error, large_left_penalty) = solver.position_metrics(150, &large_left);
    let (_, large_right_error, large_right_penalty) = solver.position_metrics(150, &large_right);
    assert!(large_right_error < large_left_error);
    assert!(large_right_penalty < large_left_penalty);
}
