...
// 中心アイデア: v053 の theta 事後グリッドを、観測数 n と
// Y=sum(D[i]-1) だけで更新できる連続事後分布の MAP=clip(Y/n, 2000, 8000) へ置換する。
// rollout本数・到着数は固定し、推定方式と計算量だけを比較する。
...
impl Solver {
    ...
    fn evaluate_candidates_rollout(
        ...
        for k in 0..ROLLOUT_SAMPLES {
            ...
            // 最軽量MAPを全rolloutで共有する。各本のP・滞在・qの乱数列と、
            // 候補間の共通乱数というv053の比較条件はそのまま保つ。
            let theta_k = theta;
            local! {
