...
impl Solver {
    ...
    fn evaluate_candidates_rollout(
        ...
        for k in 0..ROLLOUT_SAMPLES {
            ...
            // サンプル世界の自己整合: θ_k の世界では admission 閾値も θ_k で決まる。
            // incoming の (S, duration, P) は共有閾値と同じものを流用し、θ だけ差し替える。
            let threshold_k = {
                let g = &self.groups[incoming_id];
                let (gs, gd, gp) = (g.S, g.T - g.S, g.P);
                self.base_dynamic_threshold(gs, gd, gp, theta_k)
            };
            local! {
                self.trace.count_by(
                    "threshold_k_spread_permille",
                    (((threshold_k - threshold).abs() / threshold.max(1e-9)) * 1000.0)
                        .min(2000.0)
                        .round() as i64,
                );
            }
            ...
            for (i, cand) in cands.iter().enumerate() {
                totals[i] += self.rollout_one(cand, &dep_base, &arrivals, threshold_k);
            }
        }
        let mut best = 0;
