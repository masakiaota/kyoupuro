// v068_balanced_repack.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: relocation の blocker を逐次配置せず、全 blocker の旧領域と空きセルを
// 一つの domain として、多始点 priority growth で同時に再分割する。問題規則が許す
// 同時移動を直接使い、先行 blocker による逃げ道閉塞と無制限 growth fallback をなくす。
...
type RepackFrontierEntry = (usize, usize, Reverse<usize>, u64, Reverse<usize>);
type RepackResult = (Vec<(usize, Placement)>, i64, f64);
...
/// 公式 scorer と同じ整数境界判定で `round(V * 4 * sqrt(P) / L)` を求める。
/// `P` が平方数で積が半整数になる場合も、f64 の丸め誤差で 1 ずれない。
fn rounded_fee(V: i64, P: usize, perimeter: usize) -> i64 {
    if perimeter == 0 {
        return 0;
    }
    // n が答え ⇔ (2n-1)L <= 8V√P < (2n+1)L。非負なので平方して比較できる。
    let square = 64_i128 * (V as i128) * (V as i128) * (P as i128);
    let L = perimeter as i128;
    let lower_ok = |n: i64| {
        let value = (2 * (n as i128) - 1) * L;
        value <= 0 || value * value <= square
    };
    let upper_ok = |n: i64| {
        let value = (2 * (n as i128) + 1) * L;
        square < value * value
    };
    let mut result = ((V as f64) * compactness(P, perimeter)).round() as i64;
    result = result.max(0);
    while !lower_ok(result) {
        result -= 1;
    }
    while !upper_ok(result) {
        result += 1;
    }
    result
}
...
impl Solver {
    ...
    #[inline]
    fn repack_degree(&self, assigned: &[isize], owner: usize, cell: usize) -> usize {
        const DIRS: [(isize, isize); 4] = [(-1, 0), (1, 0), (0, -1), (0, 1)];
        let r = cell / self.N;
        let c = cell % self.N;
        let mut degree = 0;
        for (dr, dc) in DIRS {
            ...
            if nr >= 0
                ...
                && nc >= 0
                && nc < self.N as isize
                && assigned[(nr as usize) * self.N + (nc as usize)] == owner as isize
            {
                degree += 1;
            }
        }
    ...
    fn push_repack_frontier(
        &self,
        frontier: &mut BinaryHeap<RepackFrontierEntry>,
        domain: &Rows,
        assigned: &[isize],
        owner: RepackOwner,
        cell: usize,
        ...
        let r = cell / self.N;
        let c = cell % self.N;
        if ((domain[r] >> c) & 1) == 0 || assigned[cell] != -1 {
            return;
        }
        let sr = owner.seed / self.N;
        let sc = owner.seed % self.N;
        let distance = r.abs_diff(sr) + c.abs_diff(sc);
        let old_cell = usize::from(self.owner_cell[cell] == owner.group_id as isize);
        let degree = self.repack_degree(assigned, owner.region_index, cell);
        // frontier は必ず自領域に隣接するセルだけを持つ。この条件が、seed からの
        // growth による連結性保証そのものである。
        if degree == 0 {
            return;
        }
        frontier.push((
    ...
    #[cfg(feature = "local")]
    fn repack_region_is_connected(&self, cells: &[usize]) -> bool {
        if cells.is_empty() {
            return false;
        }
        const DIRS: [(isize, isize); 4] = [(-1, 0), (1, 0), (0, -1), (0, 1)];
    ...
    /// 全 blocker の旧領域と空きセルを一つの domain とし、各 blocker の seed から
    /// 同時に成長させて正確に P セルずつ再分割する。戻り値は
    /// (配置列, 四捨五入後の正確な fee_loss 合計, repack 後盤面の断片化メトリック)。
    fn repack_blockers(
        &mut self,
        blocker_ids: &[usize],
        target_cells: &[usize],
        ...
        local! {
            ...
            self.trace.count("balanced_repack_attempt");
        }

        let K = blocker_ids.len();
        if K == 0 {
            return Some((Vec::new(), 0, self.fragment_metric(&self.occupied_rows)));
        }

        let mut movable = [0_u64; MAX_N];
        for &group_id in blocker_ids {
            for &cell in &self.groups[group_id].cells {
                movable[cell / self.N] |= Self::bit_at(cell % self.N);
            }
        }
        let mut target = [0_u64; MAX_N];
        for &cell in target_cells {
            target[cell / self.N] |= Self::bit_at(cell % self.N);
        }

        // fixed group と incoming target を除き、blocker の旧領域は再利用可能に戻す。
        let mut domain = self.grass_rows;
        for r in 0..self.N {
            let fixed = self.occupied_rows[r] & !movable[r];
            domain[r] &= !fixed;
            domain[r] &= !target[r];
        }
        let domain_count = domain[..self.N]
            .iter()
            .map(|row| row.count_ones() as usize)
            .sum::<usize>();
        let need = blocker_ids
            .iter()
            .map(|&group_id| self.groups[group_id].P)
            .sum::<usize>();
        if domain_count < need {
            local! {
                self.trace.count("balanced_domain_shortage");
            }
            return None;
        }

        // target と blocker 集合だけから決まる seed にし、同じ入力では常に同じ分割を返す。
        let mut rng_seed = 0x9E37_79B9_7F4A_7C15_u64;
        for &cell in target_cells {
            rng_seed ^= (cell as u64)
                .wrapping_add(0x9E37_79B9_7F4A_7C15)
                .wrapping_add(rng_seed << 6)
                .wrapping_add(rng_seed >> 2);
        }
        for &group_id in blocker_ids {
            rng_seed ^= (group_id as u64)
                .wrapping_mul(0xBF58_476D_1CE4_E5B9)
                .wrapping_add(self.groups[group_id].P as u64);
        }
        let mut rng = XorShift64::new(rng_seed);

        // 大きい blocker から seed を確保する。旧領域の任意セルを、近傍の新規セルより
        // 十分強く優先し、上位5セルから決定的乱数で一つ選んで形の固定化を避ける。
        let mut seed_order = (0..K).collect::<Vec<_>>();
        seed_order.sort_by(|&a, &b| {
            self.groups[blocker_ids[b]]
                .P
                .cmp(&self.groups[blocker_ids[a]].P)
                .then_with(|| blocker_ids[a].cmp(&blocker_ids[b]))
        });
        let mut seeds = vec![usize::MAX; K];
        let mut seed_used = vec![false; self.N * self.N];
        for &k in &seed_order {
            if self.timer.reached(REPACK_LIMIT_RATIO) {
                local! {
                    self.trace.count("balanced_time_abort");
                }
                return None;
            }
            let group_id = blocker_ids[k];
            let group = &self.groups[group_id];
            let center_r = group.cells.iter().map(|&cell| cell / self.N).sum::<usize>() as f64
                / group.P as f64;
            let center_c = group.cells.iter().map(|&cell| cell % self.N).sum::<usize>() as f64
                / group.P as f64;
            let mut nearest = Vec::<(f64, usize)>::with_capacity(6);
            for (r, &row) in domain.iter().enumerate().take(self.N) {
                let mut cells = row;
                while cells != 0 {
                    let c = cells.trailing_zeros() as usize;
                    cells &= cells - 1;
                    let cell = r * self.N + c;
                    if seed_used[cell] {
                        continue;
                    }
                    let old_bias = if self.owner_cell[cell] == group_id as isize {
                        -1_000.0
                    } else {
                        0.0
                    };
                    let key = old_bias
                        + ((r as f64) - center_r).abs()
                        + ((c as f64) - center_c).abs()
                        + ((rng.next_u64() % 1_000) as f64) * 1e-6;
                    let position = nearest.partition_point(|&(current_key, current_cell)| {
                        current_key
                            .total_cmp(&key)
                            .then_with(|| current_cell.cmp(&cell))
                            .is_le()
                    });
                    if position < 5 {
                        nearest.insert(position, (key, cell));
                        nearest.truncate(5);
                    }
                }
            }
            if nearest.is_empty() {
                local! {
                    self.trace.count("balanced_seed_failure");
                }
                return None;
            }
            let chosen = (rng.next_u64() as usize) % nearest.len();
            let seed = nearest[chosen].1;
            seeds[k] = seed;
            seed_used[seed] = true;
        }
        local! {
            self.trace.count("balanced_seed_success");
        }

        let mut assigned = vec![-1_isize; self.N * self.N];
        let mut regions = vec![Vec::<usize>::new(); K];
        let mut remaining = vec![0_usize; K];
        for k in 0..K {
            assigned[seeds[k]] = k as isize;
            regions[k].push(seeds[k]);
            remaining[k] = self.groups[blocker_ids[k]].P - 1;
        }

        let mut frontiers = (0..K)
            .map(|_| BinaryHeap::<RepackFrontierEntry>::new())
            .collect::<Vec<_>>();
        const DIRS: [(isize, isize); 4] = [(-1, 0), (1, 0), (0, -1), (0, 1)];
        for k in 0..K {
            let r = seeds[k] / self.N;
            let c = seeds[k] % self.N;
            for (dr, dc) in DIRS {
                let nr = (r as isize) + dr;
                let nc = (c as isize) + dc;
                if nr < 0 || nr >= self.N as isize || nc < 0 || nc >= self.N as isize {
                ...
                let cell = (nr as usize) * self.N + (nc as usize);
                self.push_repack_frontier(
                    &mut frontiers[k],
                    &domain,
                    &assigned,
                    RepackOwner {
                        region_index: k,
                        group_id: blocker_ids[k],
                        seed: seeds[k],
                    },
                    cell,
                    &mut rng,
        ...
        let mut left = need - K;
        while left > 0 {
            if left % 32 == 0 && self.timer.reached(REPACK_LIMIT_RATIO) {
                local! {
                    self.trace.count("balanced_time_abort");
                }
                return None;
            ...
            for k in 0..K {
                ...
                while frontiers[k]
                    .peek()
                    .is_some_and(|entry| assigned[entry.4.0] != -1)
                {
                    frontiers[k].pop();
                }
                if frontiers[k].is_empty() {
                    local! {
                        self.trace.count("balanced_growth_failure");
                    }
                    return None;
                }
                let P = self.groups[blocker_ids[k]].P;
                let key = (remaining[k] as i64) * 1_000_000 / (P as i64)
                    - 10 * (frontiers[k].len() as i64)
            ...
            let k = chosen_group.expect("unfinished repack has a frontier owner");
            let entry = frontiers[k].pop().expect("chosen frontier is non-empty");
            let cell = entry.4.0;
            if assigned[cell] != -1 {
                continue;
            }
            let current_degree = self.repack_degree(&assigned, k, cell);
            if current_degree == 0 {
                local! {
                    self.trace.count("balanced_zero_degree_rejected");
                }
                return None;
            }
            if current_degree != entry.0 {
                self.push_repack_frontier(
                    &mut frontiers[k],
                    &domain,
                    &assigned,
            ...
            for (dr, dc) in DIRS {
                ...
                let next = (nr as usize) * self.N + (nc as usize);
                // この割当で新しく frontier になり得るのは、セルを獲得した k だけである。
                // 他 blocker へ追加すると自領域に隣接しないセルを後で選べてしまう。
                if remaining[k] > 0 {
                    self.push_repack_frontier(
                        &mut frontiers[k],
                        &domain,
        ...
        let mut final_occ = self.occupied_rows;
        for &group_id in blocker_ids {
            for &cell in &self.groups[group_id].cells {
                final_occ[cell / self.N] &= !Self::bit_at(cell % self.N);
            }
        }
        for &cell in target_cells {
            final_occ[cell / self.N] |= Self::bit_at(cell % self.N);
        ...
        let mut _old_cells_reused = 0_usize;
        let mut _perimeter_sum = 0_usize;
        for k in 0..K {
            let group_id = blocker_ids[k];
            let group = &self.groups[group_id];
            let mut perimeter = 0;
            for &cell in &regions[k] {
                ...
                for (dr, dc) in DIRS {
                    ...
                    if nr < 0
                        ...
                        || nc < 0
                        || nc >= self.N as isize
                        || assigned[(nr as usize) * self.N + (nc as usize)] != k as isize
                    {
                        perimeter += 1;
                    }
                }
            }
            _perimeter_sum += perimeter;
            let new_worst = group.worst_perimeter.max(perimeter);
            fee_loss += (rounded_fee(group.V, group.P, group.worst_perimeter)
                - rounded_fee(group.V, group.P, new_worst))
            .max(0);
            placements.push((
                group_id,
                Placement {
                    ...
                    cheap_score: 0.0,
                    final_score: 0.0,
                    explicit_cells: std::mem::take(&mut regions[k]),
                    ..Placement::default()
                },
            ));
        }
        ...
        local! {
            let mut seen_cells = vec![false; self.N * self.N];
            for (group_id, placement) in &placements {
                let group = &self.groups[*group_id];
                assert_eq!(placement.explicit_cells.len(), group.P);
                assert!(self.repack_region_is_connected(&placement.explicit_cells));
                for &cell in &placement.explicit_cells {
                    let r = cell / self.N;
                    let c = cell % self.N;
                    assert_ne!((domain[r] >> c) & 1, 0);
            ...
            self.trace.count("balanced_growth_success");
            self.trace
                .count_by("balanced_assigned_cells", need as i64);
            self.trace
                .count_by("balanced_old_cells_reused", _old_cells_reused as i64);
            self.trace
                .count_by("balanced_exact_fee_loss", fee_loss);
            self.trace
                .count_by("balanced_perimeter_sum", _perimeter_sum as i64);
            self.trace.count("balanced_validation_success");
        }

        let final_metric = self.fragment_metric(&final_occ);
        local! {
            self.trace.count("fragment_evaluated");
        ...
        Some((placements, fee_loss, final_metric))
    }
    ...
    fn attempt_relocation(
        ...
        let mut collected: Vec<(MovePlan, f64, f64, i64, i64)> = Vec::new();
        for (attempt, option) in options.into_iter().enumerate() {
            ...
            if surplus - (cost as f64) - (fee_loss as f64) <= baseline + 0.12 * (cost as f64) {
                continue;
            }
            let mut incoming_placement = option.placement;
            incoming_placement.explicit_cells = target_cells;
            let net = surplus - (cost as f64) - (fee_loss as f64);
            collected.push((
                MovePlan {
                    ok: true,
        ...
        for (plan, _, _, cost, fee_loss) in &collected {
            ...
            cands.push(RolloutCandidate {
                board,
                immediate: (incoming.V as f64) * C - (*cost as f64) - (*fee_loss as f64),
        ...
        local! {
            ...
            for (i, item) in collected.iter().enumerate().skip(1) {
                let key = item.1 - METRIC_TO_MONEY * item.2;
                if key > old_key {
                    old_key = key;
                    old_best = i;
                }
        ...
        local! {
            ...
            self.trace.count_by("reloc_fee_loss", *fee_loss);
        }
