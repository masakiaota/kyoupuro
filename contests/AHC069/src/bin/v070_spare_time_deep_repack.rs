// v070_spare_time_deep_repack.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: v053 の基準経路を仮想時計で守り、実時間が進捗目標より前にあるときだけ、
// 基準探索で失敗した上位 relocation target を広い beam で再試行する。
...
// 追加探索は local 1.40 秒相当の進捗目標まで使い、1.48 秒相当を実時間の安全上限とする。
// PROGRAM_TIME_LIMIT_SEC に対する比なので、提出時も同じ比率で 1.75/1.85 秒へ伸びる。
const ADAPTIVE_WALL_TARGET_RATIO: f64 = 140.0 / 152.0;
const HARD_WALL_LIMIT_RATIO: f64 = 148.0 / 152.0;
...
struct TimeKeeper {
    ...
    extra_started_at: Option<Instant>,
}
...
impl TimeKeeper {
    fn new(time_limit_sec: f64) -> Self {
        Self {
            ...
            extra_started_at: None,
        }
    }

    #[inline]
    fn reached(&self, ratio: f64) -> bool {
        self.hard_wall_reached() || self.elapsed_sec() >= self.time_limit_sec * ratio
    }

    #[inline]
    fn elapsed_sec(&self) -> f64 {
        let ongoing_extra = self
            .extra_started_at
            .map_or(0.0, |start| start.elapsed().as_secs_f64());
        (self.wall_elapsed_sec() - self.excluded_extra_sec - ongoing_extra).max(0.0)
    }

    #[inline]
    fn wall_elapsed_sec(&self) -> f64 {
    ...
    #[inline]
    fn hard_wall_reached(&self) -> bool {
        self.wall_elapsed_sec() >= self.budget_sec(HARD_WALL_LIMIT_RATIO)
    }

    fn begin_extra(&mut self) {
        assert!(self.extra_started_at.is_none());
        self.extra_started_at = Some(Instant::now());
    }

    fn end_extra(&mut self) -> f64 {
        let elapsed = self
            .extra_started_at
            .take()
            .expect("adaptive extra clock was not started")
            .elapsed()
            .as_secs_f64();
        self.excluded_extra_sec += elapsed;
        elapsed
    }

    #[cfg(feature = "local")]
    fn exclude_completed(&mut self, elapsed_sec: f64) {
        assert!(self.extra_started_at.is_none());
...
struct RepackParams {
    ...
    branches: usize,
    shape_limit: usize,
    growth_seed_limit: usize,
    deep: bool,
}

const BASE_REPACK_PARAMS: RepackParams = RepackParams {
    beam_width: 9,
    branches: 4,
    shape_limit: 8,
    growth_seed_limit: 20,
    deep: false,
};

const DEEP_REPACK_PARAMS: RepackParams = RepackParams {
    beam_width: 18,
    branches: 8,
    shape_limit: 16,
    growth_seed_limit: 40,
    deep: true,
};

type CollectedPlan = (MovePlan, f64, f64, i64, f64);
type RepackResult = (Vec<(usize, Placement)>, f64, f64);

#[derive(Clone, Copy)]
struct RelocationEconomics {
    q_value: f64,
    base_threshold: f64,
    baseline: f64,
    scale: f64,
...
impl Solver {
    ...
    fn repack_blockers(
        ...
        for group_id in order {
            ...
            for state in &beam {
                ...
                for perimeter in (minimum_perimeter(P)..=worst_perimeter + 6).step_by(2) {
                    candidates = self.scan_regular_level(
                        P,
                        &runs,
                        &weights,
                        perimeter,
        ...
        if params.deep {
            local! {
                self.trace.count("adaptive_deep_repack_success");
            }
        }
    ...
    /// 進捗に応じた実時間の包絡線より前にいる間だけ、追加探索を許可する。
    /// 初期化費用を見込んで目標の 12% を先に割り当て、残りをturnへ線形配分する。
    fn adaptive_spare_available(&self, incoming_id: usize) -> bool {
        if incoming_id * 10 < self.M || self.timer.hard_wall_reached() {
            return false;
        }
        let progress = ((incoming_id + 1) as f64) / (self.M as f64);
        let scheduled_fraction = 0.12 + 0.88 * progress;
        let target = self.timer.budget_sec(ADAPTIVE_WALL_TARGET_RATIO) * scheduled_fraction;
        self.timer.wall_elapsed_sec() < target
    }

    /// 追加探索の前後でV053用の仮想時計を止める。実時間はhard stop判定とtraceに残す。
    fn begin_adaptive_extra(&mut self) -> f64 {
        let virtual_before = self.timer.elapsed_sec();
        self.timer.begin_extra();
        virtual_before
    }

    fn end_adaptive_extra(&mut self, virtual_before: f64) {
        let _extra_sec = self.timer.end_extra();
        local! {
            let bookkeeping_start = Instant::now();
            self.trace.count_by(
                "adaptive_extra_time_us",
                (_extra_sec * 1_000_000.0).round() as i64,
            );
            self.timer
                .exclude_completed(bookkeeping_start.elapsed().as_secs_f64());
        }
        let _virtual_drift = (self.timer.elapsed_sec() - virtual_before).abs();
        local! {
            assert!(
                _virtual_drift <= 0.0005,
                "adaptive extra advanced virtual clock by {:.3}ms",
                _virtual_drift * 1000.0
            );
        }
    }

    /// target 1件を指定されたrepack幅で構築し、V053と同じ採算式を通ったplanだけを返す。
    fn build_relocation_plan(
        &mut self,
        option: &TargetOption,
        incoming: &Group,
        economics: RelocationEconomics,
        params: RepackParams,
    ) -> Option<CollectedPlan> {
        let target_cells = self.materialize(&option.placement, incoming.P);
        let (repacked, fee_loss, final_metric) =
            self.repack_blockers(&option.blockers, &target_cells, params)?;
        ...
        let surplus = {
            ...
            let threshold = economics.base_threshold
                * self.component_threshold_factor(option.placement.component_size);
            economics.scale * (economics.q_value * C - threshold)
        };
        // repack後のC低下と移動費を引き、基準案より移動費の12%分以上よいものに限る。
        if surplus - (cost as f64) - fee_loss <= economics.baseline + 0.12 * (cost as f64) {
            return None;
        }
        let mut incoming_placement = option.placement.clone();
    ...
    fn relocation_rollout_candidate(
        &self,
        incoming: &Group,
        collected: &CollectedPlan,
    ) -> RolloutCandidate {
        let (plan, _, _, cost, fee_loss) = collected;
        let mut board = self.occupied_rows;
    ...
    fn attempt_relocation(
        ...
        let economics = RelocationEconomics {
            q_value,
            base_threshold,
            baseline,
            scale,
        };
        // まずV053と同じ幅・同じ12 target・最大3 planをそのまま実行する。
        let mut collected: Vec<CollectedPlan> = Vec::new();
        let mut failed_mask = 0_u16;
        for (attempt, option) in options.iter().enumerate() {
            if attempt >= 12 || collected.len() >= 3 || self.timer.reached(REPACK_LIMIT_RATIO) {
                break;
            }
            if let Some(plan) =
                self.build_relocation_plan(option, incoming, economics, BASE_REPACK_PARAMS)
            {
                collected.push(plan);
            } else {
                failed_mask |= 1_u16 << attempt;
            }
        }

        // 基準planとbaselineの比較も追加処理より先に行い、V053の判断を確定する。
        // planがなければV053どおりrollout候補さえ作らず、深掘りでplanを得たときだけ作る。
        let (base_selected_index, base_candidate) = if collected.is_empty() {
            (None, None)
        } else {
            let mut cands: Vec<RolloutCandidate> = Vec::with_capacity(collected.len() + 1);
            for plan in &collected {
                cands.push(self.relocation_rollout_candidate(incoming, plan));
            }
            let baseline_index = cands.len();
            cands.push(self.baseline_rollout_candidate(incoming, &baseline_normal));
            ...
            local! {
                ...
                // V053の旧複合評価との比較traceも基準候補だけで維持する。
                const METRIC_TO_MONEY: f64 = 800.0;
                let mut old_best = 0;
                let mut old_key = collected[0].1 - METRIC_TO_MONEY * collected[0].2;
                for (i, plan) in collected.iter().enumerate().skip(1) {
                    let key = plan.1 - METRIC_TO_MONEY * plan.2;
                    if key > old_key {
                        old_key = key;
                        old_best = i;
                    }
            ...
            let selected_index = if winner < baseline_index {
                Some(winner)
            } else {
                None
            };
            (selected_index, Some(cands.swap_remove(winner)))
        };

        // 基準候補が3件未満なら、失敗した同じ上位targetだけを広いbeamで再試行する。
        // 各試行と追加rolloutは仮想時計から除外し、実時間の進捗包絡線で総量を制御する。
        let mut deep_collected: Vec<CollectedPlan> = Vec::new();
        if collected.len() < 3 {
            for (option_index, option) in options.iter().take(12).enumerate() {
                if failed_mask & (1_u16 << option_index) == 0 {
                    continue;
                }
                if deep_collected.len() >= 3
                    || !self.adaptive_spare_available(incoming.id)
                    || self.timer.hard_wall_reached()
                {
                    break;
                }
                let virtual_before = self.begin_adaptive_extra();
                local! {
                    self.trace.count("adaptive_deep_repack_attempt");
                }
                let plan =
                    self.build_relocation_plan(option, incoming, economics, DEEP_REPACK_PARAMS);
                if let Some(plan) = plan {
                    deep_collected.push(plan);
                    local! {
                        self.trace.count("adaptive_deep_plan_collected");
                    }
                }
                self.end_adaptive_extra(virtual_before);
                if self.timer.hard_wall_reached() {
                    break;
                }
            }
        }

        // 追加planは、先に確定したV053の勝者1件との二段目比較だけを行う。
        let mut deep_selected_index = None;
        if !deep_collected.is_empty() && !self.timer.hard_wall_reached() {
            let virtual_before = self.begin_adaptive_extra();
            let mut deep_cands = Vec::with_capacity(deep_collected.len() + 1);
            deep_cands.push(
                base_candidate
                    .clone()
                    .unwrap_or_else(|| self.baseline_rollout_candidate(incoming, &baseline_normal)),
            );
            for plan in &deep_collected {
                deep_cands.push(self.relocation_rollout_candidate(incoming, plan));
            }
            let deep_winner = local_time!(self.trace, "adaptive_deep_rollout", {
                self.evaluate_candidates_rollout(
                    &deep_cands,
                    incoming.id,
                    incoming.S,
            ...
            local! {
                self.trace.count("adaptive_deep_rollout_session");
            }
            if deep_winner > 0 {
                deep_selected_index = Some(deep_winner - 1);
                local! {
                    self.trace.count("adaptive_deep_relocation_selected");
                }
            }
            local! {
                if deep_winner == 0 {
                    self.trace.count("adaptive_deep_base_preserved");
                }
            }
            self.end_adaptive_extra(virtual_before);
        }

        let selected = if let Some(index) = deep_selected_index {
            deep_collected.swap_remove(index)
        } else if let Some(index) = base_selected_index {
            collected.swap_remove(index)
        } else {
            return MovePlan::default();
        };
        local! {
            ...
            self.trace
                .count_by(
                    "reloc_plan_collected",
                    (collected.len() + deep_collected.len() + 1) as i64,
                );
            let (plan, _, _, cost, fee_loss) = &selected;
            self.trace
    ...
    fn run<R: BufRead, W: Write>(
        ...
        local! {
            if self.timer.hard_wall_reached() {
                self.trace.count("adaptive_hard_stop");
            }
            self.trace
                .add_time_ms("adaptive_virtual_elapsed", self.timer.elapsed_sec() * 1000.0);
            self.trace
