// v074_expected_terminal_load.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: v053 の配置・rollout・relocationを固定し、admissionだけを
// 「オンラインの期待絶対需要から求めた道中価格 × 既知区間の終端需要減衰」へ置換する。
// 終端需要減衰は閾値へ一度だけ掛け、需要価格とoccupancy目標へ重ねて入れない。
