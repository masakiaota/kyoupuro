// v046_no_move_deep_biased.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。

// 中心アイデア: 因果帯reserve下の次数biased boundary swapを時間上限まで深め、
// 軽いcaseの未使用予算を追加のgrowth周長改善へ変換する。再移動は一切行わない。
...
const BIASED_SWAP_ITERATIONS: usize = 2048;
