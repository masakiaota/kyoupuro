// v045_no_move_geometry_veto.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。

// 中心アイデア: actual-future因果帯のnear-threshold vetoを、空きが再結合しやすい
// 静的park境界の単純なcaseだけへ限定する。再移動は一切行わない。
...
const CAUSAL_VETO_PARK_BOUNDARY_MAX: usize = 1_200;
...
impl Solver {
    fn new(N: usize, M: usize, grass_rows: Rows, timer: TimeKeeper) -> Self {
            ...
            for c in 0..N {
                ...
                for (dr, dc) in [(-1_isize, 0_isize), (1, 0), (0, -1), (0, 1)] {
                    ...
                    if nr < 0
                        ...
                        || nc < 0
                        || nc >= N as isize
                        || ((grass_rows[nr as usize] >> (nc as usize)) & 1) == 0
                    {
                        park_boundary += 1;
                    }
                }
        ...
        let causal_veto_enabled = park_boundary <= CAUSAL_VETO_PARK_BOUNDARY_MAX;
        ...
        local! {
            solver.trace.count_by("park_boundary", park_boundary as i64);
            solver.trace.count_by(
                "causal_veto_case_enabled",
                i64::from(solver.causal_veto_enabled),
            );
            let kept = solver
                .shapes_by_p
                .iter()
        ...
        for turn in 0..self.M {
            ...
            if passed_price_prefilter {
                ...
                if let Some(mut choices) = normal_choices {
                    ...
                    // 元のadmissionとplacement winnerを確定した後、oracleで事前固定した
                    // 因果帯だけを単方向にvetoする。rejectからacceptへ戻す経路は作らない。
                    if self.causal_veto_enabled && accepted && base_threshold > 0.0 {
                        let placement = normal.as_ref().expect("normal placement selected");
                        ...
                        if (1.0..=CAUSAL_VETO_MARGIN_MAX).contains(&margin) {
                            ...
                            if duration_ratio < CAUSAL_VETO_DURATION_RATIO_MAX {
                                ...
                                if slack <= CAUSAL_VETO_SLACK_MAX {
                                    ...
                                    // 判定値とassert値を共有し、全vetoが固定帯内であることを
                                    // releaseでも検証する。
                                    assert!(accepted);
