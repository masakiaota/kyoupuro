// v087_faithful_latest_portfolio.rs
#![allow(non_snake_case)]
// 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: v086 の C++ 忠実実行基盤へ、smooth の同一経済条件
// topology challenger と価値別 fractional-credit work scheduler を統合する。
...
/// v081 の二値 fast mode 時刻。新schedulerとのpromote/defer比較だけに使う。
const LEGACY_FAST_MODE_RATIO: f64 = 168.0 / 190.0;
/// 最初の70%はv081のfull pathを必ず保ち、残りで計算圧を連続的に上げる。
const WORK_PRESSURE_START: f64 = 0.70;
/// 通常のwork modeは決定論的に決めるが、提出時間を守る最終安全停止は残す。
const WORK_SAFETY_RATIO: f64 = 180.0 / 190.0;
...
struct Solver {
    ...
    /// 各turnのfull-work massを蓄積し、1を越えたturnだけfull pathへ送る。
    work_credit: f64,
    /// 上限利用料をprefix平均で正規化するための累積値。
    offered_gross_sum: f64,
    timer: TimeKeeper,
    output: FastOutput,
    #[cfg(feature = "local")]
    trace: TraceStats,
...
impl Solver {
    ...
    fn regular_clearance_penalty(&self, P: usize, shape: &Shape, x: usize, y: usize) -> f64 {
        ...
        for rr in 0..shape.h {
            ...
            sum += self.static_clearance_rank_prefix[x + rr][end]
                - self.static_clearance_rank_prefix[x + rr][begin];
        }
    ...
    fn explicit_clearance_penalty(&self, P: usize, cells: &[usize]) -> f64 {
        let sum = cells
            .iter()
            .map(|&id| self.static_clearance_rank[id])
            .sum::<f64>();
        let error = sum / (P as f64) - self.p_area_target[P];
        SIZE_CLEARANCE_BIAS_PER_SQRT_P * (P as f64).sqrt() * error * error
    }
    ...
    fn compute_free_info(&self, occ: &Rows, keep_cells: bool) -> FreeInfo {
        ...
        let mut queue_cells = [0_usize; MAX_N * MAX_N];
        ...
        for r in 0..self.N {
            for c in 0..self.N {
                ...
                if keep_cells {
                    info.cells.push(queue_cells[..tail].to_vec());
                }
                component_id += 1;
            }
        }
    ...
    fn fragment_metric(&self, occ: &Rows) -> f64 {
        ...
        let mut queue = [0_usize; MAX_N * MAX_N];
        let mut sizes = [0_usize; MAX_N * MAX_N];
            ...
            for c in 0..self.N {
                ...
                while head < tail {
                    ...
                    for (dr, dc) in DIRS {
                        ...
                        let next = nr as usize * self.N + nc as usize;
                        if seen[next] || !self.is_free(occ, nr as usize, nc as usize) {
                            continue;
                        }
                        seen[next] = true;
                        queue[tail] = next;
        ...
        for &size in &sizes[..component_count] {
            if size < 4 {
                metric += 100.0 * size as f64;
            } else {
                metric += 18.0 + 3.0 * (size as f64).sqrt();
    ...
    fn growth_placement(
        ...
        let mut seeds = Vec::with_capacity(seed_limit + 16);
        ...
        for seed in seeds {
            ...
            let mut frontier = BinaryHeap::with_capacity(P * 5);
    ...
    /// v081 の full/fast 二択そのものは保ち、どちらへ送るかだけを入力依存の
    /// fractional credit で決める。総仕事量を増やすのでなく、後半の高価値・大型
    /// groupへ full path を残すことが目的である。
    fn schedule_full_work(
        &mut self,
        turn: usize,
        P: usize,
        V: i64,
        ...
        let gross = (V as f64) * optimistic_C;
        self.offered_gross_sum += gross;
        let mean_gross = self.offered_gross_sum / ((turn + 1) as f64);

        // q=1、prefix平均利用料、最大Pに対してそれぞれ自然な無次元化を行う。
        // 等重みとし、特定の地形・価値帯をhard thresholdでは分岐させない。
        let q_squared = q_value * q_value;
        let quality_priority = q_squared / (1.0 + q_squared);
        let gross_priority = gross / (gross + mean_gross.max(1.0));
        let size_priority = (P as f64) / (MAX_P as f64);
        let priority = ((quality_priority + gross_priority + size_priority) / 3.0).clamp(0.0, 1.0);

        let progress = ((turn + 1) as f64) / (self.M as f64);
        let pressure =
            ((progress - WORK_PRESSURE_START) / (1.0 - WORK_PRESSURE_START)).clamp(0.0, 1.0);
        let full_mass = 1.0 - pressure * (1.0 - priority);
        self.work_credit += full_mass;
        let scheduled_full = self.work_credit + 1e-12 >= 1.0;
        if scheduled_full {
            self.work_credit -= 1.0;
        }

        let _legacy_fast = self.timer.reached(LEGACY_FAST_MODE_RATIO);
        let safety_cheap = self.timer.reached(WORK_SAFETY_RATIO);
        let full_work = scheduled_full && !safety_cheap;
        local! {
            self.trace.count_by(
                "scheduler_pressure_milli_sum",
                (pressure * 1_000.0).round() as i64,
            );
            self.trace.count_by(
                "scheduler_priority_milli_sum",
                (priority * 1_000.0).round() as i64,
            );
            self.trace.count_by(
                "scheduler_full_mass_milli_sum",
                (full_mass * 1_000.0).round() as i64,
            );
            if scheduled_full {
                self.trace.count("scheduler_scheduled_full_turn");
            } else {
                self.trace.count("scheduler_scheduled_cheap_turn");
            }
            if _legacy_fast {
                self.trace.count("scheduler_legacy_fast_turn");
            }
            if full_work {
                self.trace.count("scheduler_full_turn");
                if _legacy_fast {
                    self.trace.count("scheduler_promoted_after_legacy_fast");
                }
                if q_value >= 1.4 {
                    self.trace.count("scheduler_high_q_full");
                }
                if P >= 96 {
                    self.trace.count("scheduler_large_full");
                }
            } else {
                self.trace.count("scheduler_cheap_turn");
                if !_legacy_fast {
                    self.trace.count("scheduler_deferred_before_legacy_fast");
                }
                if q_value >= 1.4 {
                    self.trace.count("scheduler_high_q_cheap");
                }
                if P >= 96 {
                    self.trace.count("scheduler_large_cheap");
                }
            }
            if safety_cheap {
                self.trace.count("scheduler_safety_cheap");
                if scheduled_full {
                    self.trace.count("scheduler_safety_override");
                }
            }
        }
        full_work
    ...
    fn clear_group_from_board(&mut self, group_id: usize) {
        // groups と盤面配列は独立fieldなので、C++同様にcell列を複製せず解除できる。
    ...
    fn collect_relocation_targets(
        ...
        // この走査中にshape集合は変更しない。C++のconst referenceと同じ借用にする。
    ...
    fn run<R: BufRead, W: Write>(
        ...
        for turn in 0..self.M {
            ...
            let fast_mode = !self.schedule_full_work(turn, P, V, q_value, optimistic_C);
            if fast_mode {
                local! {
        ...
        local! {
            ...
            self.trace.count_by(
                "scheduler_credit_milli_remaining",
                (self.work_credit * 1_000.0).round() as i64,
            );
            self.trace
...
mod rough {
    ...
    impl RegionHashSet {
        ...
        fn insert(&mut self, value: u64) -> bool {
            ...
            while self.used[position] {
                if self.keys[position] == value {
                    return false;
                }
    ...
    struct FreeState {
        cid: [isize; MAX_C],
        comp_begin: [usize; MAX_C],
        comp_size: [usize; MAX_C],
        cell_pool: [usize; MAX_C],
        comp_count: usize,
        pool_count: usize,
        free_count: usize,
    ...
    impl FreeState {
        ...
        fn component(&self, component: usize) -> &[usize] {
            let begin = self.comp_begin[component];
            &self.cell_pool[begin..begin + self.comp_size[component]]
        }
    }
    ...
    /// C++ `Hit::operator<` はspec番号を同点比較へ入れない。
    #[derive(Clone, Copy, Default)]
    struct CompactHit {
        score: i64,
        x0: usize,
    ...
    impl PartialEq for CompactHit {
        fn eq(&self, other: &Self) -> bool {
            (self.score, self.x0, self.y0) == (other.score, other.x0, other.y0)
        }
    }

    impl Eq for CompactHit {}

    impl Ord for CompactHit {
        fn cmp(&self, other: &Self) -> Ordering {
            (self.score, self.x0, self.y0).cmp(&(other.score, other.x0, other.y0))
        }
    }

    impl PartialOrd for CompactHit {
        fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
            Some(self.cmp(other))
        }
    }

    /// peel box はC++同様にscore, x0, y0だけでheap順を決める。
    #[derive(Clone, Copy, Default)]
    struct PeelBox {
        score: i64,
        x0: usize,
    ...
    impl Ord for PeelBox {
        fn cmp(&self, other: &Self) -> Ordering {
            (self.score, self.x0, self.y0).cmp(&(other.score, other.x0, other.y0))
        }
    }

    impl PartialOrd for PeelBox {
        fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
            Some(self.cmp(other))
        }
    }

    /// 通常boxのC++ comparatorはscoreだけを比較する。
    #[derive(Clone, Copy, Default)]
    struct SearchBox {
        score: i64,
        x0: usize,
    ...
    pub(super) struct Solver {
        ...
        rng: XorShift64,
        /// v000 準拠の提出安全時計。入力・route判定・静的前計算を含む。
        overall_start: Instant,
        /// C++のrough方策時計。静的盤面前計算とcompact spec生成の完了後から数える。
        policy_start: Instant,
        policy_clock_offset_us: usize,
        grow_mark: [i32; MAX_C],
        grow_stamp: i32,
        grow_heap: BinaryHeap<Reverse<HeapNode>>,
        compact_best_heap: BinaryHeap<CompactHit>,
        compact_extra_heaps: [BinaryHeap<CompactHit>; 2],
        peel_box_heap: BinaryHeap<PeelBox>,
        search_box_heap: BinaryHeap<SearchBox>,
        ...
        same_stamp: i32,
        eval_stamp: i32,
        pair_touch_components: [Vec<usize>; 1_000],
        pair_touch_groups: Vec<u8>,
        pair_active_ids: [usize; 1_000],
        pair_active_positions: [isize; 1_000],
        same_region_checks: usize,
        grow_heap_resets: usize,
        pair_buffer_resets: usize,
        overall_hard_stops: usize,
    ...
    impl Solver {
        pub(super) fn new(
            ...
            let mut solver = Self {
                ...
                overall_start,
                policy_start: overall_start,
                policy_clock_offset_us: 0,
                grow_mark: [0; MAX_C],
                grow_stamp: 1,
                ...
                same_stamp: 1,
                eval_stamp: 1,
                pair_touch_components: std::array::from_fn(|_| Vec::new()),
                pair_touch_groups: Vec::new(),
                pair_active_ids: [0; 1_000],
                pair_active_positions: [-1; 1_000],
                same_region_checks: 0,
                grow_heap_resets: 0,
                pair_buffer_resets: 0,
                overall_hard_stops: 0,
                output_buffer: String::with_capacity(1 << 16),
                #[cfg(feature = "local")]
                trace: TraceStats::default(),
            };
            solver.initialize_board();
            solver.policy_start = Instant::now();
            solver.policy_clock_offset_us = solver
                .policy_start
                .duration_since(solver.overall_start)
                .as_micros() as usize;
            solver
        }
        ...
        pub(super) fn propose_same_economics(
            ...
            self.find_regions_with_state(P, 6, 0, Some(target_T), priority, &fs)
                ...
                .filter(|region| {
                    ...
                    component >= 0 && fs.comp_size[component as usize] == target_component_size
                })
                .min_by(|a, b| {
                    a.layout
                        .cmp(&b.layout)
        ...
        #[inline]
        fn elapsed(&self) -> f64 {
            self.policy_start.elapsed().as_secs_f64()
        }

        #[inline]
        fn overall_elapsed(&self) -> f64 {
            self.overall_start.elapsed().as_secs_f64()
        }

        #[inline]
        fn before_deadline(&self, policy_seconds_at_judge: f64) -> bool {
            self.elapsed() < scaled_time(policy_seconds_at_judge)
                && self.overall_elapsed() < scaled_time(1.88)
        }
        ...
        fn adaptive_mode(&self, turn: usize, priority: f64) -> i32 {
            ...
            if elapsed >= scaled_time(1.82) || self.overall_elapsed() >= scaled_time(1.88) {
                return -1;
            }
        ...
        fn operation_time_exhausted(&mut self) -> bool {
            ...
            if self.operation_clock_counter & 7 == 0 {
                let elapsed = self.elapsed();
                let overall_exhausted = self.overall_elapsed() >= scaled_time(1.88);
                if elapsed >= self.operation_deadline
                    || elapsed >= scaled_time(1.84)
                    || overall_exhausted
                {
                    self.operation_stopped = true;
                    self.hard_time_stops += 1;
                    if overall_exhausted {
                        self.overall_hard_stops += 1;
                    }
                    return true;
                }
            }
        ...
        fn add_compact_template_candidates(
            ...
            let mut best = std::mem::take(&mut self.compact_best_heap);
            let mut extra = [
                std::mem::take(&mut self.compact_extra_heaps[0]),
                std::mem::take(&mut self.compact_extra_heaps[1]),
            ];
            best.clear();
            extra[0].clear();
            extra[1].clear();
            let specs = &self.compact_specs[P];
            for (si, &spec) in specs.iter().enumerate() {
                ...
                for x0 in 0..=self.N - h {
                    for y0 in 0..=self.N - w {
                        ...
                        if component < 0 || fs.comp_size[component as usize] < P {
                            continue;
                        }
                        ...
                        let leftover = fs.comp_size[component as usize] - P;
                        ...
                        if best.len() < hit_limit {
                            best.push(hit);
                        } else if score < best.peek().unwrap().score {
                            best.pop();
                            best.push(hit);
                        }
                        ...
                        if cls >= 1 && cls <= 2 && extra_limit[cls - 1] > 0 {
                            ...
                            if heap.len() < extra_limit[cls - 1] {
                                heap.push(item);
                            } else if base_score < heap.peek().unwrap().score {
                                heap.pop();
                                heap.push(item);
                            }
                        }
            ...
            while let Some(hit) = best.pop() {
                hits.push(hit);
            }
            while let Some(hit) = extra[0].pop() {
                hits.push(hit);
            }
            while let Some(hit) = extra[1].pop() {
                hits.push(hit);
            }
            hits.sort_unstable_by(|a, b| {
                specs[a.spec_index]
                    .L
                    .cmp(&specs[b.spec_index].L)
                    .then(a.score.cmp(&b.score))
            });
            for hit in hits {
                let region = self.make_spec_region(specs[hit.spec_index], hit.x0, hit.y0);
                self.add_candidate(candidates, seen, region, P);
            }
            self.compact_best_heap = best;
            self.compact_extra_heaps = extra;
        }
        ...
        fn build_free_state_into(&mut self, fs: &mut FreeState) {
            ...
            fs.pool_count = 0;
            fs.free_count = 0;
            fs.pref.fill([0; MAX_N + 1]);
            fs.hpref.fill([0; MAX_N + 1]);
            fs.vpref.fill([0; MAX_N + 1]);
            for z in 0..self.N * self.N {
                ...
                fs.comp_count += 1;
                fs.comp_begin[component] = fs.pool_count;
                // C++と同じくcell_poolの未確定suffixをそのままBFS queueに使う。
                let mut head = fs.pool_count;
                fs.cell_pool[fs.pool_count] = z;
                fs.pool_count += 1;
                fs.cid[z] = component as isize;
                while head < fs.pool_count {
                    let v = fs.cell_pool[head];
                    head += 1;
                    fs.free_count += 1;
                    for &w in &self.nb[v] {
                        if w >= 0 {
                            ...
                            if self.occ[w] == -1 && fs.cid[w] < 0 {
                                ...
                                fs.cell_pool[fs.pool_count] = w;
                                fs.pool_count += 1;
                            }
                        }
                    }
                }
                fs.comp_size[component] = fs.pool_count - fs.comp_begin[component];
            }
            for x in 0..self.N {
        ...
        #[inline]
        fn push_grow_node(&mut self, node: HeapNode) {
            // C++ FixedHeap<HeapNode,16384>と同じ上限。満杯時に探索集合を拡張しない。
            if self.grow_heap.len() < 16_384 {
                self.grow_heap.push(Reverse(node));
            }
        }
        ...
        fn grow_region(
            ...
            for w in seed_neighbors {
                if w >= 0 {
                    ...
                    if self.occ[w] == -1 && self.grow_mark[w] != stamp && in_box(w, self.N) {
                        ...
                        self.push_grow_node(HeapNode { key, z: w });
                    }
                }
            }
            ...
            while region.cells.len() < P {
                ...
                if key_now != node.key {
                    self.push_grow_node(HeapNode { key: key_now, z });
                    continue;
                }
                let degree = self.selected_degree(z);
                self.grow_mark[z] = stamp;
        ...
        fn evaluate_layout(
            ...
            let size = fs.comp_size[component] as i64;
        ...
        fn find_peel_regions_with_state(
            ...
            let mut boxes = Vec::with_capacity(heap.len());
            while let Some(item) = heap.pop() {
                boxes.push(item);
            }
            self.peel_box_heap = heap;
            boxes.sort_unstable_by_key(|item| item.score);
            let mut candidates = Vec::new();
            let mut seen = RegionHashSet::new();
            for item in boxes {
                if !self.before_deadline(1.83) {
                    break;
                }
                let Some(mut region) =
                    self.peel_box_region(P, item.x0, item.y0, item.h, item.w, target_T, priority)
                else {
                    continue;
                };
        ...
        fn find_regions_with_state(
            ...
            let mut eligible: Vec<usize> = (0..fs.comp_count)
                .filter(|&component| fs.comp_size[component] >= P)
                .collect();
            ...
            if box_limit > 0 {
                let mut heap = std::mem::take(&mut self.search_box_heap);
                heap.clear();
                ...
                self.search_box_heap = heap;
                boxes.sort_unstable_by_key(|item| item.score);
                for item in boxes {
                    let SearchBox { x0, y0, h, w, .. } = item;
                    ...
                    for x in x0..x0 + h {
                        for y in y0..y0 + w {
                            ...
                            if self.occ[z] != -1
                                || fs.cid[z] < 0
                                || fs.comp_size[fs.cid[z] as usize] < P
                            {
                                continue;
                            }
                    ...
                    seed_pool.dedup();
                    seed_pool.sort_unstable_by(|&a, &b| {
                        let score = |z: usize| {
                            let dx = 2 * self.xof(z) as i32 - center_x2;
                            let dy = 2 * self.yof(z) as i32 - center_y2;
                            let degree = self.nb[z]
            ...
            eligible.sort_unstable_by_key(|&component| fs.comp_size[component]);
            ...
            for &component in &eligible[1..] {
                if fs.comp_size[component] > fs.comp_size[largest] {
                    largest = component;
                }
            }
            ...
            for component in chosen {
                // C++のspanと同じく、seed抽出だけの成分はcell列を複製しない。
                let component_cells = fs.component(component);
                if component_cells.len() == P {
                    let cells = component_cells.to_vec();
                ...
                let mut min_rank = component_cells[0];
                let mut min_x = component_cells[0];
                let mut max_x = component_cells[0];
                let mut center = component_cells[0];
                let mut max_clear = component_cells[0];
                let mut min_clear = component_cells[0];
                let mut x_min = self.N;
                let mut x_max = 0;
                let mut y_min = self.N;
                let mut y_max = 0;
                for &z in component_cells {
                ...
                if mode >= 2 {
                    seeds.push(component_cells[self.rng.next_int(component_cells.len())]);
                }
                seeds.sort_unstable();
                seeds.dedup();
            ...
            polish_order.sort_unstable_by_key(|&index| candidates[index].L);
            let mut polished = 0;
            ...
            order_by_L.sort_unstable_by(|&a, &b| {
                filtered[a]
                    .L
                    .cmp(&filtered[b].L)
        ...
        fn same_region(&mut self, region: &Region, group_id: usize) -> bool {
            ...
            if region.hash != self.groups[group_id].hash
                || region.cells.len() != self.groups[group_id].cells.len()
            {
                return false;
            }
            ...
            for group_id in 0..self.M {
                    ...
                    for &w in &self.nb[z] {
                        if w >= 0 && self.occ[w as usize] == -1 {
                            ...
                            if component >= 0 && seen_component[component as usize] != token {
                                ...
                                potential += fs.comp_size[component as usize];
                            }
                        }
                    }
                }
                ...
                for component in 0..fs.comp_count {
                    if seen_component[component] != token {
                        max_untouched = max_untouched.max(fs.comp_size[component]);
                    }
                }
                let slack = group
                    .worst_L
                ...
                if best_count < limit {
                    best[best_count] = item;
                    best_count += 1;
                } else {
                    let mut worst = 0;
                    for index in 1..best_count {
                        if less(&best[worst], &best[index]) {
                            worst = index;
                        }
                    }
                    if less(&item, &best[worst]) {
                        best[worst] = item;
                    }
                }
            }
            best[..best_count].sort_unstable_by(|a, b| {
                a.score
                    .partial_cmp(&b.score)
                    .unwrap_or(Ordering::Equal)
                    .then(a.id.cmp(&b.id))
            });
            best[..best_count].iter().map(|item| item.id).collect()
        }
        ...
        fn movable_pair_candidates(
            ...
            debug_assert!(self.M <= 1_000 && limit <= 16);
            self.pair_buffer_resets += 1;
            self.pair_active_positions.fill(-1);
            let mut active_count = 0;
            for group_id in 0..self.M {
                if self.groups[group_id].active {
                    self.pair_active_positions[group_id] = active_count as isize;
                    self.pair_active_ids[active_count] = group_id;
                    active_count += 1;
                }
            }
            self.pair_touch_groups
                .resize(active_count * active_count, 0);
            self.pair_touch_groups.fill(0);
            let mut seen_component = [-1_isize; MAX_C];
            for aa in 0..active_count {
                self.pair_touch_components[aa].clear();
                let group_id = self.pair_active_ids[aa];
                    ...
                    for &w in &self.nb[z] {
                        ...
                        if self.occ[w] == -1 {
                            ...
                            if component >= 0 && seen_component[component as usize] != aa as isize {
                                ...
                                self.pair_touch_components[aa].push(component as usize);
                            }
                        } else if self.occ[w] >= 0 {
                            let bb = self.pair_active_positions[self.occ[w] as usize];
                            if bb >= 0 && bb as usize != aa {
                                self.pair_touch_groups[aa * active_count + bb as usize] = 1;
                            }
                        }
                    }
                }
                self.pair_touch_components[aa].sort_unstable();
            }
            #[derive(Clone, Copy, Default)]
            struct Item {
                score: f64,
            ...
            for aa in 0..active_count {
                for bb in aa + 1..active_count {
                    let mut linked = self.pair_touch_groups[aa * active_count + bb] != 0
                        || self.pair_touch_groups[bb * active_count + aa] != 0;
                    let mut ia = 0;
                    let mut ib = 0;
                    let group_a = self.pair_active_ids[aa];
                    let group_b = self.pair_active_ids[bb];
                    let mut potential = self.groups[group_a].P + self.groups[group_b].P;
                    while ia < self.pair_touch_components[aa].len()
                        || ib < self.pair_touch_components[bb].len()
                    {
                        let component;
                        if ib == self.pair_touch_components[bb].len()
                            || (ia < self.pair_touch_components[aa].len()
                                && self.pair_touch_components[aa][ia]
                                    < self.pair_touch_components[bb][ib])
                        {
                            component = self.pair_touch_components[aa][ia];
                            ia += 1;
                        } else if ia == self.pair_touch_components[aa].len()
                            || self.pair_touch_components[bb][ib]
                                < self.pair_touch_components[aa][ia]
                        {
                            component = self.pair_touch_components[bb][ib];
                            ib += 1;
                        } else {
                            component = self.pair_touch_components[aa][ia];
                            ia += 1;
                        ...
                        potential += fs.comp_size[component];
                    }
        ...
        fn consider_move_plan(
            ...
            if net_gain > 0 && fee_quality >= required_fee_quality && net_gain > best.net_gain {
                best.ids.clear();
                best.dest.clear();
                best.ids.reserve(changed_count);
                best.dest.reserve(changed_count);
                for (index, &group_id) in ids.iter().enumerate() {
                    if changed[index] {
                        best.ids.push(group_id);
                        best.dest.push(placed[index].clone());
                    }
                }
        ...
        fn try_move_set(
            ...
            // C++のarray順序をそのまま再現し、同人数ならids[0]を先にする。
            let mut orders = [[usize::MAX; 2]; 2];
            let order_count;
            if count == 1 {
                orders[0][0] = ids[0];
                order_count = 1;
            } else {
                let first = usize::from(self.groups[ids[0]].P < self.groups[ids[1]].P);
                orders[0] = [ids[first], ids[1 - first]];
                orders[1] = [ids[1 - first], ids[first]];
                order_count = 2;
            }
                ...
                .filter(|_| !self.operation_time_exhausted())
            {
                ...
                for incoming in incoming_regions {
                    ...
                    for order in orders.iter().take(order_count) {
                        let mut placed = [Region::default(), Region::default()];
                        let mut has = [false; 2];
                        self.dfs_incoming_first(
                            0,
                            &order[..count],
        ...
        fn output_turn<W: Write>(
            ...
            if let Some(plan) = move_plan.filter(|plan| plan.net_gain != i64::MIN) {
                writeln!(output, "{}", plan.ids.len()).unwrap();
                for (position, &group_id) in plan.ids.iter().enumerate() {
                    writeln!(output, "{}", group_id).unwrap();
                    for &z in &plan.dest[position].cells {
                        writeln!(output, "{} {}", z / N, z % N).unwrap();
                    }
                }
            } else {
                output.push_str("0\n");
        ...
        pub(super) fn run<R: BufRead, W: Write>(
            ...
            for index in 0..self.M {
                ...
                if need_peel && self.before_deadline(1.82) && self.elapsed() < peel_allowance {
                ...
                if let Some(chosen) = chosen {
                    ...
                    if upgrade_worth
                        && self.before_deadline(1.82)
                        && self.elapsed() < upgrade_allowance
                    {
                ...
                if self.grass_count - self.occupied_cells >= P && self.before_deadline(1.82) {
            ...
            local! {
                ...
                self.trace.count_by(
                    "rough_policy_clock_offset_us",
                    self.policy_clock_offset_us as i64,
                );
                self.trace.count_by(
                    "rough_same_region_check",
                    self.same_region_checks as i64,
                );
                self.trace.count_by(
                    "rough_grow_heap_reset",
                    self.grow_heap_resets as i64,
                );
                self.trace.count_by(
                    "rough_pair_buffer_reset",
                    self.pair_buffer_resets as i64,
                );
                self.trace.count_by(
                    "rough_overall_hard_stop",
                    self.overall_hard_stops as i64,
                );
                self.trace.add_time_ms(
                    "rough_policy_elapsed",
                    self.policy_start.elapsed().as_secs_f64() * 1_000.0,
                );
                self.trace.add_time_ms(
                    "program_elapsed",
                    self.overall_start.elapsed().as_secs_f64() * 1_000.0,
                );
                self.trace.summary();
            }
            Ok(())
