// v058_pocket_packing.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: 静的草地から独立小成分と1マスの首で接続する小半島を抽出し、
// P<64 が容量と完全一致するとき、その低汎用領域そのものを通常配置候補へ加える。
// v053 の評価・admission・relocation・rolloutは変えず、候補生成だけを拡張する。
...
const STATIC_POCKET_MAX_P: usize = 63;
type Rows = [u64; MAX_N];
...
struct StaticPocket {
    cells: Vec<usize>,
    perimeter: usize,
    /// true なら、空板の連結成分から1つの関節点を除いて切り離される半島である。
    is_lobe: bool,
}
...
impl Default for Placement {
    fn default() -> Self {
        Self {
            ...
            is_static_pocket: false,
            is_static_lobe: false,
        }
    }
}
...
struct Solver {
    ...
    /// 面積Pごとの静的な袋領域。入力地形だけで決まり、占有状態には依存しない。
    static_pockets_by_p: Vec<Vec<StaticPocket>>,
...
impl Solver {
    fn new(N: usize, M: usize, R_milli: i64, grass_rows: Rows, timer: TimeKeeper) -> Self {
        ...
        let mut solver = Self {
            ...
            static_pockets_by_p: vec![Vec::new(); MAX_P + 1],
        ...
        solver.initialize_static_pockets();
        local! {
            let kept = solver
                .shapes_by_p
                .iter()
    ...
    /// 空板で完結する低汎用領域を列挙する。半島は、1マスの関節点を除くと
    /// 元の大成分から切り離される4..63マスの連結部分として定義する。
    fn initialize_static_pockets(&mut self) {
        let empty_occ = [0_u64; MAX_N];
        let info = self.compute_free_info(&empty_occ, true);
        let mut seen = HashSet::<Vec<usize>>::new();
        let mut pockets_by_p = vec![Vec::<StaticPocket>::new(); MAX_P + 1];
        let mut _independent_count = 0_i64;
        let mut _lobe_count = 0_i64;

        // 池や外周で完全に囲まれた小さな草地成分は、その全体を1つの袋とする。
        for component_cells in &info.cells {
            if !(4..=STATIC_POCKET_MAX_P).contains(&component_cells.len()) {
                continue;
            }
            let mut cells = component_cells.clone();
            cells.sort_unstable();
            if seen.insert(cells.clone()) {
                let P = cells.len();
                assert!(self.explicit_candidate_is_valid(&cells, P, &empty_occ));
                pockets_by_p[P].push(StaticPocket {
                    perimeter: self.perimeter_of_cells(&cells),
                    cells,
                    is_lobe: false,
                });
                _independent_count += 1;
            }
        }

        // 大成分の各セルを首の候補として外し、切り離される小成分を半島とする。
        // N<=50なので空板で一度だけ行うO(|G|^2)のBFSで十分軽い。
        for (component_id, component_cells) in info.cells.iter().enumerate() {
            if component_cells.len() <= STATIC_POCKET_MAX_P {
                continue;
            }
            for &cut in component_cells {
                let mut visited = vec![false; self.N * self.N];
                visited[cut] = true;
                let mut pieces = Vec::<Vec<usize>>::new();
                for &start in component_cells {
                    if visited[start] {
                        continue;
                    }
                    visited[start] = true;
                    ...
                    while let Some(id) = stack.pop() {
                        ...
                        .flatten()
                        {
                            if !visited[next] && info.component[next] == component_id as isize {
                                visited[next] = true;
                                stack.push(next);
                            }
                        }
                ...
                for mut cells in pieces {
                    if !(4..=STATIC_POCKET_MAX_P).contains(&cells.len()) {
                        continue;
                    }
                    cells.sort_unstable();
                    if !seen.insert(cells.clone()) {
        ...
        for pockets in &mut pockets_by_p {
            pockets.sort_by(|a, b| {
                a.perimeter
                    .cmp(&b.perimeter)
        ...
        self.static_pockets_by_p = pockets_by_p;
        local! {
            self.trace
                .count_by("static_independent_pocket_count", _independent_count);
            self.trace.count_by("static_lobe_count", _lobe_count);
            self.trace.count_by(
                "static_pocket_count",
                _independent_count + _lobe_count,
            );
        }
    }
    ...
    /// 現在も全マスが空いており、人数と容量が完全一致する静的袋だけを返す。
    /// 周長レベルは通常候補と同じものに限定し、異周長比較はこの実験に混ぜない。
    fn scan_static_pocket_level(
        &mut self,
        P: usize,
        occ: &Rows,
        info: &FreeInfo,
        ...
        if P > STATIC_POCKET_MAX_P {
            return Vec::new();
        }
        let mut candidates = Vec::new();
        let mut _lobe_candidates = 0_i64;
        for pocket in &self.static_pockets_by_p[P] {
            if pocket.perimeter != perimeter
                || pocket
                    .cells
                    .iter()
                    .any(|&id| ((occ[id / self.N] >> (id % self.N)) & 1) != 0)
            {
                continue;
            }
            assert!(self.explicit_candidate_is_valid(&pocket.cells, P, occ));
            let component_id = info.component[pocket.cells[0]];
            assert!(component_id >= 0);
            assert!(
                pocket
                    .cells
                    .iter()
                    .all(|&id| info.component[id] == component_id)
            );
            candidates.push(Placement {
                perimeter,
                cheap_score: pocket.cells.iter().map(|&id| weights.cell[id]).sum(),
                component_size: info.sizes[component_id as usize],
                explicit_cells: pocket.cells.clone(),
                is_static_pocket: true,
                is_static_lobe: pocket.is_lobe,
                ..Placement::default()
            });
            if pocket.is_lobe {
                _lobe_candidates += 1;
        ...
        local! {
            self.trace
                .count_by("static_pocket_candidate", candidates.len() as i64);
            self.trace
                .count_by("static_lobe_candidate", _lobe_candidates);
        }
        candidates
    }
    ...
    fn find_normal_placements(
        ...
        for perimeter in (min_L..=min_L + max_extra).step_by(2) {
            let mut candidates = self.scan_regular_level(
                P,
                &runs,
                &weights,
                perimeter,
            ...
            candidates.extend(self.scan_static_pocket_level(P, &occ, &info, &weights, perimeter));
    ...
    fn commit_normal_placement(&mut self, group_id: usize, placement: &Placement) {
        let P = self.groups[group_id].P;
        local! {
            if placement.is_static_pocket {
                self.trace.count("static_pocket_placed");
                if placement.is_static_lobe {
                    self.trace.count("static_lobe_placed");
                }
            }
        ...
        for turn in 0..self.M {
            ...
            if passed_price_prefilter {
                ...
                if let Some(mut choices) = normal_choices {
                    ...
                    if accepted
                        && placement.shape_index == usize::MAX
                        && !placement.is_static_pocket
                    {
                    ...
                    local! {
                        if placement.is_static_pocket {
                            self.trace.count("static_pocket_chosen");
                            if placement.is_static_lobe {
                                self.trace.count("static_lobe_chosen");
                            }
                        }
                    }
