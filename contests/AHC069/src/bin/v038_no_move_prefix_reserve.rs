// v038_no_move_prefix_reserve.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。

// 中心アイデア: prefix補正で下がった theta と容量reserveを整合させるため、
// v037の全経路を保ったまま再移動なし有効容量比だけを0.950へ下げる。
...
const NO_MOVE_CAPACITY_RATIO: f64 = 0.950;
...
impl ThetaModel {
    fn new(theta: f64) -> Self {
        // l は指数分布の標本を最近接整数へ丸め、l < HORIZON になるまで
        // 再生成される。ここではその正規化済み確率質量を使う。
        let norm = -(-((HORIZON as f64) - 0.5) / theta).exp_m1();
        let p0 = -(-0.5 / theta).exp_m1() / norm;
        let band = -(-1.0 / theta).exp_m1();
        let ratio = (-1.0 / theta).exp();

        // f_S(s) = sum_{l <= H-1-s} p(l)/(H-l)。prefix から全 s の
        // P(S > s) を作り、未到着 group の打切り尤度に使う。
...
impl ThetaEstimator {
    ...
    fn observe(&mut self, i: usize, S: usize, D: usize) -> f64 {
        debug_assert!(i < self.M && S < HORIZON && D >= 1);
        let l = D - 1;
            ...
            .zip(self.duration_log_likelihood.iter_mut())
        {
            *duration_like += model.log_duration_probability(l);
            // 入力は S 昇順である。既知 prefix の尤度には、未到着の全組が
            // 現在の S より後に来る確率も掛かる。
    ...
    #[cfg(feature = "local")]
    fn verify_v001_reference(&mut self) {
        // v001 の同じ推定器から得た既知小標本の事後平均と照合する。
        // 本番用の事前計算を再利用し、検証後に累積尤度だけ初期化する。
        const OBSERVATIONS: [(usize, usize); 5] = [
            (100, 1),
            (8_000, 5_001),
            (25_000, 9_001),
            (60_000, 2_001),
        ...
        const EXPECTED: [f64; 5] = [
            4_302.516_657_463_459,
            4_472.979_186_863_742,
            5_133.783_702_400_949,
            4_819.687_761_159_718,
            4_425.653_680_220_814,
        ];

        let original_M = self.M;
        self.M = OBSERVATIONS.len();
        self.duration_log_likelihood.fill(0.0);
        for (i, (&(S, D), &expected)) in OBSERVATIONS.iter().zip(&EXPECTED).enumerate() {
            let actual = self.observe(i, S, D);
            assert!(
                (actual - expected).abs() <= 0.02,
                "prefix theta reference mismatch: turn={i}, actual={actual}, expected={expected}"
            );
        }
        self.duration_log_likelihood.fill(0.0);
        self.M = original_M;
...
struct Solver {
    ...
    // 現行推定値は意思決定に使わず、prefix 補正の機構確認用 shadow として残す。
    #[cfg(feature = "local")]
    duration_sum: f64,
    #[cfg(feature = "local")]
...
impl Solver {
    fn new(N: usize, M: usize, grass_rows: Rows, timer: TimeKeeper) -> Self {
        ...
        #[cfg(feature = "local")]
        let theta_init_start = Instant::now();
        #[allow(unused_mut)]
        let mut theta_estimator = ThetaEstimator::new(M);
        #[cfg(feature = "local")]
        theta_estimator.verify_v001_reference();
        #[cfg(feature = "local")]
        let theta_init_ms = theta_init_start.elapsed().as_secs_f64() * 1_000.0;
        let mut solver = Self {
            N,
            M,
            grass_rows,
        ...
        local! {
            solver.trace.count("theta_reference_verified");
            solver.trace.add_time_ms("theta_init", theta_init_ms);
            let kept = solver
                .shapes_by_p
                .iter()
    ...
    #[cfg(feature = "local")]
    fn legacy_posterior_theta(&self) -> f64 {
    ...
    fn initialize_static_capacity(&mut self) {
        ...
        local! {
            self.trace.count_by("capacity_reserve_milli", 950);
        }
    }
    ...
    fn run<R: BufRead, W: Write>(
        ...
        for turn in 0..self.M {
            ...
            #[cfg(feature = "local")]
            let legacy_theta = self.legacy_posterior_theta();
            let theta = self.theta_estimator.observe(turn, S, duration);
            local! {
                self.trace.count("theta_prefix_turn");
                self.trace.count_by(
                    "theta_legacy_milli_sum",
                    (legacy_theta * 1_000.0).round() as i64,
                );
                self.trace.count_by(
                    "theta_prefix_milli_sum",
                    (theta * 1_000.0).round() as i64,
                );
                self.trace.count_by(
                    "theta_prefix_minus_legacy_milli_sum",
                    ((theta - legacy_theta) * 1_000.0).round() as i64,
                );
            }
            ...
            #[cfg(feature = "local")]
            let legacy_base_threshold = {
                let start = Instant::now();
                let value = self.base_dynamic_threshold(S, duration, P, legacy_theta);
                self.trace.add_time_ms(
                    "theta_legacy_threshold",
            ...
            local! {
                let legacy_passed_price_prefilter = legacy_base_threshold == 0.0
                    || q_value * optimistic_C >= 0.74 * legacy_base_threshold;
                if legacy_passed_price_prefilter {
                    self.trace.count("theta_prefilter_legacy_pass");
                }
                if passed_price_prefilter {
                    self.trace.count("theta_prefilter_prefix_pass");
                }
                if passed_price_prefilter && !legacy_passed_price_prefilter {
                    self.trace.count("theta_prefilter_prefix_only");
                } else if legacy_passed_price_prefilter && !passed_price_prefilter {
                    self.trace.count("theta_prefilter_legacy_only");
                }
            }
            if passed_price_prefilter {
                local! {
                ...
                if let Some(mut choices) = normal_choices {
                    ...
                    local! {
                        let legacy_actual_threshold = legacy_base_threshold
                            * self.component_threshold_factor(current_component_size);
                        let legacy_post_pass = legacy_base_threshold == 0.0
                            || quality >= legacy_actual_threshold;
                        self.trace.count("theta_post_candidate");
                        if legacy_post_pass {
                            self.trace.count("theta_post_legacy_pass");
                        }
                        if accepted {
                            self.trace.count("theta_post_prefix_pass");
                        }
                        if accepted && !legacy_post_pass {
                            self.trace.count("theta_post_prefix_only");
                        } else if legacy_post_pass && !accepted {
                            self.trace.count("theta_post_legacy_only");
                        }
                    }
