// v056_persistent_owner.rs
#![allow(non_snake_case)]
// 中心アイデア: v053 の探索を変えず、確定盤面の owner 配列だけを
// タイムスタンプ付き永続2次元遅延セグメントツリーへ置き換えて実用性能を測る。
...
fn decompose_cells_into_rectangles(cells: &[usize], N: usize) -> Vec<Rect> {
    ...
    for (x, &row) in rows.iter().enumerate().take(N) {
        let mut bits = row;
        let mut next = Vec::new();
        while bits != 0 {
            let y0 = bits.trailing_zeros() as usize;
            let len = (bits >> y0).trailing_ones() as usize;
            let y1 = y0 + len;
            let mask = ((1_u64 << len) - 1) << y0;
            bits &= !mask;
...
/// 長方形へのowner代入をパスコピーし、過去rootを破壊しない2次元遅延木。
/// 削除は占有bitsetだけに反映し、再配置時の新しいstampで古いownerを上書きする。
...
impl PersistentOwner2D {
    fn new(N: usize) -> Self {
        let mut x_nodes = Vec::with_capacity(32_768);
        let mut y_nodes = Vec::with_capacity(65_536);
    ...
    fn assign_rectangles(&mut self, mut root: NodeId, rectangles: &[Rect], owner: u16) -> NodeId {
        ...
        for &rect in rectangles {
            root = self.update_x(root, 0, self.N, rect, tag);
        }
        root
    }
    ...
    fn bytes_used(&self) -> usize {
        self.x_nodes.len() * std::mem::size_of::<XNode>()
            + self.y_nodes.len() * std::mem::size_of::<YNode>()
    }
}
...
struct Solver {
    ...
    occupied_rows: Rows,
    owner_root: NodeId,
    owner_arena: PersistentOwner2D,
    #[cfg(feature = "local")]
    owner_query_count: std::cell::Cell<u64>,
    #[cfg(feature = "local")]
    owner_assign_count: u64,
    #[cfg(feature = "local")]
    owner_rectangle_count: u64,
    groups: Vec<Group>,
...
impl Solver {
    fn new(N: usize, M: usize, R_milli: i64, grass_rows: Rows, timer: TimeKeeper) -> Self {
        ...
        let mut solver = Self {
            ...
            owner_arena: PersistentOwner2D::new(N),
            #[cfg(feature = "local")]
            owner_query_count: std::cell::Cell::new(0),
            #[cfg(feature = "local")]
            owner_assign_count: 0,
            #[cfg(feature = "local")]
            owner_rectangle_count: 0,
            groups,
    ...
    #[inline]
    fn owner_at(&self, r: usize, c: usize) -> isize {
        local! {
            self.owner_query_count
                .set(self.owner_query_count.get() + 1);
        }
        if self.occupied_rows[r] & Self::bit_at(c) == 0 {
            return -1;
        }
        self.owner_arena
            .owner_at(self.owner_root, r, c)
            .expect("occupied cell must have an owner") as isize
    }
    ...
    fn place_group_on_board(&mut self, group_id: usize, cells: &[usize]) {
        ...
        let rectangles = decompose_cells_into_rectangles(cells, self.N);
        self.owner_root =
            self.owner_arena
                .assign_rectangles(self.owner_root, &rectangles, group_id as u16);
        local! {
            self.owner_assign_count += 1;
            self.owner_rectangle_count += rectangles.len() as u64;
        }
    }
    ...
    fn run<R: BufRead, W: Write>(
        ...
        local! {
            self.trace
                .count_by("owner_query", self.owner_query_count.get() as i64);
            self.trace
                .count_by("owner_assign", self.owner_assign_count as i64);
            self.trace
                .count_by("owner_rectangle", self.owner_rectangle_count as i64);
            self.trace
                .count_by("owner_x_nodes", self.owner_arena.x_nodes.len() as i64);
            self.trace
                .count_by("owner_y_nodes", self.owner_arena.y_nodes.len() as i64);
            self.trace
                .count_by("owner_arena_bytes", self.owner_arena.bytes_used() as i64);
            self.trace
