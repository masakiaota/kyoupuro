// v085_departure_event_time_price.rs
#![allow(non_snake_case)]
// 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: v081 の方策を保ち、smooth admission の時間価格だけを、既知activeの
// 退去イベントと同時刻までの未到着期待負荷を合成した区間価格へ置き換える。
...
impl Solver {
    ...
    /// 既知activeの予約量と、現在以後に到着するgroupの期待負荷を同じ時刻で合成する。
    /// 後者は事前probeで選ばれた固定到着率と終端補正を使う。既知予約量だけを使うと
    /// v036のように将来到着との競合を落とすため、必ず両者を一組で価格へ渡す。
    fn event_time_bid_at(
        &mut self,
        known_area: usize,
        time: f64,
        S: usize,
        ...
        let elapsed = (time - (S as f64)).max(0.0);
        let future_start_factor = 1.0 - (-elapsed / theta).exp();
        let terminal_factor = 1.0 - (-((HORIZON as f64) - time).max(0.0) / theta).exp();
        let future_area = offered_area * future_start_factor * terminal_factor;
        let local_offer = (known_area as f64) + future_area;
        if local_offer <= self.effective_capacity {
            return (0.0, future_area);
        }
        let fraction = self.effective_capacity / local_offer;
        (
            self.q_threshold_for_fraction(fraction) * self.compactness_bar,
    ...
    /// v081の旧しきい値と、退去イベントで区切った区間価格の積分値を返す。
    /// 旧値は配置探索量を変えないprefilter専用であり、実際の経済判断には新値を使う。
    fn dynamic_thresholds(
        ...
        let legacy_threshold = threshold * multiplier;

        let end = S + duration;
        let mut departure_events: Vec<(usize, usize)> = self
            .departures
            .iter()
            .filter_map(|&Reverse((T, group_id))| {
                let group = &self.groups[group_id];
                (group.active && S < T && T < end).then_some((T, group.P))
            })
            .collect();
        departure_events.sort_unstable_by_key(|&(T, _)| T);

        let mut cursor = S;
        let mut known_area = self.occupied_count();
        debug_assert_eq!(
            known_area,
            self.departures
                .iter()
                .filter_map(|&Reverse((_, group_id))| {
                    let group = &self.groups[group_id];
                    group.active.then_some(group.P)
                })
                .sum::<usize>(),
            "active departure areas must match occupied cells",
        );
        let mut event_index = 0;
        let mut event_average_bid = 0.0;
        let mut _covered_duration = 0_usize;
        let mut _interval_count = 0_i64;
        let mut _known_area_sum = 0_i64;
        let mut _future_area_milli_sum = 0_i64;
        let mut _local_offer_milli_sum = 0_i64;
        let mut _point_bid_milli_sum = 0_i64;
        let D = duration as f64;

        while cursor < end {
            let next = departure_events.get(event_index).map_or(end, |&(T, _)| T);
            debug_assert!(cursor < next);
            let midpoint = 0.5 * ((cursor + next) as f64);
            let (point_bid, future_area) =
                self.event_time_bid_at(known_area, midpoint, S, theta, offered_area);
            event_average_bid += ((next - cursor) as f64 / D) * point_bid;
            _covered_duration += next - cursor;
            _interval_count += 1;
            _known_area_sum += known_area as i64;
            _future_area_milli_sum += (future_area * 1_000.0).round() as i64;
            _local_offer_milli_sum +=
                (((known_area as f64) + future_area) * 1_000.0).round() as i64;
            _point_bid_milli_sum += (point_bid * 1_000.0).round() as i64;

            cursor = next;
            while event_index < departure_events.len() && departure_events[event_index].0 == cursor
            {
                let departing_area = departure_events[event_index].1;
                debug_assert!(known_area >= departing_area);
                known_area -= departing_area;
                event_index += 1;
            }
        }
        debug_assert_eq!(event_index, departure_events.len());
        debug_assert_eq!(_covered_duration, duration);
        let event_threshold = event_average_bid * (D / theta).powf(0.1);

        local! {
            self.trace.count("time_price_turn");
            self.trace.count_by("time_price_interval", _interval_count);
            self.trace.count_by(
                "time_price_departure_split",
                departure_events.len() as i64,
            );
            self.trace
                .count_by("time_price_known_area", _known_area_sum);
            self.trace.count_by(
                "time_price_future_area_milli",
                _future_area_milli_sum,
            );
            self.trace.count_by(
                "time_price_local_offer_milli",
                _local_offer_milli_sum,
            );
            self.trace
                .count_by("time_price_point_bid_milli", _point_bid_milli_sum);
            self.trace.count_by(
                "time_price_legacy_threshold_milli",
                (legacy_threshold * 1_000.0).round() as i64,
            );
            self.trace.count_by(
                "time_price_event_threshold_milli",
                (event_threshold * 1_000.0).round() as i64,
            );
            if event_threshold + 1e-12 < legacy_threshold {
                self.trace.count("time_price_lower_turn");
            } else if event_threshold > legacy_threshold + 1e-12 {
                self.trace.count("time_price_higher_turn");
            }
        }
        (legacy_threshold, event_threshold)
    }
    ...
    fn run<R: BufRead, W: Write>(
        ...
        for turn in 0..self.M {
            ...
            let (legacy_base_threshold, base_threshold) =
                local_time!(self.trace, "time_price_curve", {
                    self.dynamic_thresholds(S, duration, P, theta)
                });
            ...
            // 新価格による探索量・時計軌道の変化を混ぜない。候補が見つかった後の受否だけを
            // 比べるため、0.74 prefilter はv081の旧しきい値と完全に同じに保つ。
            let passed_price_prefilter = legacy_base_threshold == 0.0
                || q_value * optimistic_C >= 0.74 * legacy_base_threshold;
            local! {
                assert_eq!(
                    passed_price_prefilter,
                    legacy_base_threshold == 0.0
                        || q_value * optimistic_C >= 0.74 * legacy_base_threshold,
                    "event time price must not change the v081 prefilter",
                );
                self.trace.count("time_price_prefilter_legacy_turn");
            }
            if passed_price_prefilter {
                ...
                if let Some(mut choices) = normal_choices {
                    ...
                    let component_factor = self.component_threshold_factor(current_component_size);
                    let legacy_actual_threshold = legacy_base_threshold * component_factor;
                    let actual_threshold = base_threshold * component_factor;
                    let quality = q_value * compactness(P, current_perimeter);
                    let _legacy_accepted =
                        legacy_base_threshold == 0.0 || quality >= legacy_actual_threshold;
                    accepted = base_threshold == 0.0 || quality >= actual_threshold;

                    local! {
                        if _legacy_accepted {
                            self.trace.count("time_price_legacy_pass_candidate");
                        }
                        if accepted {
                            self.trace.count("time_price_event_pass_candidate");
                        }
                        if !_legacy_accepted && accepted {
                            self.trace.count("time_price_rescue_candidate");
                            self.trace.count_by(
                                "time_price_rescue_actual_fee",
                                ((V as f64) * compactness(P, current_perimeter)).round() as i64,
                            );
                            self.trace.count_by(
                                "time_price_rescue_ideal_fee",
                                ((V as f64) * optimistic_C).round() as i64,
                            );
                        } else if _legacy_accepted && !accepted {
                            self.trace.count("time_price_drop_candidate");
                            self.trace.count_by(
                                "time_price_drop_actual_fee",
                                ((V as f64) * compactness(P, current_perimeter)).round() as i64,
                            );
                            self.trace.count_by(
                                "time_price_drop_ideal_fee",
                                ((V as f64) * optimistic_C).round() as i64,
                            );
                        }
                    }
