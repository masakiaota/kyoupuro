// v016_low_r_slack2.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: R<=0.02 に限り、通常受理済みの L=L_min+2 も既存 relocation の
// 発動候補へ加える。移動が安いケースの軽微なコンパクト度損失だけを回収し、target・
// repack・採算式・ロールアウト・admission・時間予算は v014 から変えない。
...
impl Solver {
    ...
    fn run<R: BufRead, W: Write>(
        ...
        for turn in 0..self.M {
            ...
            // 受け入れ成立時は従来の slack>=4 に加え、移動が安い R<=0.02 だけ
            // slack=2 も既存の採算・ロールアウト評価へ渡す。
            let min_L = minimum_perimeter(P);
            let low_r_slack2_eligible = accepted
                && self.R_milli <= 20
                && normal
                    .as_ref()
                    .is_some_and(|placement| placement.perimeter == min_L + 2);
            local! {
                if low_r_slack2_eligible {
                    self.trace.count("low_r_slack2_eligible");
                }
            }
            ...
            if !fast_mode && reloc_worth_trying && q_value * optimistic_C >= 0.88 * base_threshold {
                local! {
                    ...
                    if low_r_slack2_eligible {
                        self.trace.count("low_r_slack2_attempt");
                    }
                }
                ...
                if move_plan.ok {
                    local! {
                        ...
                        if low_r_slack2_eligible {
                            self.trace.count("low_r_slack2_success");
                        }
                    }
                    accepted = true;
                }
