// v050_move_strong_biased.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: v017 の受理済み悪形 growth のうち回収可能利用料が大きいものだけ、
// v047 の次数 biased boundary swap で後処理する。改善形状を relocation の baseline にし、
// 局所改善と既存group移動を同じ利用料尺度で補完させる。
...
struct Solver {
    ...
    reloc_spent_sec: f64,
    /// 圧縮フェーズに費やした累積秒数。relocation 予算のサブ上限 (1/3) で抑える。
    compaction_spent_sec: f64,
    /// 各グループが最後に移動したターン。同一ターンの二重移動 (出力違反) を防ぐ。
...
impl Solver {
    ...
    /// 退去処理を行い、退去した件数を返す。圧縮フェーズの発動判定に使う。
    ...
    /// 退去駆動圧縮: 最大空き成分と分断された小成分があるとき、小成分と最大成分の
    /// 両方に隣接するグループを worst 以下の形 (fee_loss=0) で移し、跡地で空きを
    /// 接続する。移動の使途を「incoming のための場所空け」から「空き供給の秩序維持」
    /// へ広げる部品。採算は incoming を先頭に加えたロールアウト差分で判定する。
    #[allow(clippy::too_many_arguments)]
    fn try_compaction(
        &mut self,
        incoming_id: usize,
        ...
        // 圧縮は relocation 予算の 1/3 までに抑える (救済移動の時間を奪わないため)。
        if self.compaction_spent_sec >= self.timer.budget_sec(RELOC_TIME_BUDGET_RATIO) / 3.0 {
            local! {
                self.trace.count("compaction_budget_hit");
            }
            return None;
        }
        let occ = self.occupied_rows;
        ...
        let max_comp = info
            ...
            .map(|(cid, _)| cid)?;
        // 統合対象: 最大成分以外で最も大きい小成分 (4 <= size < MAX_P)。
        let mut small: Option<(usize, usize)> = None;
        for (cid, &size) in info.sizes.iter().enumerate() {
            if cid == max_comp || size < 4 || size >= MAX_P {
                continue;
            }
            if small.is_none_or(|(_, s)| size > s) {
                small = Some((cid, size));
            }
        }
        let (small_id, small_size) = small?;
        local! {
            self.trace.count("compaction_attempt");
        }

        // 小成分の代表セル・最大成分の代表セル・小成分に隣接する active グループを集める。
        let mut small_rep = usize::MAX;
        let mut max_rep = usize::MAX;
        let mut adj: Vec<usize> = Vec::new();
        const DIRS: [(isize, isize); 4] = [(-1, 0), (1, 0), (0, -1), (0, 1)];
        for id in 0..self.N * self.N {
            if info.component[id] == max_comp as isize && max_rep == usize::MAX {
                max_rep = id;
            }
            if info.component[id] == small_id as isize {
                if small_rep == usize::MAX {
                    small_rep = id;
                }
                let r = id / self.N;
                let c = id % self.N;
                for (dr, dc) in DIRS {
                    ...
                    let owner = self.owner_cell[(nr as usize) * self.N + (nc as usize)];
                    if owner >= 0 {
                        adj.push(owner as usize);
                    }
                }
            }
        }
        if small_rep == usize::MAX || max_rep == usize::MAX {
            return None;
        }
        adj.sort_unstable();
        adj.dedup();
        // 移動費の安いグループから最大 2 体試す。
        adj.sort_by_key(|&g| self.movement_cost(g));

        let start = self.timer.elapsed_sec();
        let mut result = None;
        for &g in adj.iter().take(2) {
            if !self.groups[g].active
                || self.turn_moved[g] == self.current_turn
                || self.groups[g].move_count >= 4
            {
                continue;
            }
            // g を外した盤面で小成分と最大成分が接続するか確認する。
            let mut without = occ;
            for &cell in &self.groups[g].cells {
                without[cell / self.N] &= !Self::bit_at(cell % self.N);
            }
            let info2 = self.compute_free_info(&without, false);
            if info2.component[small_rep] < 0
                || info2.component[small_rep] != info2.component[max_rep]
            {
                continue;
            }
            // g の行き先: 規則形状で、接続を塞がず断片化が最小の位置。worst 超過は
            // fee_loss (C 低下損失) を採算へ載せて許す (B-010 と同じ「禁止でなく価格」)。
            let g_P = self.groups[g].P;
            let g_worst = self.groups[g].worst_perimeter;
            let runs = self.build_run_table(&without);
            let weights = self.build_weight_data(&without, &info2, incoming_T, theta, true);
            let mut choice: Option<(Placement, f64)> = None;
            for perimeter in (minimum_perimeter(g_P)..=g_worst + 6).step_by(2) {
                let scan = self.scan_regular_level(g_P, &runs, &weights, perimeter, 4, 8);
                for mut cand in scan {
                    let cells = self.materialize(&cand, g_P);
                    let mut board = without;
                    for &cell in &cells {
                        board[cell / self.N] |= Self::bit_at(cell % self.N);
                    }
                    let binfo = self.compute_free_info(&board, false);
                    // 行き先が接続路を再び塞いだら意味がない。
                    if binfo.component[small_rep] < 0
                        || binfo.component[small_rep] != binfo.component[max_rep]
                    {
                        continue;
                    }
                    cand.explicit_cells = cells;
                    if choice
                        .as_ref()
                        .is_none_or(|(_, metric)| binfo.metric < *metric)
                    {
                        choice = Some((cand, binfo.metric));
                    }
                }
                if choice.is_some() {
                    break;
            ...
            let Some((placement, _)) = choice else {
                continue;
            };
            // 採算: incoming を先頭に加えたロールアウト差分 − 移動費 − fee_loss > 0。
            let move_cost = self.movement_cost(g);
            let fee_loss = if placement.perimeter > g_worst {
                (self.groups[g].V as f64)
                    * (compactness(g_P, g_worst) - compactness(g_P, placement.perimeter))
            } else {
                0.0
            };
            let mut board = without;
            ...
            let incoming_dur = incoming_T - incoming_S;
            let q = (incoming_V as f64) / ((incoming_P as f64) * (incoming_dur as f64).powf(0.9));
            let first = FutureArrival {
                at: incoming_S,
                dur: incoming_dur,
                P: incoming_P,
                q,
                value: q * (incoming_P as f64) * (incoming_dur as f64).powf(0.9),
            };
            let cands = [
                RolloutCandidate {
                    board: occ,
                ...
                RolloutCandidate {
                    board,
                    immediate: -(move_cost as f64) - fee_loss,
                    incoming_dep: None,
                    overrides: vec![(g, placement.explicit_cells.clone())],
                },
            ];
            ...
            if winner == 1 {
                local! {
                    self.trace.count("compaction_success");
                    self.trace.count_by("compaction_gain_sum", small_size as i64);
                }
                result = Some((g, placement));
                break;
            } else {
                local! {
                    self.trace.count("compaction_rollout_reject");
                }
            }
        }
        let spent = self.timer.elapsed_sec() - start;
        self.compaction_spent_sec += spent;
        self.reloc_spent_sec += spent;
        result
    }

    /// 圧縮移動を盤面と group 状態へ反映する。出力は run() 側で move ブロックに合成する。
    fn commit_compaction_move(&mut self, group_id: usize, placement: &Placement) {
        self.clear_group_from_board(group_id);
        let cells = placement.explicit_cells.clone();
        {
                    ...
                    while ys != 0 {
                        ...
                        for rr in 0..shape.h {
                            ...
                            for c in begin..end {
                                ...
                                // 同一ターンに圧縮で動いたグループは二重移動 (出力違反) になるため除外。
    ...
    /// extra_first: 到着列の先頭に確定情報として挿入する到着 (圧縮判定で、これから
    /// 処理する incoming 自身を含めるために使う)。None なら従来どおり。
    fn evaluate_candidates_rollout_with(
        ...
        for k in 0..ROLLOUT_SAMPLES {
            ...
            let mut arrivals = self.make_future_arrivals(incoming_id, now, theta, seed);
    ...
    /// extra は圧縮移動 (このターン既に盤面へ反映済み)。relocation の moved と
    /// 合成して 1 つの移動ブロックとして出力する。同一グループの重複は turn_moved で
    /// 発生しない前提を debug_assert で確認する。
    fn print_move_block<W: Write>(
        &self,
        writer: &mut W,
        plan: Option<&MovePlan>,
        extra: Option<&(usize, Placement)>,
    ) -> io::Result<()> {
        let moved: &[(usize, Placement)] = match plan.filter(|plan| plan.ok) {
            Some(plan) => &plan.moved,
            None => &[],
        };
        let total = moved.len() + usize::from(extra.is_some());
        if total == 0 {
            writeln!(writer, "0")?;
            writer.flush()?;
            return Ok(());
        ...
        if let Some((group_id, placement)) = extra {
            debug_assert!(moved.iter().all(|(g, _)| g != group_id));
    ...
    fn run<R: BufRead, W: Write>(
        ...
        for turn in 0..self.M {
            ...
            // 退去駆動圧縮: 空きの形が変わった (退去があった) ターンだけ、小空き成分の
            // 統合を試す。incoming の処理はすべて圧縮後の盤面で行われる。
            let mut compaction: Option<(usize, Placement)> = None;
            if expired_now > 0 && !fast_mode {
                compaction = local_time!(self.trace, "compaction", {
                    self.try_compaction(id, S, T, P, V, theta, base_threshold)
                });
                if let Some((gid, placement)) = &compaction {
                    self.commit_compaction_move(*gid, &placement.clone());
                }
            }
            ...
            if move_plan.ok {
                ...
                self.print_move_block(writer, Some(&move_plan), compaction.as_ref())?;
                ...
                self.print_move_block(writer, None, compaction.as_ref())?;
                if accepted {
