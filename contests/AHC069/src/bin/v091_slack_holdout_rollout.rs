// v091_slack_holdout_rollout.rs
#![allow(non_snake_case)]
// 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: v090 の基準探索を先に保ち、smooth の未決着な同一経済条件候補だけを
// 実測 slack 内の fresh posterior scenario で再確認する。
...
impl Solver {
    ...
    fn evaluate_candidates_rollout(
            ...
            && self.M - 1 - incoming_id >= DEEP_ROLLOUT_ARRIVALS
        {
            ...
            let minimum_slack = self.timer.budget_sec(SPARE_ROLLOUT_MIN_SLACK_RATIO);
            let base_ready = self.deep_rollout_sessions >= base_paced_limit;
            let gap_ready = gap <= DEEP_ROLLOUT_GAP_RATIO * scale;
            let session_ready = self.spare_rollout_sessions < spare_paced_limit;
            let slack_ready = real_elapsed + minimum_slack <= pace_deadline;
            local! {
            ...
            if base_ready && session_ready && gap_ready && slack_ready {
                ...
                for k in 0..SPARE_ROLLOUT_SAMPLES {
                    if self.timer.real_elapsed_sec() >= pace_deadline {
                        _budget_stopped = true;
                        break;
                    }
                    let sample_index = SPARE_ROLLOUT_SEED_OFFSET + k;
