// v069_adaptive_time_budget.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: V053の配置・admission・relocation判断を固定したまま、進捗時点の
// 全体・relocation CPU時間を完走予測し、余裕があるケースだけ通常配置候補とrepack試行を
// 段階的に拡張する。追加候補も既存のposterior rolloutで比較するため、予算不足時には
// V053と同じ候補集合へ戻る。
