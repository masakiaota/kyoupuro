// v071_articulation_growth.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。
// 中心アイデア: v069 の admission・posterior rollout・adaptive 探索水準・relocation を
// 固定したまま、通常配置の growth だけで、同じ共有辺数の frontier 候補から空き連結成分を
// 大きく切断しないセルを優先する。repack growth は従来順序のままとして効果を分離する。
