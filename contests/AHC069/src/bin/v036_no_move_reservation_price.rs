// v036_no_move_reservation_price.rs
#![allow(non_snake_case)] // 問題文の `N`, `M`, `S`, `T`, `P`, `V` を対応づけたまま使う。

// 中心アイデア: admission の滞在区間価格を、既知 active group の退去時刻を使って
// 評価時刻ごとの予約量で補正する。再移動は一切行わない。
...
impl Solver {
    ...
    /// `time` より後まで滞在する既知 active group の面積合計。
    /// 現在の occupied count ではなく、評価時刻までに確実に退去する面積を除いた予約量を使う。
    fn reserved_area_at(&self, time: f64) -> usize {
        self.departures
            .iter()
            .filter_map(|&Reverse((T, group_id))| {
                let group = &self.groups[group_id];
                (group.active && (T as f64) > time).then_some(group.P)
            })
            .sum()
    }

    /// v035 の旧しきい値と、既知退去列で各積分点を補正した新しきい値を返す。
    /// 旧値は prefilter の探索量を変えないため、v035 と演算順も含めて同じ計算を保つ。
    fn dynamic_thresholds(
        ...
        let old_threshold = threshold * multiplier;

        // 同じ Gauss-Legendre 3 点を使い、各時刻の local bid をその時刻まで残る
        // 既知予約量で補正する。到着時 occupancy の一括補正は新値へは掛けない。
        let times = [
            (S as f64) + X0 * D,
            (S as f64) + X1 * D,
            (S as f64) + X2 * D,
        ...
        let mut reservation_average_bid = 0.0;
        let mut reserved_areas = [0_usize; 3];
        let mut target_areas = [0.0_f64; 3];
        for point in 0..3 {
            let time = times[point];
            let reserved = self.reserved_area_at(time);
            let target = self
                .effective_capacity
                .min(offered_area * Self::boundary_load_factor(time, theta));
            let point_error =
                ((reserved as f64) + 0.5 * (P as f64) - target) / self.effective_capacity.max(1.0);
            let point_multiplier = (0.70 * point_error).exp().clamp(0.82, 1.80);
            let local_bid = self.local_bid_at(time, theta, offered_area);
            reservation_average_bid += weights[point] * local_bid * point_multiplier;
            reserved_areas[point] = reserved;
            target_areas[point] = target;
        }
        let reservation_threshold = reservation_average_bid * (D / theta).powf(0.1);

        local! {
            self.trace.count("reservation_price_turn");
            for point in 0..3 {
                self.trace.count_by(
                    "reservation_reserved_area",
                    reserved_areas[point] as i64,
                );
                self.trace.count_by(
                    "reservation_target_area",
                    target_areas[point].round() as i64,
                );
            }
            self.trace.count_by(
                "reservation_threshold_old_milli",
                (old_threshold * 1_000.0).round() as i64,
            );
            self.trace.count_by(
                "reservation_threshold_new_milli",
                (reservation_threshold * 1_000.0).round() as i64,
            );
        }

        (old_threshold, reservation_threshold)
    }
    ...
    fn run<R: BufRead, W: Write>(
        ...
        for turn in 0..self.M {
            ...
            let (legacy_base_threshold, reservation_base_threshold) =
            ...
            let mut normal = None;
            // 配置探索量を v035 と一致させるため、prefilter だけは旧しきい値を使う。
            ...
            local! {
                assert_eq!(
                    ...
                    legacy_base_threshold == 0.0
                        || q_value * optimistic_C >= 0.74 * legacy_base_threshold,
                    "reservation price must not change the v035 prefilter",
                );
            }
            if passed_price_prefilter {
                ...
                if let Some(mut choices) = normal_choices {
                    ...
                    let reservation_actual_threshold =
                        reservation_base_threshold * component_factor;
                    ...
                    accepted = reservation_base_threshold == 0.0
                        || quality >= reservation_actual_threshold;

                    // 旧/新 admission の比較は、同じ実配置候補が存在する場合に限る。
                    local! {
                        if _legacy_accepted {
                            self.trace.count("reservation_price_old_pass_candidate");
                        }
                        if accepted {
                            self.trace.count("reservation_price_new_pass_candidate");
                        }
                        if !_legacy_accepted && accepted {
                            self.trace.count("reservation_price_rescue_candidate");
                            self.trace.count_by(
                                "reservation_price_rescue_actual_fee",
                                ((V as f64) * compactness(P, current_perimeter)).round() as i64,
                            );
                            self.trace.count_by(
                                "reservation_price_rescue_ideal_fee",
                                ((V as f64) * optimistic_C).round() as i64,
                            );
                        } else if _legacy_accepted && !accepted {
                            self.trace.count("reservation_price_drop_candidate");
                            self.trace.count_by(
                                "reservation_price_drop_actual_fee",
                                ((V as f64) * compactness(P, current_perimeter)).round() as i64,
                            );
                            self.trace.count_by(
                                "reservation_price_drop_ideal_fee",
                                ((V as f64) * optimistic_C).round() as i64,
                            );
                        }
                    }
