// v022_cost_weight.rs
// v021_expand_quality の評価式 score = W*T + 木距離和 の手数換算重み W だけを
// 楽観値 10(1 手 = 最大 shift ぶん)から実測消化率に基づく 4 へ較正する
// 1 定数の最小差分実験(Q-C12)。v021 の 0000 実測では 1 手あたり約 4.4 距離を
// 消化しており、重み 10 は手数を過大に嫌って potential 削減の機会を捨てていた
// 疑いがある。遷移・拡大・探索は v021 と同一。
use std::{
