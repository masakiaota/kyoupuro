// v023_fixed_beam96.rs
// v011_support_expand の探索スケジュールだけを固定幅 96 の単一 pass へ置き換える実験(IDEA-D21)。
// 幅 1/2/8/32/... の反復再始動と未完 pass の破棄をなくし、同じ状態・候補・評価・
// 支持拡大へ計算時間を集中させる。pass_seed は 1 に固定し、fallback は置かない。
use std::{
...
const FIXED_BEAM_WIDTH: usize = 96;
...
fn main() {
    ...
    // 反復幅拡大は行わず、初期状態から固定幅 96 の pass を一度だけ完走させる。
    let best = beam_pass(
        &input,
        &initial,
        &tree,
    ...
    trace.count_by("fixed_beam_width", FIXED_BEAM_WIDTH as i64);
    trace.count_by("fixed_t", best_cost as i64);
