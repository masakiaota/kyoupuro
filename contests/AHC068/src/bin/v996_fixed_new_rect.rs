...
fn main() {
    ...
    for p in order {
        ...
        let mut dist = vec![usize::MAX; N2 * (out.len() + 1)];
        let s = source_of[p];
        ...
        let mut goal_id = !0;
        let mut goal_dist = usize::MAX;
        while let Some((q, t, d, id)) = que.pop_front() {
            if dist[t * N2 + q] != d {
                continue;
            }
            if goal_id != !0 && d > goal_dist {
                break;
            }
            if q == p && t == out.len() {
...
fn existing_branches(op: FlexRect, pos: usize) -> Vec<Branch> {
    let (axis, perp) = axis_perp(op, pos);
    let k = op.k();
    let mut out = Vec::new();
    ...
    if let Some((lo, hi)) = intersect_signed(
        ...
        out.push(Branch { to: pos, decision });
    }
...
fn build_flex_rect(mv: NewMove, blocked: &[bool], input: &Input) -> FlexRect {
    ...
    // S3 の反証用簡素化では操作族を広げず、注目カードだけを含む最初の合法長方形へ固定する。
    for slide in start_lo..=start_hi {
        let op = FlexRect {
            d: mv.d,
            k: mv.k,
            slide_lo: slide as u8,
            slide_hi: slide as u8,
            min_l: core as u8,
            min_r: (core + 1) as u8,
            max_l: core as u8,
            max_r: (core + 1) as u8,
        };
        if rect_legal_blocked(representative(op), blocked, input) {
            return op;
        }
    }
    panic!("new move must have a legal rectangle");
}
