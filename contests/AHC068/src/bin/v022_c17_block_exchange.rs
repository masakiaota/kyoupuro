// v022_c17_block_exchange.rs
// v011_support_expand の搬送hopだけを IDEA-C17 で強化する単独差分実験。
// 同じ from -> to を実現する全ての長手位置について、anchor を含む合法・全activeな
// 支持区間の厳密 min-sum を求め、矩形全体の木距離deltaが最小の帯を共同選択する。
// 固定木・葉確定順・搬送経路・評価式(10*T + 木距離和)・探索・時間配分は v011 と同一。
use std::{
