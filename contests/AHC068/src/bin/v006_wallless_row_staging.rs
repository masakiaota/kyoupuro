...
/// 壁なし盤面でだけ使う。許可区間内の等長隣接ブロック交換により、
/// 指定位置を移す最短手順を位置グラフ上の BFS で復元する。
fn line_swap_path(
    ...
    assert!(interval_start <= source && source < interval_end);
    assert!(interval_start <= target && target < interval_end);
    if source == target {
        return Vec::new();
    }

    let mut previous = [usize::MAX; N];
    let mut previous_swap = [None; N];
    let mut queue = VecDeque::new();
    previous[source] = source;
    queue.push_back(source);

    while let Some(position) = queue.pop_front() {
        for start in interval_start..interval_end {
            for distance in 1..=(interval_end - start) / 2 {
                let middle = start + distance;
                let end = middle + distance;
                let next = if (start..middle).contains(&position) {
                    position + distance
                ...
                previous_swap[next] = Some(LineSwap { start, distance });
                if next == target {
                    queue.clear();
                    break;
                }
    ...
    while position != source {
        path.push(previous_swap[position].unwrap());
...
fn find_card(state: &BoardState, card: usize) -> usize {
    (0..CELLS)
        .find(|&cell| state.card_at(cell) == card)
        .unwrap()
}
...
fn move_card_horizontally(
    ...
    let source = find_card(state, card);
    let row = source / N;
    let source_col = source % N;
    for swap in line_swap_path(interval_start, N, source_col, target_col) {
        apply_and_record(
            state,
            input,
            operations,
    ...
    assert_eq!(find_card(state, card), row * N + target_col);
}

fn move_card_vertically(
    state: &mut BoardState,
    input: &Input,
    operations: &mut Vec<Operation>,
    card: usize,
    ...
    let source = find_card(state, card);
    assert_eq!(source % N, col);
    let source_row = source / N;
    for swap in line_swap_path(interval_start, N, source_row, target_row) {
        apply_and_record(
            state,
            input,
            operations,
    ...
    assert_eq!(find_card(state, card), target_row * N + col);
}

fn is_wallless(input: &Input) -> bool {
    !input
        .vertical_walls
        .iter()
        .flatten()
...
/// 行 i のカードを一度行 i+1 に揃えてから、幅 N の一手で確定行へ送る。
/// 横移動は staging 行の確定済み prefix を、縦移動は過去の確定行を触らない。
fn build_row_staging_candidate(input: &Input) -> Vec<Operation> {
    assert!(is_wallless(input));
    ...
    for i in 0..N - 2 {
        ...
        for j in 0..N {
            let card = i * N + j;
            let source_row = find_card(&state, card) / N;
            assert!(source_row >= i);
            let horizontal_start = if source_row == staging_row { j } else { 0 };
            move_card_horizontally(
                &mut state,
                input,
                &mut operations,
                card,
            ...
            move_card_vertically(&mut state, input, &mut operations, card, staging_row, j, i);
        }
        assert!((0..N).all(|j| state.card_at(staging_row * N + j) == i * N + j));
        ...
        assert!((0..N).all(|j| state.card_at(i * N + j) == i * N + j));
    }

    // 行 18 は全 20 列を明示的に確定する。最後の対象が行 19 に残る場合もある。
    let final_target_row = N - 2;
    for j in 0..N {
        let card = final_target_row * N + j;
        let source_row = find_card(&state, card) / N;
        assert!(source_row == final_target_row || source_row == N - 1);
        let horizontal_start = if source_row == final_target_row { j } else { 0 };
        move_card_horizontally(
            &mut state,
            input,
            &mut operations,
        ...
        assert_eq!(state.card_at(final_target_row * N + j), card);
    }

    // 正しいカード集合だけでは順序は決まらないため、最終行も suffix 内で明示ソートする。
    let last_row = N - 1;
    for j in 0..N {
        let card = last_row * N + j;
        assert_eq!(find_card(&state, card) / N, last_row);
        move_card_horizontally(&mut state, input, &mut operations, card, j, j);
        assert_eq!(state.card_at(last_row * N + j), card);
    }
...
fn main() {
    ...
    // v003 の timed search は凍結したまま先に完走させ、残り時間で独立候補を比較する。
    let mut row_staging_ops = 0_usize;
    let mut row_staging_selected = false;
    if is_wallless(&input) {
        trace.count_by("row_staging_cases", 1);
        trace.count_by("staged_rows", (N - 2) as i64);
        trace.count_by("staged_cards", ((N - 2) * N) as i64);
        trace.count_by("row_commit_ops", (N - 2) as i64);

        let row_operations = build_row_staging_candidate(&input);
        row_staging_ops = row_operations.len();
        complete_candidates += 1;
        if row_operations.len() < best_operations.len() {
            best_operations = row_operations;
            best_prefix_ops = 0;
            best_tail_ops = 0;
            best_updates += 1;
            row_staging_selected = true;
        }
    } else {
        trace.count_by("row_staging_cases", 0);
        trace.count_by("staged_rows", 0);
        trace.count_by("staged_cards", 0);
        trace.count_by("row_commit_ops", 0);
    }
    trace.count_by("row_staging_ops", row_staging_ops as i64);
    trace.count_by("row_staging_selected", row_staging_selected as i64);
    trace.count_by("row_staging_final_e", 0);
