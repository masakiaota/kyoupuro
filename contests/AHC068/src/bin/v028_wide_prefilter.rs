// v028_wide_prefilter.rs
// v026 の幅32以下を保ち、未完予測の幅128以降だけ葉候補を安価proxy上位16件へ絞る。
use std::{
...
const LEAF_PREFILTER_LIMIT: usize = 16;
const WIDE_BEAM_START: usize = 128;
const FULL_PASS_SAFETY: f64 = 1.05;
...
struct MechanismStats {
    ...
    wide_prefilter_passes: u64,
    wide_full_passes: u64,
    leaf_proxy_seen: u64,
    leaf_proxy_kept: u64,
    leaf_proxy_capped_parents: u64,
...
impl MechanismStats {
    fn flush(&self, trace: &mut TraceStats) {
        ...
        trace.count_by("wide_prefilter_passes", self.wide_prefilter_passes as i64);
        trace.count_by("wide_full_passes", self.wide_full_passes as i64);
        trace.count_by("leaf_proxy_seen", self.leaf_proxy_seen as i64);
        trace.count_by("leaf_proxy_kept", self.leaf_proxy_kept as i64);
        trace.count_by(
            "leaf_proxy_capped_parents",
            self.leaf_proxy_capped_parents as i64,
        );
    }
}
...
/// 固定木経路を最大 shift ごとに束ねた楽観搬送手数。
#[inline]
fn optimistic_transport_hops(state: &SearchState, tree: &TreeData, target: usize) -> u16 {
    let mut current = state.position[target] as usize;
    let mut hops = 0_u16;
    while current != target {
        let first = tree.next[current * CELLS + target] as usize;
        ...
        let mut at = first;
        let mut straight_len = 1;
        while straight_len < MAX_SINGLE_AXIS_SHIFT && at != target {
            let next = tree.next[at * CELLS + target] as usize;
            let next_step_i = next as i32 / N as i32 - at as i32 / N as i32;
            let next_step_j = next as i32 % N as i32 - at as i32 % N as i32;
            if (next_step_i, next_step_j) != (step_i, step_j) {
                break;
            }
            at = next;
            straight_len += 1;
...
fn beam_pass(
    ...
    width: usize,
    pass_seed: u64,
    leaf_prefilter_limit: Option<usize>,
    ...
    let mut leaf_proxies = Vec::with_capacity(64);
    ...
    for _depth in 0..CELLS - 1 {
        ...
        let mut candidates =
            Vec::with_capacity(beam.len().saturating_mul(leaf_prefilter_limit.unwrap_or(8)));
        for (parent_index, parent) in beam.iter_mut().enumerate() {
            ...
            for leaf in 0..CELLS {
                ...
                let (proxy, proxy_tie) = if leaf_prefilter_limit.is_some() {
                    let current = parent.position[leaf] as usize;
                    let distance = tree.distance[current * CELLS + leaf] as i32;
                    let hops = optimistic_transport_hops(parent, tree, leaf) as i32;
                    stats.leaf_proxy_seen += 1;
                    (
                        EVAL_COST_WEIGHT as i32 * hops - distance,
                        tie_hash(
                            parent.hash_a
                                ^ parent.hash_b
                                ^ (leaf as u64).wrapping_mul(0x9e37_79b9_7f4a_7c15),
                            pass_seed,
                        ),
                    )
                } else {
                ...
                leaf_proxies.push((proxy, proxy_tie, leaf as u16));
            }

            if let Some(limit) = leaf_prefilter_limit {
                if leaf_proxies.len() > limit {
                    leaf_proxies.select_nth_unstable(limit);
                    leaf_proxies.truncate(limit);
                    stats.leaf_proxy_capped_parents += 1;
                }
                leaf_proxies.sort_unstable();
                stats.leaf_proxy_kept += leaf_proxies.len() as u64;
            }

            for &(_, _, leaf) in &leaf_proxies {
...
fn main() {
    ...
    let mut last_completed_pass_sec = None;
    loop {
        time_keeper.force_update();
        if time_keeper.elapsed_sec() >= search_deadline_sec {
            break;
        }
        let remaining_sec = search_deadline_sec - time_keeper.elapsed_sec();
        let use_prefilter = width >= WIDE_BEAM_START
            && (width > WIDE_BEAM_START
                || last_completed_pass_sec
                    .map_or(true, |last| last * 4.0 * FULL_PASS_SAFETY > remaining_sec));
        if width >= WIDE_BEAM_START {
            if use_prefilter {
                stats.wide_prefilter_passes += 1;
            } else {
                stats.wide_full_passes += 1;
            }
        }
        ...
        let result = beam_pass(
            ...
            width,
            pass_seed,
            use_prefilter.then_some(LEAF_PREFILTER_LIMIT),
            true,
            search_deadline_sec,
            &mut time_keeper,
            &mut stats,
        ...
        last_completed_pass_sec = Some(pass_started.elapsed().as_secs_f64());
    ...
    #[cfg(feature = "local")]
    {
        ...
        if stats.leaf_proxy_seen > 0 {
            assert!(stats.leaf_proxy_kept * 5 <= stats.leaf_proxy_seen * 3);
        }
    }
