// v024_warp_pairwise.rs
// v012_overlap_warp の選択規準を修正した再挑戦(IDEA-C18 の再開条件)。
// v012 の敗因は per-distance 正規化が通常側 2 手目の支持拡大益を無視したこと。
// 本版は直線区間 L>=12 のときだけ、同一到達点(from+2d)への
//   (a) 通常 2 手プラン: 1 手目(拡大済み)を仮適用し 2 手目の best_mapping+拡大まで実測
//   (b) 重ね合成ワープ 2 手(d=min(9,L/2)>=6): 等間隔 3 セル巡回、帯内は無傷
// を木距離和変化のみで比較する(到達点・手数が同一なので完全に公平)。
// 通常 1 手で届く範囲(2d<=10)はワープ対象外。評価式・拡大・探索は v022 と同一。
use std::{
...
/// ワープを検討する最小シフト。2d<=10 は通常 1 手で届き手数で必ず劣るため対象外。
const WARP_MIN_D: usize = 6;
...
fn apply_transport(
    ...
    while current != target {
        ...
        // 長い直線では、同一到達点(from+2d)への「通常 2 手プラン」と「ワープ 2 手」を
        // 木距離和変化のみで比較する(到達点・手数が同一なので公平)。
        let mut warp_choice = None;
        if straight_len >= 2 * WARP_MIN_D {
                ...
                try_warp(state, tree, zobrist, current, step_i, step_j, straight_len)
            {
                ...
                // 通常側: いま選んだ 1 手目(拡大済み)を仮適用し、同じ到達点への
                // 2 手目(best_mapping + 拡大)の差分まで実測してから戻す。
                let delta_first = operation_delta_potential(state, tree, op);
                apply_operation(state, tree, zobrist, op);
                let normal_two_delta = best_mapping_operation(input, state, tree, next_position, warp_to)
                    .map(|second| {
                        let (second, _, _) = expand_support(input, state, tree, second);
                        delta_first + operation_delta_potential(state, tree, second)
                    });
                apply_operation(state, tree, zobrist, op);
                if let Some(normal_delta) = normal_two_delta {
                    if warp_delta < normal_delta {
