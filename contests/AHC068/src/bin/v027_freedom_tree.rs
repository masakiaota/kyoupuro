// v027_freedom_tree.rs
// v022_cost_weight の全域木だけを、セルが参加できる合法操作数(自由度)を使う
// 最短路木へ置き換える。自由度の高いセルを根とし、同じ BFS 距離層では
// 自由度の高い親から子を獲得することで、細通路と外周を木の末端へ集める。
// 葉剥がしは末端から進むため、swap の選択肢が少ない場所を先に保護確定し、
// 完成領域を端から内側へ進める。搬送・支持拡大・評価・探索は v022 と同一。
use std::{
...
/// そのセルを実際に動かせる合法操作の総数を数える。
/// 外周や細通路では参加できる長方形が減るため、この値が swap 自由度の代理になる。
...
fn build_spanning_tree(input: &Input) -> TreeData {
    ...
    // 同自由度なら外周から遠いセルを根にし、最後の同点は座標順で固定する。
    let root = (0..CELLS)
        .max_by_key(|&cell| (freedom[cell], edge_depth(cell), CELLS - cell))
        .unwrap();
    let mut parent = [NO_CELL; CELLS];
    parent[root] = root as u16;
    let mut open_distance = [u16::MAX; CELLS];
    open_distance[root] = 0;
    let mut frontier = vec![root];

    // BFS の距離層は保ち、同じ層では自由度の高いセルを先に展開する。
    // そのセルが子を引き受けるので、自由度の低いセルほど葉になりやすい。
    while !frontier.is_empty() {
        frontier.sort_unstable_by(|&a, &b| {
            (freedom[b], edge_depth(b), CELLS - b).cmp(&(freedom[a], edge_depth(a), CELLS - a))
        });
        ...
        for &cell in &frontier {
            ...
            for &next in &neighbors[..len] {
                if parent[next] == NO_CELL {
                    ...
                    open_distance[next] = open_distance[cell] + 1;
    ...
    let mut shortest_distance = [u16::MAX; CELLS];
    shortest_distance[root] = 0;
    let mut shortest_queue = VecDeque::with_capacity(CELLS);
    shortest_queue.push_back(root);
    while let Some(cell) = shortest_queue.pop_front() {
        let (neighbors, len) = open_neighbors(input, cell);
        for &next in &neighbors[..len] {
            if shortest_distance[next] == u16::MAX {
                shortest_distance[next] = shortest_distance[cell] + 1;
                shortest_queue.push_back(next);
            }
        }
    }
    assert_eq!(open_distance, shortest_distance);
    for cell in 0..CELLS {
        if cell != root {
            assert_eq!(
                open_distance[parent[cell] as usize] + 1,
                open_distance[cell]
            );
        }
    }
...
fn record_peel_profile(order: &[u16], tree: &TreeData, trace: &mut TraceStats) {
    const PROFILE_SIZE: usize = 100;
    assert!(order.len() >= PROFILE_SIZE * 2);
    ...
    let late_freedom_sum: u64 = late
        ...
        .map(|&cell| tree.freedom[cell as usize] as u64)
        .sum();
    let early_edge_depth_sum: usize = early.iter().map(|&cell| edge_depth(cell as usize)).sum();
    let late_edge_depth_sum: usize = late.iter().map(|&cell| edge_depth(cell as usize)).sum();

    trace.count_by("early100_freedom_sum", early_freedom_sum as i64);
    trace.count_by("late100_freedom_sum", late_freedom_sum as i64);
    trace.count_by(
        "freedom_profile_pass",
        (early_freedom_sum < late_freedom_sum) as i64,
    );
    trace.count_by("early100_edge_depth_sum", early_edge_depth_sum as i64);
    trace.count_by("late100_edge_depth_sum", late_edge_depth_sum as i64);
    trace.count_by(
        "edge_profile_pass",
        (early_edge_depth_sum < late_edge_depth_sum) as i64,
    );
    trace.count_by("freedom_tree_root", tree.root as i64);
    trace.count_by("freedom_min", *tree.freedom.iter().min().unwrap() as i64);
    trace.count_by("freedom_max", *tree.freedom.iter().max().unwrap() as i64);
}
...
fn main() {
    ...
    record_peel_profile(&best_order, &tree, &mut trace);
