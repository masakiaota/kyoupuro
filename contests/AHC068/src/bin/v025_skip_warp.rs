// v025_skip_warp.rs
// 重ね合成ワープ(IDEA-C18)の第 2 応用: 保護制約の 3 セル緩和による直線短絡。
// ワープ 2 手は完了後に等間隔 3 セル(from/mid/to)以外を完全復元するため、
// active の要求を帯全体から 3 セルだけへ緩められる。current と target が同軸の
// とき、間の inactive(確定済み)セルや木経路外の領域を「跨ぐ」直線ワープを候補化し、
// score 増分(手数×W + 木距離和変化)が通常 1 手より小さいときに適用する。
// 固定木の蛇行(木距離 >> 直線距離)と終盤の確定領域迂回を刈るのが狙いで、
// 評価式(固定木距離の差分更新)・支持拡大・探索は v022 と同一。
use std::{
...
/// 確定セル通過の直線短絡ワープ(IDEA-C18 第 2 応用)。current と target が同軸の
/// とき、同じ細長帯を 1 セルずらして 2 連発するワープで target 方向へ 2d 跳ぶ。
/// 正味の置換は等間隔 3 セル (from, mid, to) の巡回で帯内の他セルは完全復元される
/// ため、active の要求は 3 セルだけでよく、間の inactive(確定済み)セルは跨げる。
/// 帯の壁なしは必須(壁は越えられない)。戻り値は (op 2 つ, 到達セル, 2 手合計の
/// 木距離和変化, 跨いだ inactive セル数)。
fn try_skip_warp(
    input: &Input,
    state: &mut SearchState,
    tree: &TreeData,
    ...
    current: usize,
    target: usize,
) -> Option<([Operation; 2], usize, i32, u64)> {
    let cur_row = current / N;
    let cur_col = current % N;
    let tgt_row = target / N;
    let tgt_col = target % N;

    let (op_first, op_second, to, mid) = if cur_col == tgt_col {
        let dist_axis = cur_row.abs_diff(tgt_row);
        if dist_axis < 4 {
            return None;
        }
        let d = (dist_axis / 2).min(MAX_SINGLE_AXIS_SHIFT - 1);
        let band = 2 * d;
        if tgt_row > cur_row {
            let to = current + band * N;
            let a = Operation { direction: Direction::Vertical, r: cur_row, c: cur_col, h: band, w: 1 };
            let b = Operation { direction: Direction::Vertical, r: cur_row + 1, c: cur_col, h: band, w: 1 };
            (a, b, to, current + d * N)
        } else {
            let to = current - band * N;
            let to_row = to / N;
            let a = Operation { direction: Direction::Vertical, r: to_row + 1, c: cur_col, h: band, w: 1 };
            let b = Operation { direction: Direction::Vertical, r: to_row, c: cur_col, h: band, w: 1 };
            (a, b, to, current - d * N)
        }
    } else if cur_row == tgt_row {
        let dist_axis = cur_col.abs_diff(tgt_col);
        if dist_axis < 4 {
            return None;
        }
        let d = (dist_axis / 2).min(MAX_SINGLE_AXIS_SHIFT - 1);
        ...
        if tgt_col > cur_col {
            let to = current + band;
            let a = Operation { direction: Direction::Horizontal, r: cur_row, c: cur_col, h: 1, w: band };
            let b = Operation { direction: Direction::Horizontal, r: cur_row, c: cur_col + 1, h: 1, w: band };
            (a, b, to, current + d)
        } else {
            let to = current - band;
            let to_col = to % N;
            let a = Operation { direction: Direction::Horizontal, r: cur_row, c: to_col + 1, h: 1, w: band };
            let b = Operation { direction: Direction::Horizontal, r: cur_row, c: to_col, h: 1, w: band };
            (a, b, to, current - d)
        }
    } else {
        return None;
    ...
    // active の要求は巡回で実際に動く 3 セルのみ。帯の壁なしは両 op の合法性で判定する。
    if !state.is_active(mid) || !state.is_active(to) {
        return None;
    }
    if !is_legal_operation(input, op_first) || !is_legal_operation(input, op_second) {
        return None;
    }

    // 跨いだ inactive セル数(発火局面の観測用)。
    let lo = current.min(to);
    let hi = current.max(to);
    let stride = if cur_col == tgt_col { N } else { 1 };
    let mut skipped = 0;
    let mut cell = lo;
    while cell <= hi {
    ...
    // 仮説の対象局面に限定する: inactive を実際に跨ぐか、木経路がこの直線より
    // 遠回り(蛇行)しているときだけ候補にする。跨ぎゼロ・蛇行なしの局面は通常
    // 2 手 + 支持拡大とほぼ等価で、1 手 vs 2 手の greedy 比較が過剰選択を招く
    // (v012/v024 の学び)ため対象外とする。
    let band_len = hi.abs_diff(lo) / stride;
    if skipped == 0 && (tree.distance[current * CELLS + to] as usize) <= band_len {
        return None;
    }

    // 2 手合計の正確な木距離和変化は、1 手目を仮適用してから 2 手目を測る。
    // undo は対合性から同じ操作の再適用でよい(inactive セルも 2 手で完全復元される)。
    ...
    apply_operation(state, tree, zobrist, op_first);
    Some(([op_first, op_second], to, delta_first + delta_second, skipped))
}
...
fn apply_transport(
    ...
    while current != target {
        ...
        // 同軸の target へは、確定セルを跨ぐ直線短絡ワープを検討する。
        // 2 手 vs 1 手を、ビームが最終的に使う score 増分そのもので比較する
        // (到達点の差は木距離和の変化 = 注目カードの寄与に正確に反映される)。
        let mut skip_choice = None;
        if let Some((skip_ops, skip_to, skip_delta, skipped)) =
            try_skip_warp(input, state, tree, zobrist, current, target)
        {
            stats.skip_attempts += 1;
            let normal_delta = operation_delta_potential(state, tree, op);
            if 2 * (EVAL_COST_WEIGHT as i64) + (skip_delta as i64)
                < (EVAL_COST_WEIGHT as i64) + (normal_delta as i64)
            {
                skip_choice = Some((skip_ops, skip_to, skipped));
            }
        }

        if let Some((skip_ops, skip_to, skipped)) = skip_choice {
            stats.skip_selected += 1;
            stats.skip_over_inactive += skipped;
            for skip_op in skip_ops {
                touched_pairs += apply_operation(state, tree, zobrist, skip_op);
                scratch.push(skip_op);
            }
            current = state.position[target] as usize;
            assert_eq!(current, skip_to);
        } else {
            if added > 0 {
...
impl MechanismStats {
    fn flush(&self, trace: &mut TraceStats) {
        ...
        trace.count_by("skip_attempts", self.skip_attempts as i64);
        trace.count_by("skip_selected", self.skip_selected as i64);
        trace.count_by("skip_over_inactive", self.skip_over_inactive as i64);
    }
}
