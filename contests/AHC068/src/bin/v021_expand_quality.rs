// v021_expand_quality.rs
// v011_support_expand の「拡大の選択品質」だけを変える最小差分実験。
// (a) expand_support の停止則を「累積が負の間だけ」の貪欲から、合法な限り走査して
//     累積 argmin 位置まで採用する固定端 min-sum へ(正の行を挟んで先の負まで届く)。
// (b) best_mapping_operation の帯位置比較に、両隣 1 本ずつの拡大差分の負部分
//     (expand_probe)を加算し、「拡大しやすい帯位置」を選べるようにする。
// 適用時の拡大・評価式(10*T + 木距離和)・探索(反復幅拡大ビーム)は v011 と同一。
use std::{
