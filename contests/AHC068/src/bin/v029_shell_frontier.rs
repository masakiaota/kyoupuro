// v029_shell_frontier.rs
// v026_joint_block_weight の固定最短路木と厳密共同支持選択を保ち、確定する葉を
// 外部に露出した最外層へ直接制約する。完成領域を外周から連続して広げることで、
// 中央 active 領域に穴を作らず、大きな長方形による一括輸送の余地を温存する。
// 同じ外周層では、参加できる合法操作数が少ない細通路セルを先に確定する。
use std::{
...
#[inline]
fn edge_depth(cell: usize) -> usize {
    let i = cell / N;
    let j = cell % N;
    i.min(j).min(N - 1 - i).min(N - 1 - j)
}
...
struct SearchState {
    ...
    degree: [u8; CELLS],
    active_count: u16,
    // 同じ深さのbeam状態を外周から内側へ辞書順比較する累積量。
    frontier_violations: u16,
    shell_prefix: u16,
    freedom_prefix: u64,
    operations_used: u32,
    potential: i32,
...
/// 盤外または既に確定したセルへ幾何的に接していれば、完成領域の外向き境界にある。
/// 壁越しでも、そのセルをinactiveにしたとき中央へ新しい孤立穴は作らない。
#[inline]
fn is_frontier_exposed(state: &SearchState, cell: usize) -> bool {
    let i = cell / N;
    let j = cell % N;
    i == 0
        || j == 0
        || i + 1 == N
        || j + 1 == N
        || !state.is_active(cell - N)
        || !state.is_active(cell + N)
        || !state.is_active(cell - 1)
        || !state.is_active(cell + 1)
}
...
fn remove_leaf(
    ...
    let was_exposed = is_frontier_exposed(state, leaf);
    state.frontier_violations += (!was_exposed) as u16;
    state.shell_prefix += edge_depth(leaf) as u16;
    state.freedom_prefix += tree.freedom[leaf] as u64;
...
fn verify_state(state: &SearchState, tree: &TreeData, zobrist: &Zobrist) {
    ...
    for cell in 0..CELLS {
        ...
        if state.is_active(cell) {
            ...
            assert_eq!(card, cell);
            shell_prefix += edge_depth(cell) as u16;
    ...
    assert_eq!(shell_prefix, state.shell_prefix);
    assert_eq!(freedom_prefix, state.freedom_prefix);
    assert!(state.frontier_violations <= CELLS as u16 - state.active_count);
    assert_eq!(
        state
            .active
            .iter()
...
struct MechanismStats {
    ...
    best_updates: u64,
    frontier_leaf_seen: u64,
    frontier_leaf_eligible: u64,
    frontier_exposed_parents: u64,
    frontier_nonexposed_parents: u64,
...
impl MechanismStats {
    fn flush(&self, trace: &mut TraceStats) {
        ...
        trace.count_by("frontier_leaf_seen", self.frontier_leaf_seen as i64);
        trace.count_by("frontier_leaf_eligible", self.frontier_leaf_eligible as i64);
        trace.count_by(
            "frontier_exposed_parents",
            self.frontier_exposed_parents as i64,
        );
        trace.count_by(
            "frontier_nonexposed_parents",
            self.frontier_nonexposed_parents as i64,
        );
...
fn beam_pass(
    ...
    for _depth in 0..CELLS - 1 {
        ...
        for (parent_index, parent) in beam.iter_mut().enumerate() {
            ...
            let mut min_any_shell = usize::MAX;
            let mut min_exposed_shell = usize::MAX;
            for leaf in 0..CELLS {
                if parent.is_active(leaf) && parent.degree[leaf] == 1 {
                    leaf_count += 1;
                    min_any_shell = min_any_shell.min(edge_depth(leaf));
                    if is_frontier_exposed(parent, leaf) {
                        min_exposed_shell = min_exposed_shell.min(edge_depth(leaf));
                    }
                }
            }
            assert!(leaf_count > 0);
            stats.frontier_leaf_seen += leaf_count as u64;
            let require_exposed = min_exposed_shell != usize::MAX;
            let eligible_shell = if require_exposed {
                stats.frontier_exposed_parents += 1;
                min_exposed_shell
            } else {
                stats.frontier_nonexposed_parents += 1;
                min_any_shell
            };
            ...
            for leaf in 0..CELLS {
                ...
                let exposed = is_frontier_exposed(parent, leaf);
                if (require_exposed && !exposed) || edge_depth(leaf) != eligible_shell {
                    continue;
                }
                stats.frontier_leaf_eligible += 1;
                ...
                let (neighbor, was_exposed) = remove_leaf(parent, tree, zobrist, leaf);
                let candidate = Candidate {
                    frontier_violations: parent.frontier_violations,
                    shell_prefix: parent.shell_prefix,
                ...
                restore_leaf(parent, tree, zobrist, leaf, neighbor, was_exposed);
        ...
        for (selected_index, candidate) in selected.into_iter().enumerate() {
            ...
            assert_eq!(child.frontier_violations, candidate.frontier_violations);
            assert_eq!(child.shell_prefix, candidate.shell_prefix);
    ...
    let best_index = (0..beam.len())
        .min_by_key(|&index| {
            (
                beam[index].frontier_violations,
                beam[index].shell_prefix,
    ...
    Some(PassResult {
        ...
        frontier_violations: best.frontier_violations,
        shell_prefix: best.shell_prefix,
...
fn record_shell_profile(order: &[u16], tree: &TreeData, trace: &mut TraceStats) {
    const PROFILE_SIZE: usize = 100;
    const OUTER_RING_SIZE: usize = 4 * (N - 1);
    assert!(order.len() >= PROFILE_SIZE);
    let early = &order[..PROFILE_SIZE];
    let late = &order[order.len() - PROFILE_SIZE..];
    let first76_boundary = order[..OUTER_RING_SIZE]
        .iter()
        .filter(|&&cell| edge_depth(cell as usize) == 0)
        .count();
    let early_shell_sum: usize = early.iter().map(|&cell| edge_depth(cell as usize)).sum();
    ...
    let mut degree = tree.initial_degree;
    let profile_exposed = |active: &[bool; CELLS], cell: usize| {
        let i = cell / N;
        let j = cell % N;
        i == 0
            || j == 0
    ...
    let mut shell_gate_violations = 0_usize;
    for &leaf_u16 in order {
        let leaf = leaf_u16 as usize;
        assert!(active[leaf] && degree[leaf] == 1);
        let mut min_any_shell = usize::MAX;
        let mut min_exposed_shell = usize::MAX;
        for cell in 0..CELLS {
            if active[cell] && degree[cell] == 1 {
                min_any_shell = min_any_shell.min(edge_depth(cell));
                if profile_exposed(&active, cell) {
                    min_exposed_shell = min_exposed_shell.min(edge_depth(cell));
                }
            }
        }
        let require_exposed = min_exposed_shell != usize::MAX;
        ...
        if (require_exposed && !profile_exposed(&active, leaf))
            || edge_depth(leaf) != eligible_shell
        {
            shell_gate_violations += 1;
        }
        active[leaf] = false;
        degree[leaf] = 0;
        let neighbor = (0..tree.neighbor_count[leaf] as usize)
            .map(|k| tree.neighbors[leaf][k] as usize)
            .find(|&cell| active[cell])
            .unwrap();
        degree[neighbor] -= 1;
    }
    assert_eq!(shell_gate_violations, 0);
    trace.count_by("first76_boundary", first76_boundary as i64);
    trace.count_by("early100_edge_depth_sum", early_shell_sum as i64);
    trace.count_by("early100_freedom_sum", early_freedom_sum as i64);
    trace.count_by("late100_freedom_sum", late_freedom_sum as i64);
    trace.count_by("shell_gate_violations", shell_gate_violations as i64);
}
...
fn main() {
    ...
    let greedy_frontier_violations = greedy.frontier_violations;
    let mut best = greedy;
    ...
    match width_two {
        Some(width_two)
            if (
                width_two.frontier_violations,
                width_two.shell_prefix,
                width_two.cost,
                width_two.freedom_prefix,
            ) < (
                best.frontier_violations,
                best.shell_prefix,
                best.cost,
                best.freedom_prefix,
            ) =>
    ...
    loop {
        ...
        if (
            result.frontier_violations,
            result.shell_prefix,
            result.cost,
            result.freedom_prefix,
        ) < (
            best.frontier_violations,
            best.shell_prefix,
            best.cost,
    ...
    let best_frontier_violations = best.frontier_violations;
    let best_shell_prefix = best.shell_prefix;
    let best_freedom_prefix = best.freedom_prefix;
    let best_order = best.order;
    record_shell_profile(&best_order, &tree, &mut trace);
    ...
    assert_eq!(final_state.frontier_violations, best_frontier_violations);
    assert_eq!(final_state.shell_prefix, best_shell_prefix);
    ...
    trace.count_by(
        "greedy_frontier_violations",
        greedy_frontier_violations as i64,
    );
    trace.count_by("final_t", operations.len() as i64);
    trace.count_by(
        "frontier_violations",
        final_state.frontier_violations as i64,
    );
