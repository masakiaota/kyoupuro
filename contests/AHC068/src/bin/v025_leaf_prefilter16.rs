// v025_leaf_prefilter16.rs
// v022_cost_weight の各親で、全 active 葉を安価 proxy により上位 16 件へ予選してから
// 既存の apply_transport/undo による正確評価へ渡す実験(IDEA-D22)。
// 状態・固定木・搬送・支持拡大・評価式・反復幅拡大・時間配分は v022 と同一。
use std::{
...
/// 盤面や合法矩形を調べず、固定木経路の同方向連続辺を最大 shift ずつ束ねた hop 数を返す。
/// 実搬送は active 穴や壁で短い shift に分かれ得るため、正確値ではなく安価な楽観値である。
#[inline]
...
fn beam_pass(
    ...
    for _depth in 0..CELLS - 1 {
        ...
        let mut candidates = Vec::with_capacity(beam.len().saturating_mul(LEAF_PREFILTER_LIMIT));
        for (parent_index, parent) in beam.iter_mut().enumerate() {
            ...
            for leaf in 0..CELLS {
                ...
                let estimated_delta = EVAL_COST_WEIGHT as i32 * hops - distance;
                let proxy_tie = tie_hash(
                    parent.hash_a
                        ^ parent.hash_b
                        ^ (leaf as u64).wrapping_mul(0x9e37_79b9_7f4a_7c15),
                    pass_seed,
                );
                leaf_proxies.push((estimated_delta, proxy_tie, leaf as u16));
                stats.leaf_proxy_seen += 1;
            }

            if leaf_proxies.len() > LEAF_PREFILTER_LIMIT {
                leaf_proxies.select_nth_unstable(LEAF_PREFILTER_LIMIT);
                leaf_proxies.truncate(LEAF_PREFILTER_LIMIT);
