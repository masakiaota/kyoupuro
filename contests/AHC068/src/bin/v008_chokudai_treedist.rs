// v008_chokudai_treedist.rs
// v005_chokudai_free の目的関数だけを差し替えた最小差分実験。
// キュー優先度・候補生成のペア益・tail 発火判定を、needs 和から
// 「葉剥がし tail 長の一次推定 tail_est = Σ 全域木上距離(セル, そのカードの目標セル)」
// に統一する。tail_est は総 T 推定そのものなので、探索の各手が総手数を直接削る。
// 探索構造(自由 δ 帯 Kadane + chokudai + anytime tail)とパラメタは v005 と同一。
...
/// tail_est がこの幅以上で記録更新したときだけ tail を実走する。
const TAIL_EST_GAP: i32 = 16;
...
/// 探索ノードが持つ最小状態。目的関数 tail_est は両軸共通なので needs 系は持たない。
#[derive(Clone)]
struct Board {
    cells: [u16; CELLS],
    e: i32,
    /// 葉剥がし tail 長の一次推定: Σ_cell 全域木上距離(cell, そのセルのカードの目標セル)。
    /// 総 T ≈ prefix 手数 + tail_est なので、これがキュー優先度の主項になる。
    tail_est: i32,
    hash: u64,
}

impl Board {
    fn new(initial_board: &[usize; CELLS], zobrist: &[u64], tree_dist: &[u16]) -> Self {
        ...
        for cell in 0..CELLS {
            ...
            e += (card != cell) as i32;
            tail_est += tree_dist[cell * CELLS + card] as i32;
    ...
    /// 1 ペア (a,b) のカードを交換し、E・tail_est・hash を差分更新する。
    #[inline]
    fn swap_pair(&mut self, a: usize, b: usize, zobrist: &[u64], tree_dist: &[u16]) {
        let ca = self.cells[a] as usize;
        let cb = self.cells[b] as usize;
        self.e -= (ca != a) as i32 + (cb != b) as i32;
        self.e += (ca != b) as i32 + (cb != a) as i32;
        self.tail_est -= tree_dist[a * CELLS + ca] as i32 + tree_dist[b * CELLS + cb] as i32;
        self.tail_est += tree_dist[b * CELLS + ca] as i32 + tree_dist[a * CELLS + cb] as i32;
    ...
    fn apply(&mut self, op: u32, zobrist: &[u64], tree_dist: &[u16]) {
        let (is_v, r, c, h, w) = decode_op(op);
        if is_v {
            let half = h / 2;
            for x in 0..half {
                for j in c..c + w {
                    let a = (r + x) * N + j;
                    self.swap_pair(a, a + half * N, zobrist, tree_dist);
                }
            }
        } else {
            let half = w / 2;
            for i in r..r + h {
                for y in 0..half {
                    let a = i * N + c + y;
                    self.swap_pair(a, a + half, zobrist, tree_dist);
                }
            }
        }
    }
...
/// 縦帯・横帯 200 本それぞれについて、木距離減少(= tail_est の減少 = 総 T 推定の改善)を
/// 最大化する (c,w) または (r,h) を非空 Kadane で求め、pool に帯ごとの最良候補を積む。
    ...
    for delta in 1..=MAX_SHIFT {
        ...
        for r in 0..=(N - hgt) {
            ...
            for j in 0..N {
                ...
                // 益 = 交換で減る木距離(tail_est の減少ぶん)を 1 列ぶん集計する。
                let mut col_score = 0;
                for x in 0..delta {
                    let a = (r + x) * N + j;
                    let b = a + delta * N;
                    ...
                    col_score += tree_dist[a * CELLS + ca] as i32
                        + tree_dist[b * CELLS + cb] as i32
                        - tree_dist[b * CELLS + ca] as i32
                        - tree_dist[a * CELLS + cb] as i32;
                }
        ...
        for c in 0..=(N - wid) {
            ...
            for i in 0..N {
                ...
                for y in 0..delta {
                    ...
                    let ca = cells[a] as usize;
                    let cb = cells[b] as usize;
                    row_score += tree_dist[a * CELLS + ca] as i32
                        + tree_dist[b * CELLS + cb] as i32
                        - tree_dist[b * CELLS + ca] as i32
                        - tree_dist[a * CELLS + cb] as i32;
                }
...
struct Tail {
    ...
    order: Vec<usize>,
    /// 全域木上の全対距離。tail_est(葉剥がし手数の一次近似)の差分更新に使う。
    tree_dist: Vec<u16>,
}
...
impl Tail {
    fn new(input: &Input) -> Self {
        ...
        // 木の無向隣接から全対の木上距離を BFS で前計算する(400 本、計 16 万エントリ)。
        let mut tree_adj: Vec<Vec<usize>> = vec![Vec::new(); CELLS];
        for v in 1..CELLS {
            tree_adj[parent[v]].push(v);
            tree_adj[v].push(parent[v]);
        }
        let mut tree_dist = vec![u16::MAX; CELLS * CELLS];
        let mut bfs = Vec::with_capacity(CELLS);
        for s in 0..CELLS {
            let base = s * CELLS;
            tree_dist[base + s] = 0;
            bfs.clear();
            ...
            while head < bfs.len() {
                let v = bfs[head];
                head += 1;
                let dv = tree_dist[base + v];
                for &to in &tree_adj[v] {
                    if tree_dist[base + to] == u16::MAX {
                        tree_dist[base + to] = dv + 1;
                        bfs.push(to);
                    }
                }
            }
...
fn node_key(n: &Node) -> (i32, i32, u32) {
    (n.board.tail_est, n.board.e, n.tie)
}
...
fn main() {
    ...
    let root = Board::new(&input.initial_board, &zobrist, &tail.tree_dist);
    ...
    local! {
        ...
        trace.count_by("tail_est_initial", root.tail_est as i64);
    }
    ...
    let root_tail_est = root.tail_est;
    queues[0].push(Node {
        board: root,
        arena: 0,
        tie: rng.next() as u32,
    ...
    let mut tail_gate = root_tail_est;
    ...
    while running {
        ...
        for t in 0..MAX_PREFIX_DEPTH {
            ...
            generate_candidates(&node.board, &walls, &tail.tree_dist, &mut rng, &mut pool);
            for cand in pool.iter() {
                let mut child = node.board.clone();
                child.apply(cand.op, &zobrist, &tail.tree_dist);
                ...
                // tail_est の記録を大きく更新したノードにだけ tail を実走し、総手数でベストを争う。
                if child.tail_est + TAIL_EST_GAP <= tail_gate {
                    tail_gate = child.tail_est;
    ...
    // 打ち切り時: キューに残った tail_est 最良ノード 1 つに最終 tail を当て、取りこぼしを防ぐ。
    let mut final_best: Option<(i32, i32, usize, u32)> = None;
    for t in 0..=MAX_PREFIX_DEPTH {
        for node in &queues[t] {
            let key = (node.board.tail_est, node.board.e);
            if final_best.is_none_or(|(be, bn, _, _)| key < (be, bn)) {
                final_best = Some((node.board.tail_est, node.board.e, t, node.arena));
