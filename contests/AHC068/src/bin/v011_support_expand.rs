// v011_support_expand.rs
// v004_differential_beam の搬送だけを最小差分で強化する実験(IDEA-D5)。
// apply_transport の各直線ステップで選ばれた幅 1 / 高さ 1 の運搬帯を、from を含む
// 連続・合法・全セル active な範囲内で「追加行/列の木距離和差分の累積が負である限り」
// 直交方向へ貪欲に伸ばす(expand_support)。注目カードの写像は帯の直交幅に依存しない
// ため搬送経路は不変で、巻き添えだけが「害最小」から「益最大」へ変わる。
// 帯の選択規準・評価式(10*T + 木距離和)・探索(反復幅拡大ビーム)は v004 と同一。
use std::{
