...
def run_case(
    ...
    try:
        # AHC068 の公式 vis は cwd に vis.html を書く。並列評価で衝突しないよう、
        # case ごとの専用ディレクトリで scorer を起動する。
        vis_dir = output_dir / f"{case_name}.vis"
...
def run_eval_locked(
    ...
    build_binary(TOOLS_MANIFEST, "vis")

    solver_bin = SOLVER_BIN_DIR / args.bin_name
    score_bin = TOOLS_BIN_DIR / "vis"
