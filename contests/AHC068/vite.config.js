...
const CASE_SORT_OPTIONS = [
  ...
  { key: "wall_edges_asc", label: "wall_edges ∧" },
  { key: "wall_edges_desc", label: "wall_edges ∨" },
];
...
function parseCaseMeta(filePath) {
  ...
  const wallStart = 1 + N * N;
  const verticalRows = tokens.slice(wallStart, wallStart + N);
  const horizontalRows = tokens.slice(wallStart + N, wallStart + N + N - 1);
  if (verticalRows.length !== N || horizontalRows.length !== N - 1) {
    return null;
  }
  if (
    verticalRows.some((row) => !/^[01]+$/.test(row) || row.length !== N - 1) ||
    horizontalRows.some((row) => !/^[01]+$/.test(row) || row.length !== N)
  ) {
    return null;
  }

  const wallEdges = [...verticalRows, ...horizontalRows].reduce(
    (count, row) => count + [...row].filter((ch) => ch === "1").length,
    0,
  );
  return { wallEdges };
}
