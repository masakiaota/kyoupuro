...
const CASE_SORT_OPTIONS = [
  ...
  { key: "initial_correct_position_desc", label: "初期正位置 ∨" },
  { key: "initial_correct_position_asc", label: "初期正位置 ∧" },
  { key: "initial_correct_line_desc", label: "初期正線 ∨" },
  { key: "initial_correct_line_asc", label: "初期正線 ∧" },
  { key: "inversions_desc", label: "転倒数 ∨" },
  { key: "inversions_asc", label: "転倒数 ∧" },
];
...
function parseCaseMeta(filePath) {
  ...
  const cars = tokens.slice(1, 1 + r * 10);
  if (cars.length !== r * 10 || cars.some((value) => !Number.isInteger(value))) {
    return null;
  }

  let initialCorrectPosition = 0;
  let initialCorrectLine = 0;
  for (let line = 0; line < r; line += 1) {
    for (let pos = 0; pos < 10; pos += 1) {
      const car = cars[line * 10 + pos];
      if (Math.floor(car / 10) === line) {
        initialCorrectLine += 1;
      }
      if (car === line * 10 + pos) {
        initialCorrectPosition += 1;
      }
    }
  }
  ...
  return { initialCorrectPosition, initialCorrectLine, inversions };
}
