...
def detect_score_bin_name() -> str:
    tools_bin_dir = ROOT_DIR / "tools" / "src" / "bin"
    if (tools_bin_dir / "score.rs").is_file():
        return "score"
    if (tools_bin_dir / "vis.rs").is_file():
        return "vis"
    raise SystemExit(
        "error: neither tools/src/bin/score.rs nor tools/src/bin/vis.rs was found"
    )
...
def warmup_binaries(
    ...
    output_path = output_dir / ".warmup.txt"
    err_path = output_dir / ".warmup.err"

    if verbose:
        eprint(f"warmup: {case_path.name}")
    ...
    if run_result.returncode != 0:
        raise SystemExit(f"error: warmup solver failed: {case_path.name}")
    ...
    if score_result.returncode != 0:
        raise SystemExit(f"error: warmup scorer failed: {case_path.name}")
    if parse_score(score_result.stdout) is None:
        raise SystemExit(f"error: warmup scorer output was not parseable: {case_path.name}")
...
def main() -> int:
    ...
    score_bin_name = detect_score_bin_name()

    build_binary(SOLVER_MANIFEST, args.bin_name)
    build_binary(TOOLS_MANIFEST, score_bin_name)

    solver_bin = SOLVER_BIN_DIR / args.bin_name
    score_bin = TOOLS_BIN_DIR / score_bin_name
