...
const STORAGE_KEY = "ahc064:case-visualizer:v1";
...
const state = {
  ...
  cases: [],
  currentBin: typeof savedState.currentBin === "string" ? savedState.currentBin : "",
  currentCase:
    typeof savedState.currentCase === "string" && savedState.currentCase
      ? savedState.currentCase
      : null,
  loadSeq: 0,
...
function restoreSpeed() {
  if (typeof savedState.speed !== "string") {
    return;
  }
  const optionValues = new Set(Array.from(els.speed.options, (option) => option.value));
  if (optionValues.has(savedState.speed)) {
    els.speed.value = savedState.speed;
  }
}

function restoreTurn() {
  if (savedState.currentBin !== state.currentBin || savedState.currentCase !== state.currentCase) {
    return;
  }
  if (!Number.isFinite(Number(savedState.turn))) {
    return;
  }
  const maxTurn = getMaxTurn();
  const turn = Math.max(0, Math.min(maxTurn, Number(savedState.turn) || 0));
  els.turn.value = String(turn);
  state.playbackTurnFloat = turn;
  syncTurnDisplay();
  render();
}
...
els.speed.addEventListener("change", saveViewState);
