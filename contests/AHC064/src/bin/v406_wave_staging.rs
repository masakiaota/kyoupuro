...
fn push_unique_move(candidates: &mut Vec<Move>, state: &State, mv: Move) {
    if is_valid_move(state, mv) && !candidates.iter().any(|&x| x == mv) {
        candidates.push(mv);
    }
}
...
fn push_dep_to_sidings(
    ...
    for j in 0..R {
        ...
        push_unique_move(candidates, state, Move::dep_to_siding(i, j, block.len));
    }
}
...
fn push_siding_to_deps(
    ...
    for i in 0..R {
        ...
        push_unique_move(candidates, state, Move::siding_to_dep(i, j, block.len));
    }
}
...
fn expose_dep_tail_candidates(
    ...
    if block.area == AREA_DEP {
        ...
        push_dep_to_sidings(state, &mut candidates, block.line, tail, avoid_sid);
    } else if block.start == 0 {
        push_siding_to_deps(state, &mut candidates, block.line, block, avoid_dep);
    } else {
        let head = sid_head_block(state, block.line)?;
        if block_contains_other_protected(head, protected, iv) {
            return Some((false, candidates));
        }
        push_siding_to_deps(state, &mut candidates, block.line, head, avoid_dep);
    }
...
fn expose_sid_head_candidates(
    ...
    if block.area == AREA_SID {
        ...
        push_dep_to_sidings(state, &mut candidates, block.line, block, avoid_sid);
    } else {
        let tail = dep_tail_block(state, block.line)?;
        if block_contains_other_protected(tail, protected, iv) {
            return Some((false, candidates));
...
fn job_step_candidates(
    ...
    if let (Some(a_probe), Some(b_probe)) = (find_block(state, job.a), find_block(state, job.b)) {
            ...
            && b_probe.end_pos() <= a_probe.start
        {
            ...
            push_siding_to_deps(state, &mut candidates, a_probe.line, head, avoid_dep);
            return candidates;
        }
    }
    ...
    if let Some(b_probe) = find_block(state, job.b) {
        ...
        {
            let mut candidates = Vec::new();
            push_dep_to_sidings(state, &mut candidates, a_block.line, a_block, avoid_sid);
            return candidates;
        }
    }
...
fn select_non_crossing_moves(state: &State, candidates: &[Move]) -> Vec<Move> {
    ...
    for &mv in candidates {
        ...
        let weight = move_weight(state, mv);
        match edge[mv.i][mv.j] {
            Some((old, _)) if old >= weight => {}
            _ => edge[mv.i][mv.j] = Some((weight, mv)),
        }
    }
...
fn simulate_subset_score(state: &State, jobs: &[MergeJob], gain: i32) -> Option<i32> {
    ...
    for _ in 0..120 {
        ...
        let mut candidates = Vec::new();
        for &job in &active {
            for mv in job_step_candidates(&st, job, &avoid_dep, &avoid_sid, &protected) {
                push_unique_move(&mut candidates, &st, mv);
            }
        }
...
fn execute_wave(state: &mut State, output: &mut Output, mut jobs: Vec<MergeJob>) -> bool {
    ...
    for _ in 0..120 {
        ...
        let mut candidates = Vec::new();
        for &job in &jobs {
            for mv in job_step_candidates(state, job, &avoid_dep, &avoid_sid, &protected) {
                push_unique_move(&mut candidates, state, mv);
            }
        }
...
fn solve(input: &Input) -> Output {
    ...
    let debug = std::env::var_os("V406_DEBUG").is_some();
