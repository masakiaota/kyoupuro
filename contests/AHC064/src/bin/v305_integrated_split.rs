...
fn split_prog_key(p: &[usize], split: usize, aux_cap: usize) -> u64 {
    ((perm_code_from_vec(p) as u64) << 8) | ((split as u64) << 4) | aux_cap as u64
}

fn bad_prog(prog: &[AbsOp]) -> bool {
    !prog.is_empty() && prog[0].t == OP_BAD
}

fn get_prog_for_perm_split(
    p: &[usize],
    split: usize,
    aux_cap: usize,
    memo: &mut HashMap<u64, Vec<AbsOp>>,
) -> Vec<AbsOp> {
    let pkey = split_prog_key(p, split, aux_cap);
    ...
    let mut parent: HashMap<u64, (u64, AbsOp)> = HashMap::with_capacity(20000);
    ...
    while let Some(cur) = queue.pop_front() {
        let l = decode_prog_key(cur, &mut s, &mut a);
        if l == split && a.is_empty() && s.len() == INIT_LEN - split {
            let mut ok = true;
            for (i, &x) in s.iter().enumerate() {
                if x as usize != split + i {
                    ok = false;
                    break;
                }
            }
        ...
        maxk = if l >= split { 0 } else { maxk.min(split - l) };
        for k in 1..=maxk {
            let ns = encode_prog_key(&s[k..], &a, l + k);
            if visited.insert(ns) {
                parent.insert(ns, (cur, AbsOp { t: OP_T, k }));
    ...
    let fail = vec![AbsOp { t: OP_BAD, k: 0 }];
    memo.insert(pkey, fail.clone());
    fail
}
...
fn choose_split_plan(
    sroute: &[Vec<usize>],
    split_memo: &mut HashMap<u64, Vec<AbsOp>>,
) -> SplitPlan {
    let mut best = SplitPlan {
        first_group: Vec::new(),
        sf: INIT_LEN,
        ss: INIT_LEN,
        est_turns: 1_000_000,
    };
    let positions = sroute
        .iter()
        .map(|row| row.iter().map(|&x| pos_of(x)).collect::<Vec<_>>())
        .collect::<Vec<_>>();
    ...
    for mask in 0usize..(1usize << R) {
        ...
        for sf in 0..=INIT_LEN {
            let cap2 = DEP_CAP - sf;
            if cap2 == 0 {
                continue;
            }
            for ss in 0..=INIT_LEN {
                let mut progs = vec![Vec::new(); R];
                let mut ok = true;
                for &r in &g {
                    progs[r] = get_prog_for_perm_split(&positions[r], sf, DEP_CAP, split_memo);
                    if bad_prog(&progs[r]) {
                        ok = false;
                        break;
                    }
                ...
                for &r in &c {
                    progs[r] = get_prog_for_perm_split(&positions[r], ss, cap2, split_memo);
                    if bad_prog(&progs[r]) {
                        ok = false;
                        break;
                    }
                ...
                let t1 = schedule_group_est(&g, &c, &progs);
                let t2 = schedule_group_est(&c, &g, &progs);
                let fin = if sf < INIT_LEN || ss < INIT_LEN { 1 } else { 0 };
                let est = t1 + t2 + fin;
                if est < best.est_turns {
                    best.est_turns = est;
                    best.first_group = g.clone();
                    best.sf = sf;
                    best.ss = ss;
                }
            }
...
fn execute_split_sort(
    sim: &mut Sim,
    sroute: &[Vec<usize>],
    plan: &SplitPlan,
    split_memo: &mut HashMap<u64, Vec<AbsOp>>,
) -> bool {
    if plan.est_turns >= 1_000_000 {
        return false;
    }

    let mut in_g = 0usize;
    for &r in &plan.first_group {
        in_g |= 1usize << r;
    }
    let mut c = Vec::new();
    for r in 0..R {
    ...
    let mut progs = vec![Vec::new(); R];
    for r in 0..R {
        let split = if (in_g >> r) & 1 == 1 {
            plan.sf
        } else {
            plan.ss
        };
        let cap = if (in_g >> r) & 1 == 1 {
            DEP_CAP
        } else {
            DEP_CAP - plan.sf
        };
        progs[r] = get_prog_for_perm_split(&positions[r], split, cap, split_memo);
        if bad_prog(&progs[r]) {
            return false;
        }
    }

    execute_group_sort(sim, &plan.first_group, &c, &progs);
    execute_group_sort(sim, &c, &plan.first_group, &progs);
    ...
    for r in 0..R {
        ...
        if split < INIT_LEN {
            fin.push(Move::siding_to_dep(r, r, INIT_LEN - split));
        }
    }
    sim.add_turn(fin);
    true
...
fn make_solution_from_distribution_split(
    db: &DistBuilder,
    nodes: &[DistNode],
    final_id: usize,
    init: &[Vec<usize>],
    ...
    let sroute = sim.sid.clone();
    let progs = build_sort_progs(&sroute, memo);
    let current = choose_sort_partition(&progs);
    let split = choose_split_plan(&sroute, split_memo);
    if split.est_turns >= current.est_turns {
        return None;
    }
    if !execute_split_sort(&mut sim, &sroute, &split, split_memo) {
        return None;
    }
    assert!(sim.is_goal());
...
fn line_in_pending_c(pending: &[PendingC], line: usize) -> bool {
    pending.iter().any(|p| p.r == line || p.aux == line)
}

fn shallow_block_depth(sim: &Sim, r: usize, fixed: &[usize; R]) -> Option<usize> {
    if fixed[r] >= INIT_LEN || sim.sid[r].len() <= 1 {
        return None;
    }
    let target = target_id(r, fixed[r]);
    let max_d = C_MAX_DEPTH.min(sim.sid[r].len() - 1);
    (1..=max_d).find(|&d| sim.sid[r][d] == target)
}
...
fn choose_c_aux(
    ...
    for aux in 0..R {
        if aux == r || line_in_pending_c(pending, aux) || src_remaining[aux] != 0 {
            continue;
        }
        if sim.dep[aux].len() != fixed[aux] || sim.dep[aux].len() + k > DEP_CAP {
            continue;
        }
        let mv = Move::siding_to_dep(aux, r, k);
        if !can_add_move_to_turn(ops, mv) {
            continue;
        }
        let key = aux.abs_diff(r) * 100 + sim.dep[aux].len();
        if key < best_key {
            best_key = key;
            best = Some(aux);
        }
...
fn schedule_pending_c(
    ...
    let mut actions = vec![None; pending.len()];
    for (idx, &p) in pending.iter().enumerate() {
        match p.stage {
            CStage::Commit => {
                let len = ready_commit_len(sim, p.r, fixed, src_remaining, commit_mask);
                if len == 0 {
                    continue;
                }
                let mv = Move::siding_to_dep(p.r, p.r, len);
                if can_add_move_to_turn(ops, mv) {
                    ops.push(mv);
                    actions[idx] = Some(CAction::Commit(len));
                }
            }
            CStage::Return => {
                let mv = Move::dep_to_siding(p.aux, p.r, p.k);
                if can_add_move_to_turn(ops, mv) {
                    ops.push(mv);
                    actions[idx] = Some(CAction::Return);
                }
            }
        }
    }
...
fn make_solution_from_distribution_mixed_c(
    db: &DistBuilder,
    nodes: &[DistNode],
    final_id: usize,
    init: &[Vec<usize>],
    ...
    for dist_ops in dturns {
        let mut ops = dist_ops.clone();
        let actions = schedule_pending_c(
            &sim,
            &pending,
            &fixed,
            &src_remaining,
        ...
        let mut commits = Vec::new();
        for r in 0..R {
            if line_in_pending_c(&pending, r) {
                continue;
            }
            let len = ready_commit_len(&sim, r, &fixed, &src_remaining, commit_mask);
            if len == 0 {
        ...
        for r in 0..R {
            if ((commit_mask >> r) & 1 == 0)
                || line_in_pending_c(&pending, r)
                || starts.iter().any(|p: &PendingC| p.r == r || p.aux == r)
                || src_remaining[r] != 0
                || sim.dep[r].len() != fixed[r]
            {
                continue;
            }
            if let Some(k) = shallow_block_depth(&sim, r, &fixed) {
                let mut active = pending.clone();
                active.extend_from_slice(&starts);
                if let Some(aux) = choose_c_aux(&sim, r, k, &fixed, &src_remaining, &active, &ops) {
                    let mv = Move::siding_to_dep(aux, r, k);
                    ops.push(mv);
                    starts.push(PendingC {
                        r,
                        aux,
                        k,
                        stage: CStage::Commit,
        ...
        for (idx, &p) in pending.iter().enumerate() {
            if let Some(CAction::Commit(len)) = actions[idx] {
                fixed[p.r] += len;
            }
        }
        for mv in commits {
        ...
        for (idx, mut p) in pending.into_iter().enumerate() {
            match actions[idx] {
                Some(CAction::Commit(_)) => {
                    p.stage = CStage::Return;
                    next_pending.push(p);
                }
                Some(CAction::Return) => {}
...
fn solve_route_strategy(
    ...
    for id in final_ids {
        ...
        if let Some(split) =
            make_solution_from_distribution_split(&db, &nodes, id, init, memo, &mut split_memo)
        {
            if split.turn_count < best.turn_count {
                best = split;
            }
        }
        ...
        if let Some(mixed_c) = make_solution_from_distribution_mixed_c(&db, &nodes, id, init, memo)
        {
            if mixed_c.turn_count < best.turn_count {
                best = mixed_c;
            }
        }
    }
    ...
    if mode == 3 {
        for pen in [10, 50] {
            ...
            if let Some(split) = make_solution_from_distribution_split(
                &db,
                &greedy_nodes,
                gid,
                init,
            ...
            if let Some(mixed_c) =
                make_solution_from_distribution_mixed_c(&db, &greedy_nodes, gid, init, memo)
            {
                if mixed_c.turn_count < best.turn_count {
                    best = mixed_c;
                }
