...
fn choose_wave_jobs(state: &State) -> Vec<MergeJob> {
    ...
    let mut used = [[false; INIT_LEN]; R];
    let mut used_dep_line = [false; R];
    let mut used_sid_line = [false; R];
    let mut jobs = Vec::new();
    for (_gain, a, b) in cands {
        ...
        for block in [a, b] {
            if block.area == AREA_DEP {
                if used_dep_line[block.line] {
                    ok = false;
                }
            } else if used_sid_line[block.line] {
                ok = false;
            }
        }
        if !ok {
        ...
        for block in [a, b] {
            if block.area == AREA_DEP {
                used_dep_line[block.line] = true;
            } else {
                used_sid_line[block.line] = true;
            }
        }
        jobs.push(job);
    }
...
fn job_can_start(state: &State, job: MergeJob) -> bool {
    let jobs = [job];
...
fn solve(input: &Input) -> Output {
    ...
    let debug = std::env::var_os("V404_DEBUG").is_some();
