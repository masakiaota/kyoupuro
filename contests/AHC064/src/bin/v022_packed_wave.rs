...
impl Solver {
    ...
    fn compatible_with_turn(mv: Move, moves: &[Move]) -> bool {
        for &other in moves {
            if mv.i == other.i || mv.j == other.j {
                return false;
            }
            if mv.i < other.i {
                if mv.j >= other.j {
    ...
    fn pack_turn_reserved(
        &mut self,
        mut mandatory: Vec<Move>,
        policy: DirectPolicy,
        rng: &mut XorShift64,
        reserved_dep: &[bool; R],
        reserved_sid: &[bool; R],
        ...
        mandatory.sort_unstable_by_key(|mv| (mv.i, mv.j));
        if mandatory.len() >= 5 {
            self.add_turn(mandatory);
            return;
        }

        let candidates = self.make_direct_candidates(policy, rng);
        let capacity = 5 - mandatory.len();
        ...
        for (idx, cand) in candidates.iter().enumerate() {
            ...
            let mv = cand.mv;
            if reserved_dep[mv.i] || reserved_sid[mv.j] {
                continue;
            }
            if !Self::compatible_with_turn(mv, &mandatory) {
                continue;
            }
            by_i[mv.i].push(idx);
        }
        ...
        let mut dp = vec![vec![vec![None::<(i32, Vec<usize>)>; capacity + 1]; R + 1]; R + 1];
        dp[0][0][0] = Some((0, Vec::new()));
        for i in 0..R {
            let mut next = vec![vec![None::<(i32, Vec<usize>)>; capacity + 1]; R + 1];
            for last in 0..=R {
                for cnt in 0..=capacity {
        ...
        for last in 0..=R {
            for cnt in 1..=capacity {
                if let Some((score, path)) = dp[R][last][cnt].clone() {
                    Self::relax_direct_dp(&mut best, score + 500 * cnt as i32, path);
                }
            }
        }
        ...
        if let Some((_, path)) = best {
            for idx in path {
                mandatory.push(candidates[idx].mv);
            }
        }
        self.add_turn(mandatory);
    }
    ...
    fn execute_scan_batch(
        ...
        let mut reserved_dep = [false; R];
        let mut reserved_sid = [false; R];
        for &r in active_p {
            reserved_dep[r] = true;
            reserved_sid[r] = true;
        }
        ...
        for step in 0..max_seg {
            ...
            self.pack_turn_reserved(mb, policy, rng, &reserved_dep, &reserved_sid);
            ...
            self.pack_turn_reserved(mt, policy, rng, &reserved_dep, &reserved_sid);
        }
        ...
        self.pack_turn_reserved(mr, policy, rng, &reserved_dep, &reserved_sid);
    }
    ...
    fn execute_one_direct_scan_batch(
        &mut self,
        policy: DirectPolicy,
        rng: &mut XorShift64,
    ) -> bool {
        ...
        for a in (1..=max_a).rev() {
            for start in 0..=need_buf.len() - a {
                ...
                if self.assign_buffers_for(&active_p, &plans, &active, &mut buffers) {
                    self.execute_scan_batch(&active_p, &buffers, &[], &plans, policy, rng);
                    return true;
                }
            }
        }
    ...
    fn solve_direct5_unified(&mut self, policy: DirectPolicy, rng: &mut XorShift64) -> bool {
        ...
        while !self.complete() {
            ...
            if self.execute_one_direct_scan_batch(policy, rng) {
                progressed = true;
            }
