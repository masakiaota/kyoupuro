...
fn finish_phase2_group_dp(mut state: State, mut output: Output) -> Output {
    for c in 0..INIT_LEN {
        for r in 0..R {
...
fn make_scan_plan(state: &State, r: usize) -> ScanPlan {
    let mut plan = ScanPlan::new();
    let pref = state.dep_len(r);
    if pref >= INIT_LEN {
        return plan;
    }

    let mut pos = [0usize; INIT_LEN];
    for i in 0..state.sid_len(r) {
        let car = state.sid_car(r, i);
        pos[Input::target_pos(car)] = i;
    }
...
fn assign_scan_buffers(
    ...
    for &r in active_p {
        ...
        while ci < cand.len() {
            let b = cand[ci];
            ci += 1;
            if state.dep_len(b) + plans[r].total_skip <= DEP_CAP {
                buffers.push(b);
                ok = true;
                break;
            }
...
fn emit_scan_turn(output: &mut Output, state: &mut State, moves: Vec<Move>) {
    if !moves.is_empty() {
...
fn execute_scan_batch(
    ...
    for step in 0..max_seg {
        let mut buffer_moves = Vec::new();
        for t in 0..active_count {
            let r = active_p[t];
            if step < plans[r].chunks.len() && plans[r].chunks[step] > 0 {
                buffer_moves.push(Move::siding_to_dep(buffers[t], r, plans[r].chunks[step]));
            }
        }
        emit_scan_turn(output, state, buffer_moves);

        let mut target_moves = Vec::new();
        for &r in active_p {
            if step < plans[r].segs.len() {
                target_moves.push(Move::siding_to_dep(r, r, plans[r].segs[step]));
            }
        }
        if !z_done {
            for &r in active_z {
                if plans[r].take > 0 {
                    target_moves.push(Move::siding_to_dep(r, r, plans[r].take));
                }
            }
            z_done = true;
        }
        emit_scan_turn(output, state, target_moves);
    }
    ...
    for t in 0..active_count {
        ...
        if plans[r].total_skip > 0 {
            restore_moves.push(Move::dep_to_siding(buffers[t], r, plans[r].total_skip));
        }
    }
    emit_scan_turn(output, state, restore_moves);
}

fn finish_phase2_scan(mut state: State, mut output: Output) -> Output {
    ...
    loop {
        ...
        let mut plans = (0..R).map(|_| ScanPlan::new()).collect::<Vec<_>>();
        let mut need_buf = Vec::new();
        let mut no_buf = Vec::new();

        for r in 0..R {
            if state.dep_len(r) < INIT_LEN {
                plans[r] = make_scan_plan(&state, r);
        ...
        while ptr < need_buf.len() {
            ...
            for a in (1..=max_a).rev() {
                ...
                if assign_scan_buffers(&state, &active_p, &plans, &active, &mut buffers) {
                    best_a = a;
            ...
            if !found {
                let r = need_buf[ptr];
                let car = Input::target_id(r, state.dep_len(r));
                let p = state.sid_pos(r, car);
                let mut buffer = None::<usize>;
                for x in 0..R {
                    if x != r && state.dep_len(x) + p <= DEP_CAP {
                        buffer = Some(x);
                        break;
                    }
                }
                ...
                emit_scan_turn(
                    ...
                    &mut state,
                    if p > 0 {
                        vec![Move::siding_to_dep(b, r, p)]
                    } else {
                        Vec::new()
                    },
                );
                emit_turn(&mut output, &mut state, vec![Move::siding_to_dep(r, r, 1)]);
                emit_scan_turn(
                    &mut output,
                    &mut state,
                    if p > 0 {
                        vec![Move::dep_to_siding(b, r, p)]
                    } else {
                        Vec::new()
                    },
                );
        ...
        for r in no_buf {
            if state.dep_len(r) < INIT_LEN {
                let plan = make_scan_plan(&state, r);
                if plan.total_skip == 0 && plan.take > 0 {
                    direct_moves.push(Move::siding_to_dep(r, r, plan.take));
                }
            }
        }
        emit_scan_turn(&mut output, &mut state, direct_moves);
    }
...
fn build_output(
    ...
    let grouped_state = state.clone();
    let grouped_output = output.clone();
    let best = finish_phase2_group_dp(state, output);
    let scan = finish_phase2_scan(grouped_state, grouped_output);
    if scan.turns.len() < best.turns.len() {
        scan
    } else {
        best
    }
