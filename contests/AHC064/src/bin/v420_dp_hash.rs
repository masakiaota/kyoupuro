...
const MOVE_DEP_TO_SIDING: u8 = 0;
const MOVE_SIDING_TO_DEP: u8 = 1;
...
fn hash128(x: u64) -> u128 {
    let lo = splitmix64(x);
    let hi = splitmix64(x ^ 0xD1B5_4A32_D192_ED03);
    ((hi as u128) << 64) | lo as u128
}

fn car_pos_hash(car: usize, area: u8, line: u8, slot: u8) -> u128 {
    let x = car as u64
        | ((area as u64) << 8)
        | ((line as u64) << 12)
        | ((slot as u64) << 16)
...
fn dep_len_hash(line: usize, len: u8) -> u128 {
    hash128((line as u64) | ((len as u64) << 8) | (0x22_u64 << 24))
}

fn sid_head_hash(line: usize, head: u8) -> u128 {
    hash128((line as u64) | ((head as u64) << 8) | (0x33_u64 << 24))
}

fn sid_len_hash(line: usize, len: u8) -> u128 {
    hash128((line as u64) | ((len as u64) << 8) | (0x44_u64 << 24))
}
...
impl State {
    fn new(input: &Input) -> Self {
        ...
        for r in 0..R {
            hash ^= dep_len_hash(r, INIT_LEN as u8);
            hash ^= sid_head_hash(r, 0);
            hash ^= sid_len_hash(r, 0);
            for c in 0..INIT_LEN {
                ...
                hash ^= car_pos_hash(car, AREA_DEP as u8, r as u8, c as u8);
            }
        }
    ...
    fn recompute_hash(&self) -> u128 {
        let mut hash = 0u128;
        for i in 0..R {
            hash ^= dep_len_hash(i, self.dep_len[i]);
            hash ^= sid_head_hash(i, self.sid_head[i]);
            hash ^= sid_len_hash(i, self.sid_len[i]);
        }
    ...
    fn apply_move(&mut self, mv: Move) {
        ...
        let j = mv.j();
        let k = mv.k();
        self.hash ^= dep_len_hash(i, self.dep_len[i]);
        self.hash ^= sid_head_hash(j, self.sid_head[j]);
        self.hash ^= sid_len_hash(j, self.sid_len[j]);
        ...
        if mv.kind == MOVE_DEP_TO_SIDING {
            ...
            for offset in 0..k {
                ...
                self.hash ^= car_pos_hash(car_idx, AREA_DEP as u8, mv.i, dep_pos as u8);
                self.hash ^= car_pos_hash(car_idx, AREA_SID as u8, mv.j, sid_slot as u8);
                self.sid[j][sid_slot] = car;
                self.car_area[car_idx] = AREA_SID as u8;
                self.car_line[car_idx] = mv.j;
...
const EMPTY_DP_CAND: DpCand = DpCand {
    sum_delta_p: i32::MIN,
    ep_score: i32::MIN,
    dep_need_mask: 0,
    sid_head_mask: 0,
...
impl CandList {
    ...
    fn push_raw(&mut self, cand: DpCand) {
        let pos = self.len as usize;
        self.data[pos] = cand;
        self.len += 1;
    }

    fn sorted_vec(&self) -> Vec<DpCand> {
        let mut v = self.data[..self.len as usize].to_vec();
        v.sort_by(dp_cand_cmp);
        v
    }
}
...
#[cfg(debug_assertions)]
fn move_delta_p_slow(state: &State, mv: Move) -> i32 {
    let i = mv.i();
    let j = mv.j();
    let k = mv.k();
    let before = pair_score(state, i, j);
    let dep_len = state.dep_len(i);
    let sid_len = state.sid_len(j);
    ...
    if mv.kind == MOVE_DEP_TO_SIDING {
        dep_after_len = dep_len - k;
        sid_after_len = sid_len + k;
        for pos in 0..dep_after_len {
            dep_after[pos] = state.dep[i][pos];
        }
        for pos in 0..k {
            sid_after[pos] = state.dep[i][dep_after_len + pos];
        }
        for pos in 0..sid_len {
            sid_after[k + pos] = state.sid_car(j, pos) as u8;
        }
    } else {
        dep_after_len = dep_len + k;
        sid_after_len = sid_len - k;
        ...
        for pos in 0..k {
            dep_after[dep_len + pos] = state.sid_car(j, pos) as u8;
        }
        for pos in k..sid_len {
            sid_after[pos - k] = state.sid_car(j, pos) as u8;
        }
    }

    dep_line_score_buf(i, &dep_after, dep_after_len) + sid_line_score_buf(&sid_after, sid_after_len)
        - before
}

fn adjacent_pair_score(prev: usize, car: usize) -> i32 {
    if prev % INIT_LEN + 1 < INIT_LEN && car == prev + 1 {
        1
    } else {
        0
...
fn dep_anchor_score(i: usize, first: usize) -> i32 {
    if first == i * INIT_LEN {
        1
    } else {
        0
...
fn move_delta_p_fast(state: &State, mv: Move) -> i32 {
    let i = mv.i();
    let j = mv.j();
    let k = mv.k();
    let dep_len = state.dep_len(i);
    ...
    if mv.kind == MOVE_DEP_TO_SIDING {
        let dep_after_len = dep_len - k;

        if dep_after_len == 0 {
            delta -= dep_anchor_score(i, state.dep[i][0] as usize);
        } else {
            let left = state.dep[i][dep_after_len - 1] as usize;
            let right = state.dep[i][dep_after_len] as usize;
            delta -= adjacent_pair_score(left, right);
        }

        if sid_len > 0 {
            let moved_last = state.dep[i][dep_len - 1] as usize;
            let sid_first = state.sid_car(j, 0);
            delta += adjacent_pair_score(moved_last, sid_first);
        }
    } else {
        let moved_first = state.sid_car(j, 0);
        let moved_last = state.sid_car(j, k - 1);

        if dep_len == 0 {
            delta += dep_anchor_score(i, moved_first);
        } else {
            let dep_last = state.dep[i][dep_len - 1] as usize;
            delta += adjacent_pair_score(dep_last, moved_first);
        }

        if k < sid_len {
            let remaining_first = state.sid_car(j, k);
            delta -= adjacent_pair_score(moved_last, remaining_first);
        }
    }
...
fn move_delta_p(state: &State, mv: Move) -> i32 {
    let delta = move_delta_p_fast(state, mv);
    #[cfg(debug_assertions)]
    {
        let slow = move_delta_p_slow(state, mv);
        debug_assert_eq!(delta, slow, "move_delta_p mismatch: {:?}", mv);
    }
    delta
}
...
fn endpoint_after_move(state: &State, mv: Move) -> (Option<usize>, Option<usize>) {
    ...
    if mv.kind == MOVE_DEP_TO_SIDING {
        ...
        let dep_need = if dep_after_len == 0 {
            ...
            dep_need_from_tail(state.dep[i][dep_after_len - 1] as usize)
        };
        let sid_head = Some(state.dep[i][dep_after_len] as usize);
        (dep_need, sid_head)
    } else {
        let dep_tail = state.sid_car(j, k - 1);
        let dep_need = dep_need_from_tail(dep_tail);
        let sid_after_len = sid_len - k;
        let sid_head = if sid_after_len == 0 {
            None
        } else {
            Some(state.sid_car(j, k))
        };
        (dep_need, sid_head)
    }
}
...
fn push_dp_cand(cell: &mut CandList, cand: DpCand) {
    for pos in 0..cell.len as usize {
        let old = cell.data[pos];
        if old.dep_need_mask == cand.dep_need_mask && old.sid_head_mask == cand.sid_head_mask {
            if better_dp_cand(&cand, &old) {
                cell.data[pos] = cand;
            }
            return;
        }
    ...
    if (cell.len as usize) < DP_BEAM {
        cell.push_raw(cand);
        return;
    }

    let mut worst = 0;
    for pos in 1..cell.len as usize {
        if better_dp_cand(&cell.data[worst], &cell.data[pos]) {
            worst = pos;
        }
    }
    if better_dp_cand(&cand, &cell.data[worst]) {
        cell.data[worst] = cand;
    }
}
...
fn run_beam_dp_for_one_turn(state: &State, rng: &mut XorShift) -> Vec<DpCand> {
    ...
    let mut dp = [[CandList::new(); R + 1]; R + 1];
    dp[0][0].push_raw(DpCand::start(rng.next_u64()));
    ...
    for i in 0..=R {
        for j in 0..=R {
            ...
            for cand_pos in 0..cell.len as usize {
                let cand = cell.data[cand_pos];
                if i < R {
