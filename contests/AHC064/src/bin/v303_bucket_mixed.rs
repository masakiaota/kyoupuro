...
impl Sim {
    ...
    fn has_dep_cars(&self) -> bool {
        self.dep.iter().any(|line| !line.is_empty())
    }

    fn suffix_route_run_len(&self, i: usize) -> usize {
        let len = self.dep[i].len();
        let j = route_of(*self.dep[i].last().unwrap());
        let mut k = 0usize;
        while k < len {
            let car = self.dep[i][len - 1 - k];
            if route_of(car) != j {
                break;
            }
            k += 1;
    ...
    fn make_bucket_candidates(
        ...
        for i in 0..R {
            if self.dep[i].is_empty() {
                continue;
            }

            let j = route_of(*self.dep[i].last().unwrap());
            let max_k = self.suffix_route_run_len(i);
            let mut k = max_k;
            if max_k > 1 && rng.usize(1000) < policy.split_permille as usize {
                k = 1 + rng.usize(max_k);
            }
            ...
            for offset in 0..k {
                let car = self.dep[i][len - k + offset];
                pos_sum += pos_of(car) as i32;
            }

            candidates.push(BucketCandidate { i, j, k, pos_sum });
        }

        candidates
    }

    fn choose_bucket_moves(&self, policy: BucketPolicy, rng: &mut XorShift64) -> Vec<Move> {
        let candidates = self.make_bucket_candidates(policy, rng);
        ...
        for a in 0..n {
            ...
            for b in 0..a {
                if candidates[b].j < cand.j {
                    ...
                    if next > dp[a] || (next == dp[a] && (rng.next_u64() & 1) == 1) {
                        dp[a] = next;
                        prev[a] = b;
                    }
                }
            ...
            if dp[a] > dp[best] || (dp[a] == dp[best] && (rng.next_u64() & 1) == 1) {
                best = a;
            }
        }
    ...
    fn distribute_with_bucket_policy(&mut self, policy: BucketPolicy, rng: &mut XorShift64) {
        while self.has_dep_cars() {
            let moves = self.choose_bucket_moves(policy, rng);
            self.add_turn(moves);
        }
    }
}
...
fn make_solution_from_turns_mixed_all(
    init: &[Vec<usize>],
    dturns: &[Vec<Move>],
    memo: &mut HashMap<usize, Vec<AbsOp>>,
) -> Option<CandidateSolution> {
...
fn make_bucket_distribution_turns(
    init: &[Vec<usize>],
    policy: BucketPolicy,
    rng: &mut XorShift64,
) -> Vec<Vec<Move>> {
    let mut sim = Sim::new(init);
    sim.distribute_with_bucket_policy(policy, rng);
    sim.turns
}
...
fn solve(init: &[Vec<usize>]) -> CandidateSolution {
    ...
    let mut rng = XorShift64::new(input_seed(init));
    while start.elapsed().as_secs_f64() < TIME_LIMIT_SEC {
        let policy = BucketPolicy::random(&mut rng);
        let dturns = make_bucket_distribution_turns(init, policy, &mut rng);

        let sol = make_solution_from_turns(init, &dturns, &mut memo);
        if sol.turn_count < best.turn_count {
            best = sol;
        }

        if let Some(mixed) = make_solution_from_turns_mixed_all(init, &dturns, &mut memo) {
            if mixed.turn_count < best.turn_count {
                best = mixed;
            }
        }
