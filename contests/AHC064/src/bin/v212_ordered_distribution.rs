...
fn update_best_from_distribution(
    best: &mut CandidateSolution,
    db: &DistBuilder,
    nodes: &[DistNode],
    final_id: usize,
    init: &[Vec<usize>],
    ...
    let sol = make_solution_from_distribution(db, nodes, final_id, init, memo);
    if sol.turn_count < best.turn_count {
        *best = sol;
    }

    let mixed = make_solution_from_distribution_mixed_b(db, nodes, final_id, init, memo);
...
fn solve_route_strategy(
    ...
    for id in final_ids {
        update_best_from_distribution(&mut best, &db, &nodes, id, init, memo);
    }

    if mode == 3 {
        let mut ordered_nodes = Vec::new();
        let ordered_ids = db.beam_final_nodes(&mut ordered_nodes, 1200, 80, 2000, 2, 12);
        for id in ordered_ids {
            update_best_from_distribution(&mut best, &db, &ordered_nodes, id, init, memo);
        }

        for pen in [10, 50] {
            let mut greedy_nodes = Vec::new();
            let gid = db.greedy_final_node(&mut greedy_nodes, pen);
            update_best_from_distribution(&mut best, &db, &greedy_nodes, gid, init, memo);
        }
    }
