...
fn make_solution_from_bucket_distribution(
    init: &[Vec<usize>],
    policy: BucketPolicy,
    rng: &mut XorShift64,
    memo: &mut HashMap<usize, Vec<AbsOp>>,
...
fn solve(init: &[Vec<usize>]) -> CandidateSolution {
    ...
    while start.elapsed().as_secs_f64() < TIME_LIMIT_SEC {
        ...
        let candidate = make_solution_from_bucket_distribution(init, policy, &mut rng, &mut memo);
