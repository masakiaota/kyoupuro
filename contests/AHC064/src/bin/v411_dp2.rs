...
fn relaxed_anchor_score(dep: &[usize], sid: &[usize], r: usize) -> i32 {
    let target = r * INIT_LEN;
    if dep.last().copied() == Some(target)
        || (dep.is_empty() && sid.first().copied() == Some(target))
    {
        1
    } else {
        0
...
fn affected_score(state: &State, i: usize, j: usize) -> i32 {
    let mut score = adjacent_pair_score(&state.dep[i]) + adjacent_pair_score(&state.sid[j]);
    score += relaxed_anchor_score(&state.dep[i], &state.sid[i], i);
    if i != j {
        score += relaxed_anchor_score(&state.dep[j], &state.sid[j], j);
    }
    score
}

fn affected_score_after(
    state: &State,
    i: usize,
    j: usize,
    dep_i: &[usize],
    ...
    let mut score = adjacent_pair_score(dep_i) + adjacent_pair_score(sid_j);
    if i == j {
        score += relaxed_anchor_score(dep_i, sid_j, i);
    } else {
        score += relaxed_anchor_score(dep_i, &state.sid[i], i);
        score += relaxed_anchor_score(&state.dep[j], sid_j, j);
    }
    score
}

fn move_delta_p(state: &State, mv: Move) -> i32 {
    let before = affected_score(state, mv.i, mv.j);
    ...
    affected_score_after(state, mv.i, mv.j, &dep, &sid) - before
}
...
fn solve(input: &Input) -> Output {
    ...
    let debug = std::env::var_os("V411_DEBUG").is_some();
