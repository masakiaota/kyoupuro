...
struct Config {
    ...
    near_bonus: i32,
    full_bonus: i32,
    direct_main_bonus: i32,
    direct_alt_bonus: i32,
    stage_map: u8,
}
...
fn staging_line(iv: Interval, config: &Config) -> usize {
    match config.stage_map {
        1 => (iv.r + iv.u) % R,
        2 => iv.r,
        3 => (iv.r + INIT_LEN - 1 - iv.l) % R,
        _ => (iv.r + iv.l) % R,
    }
}

fn staging_dep_line(iv: Interval, config: &Config) -> usize {
    if iv.l == 0 {
        iv.r
    } else {
        staging_line(iv, config)
    }
}

fn staging_sid_line(iv: Interval, config: &Config) -> usize {
    staging_line(iv, config)
}
...
fn push_dep_to_sidings(
    ...
    for j in 0..R {
        ...
        if preferred_sid == Some(j) {
            bonus += config.stage_bonus;
        }
        bonus += ((SIDING_CAP - state.sid[j].len()) as i32) * config.load_bonus;
        if state.sid[j].is_empty() {
            bonus += config.empty_bonus;
        }
        if i.abs_diff(j) <= 1 {
            bonus += config.near_bonus;
        }
        push_candidate(
            candidates,
            state,
...
fn push_siding_to_deps(
    ...
    for i in 0..R {
        ...
        bonus += ((DEP_CAP - state.dep[i].len()) as i32) * config.load_bonus;
...
fn expose_dep_tail_candidates(
    ...
    if block.area == AREA_DEP {
        ...
        push_siding_to_deps(
            ...
            block,
            avoid_dep,
            Some(staging_dep_line(iv, config)),
        );
    } else {
...
fn expose_sid_head_candidates(
    ...
    if block.area == AREA_SID {
        ...
        push_dep_to_sidings(
            ...
            block,
            avoid_sid,
            Some(staging_sid_line(iv, config)),
        );
    } else {
        let tail = dep_tail_block(state, block.line)?;
        if block_contains_other_protected(tail, protected, iv) {
...
fn job_step_candidates(
    ...
    let Some((a_ready, a_moves)) =
        expose_dep_tail_candidates(state, config, job.a, avoid_dep, avoid_sid, protected)
    else {
        return Vec::new();
    };
    if !a_ready {
    ...
    if let Some(b_probe) = find_block(state, job.b) {
            ...
            && b_probe.end_pos() <= a_block.start
        {
            ...
            push_dep_to_sidings(
                ...
                a_block,
                avoid_sid,
                Some(staging_sid_line(job.a, config)),
            );
            return candidates;
        }
    }
    ...
    let Some((b_ready, b_moves)) =
        expose_sid_head_candidates(state, config, job.b, &avoid_dep_for_b, avoid_sid, protected)
    else {
        return Vec::new();
    };
    if !b_ready {
    ...
    for _ in 0..120 {
        ...
        for &job in &active {
            candidates.extend(job_step_candidates(
                &st, config, job, &avoid_dep, &avoid_sid, &protected,
            ));
        }
...
fn choose_wave_jobs(state: &State, config: &Config) -> Vec<MergeJob> {
    ...
    cands.truncate(config.cand_limit);
    ...
    for &(gain, a, b) in &cands {
        ...
        for node in &beam {
            if node.jobs.len() >= config.max_active_jobs || (node.used[a.r] & bits) != 0 {
                continue;
            }
    ...
    for node in beam {
        ...
        if let Some(score) = simulate_subset_score(state, config, &node.jobs, node.gain) {
            if score > best_score {
                best_score = score;
                best_jobs = node.jobs;
            }
...
fn execute_wave(
    ...
    for _ in 0..120 {
        ...
        for &job in &jobs {
            candidates.extend(job_step_candidates(
                state, config, job, &avoid_dep, &avoid_sid, &protected,
            ));
        }
...
fn solve_with_config(input: &Input, config: Config) -> Output {
    let mut state = State::new(input);
    let mut output = Output::new();
    let debug = std::env::var_os("V409_DEBUG").is_some();

    while completed(&state).iter().any(|&done| !done) {
        let jobs = choose_wave_jobs(&state, &config);
        if jobs.is_empty() {
            if debug {
        ...
        if !execute_wave(&mut state, &mut output, &config, jobs) {
            if debug {
                let done = completed(&state).iter().filter(|&&x| x).count();
...
fn move_count(output: &Output) -> usize {
    output.turns.iter().map(Vec::len).sum()
}

fn portfolio() -> [Config; 7] {
    let base = Config {
        max_active_jobs: 9,
        cand_limit: 36,
        beam_width: 128,
        ...
        near_bonus: 120,
        full_bonus: 7_000,
        direct_main_bonus: 3_000,
        direct_alt_bonus: 400,
        stage_map: 0,
    };
    [
        base,
...
fn solve(input: &Input) -> Output {
    let configs = portfolio();
    let mut best = solve_with_config(input, configs[0]);
    for &config in configs.iter().skip(1) {
        let cand = solve_with_config(input, config);
        let better = cand.turns.len() < best.turns.len()
            || (cand.turns.len() == best.turns.len() && move_count(&cand) < move_count(&best));
        if better {
            best = cand;
        }
    }
