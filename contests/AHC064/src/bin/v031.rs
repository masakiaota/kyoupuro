...
fn noncrossing_pairs(pairs: &[(usize, usize)]) -> bool {
    let mut used_i = 0usize;
    let mut used_j = 0usize;
    for &(i, j) in pairs {
        let bi = 1usize << i;
        let bj = 1usize << j;
        if (used_i & bi) != 0 || (used_j & bj) != 0 {
            return false;
        }
        used_i |= bi;
        used_j |= bj;
...
impl DistBuilder {
    ...
    fn gen_masks_rec(
        ...
        Self::gen_masks_rec(i + 1, used_j, top, res, mask, cnt);
        let j = top[i];
        if j >= 0 {
            let bit = 1usize << (j as usize);
            if (used_j & bit) == 0 {
                Self::gen_masks_rec(i + 1, used_j | bit, top, res, mask | (1usize << i), cnt + 1);
            }
        }
    }
    ...
    fn legal_masks(&self, top: &[isize; R]) -> Vec<(usize, usize)> {
        ...
        Self::gen_masks_rec(0, 0, top, &mut res, 0, 0);
        res
    }
...
fn make_global_sort_move(r: usize, op: AbsOp, aux: usize) -> Move {
    if op.t == OP_A {
        Move::siding_to_dep(aux, r, op.k)
    } else if op.t == OP_B {
        Move::dep_to_siding(aux, r, op.k)
    } else {
        Move::siding_to_dep(r, r, op.k)
    }
}
...
fn distribution_complete_times(dturns: &[Vec<Move>]) -> ([usize; R], [usize; R]) {
    let mut source_remaining = [INIT_LEN; R];
    let mut sid_remaining = [INIT_LEN; R];
    let mut source_done_time = [0usize; R];
    let mut sid_closed_time = [0usize; R];

    for (turn, ops) in dturns.iter().enumerate() {
        let t = turn + 1;
        for &mv in ops {
            source_remaining[mv.i] -= mv.k;
            sid_remaining[mv.j] -= mv.k;
            if source_remaining[mv.i] == 0 && source_done_time[mv.i] == 0 {
                source_done_time[mv.i] = t;
            }
            if sid_remaining[mv.j] == 0 && sid_closed_time[mv.j] == 0 {
                sid_closed_time[mv.j] = t;
            }
        }
    }

    (source_done_time, sid_closed_time)
}

fn best_aux_order_for_overlap(
    g: &[usize],
    c: &[usize],
    progs: &[Vec<AbsOp>],
    source_done_time: &[usize; R],
    sid_closed_time: &[usize; R],
) -> Vec<usize> {
    fn dfs(
        pos: usize,
        g: &[usize],
        ...
        used: &mut [bool; R],
        cur: &mut Vec<usize>,
        best_key: &mut (usize, usize),
        best: &mut Vec<usize>,
    ) {
        if pos == g.len() {
            let mut max_finish = 0usize;
            let mut sum_finish = 0usize;
            for (idx, &r) in g.iter().enumerate() {
                let a = cur[idx];
                let start = source_done_time[r]
                    .max(sid_closed_time[r])
                    .max(source_done_time[a]);
                let finish = start + progs[r].len();
                max_finish = max_finish.max(finish);
                sum_finish += finish;
            }
            let key = (max_finish, sum_finish);
            if key < *best_key {
                *best_key = key;
                *best = cur.clone();
            }
    ...
    let mut used = [false; R];
    let mut cur = Vec::with_capacity(c.len());
    let mut best = c.to_vec();
...
fn execute_overlap_partition(
    ...
    let mut sim = Sim::new(init);
    let mut dist_turn = 0usize;
    let mut source_remaining = [INIT_LEN; R];
    let mut sid_remaining = [INIT_LEN; R];
    let mut source_done = [false; R];
    let mut sid_closed = [false; R];
    let mut idx = [0usize; R];
    ...
    for _guard in 0..1000 {
        ...
        let mut dist_events = Vec::<Move>::new();
        let mut park_now = false;
        let mut restore_now = false;

        if dist_turn < dturns.len() {
            for &mv in &dturns[dist_turn] {
                assert!(add_move_if_free(
                    &mut moves,
                    &mut used_dep,
                    &mut used_sid,
                    mv
        ...
        if !parked {
            for &r in g {
                ...
                let mv = make_global_sort_move(r, progs[r][idx[r]], aux[r]);
                if add_move_if_free(&mut moves, &mut used_dep, &mut used_sid, mv) {
                    sort_events.push(r);
                }
            }

            if g.iter().all(|&r| idx[r] == progs[r].len()) {
                let mut ok = true;
                let mut park_moves = Vec::new();
                for &r in g {
                    let mv = Move::dep_to_siding(r, r, INIT_LEN);
                    if used_dep[mv.i] || used_sid[mv.j] {
                        ok = false;
                        break;
                    }
                ...
                if ok {
                    for mv in park_moves {
                        add_move_if_free(&mut moves, &mut used_dep, &mut used_sid, mv);
                    }
                    park_now = true;
                }
            }
            ...
            for &r in c {
                if idx[r] >= progs[r].len() || !source_done[r] || !sid_closed[r] {
                    continue;
                }
                let mv = make_global_sort_move(r, progs[r][idx[r]], aux[r]);
                if add_move_if_free(&mut moves, &mut used_dep, &mut used_sid, mv) {
            ...
            if c.iter().all(|&r| idx[r] == progs[r].len()) {
                let mut ok = true;
                let mut restore_moves = Vec::new();
                for &r in g {
                    let mv = Move::siding_to_dep(r, r, INIT_LEN);
                    if used_dep[mv.i] || used_sid[mv.j] {
                        ok = false;
                        break;
                    }
                    restore_moves.push(mv);
                }
                if ok {
                    for mv in restore_moves {
                        add_move_if_free(&mut moves, &mut used_dep, &mut used_sid, mv);
                    }
                    restore_now = true;
                }
        ...
        assert!(!moves.is_empty(), "overlap scheduler got stuck");
        sim.add_turn(moves);

        if dist_turn < dturns.len() {
            dist_turn += 1;
        }
        for mv in dist_events {
            source_remaining[mv.i] -= mv.k;
            sid_remaining[mv.j] -= mv.k;
            if source_remaining[mv.i] == 0 {
                source_done[mv.i] = true;
            }
            if sid_remaining[mv.j] == 0 {
                sid_closed[mv.j] = true;
            }
        }
        for r in sort_events {
            idx[r] += 1;
        }
        if park_now {
            parked = true;
...
fn make_solution_overlap_partition(
    db: &DistBuilder,
    nodes: &[DistNode],
    final_id: usize,
    init: &[Vec<usize>],
    ...
    let mut dist_sim = Sim::new(init);
    for ops in &dturns {
        dist_sim.add_turn(ops.clone());
    }
    let sroute = dist_sim.sid.clone();
    let progs = build_sort_progs(&sroute, memo);
    let (source_done_time, sid_closed_time) = distribution_complete_times(&dturns);
    ...
    for mask in 0usize..(1usize << R) {
        ...
        c = best_aux_order_for_overlap(&g, &c, &progs, &source_done_time, &sid_closed_time);

        let sol = execute_overlap_partition(init, &dturns, &progs, &g, &c);
...
fn solve_route_strategy(
    ...
    for id in final_ids {
        ...
        let overlap = make_solution_overlap_partition(&db, &nodes, id, init, memo);
        if overlap.turn_count < best.turn_count {
            best = overlap;
        }
    }
    ...
    if mode == 3 {
        for pen in [10, 50] {
            ...
            let overlap = make_solution_overlap_partition(&db, &greedy_nodes, gid, init, memo);
            if overlap.turn_count < best.turn_count {
                best = overlap;
            }
        }
...
fn noncrossing_subset(moves: &[Move], mask: usize) -> bool {
...
fn split_cost(moves: &[Move]) -> usize {
    let n = moves.len();
    if n == 0 {
        return 0;
    }
    ...
    let size = 1usize << n;
    let mut ok = vec![false; size];
    for (mask, slot) in ok.iter_mut().enumerate().skip(1) {
        *slot = noncrossing_subset(moves, mask);
    }
...
fn relaxed_stats(turns: &[Vec<Move>]) -> (usize, usize, usize, usize, usize) {
    let logical_turns = turns.len();
    let move_count = turns.iter().map(Vec::len).sum::<usize>();
    let mut split_turns = 0usize;
    let mut crossing_turns = 0usize;
    let mut max_width = 0usize;

    for moves in turns {
        max_width = max_width.max(moves.len());
        let cost = split_cost(moves);
        split_turns += cost;
        if cost > 1 {
            crossing_turns += 1;
        }
...
fn main() {
    let init = read_input();
    let sol = solve(&init);
    let (logical_turns, split_turns, crossing_turns, max_width, move_count) =
        relaxed_stats(&sol.turns);
    eprintln!(
        "v031 logical_turns={} split_turns={} crossing_turns={} max_width={} moves={}",
        logical_turns, split_turns, crossing_turns, max_width, move_count
    );
