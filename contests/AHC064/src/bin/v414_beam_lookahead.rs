...
fn best_dp_turn_gain(state: &State) -> i32 {
    turn_candidates(state)
        .into_iter()
        .map(|turn| turn.gain)
        .max()
        .unwrap_or(0)
...
fn best_beam_turn(state: &State) -> Vec<Move> {
    let mut best_score = i32::MIN;
    let mut best_moves = Vec::new();

    for turn in turn_candidates(state) {
        let next_state = apply_turn_clone(state, &turn.moves);
        let score = turn.gain + best_dp_turn_gain(&next_state);
        if score > best_score {
            best_score = score;
            best_moves = turn.moves;
        }
    }
...
fn execute_greedy_dp_improvements(state: &mut State, output: &mut Output) {
    for _ in 0..200 {
...
fn solve_attempt(input: &Input, use_beam: bool) -> Option<Output> {
    let mut state = State::new(input);
    let mut output = Output::new();
    let debug = std::env::var_os("V414_DEBUG").is_some();

    if use_beam {
        execute_dp_improvements(&mut state, &mut output);
    } else {
        execute_greedy_dp_improvements(&mut state, &mut output);
    }
...
fn solve(input: &Input) -> Output {
    solve_attempt(input, true)
        .or_else(|| solve_attempt(input, false))
