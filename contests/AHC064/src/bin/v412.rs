...
fn build_edges(state: &State) -> [[Option<DpEdge>; R]; R] {
...
fn best_dp_gain(state: &State) -> i32 {
...
fn push_candidate(cands: &mut Vec<TurnCandidate>, cand: TurnCandidate) {
...
fn prune_candidates(cands: &mut Vec<TurnCandidate>) {
    const BEAM: usize = 64;
    cands.sort_by(|a, b| {
        b.gain
            .cmp(&a.gain)
...
fn apply_turn_clone(state: &State, moves: &[Move]) -> State {
    ...
    for i in 0..=R {
        for j in 0..=R {
            ...
            if i < R {
                ...
                for cand in cur {
                    push_candidate(&mut dp[i + 1][j], cand);
                }
                prune_candidates(&mut dp[i + 1][j]);
            }

            if j < R {
                let cur = dp[i][j].clone();
                for cand in cur {
                    push_candidate(&mut dp[i][j + 1], cand);
                }
                prune_candidates(&mut dp[i][j + 1]);
            }
            ...
            if i < R && j < R {
                if let Some(e) = edge[i][j] {
                    ...
                    for mut cand in cur {
                        cand.gain += e.gain;
                        cand.moves.push(e.mv);
                        push_candidate(&mut dp[i + 1][j + 1], cand);
                    }
                    prune_candidates(&mut dp[i + 1][j + 1]);
                }
            }
        }
    }

    let best_gain = dp[R][R].iter().map(|cand| cand.gain).max().unwrap_or(0);
    if best_gain <= 0 {
        return Vec::new();
    }

    let mut best_moves = Vec::new();
    let mut best_next_gain = i32::MIN;
    for cand in dp[R][R].iter().filter(|cand| cand.gain == best_gain) {
        let next = apply_turn_clone(state, &cand.moves);
        let next_gain = best_dp_gain(&next);
        if next_gain > best_next_gain {
            best_next_gain = next_gain;
            best_moves = cand.moves.clone();
        }
    }
    best_moves
}
...
fn solve(input: &Input) -> Output {
    ...
    let debug = std::env::var_os("V412_DEBUG").is_some();
