...
impl Solver {
    ...
    fn suffix_run_len(&self, i: usize) -> usize {
        ...
        let j = Self::target_line(*self.dep[i].last().unwrap());
        let mut k = 0usize;
        while k < len {
    ...
    fn make_bucket_candidates(
        ...
        for i in 0..R {
            ...
            let j = Self::target_line(*self.dep[i].last().unwrap());
            let max_k = self.suffix_run_len(i);
            let mut k = max_k;
    ...
    fn initial_grouping_bucket(&mut self, policy: BucketPolicy, rng: &mut XorShift64) {
    ...
    fn finish_grouped_with_best_phase2(&mut self) {
        for j in 0..R {
    ...
    fn solve(&mut self) {
        ...
        self.finish_grouped_with_best_phase2();

        let mut best_dep = self.dep.clone();
        let mut best_sid = self.sid.clone();
        let mut best_turns = self.turns.clone();
        ...
        while start.elapsed().as_secs_f64() < TIME_LIMIT_SEC {
            ...
            candidate.initial_grouping_bucket(policy, &mut rng);
            candidate.finish_grouped_with_best_phase2();

            if candidate.turns.len() < best_turns.len() {
                best_dep = candidate.dep;
                best_sid = candidate.sid;
                best_turns = candidate.turns;
            }
        }
