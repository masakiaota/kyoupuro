...
const BEAM_WIDTH: usize = 220;
const K_LOOK: usize = 60;
const EXTRA_DEPTH: usize = 10;
const DMAX: usize = 80;
const LEN_PENALTY: f32 = 1.2;
const BEAM_TIME_LIMIT: f64 = 1.94;
...
impl Solver {
    ...
    fn build_loops(&mut self) {
        ...
        let mut cols = self.split_cols_around_exit(V_BLOCK, COL_OFFSET);
        cols.reverse();
        for (c, w) in cols {
            let cells = self.rect_cycle(0, N, c, w);
            self.solution.add_conveyor(&cells);
        }
        for (r, h) in self.split_rows(H_BLOCK, ROW_OFFSET) {
