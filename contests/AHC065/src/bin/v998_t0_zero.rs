...
impl<'a> Solver<'a> {
    ...
    fn solve(&mut self) -> ResultData {
        ...
        let (moves, delivered) = self.solve0_with_count(0);
        let score = if delivered == self.input.nn {
            1_000_000.0
                + (1_000_000.0 * (100_000.0 / moves.len() as f64).ln() / 2.0_f64.ln()).round()
        } else {
            (1_000_000.0 * delivered as f64 / self.input.nn as f64).round()
        };
    ...
    fn solve0_with_count(&self, t0: i32) -> (Vec<Move>, usize) {
