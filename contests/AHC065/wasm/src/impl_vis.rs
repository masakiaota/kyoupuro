...
pub fn parse_input(f: &str) -> Result<Input, String> {
    let mut f = f.split_whitespace();
    let N = read(f.next(), 1..=200usize)?;
    let mut a = mat![0; N; N];
    let mut seen = vec![false; N * N];
    for i in 0..N {
        for j in 0..N {
            let v = read(f.next(), 0..=(N * N - 1) as i32)?;
            if seen[v as usize] {
                return Err(format!("Duplicated box number: {}", v));
            }
...
pub fn gen(seed: u64) -> Input {
    ...
    let N = 20;
    let mut a0 = (0..(N * N) as i32).collect::<Vec<_>>();
    a0.shuffle(&mut rng);
    let mut a = mat![0; N; N];
    for i in 0..N {
        for j in 0..N {
...
pub fn compute_score_details(
    ...
    for &(m, d) in &out.out[..t.min(out.out.len())] {
...
pub fn calc_max_turn(input: &str, output: &str) -> usize {
    ...
    let Ok(output) = parse_output(&input, output) else {
        return 0;
    };
    output.out.len()
}
...
pub fn visualize(input: &str, output: &str, turn: usize) -> Result<(i64, String, String), String> {
    ...
    if output.trim().is_empty() {
        ...
        return Ok(vis(&input, &out, 0, true));
    }

    match parse_output(&input, output) {
        Ok(out) => Ok(vis(&input, &out, turn, true)),
        Err(err) => {
            let out = Output {
                conveyers: vec![],
                out: vec![],
            };
            let (_, _, svg) = vis(&input, &out, 0, true);
            Ok((0, err, svg))
        }
    }
}
