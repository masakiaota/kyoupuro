...
impl Solver {
    ...
    fn collect_candidates(
        ...
        for v in 0..b {
            ...
            let (cell, cell_depth) = cell_depths[0];
            let cells = cell_depths
                .drain(..)
                .map(|(cell, _)| cell)
                .collect::<Vec<_>>();
            let trip_cost = cell_depth;
    ...
    fn anneal_assignment(
        ...
        for i in 0..n {
            ...
            by_switch[i].sort_unstable_by(|&a, &b| {
                let ca = &cands[a];
                let cb = &cands[b];
                let sa = 12 * ca.len as i32 + 5 * ca.depth as i32 + 4 * ca.trip_cost;
                let sb = 12 * cb.len as i32 + 5 * cb.depth as i32 + 4 * cb.trip_cost;
...
fn make_assignment(
    ...
    for &si in &order {
        ...
        for (ci, c) in cands.iter().enumerate() {
            ...
            let len_score = c.depth as f64 + 1.5 * c.len as f64 + 0.65 * trip_score;
            let score = if mode == 1 {
                1000.0 * c.len as f64 + c.depth as f64 + 4.0 * trip_score + rng.next_double()
            } else if mode == 3 {
                (si + 1) as f64 * 200.0 + c.len as f64 * 20.0 + 1.5 * trip_score + rng.next_double()
            } else {
