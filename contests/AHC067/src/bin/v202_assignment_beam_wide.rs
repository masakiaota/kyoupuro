...
impl Solver {
    ...
    fn assignment_beam(
        ...
        for (ci, c) in cands.iter().enumerate() {
            ...
            candidate_value[ci] = c.trip_cost as f64
                ...
                + 2.5 * c.len as f64
                + 0.8 * c.depth as f64
                + 1.5 * c.cell_depth.max(0) as f64;
        }

        let mut beam = vec![AssignBeamState::new()];
        for &si in order {
            ...
            for state in &beam {
                for (ci, c) in cands.iter().enumerate() {
                    ...
                    ns.assign[si] = ci;
                    ns.score += cnt[si] as f64 * candidate_value[ci]
                        + (si as f64 + 1.0) * c.len as f64
                        + c.depth as f64 * 0.25;
    ...
    fn best_assignment_beam_solution(
        &mut self,
        cands: &[Candidate],
        n: usize,
        final_edges: &[usize],
        ...
        for order in &orders {
            ...
            let mut got = self.assignment_beam(cands, n, order, beam_width, eval_keep);
        ...
        for assign in assignments.into_iter().take(eval_keep * orders.len()) {
            if timer.sec() >= deadline {
                break;
            }
    ...
    fn solve_chinese_original(&mut self, timer: &Timer, deadline: f64) -> Solution {
        ...
        for t in 1..path.len() {
            ...
            for n in (2..=10).rev() {
                ...
                if n == K && timer.sec() < deadline {
                    if let Some((assign, sol)) = self.best_assignment_beam_solution(
                        &cands,
                        n,
                        &[final_edge],
                        &[],
    ...
    fn solve_augmented_goal_cage(&mut self, timer: &Timer, deadline: f64) -> Solution {
        ...
        while timer.sec() < deadline {
            ...
            if n == K && timer.sec() < deadline {
                if let Some((_assign, sol)) = self.best_assignment_beam_solution(
                    &cands,
                    n,
                    &final_edges,
                    &extra,
