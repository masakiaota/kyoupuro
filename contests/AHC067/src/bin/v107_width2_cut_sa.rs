...
const WIDTH2_SEARCH_LIMIT_SEC: f64 = PROGRAM_TIME_LIMIT_SEC * 0.965;
const WIDTH2_REGION_LIMIT: usize = 260;
const WIDTH2_FINAL_LIMIT: usize = 80;
...
impl Solver {
    ...
    fn find_bridges_with_banned_edge(&self, banned: usize) -> Vec<bool> {
        fn dfs(
            solver: &Solver,
            banned: usize,
            v: usize,
            ...
            *timer += 1;
            ...
            for &(to, e) in &solver.adj[v] {
                ...
                if tin[to] >= 0 {
                    ...
                    dfs(solver, banned, to, e, tin, low, timer, is_bridge);
                    low[v] = low[v].min(low[to]);
                    if low[to] > tin[v] {
                        is_bridge[e] = true;
                    }
    ...
    #[inline(always)]
    fn bit_set(bits: &mut [u64; BIT_WORDS], cell: usize) {
        bits[cell >> 6] |= 1_u64 << (cell & 63);
    }

    #[inline(always)]
    fn bit_contains(bits: &[u64; BIT_WORDS], cell: usize) -> bool {
        ((bits[cell >> 6] >> (cell & 63)) & 1) != 0
    }

    fn bit_subset(a: &[u64; BIT_WORDS], b: &[u64; BIT_WORDS]) -> bool {
        a.iter().zip(b.iter()).all(|(&x, &y)| (x & !y) == 0)
    }

    fn bfs_bits_without_edges(&self, banned_a: usize, banned_b: usize) -> [u64; BIT_WORDS] {
        let mut bits = [0_u64; BIT_WORDS];
        let mut q = [0usize; C];
        let mut h = 0usize;
        let mut t = 0usize;
        Self::bit_set(&mut bits, START);
        q[t] = START;
        t += 1;
        ...
        while h < t {
            ...
            h += 1;
            for &(to, e) in &self.adj[v] {
                if e == banned_a || e == banned_b || Self::bit_contains(&bits, to) {
                    continue;
                }
                Self::bit_set(&mut bits, to);
                q[t] = to;
                t += 1;
            }
        }
    ...
    fn complement_region_cells(&self, bits: &[u64; BIT_WORDS]) -> (Vec<usize>, [u64; BIT_WORDS]) {
        let mut cells = Vec::new();
        let mut region_bits = [0_u64; BIT_WORDS];
        for cell in 0..C {
            if self.grid[cell] == b'.' && !Self::bit_contains(bits, cell) {
                cells.push(cell);
                Self::bit_set(&mut region_bits, cell);
            }
        }
        (cells, region_bits)
    }

    fn choose_region_cell(&self, region: &Region2, cut: Cut2) -> usize {
        let mut d = [-1_i32; C];
        let mut q = [0usize; C];
        let mut h = 0usize;
        let mut t = 0usize;
        ...
        for ei in 0..cut.len {
            ...
            for cell in [edge.a, edge.b] {
                if Self::bit_contains(&region.bits, cell) && d[cell] < 0 {
                    d[cell] = 0;
                    q[t] = cell;
                    t += 1;
                }
        ...
        while h < t {
            ...
            for &(to, _) in &self.adj[v] {
                if !Self::bit_contains(&region.bits, to) || d[to] >= 0 {
                    continue;
                }
                d[to] = d[v] + 1;
                q[t] = to;
    ...
    fn enumerate_width2_cuts(&self) -> (Vec<Cut2>, Vec<Region2>, Vec<FinalCut2>) {
        let natural_bridge = self.find_bridges();
        ...
        for banned in 0..self.edges.len() {
            if natural_bridge[banned] {
                continue;
            }
            let bridges = self.find_bridges_with_banned_edge(banned);
            for other in (banned + 1)..self.edges.len() {
                if natural_bridge[other] || !bridges[other] {
                    continue;
                }
                if !pair_seen.insert((banned, other)) {
                    continue;
                }

                let prefix_bits = self.bfs_bits_without_edges(banned, other);
                let start_goal_connected = Self::bit_contains(&prefix_bits, GOAL);
                let cut_id = cuts.len();
                cuts.push(Cut2 {
                    edges: [banned, other],
                    len: 2,
                ...
                if start_goal_connected {
                    let (cells, bits) = self.complement_region_cells(&prefix_bits);
                    if !cells.is_empty()
                        && !Self::bit_contains(&bits, START)
                        && !Self::bit_contains(&bits, GOAL)
                        && region_seen.insert(bits)
                    {
                ...
                } else if final_seen.insert(prefix_bits) {
                    finals.push(FinalCut2 {
                        cut: cut_id,
                        prefix_bits,
                    });
                }
        ...
        regions.sort_unstable_by(|a, b| b.size.cmp(&a.size).then_with(|| a.cut.cmp(&b.cut)));
        if regions.len() > WIDTH2_REGION_LIMIT {
            regions.truncate(WIDTH2_REGION_LIMIT);
        }

        finals.sort_unstable_by(|a, b| a.cut.cmp(&b.cut));
        if finals.len() > WIDTH2_FINAL_LIMIT {
            finals.truncate(WIDTH2_FINAL_LIMIT);
        }
    ...
    fn build_width2_candidates(
        ...
        for (idx, region) in regions.iter().enumerate() {
            if Self::bit_subset(&region.bits, &final_cut.prefix_bits) {
                active.push(idx);
            }
        }
        if active.is_empty() {
        ...
        for &ri in &active {
            ...
            chain.push((region.size, region.cut));
            for &aj in &active {
                if aj == ri {
                    continue;
                }
                let anc = &regions[aj];
                if region.size < anc.size && Self::bit_subset(&region.bits, &anc.bits) {
                    chain.push((anc.size, anc.cut));
                }
            }
            chain.sort_unstable_by(|a, b| a.0.cmp(&b.0).then_with(|| a.1.cmp(&b.1)));
            chain.dedup_by_key(|(_, cut)| *cut);

            let mut tail = Vec::new();
            let mut total_cost = 0usize;
            for (_, cut_id) in chain {
                let cost = cuts[cut_id].len;
            ...
            let cell = self.choose_region_cell(region, cuts[region.cut]);
            cands.push(Candidate2 {
                cell,
                len: tail.len(),
                depth: tail.len(),
                full: tail.clone(),
        ...
        cands.truncate(180.min(cands.len()));
        cands
    }

    fn set_cut_door(door_kind: &mut [i32], cuts: &[Cut2], cut_id: usize, g: i32) -> bool {
        let cut = cuts[cut_id];
    ...
    fn build_from_width2_assignment(
        &mut self,
        cuts: &[Cut2],
        cands: &[Candidate2],
        assign: &[usize],
        ...
        for i in 0..n {
            ...
            if i > 0 {
                for j in 0..(i - 1) {
                    if !Self::set_cut_door(&mut door_kind, cuts, c.tail[j], (2 * j) as i32) {
                        return Solution::new();
                    }
                }
                if !Self::set_cut_door(
                    &mut door_kind,
                    cuts,
                    c.tail[i - 1],
                    (2 * (i - 1) + 1) as i32,
        ...
        if !Self::set_cut_door(&mut door_kind, cuts, final_cut, (2 * (n - 1) + 1) as i32) {
            return Solution::new();
        }
    ...
    fn make_width2_assignment(
        ...
        for &si in &order {
            ...
            for (ci, c) in cands.iter().enumerate() {
                ...
                let mut ok = true;
                for &cut_id in &c.full {
                    let cut = cuts[cut_id];
                    for idx in 0..cut.len {
                        if used_door[cut.edges[idx]] {
                            ok = false;
                            break;
                        }
                    }
                ...
                for p in 0..si {
                    let cut = cuts[c.tail[p]];
                    for idx in 0..cut.len {
                        if protected_edge[cut.edges[idx]] {
                            ok = false;
                            break;
                        }
                    }
                ...
                let cut_cost: usize = c.tail.iter().map(|&cut_id| cuts[cut_id].len).sum();
                let len_score = c.depth as f64 + 0.8 * cut_cost as f64;
                let score = if mode == 1 {
            ...
            let take = if mode >= 4 && picks.len() >= 2 {
                rng.next_int(5.min(picks.len()))
            } else {
                0
            };
            let ci = picks[take].1;
            ...
            for &cut_id in &cands[ci].full {
                let cut = cuts[cut_id];
                for idx in 0..cut.len {
                    protected_edge[cut.edges[idx]] = true;
                }
            }
            for p in 0..si {
                let cut = cuts[cands[ci].tail[p]];
                for idx in 0..cut.len {
                    used_door[cut.edges[idx]] = true;
                }
            }
        }
    ...
    fn valid_width2_assignment(
        ...
        let mut owner = vec![-1_i32; self.edges.len()];
        let mut used_cand = vec![false; cands.len()];
        let mut used_cell = [false; C];

        for i in 0..n {
            let ci = assign[i];
            if ci >= cands.len() || used_cand[ci] || cands[ci].len < i {
                return false;
            }
            used_cand[ci] = true;
    ...
    fn anneal_width2_assignment(
        ...
        if !self.valid_width2_assignment(cuts, cands, n, &init_assign) {
            return;
        }

        let cur_sol = self.build_from_width2_assignment(cuts, cands, &init_assign, n, final_cut);
        if cur_sol.t == 0 {
            return;
        }
        ...
        while timer.sec() < WIDTH2_SEARCH_LIMIT_SEC {
            let si = if (rng.next() & 7) == 0 {
                rng.next_int(n)
            } else {
                let r = rng.next_int(100);
            ...
            cur[si] = ni;
            if !self.valid_width2_assignment(cuts, cands, n, &cur) {
                cur[si] = old;
                continue;
            }

            let nxt = self.build_from_width2_assignment(cuts, cands, &cur, n, final_cut);
            let nt = nxt.t;
            if nt == 0 {
                cur[si] = old;
                continue;
            ...
            let progress = 1.0_f64.min(timer.sec() / WIDTH2_SEARCH_LIMIT_SEC);
            let temp = 1800.0 * (1.0 - progress) + 25.0 * progress;
            let mut accept = nt >= cur_t;
            if !accept {
                accept = rng.next_double() < ((nt as f64 - cur_t as f64) / temp).exp();
            }
    ...
    fn solve_width2_cut_chinese(&mut self, timer: &Timer) -> Solution {
        if timer.sec() > WIDTH2_SEARCH_LIMIT_SEC {
            return Solution::new();
        }

        let (cuts, regions, finals) = self.enumerate_width2_cuts();
        if cuts.is_empty() || regions.is_empty() || finals.is_empty() {
            return Solution::new();
        }

        let mut rng = XorShift::new(
            0x9e37_79b9_7f4a_7c15_u64 ^ self.edges.len() as u64 ^ regions.len() as u64 * 1009,
        );
        let mut best = Solution::new();
        let mut best_cands = Vec::new();
        let mut best_assign = Vec::new();
        let mut best_final_cut = usize::MAX;
        let mut best_n = 0usize;

        for final_info in &finals {
            if timer.sec() > WIDTH2_SEARCH_LIMIT_SEC {
                break;
            }
            let cands = self.build_width2_candidates(&cuts, &regions, final_info);
            if cands.is_empty() {
                continue;
            }
            ...
            for n in (2..=9).rev() {
                ...
                for mode in 0..attempts {
                    ...
                    let real_mode = if mode < 5 { mode } else { 4 };
                    let mut assign = Vec::new();
                    if !self.make_width2_assignment(
                        &cuts,
                        &cands,
                        n,
                        real_mode,
                    ...
                    let sol = self.build_from_width2_assignment(
                        &cuts,
                        &cands,
                        &assign,
                        n,
        ...
        if best_final_cut != usize::MAX && timer.sec() < WIDTH2_SEARCH_LIMIT_SEC {
            self.anneal_width2_assignment(
                &cuts,
                &best_cands,
                best_n,
                best_final_cut,
...
fn main() {
    ...
    if best.t < 900 {
        let width2 = solver.solve_width2_cut_chinese(&timer);
        if width2.t > best.t {
            best = width2;
        }
    }
