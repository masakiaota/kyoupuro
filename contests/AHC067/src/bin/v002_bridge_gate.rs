...
struct SolveStats {
    ...
    baseline_t: usize,
    best_estimated_t: usize,
    verified_t: usize,
    used_bridge_gate: bool,
}
...
fn solve(input: &Input) -> (Output, SolveStats) {
    ...
    let baseline = CastlePlan::empty();
    stats.baseline_t = baseline.calc_t_with_scratch(input, &mut scratch);

    let best_gate = find_best_bridge_gate(input, &mut stats);
    let Some(gate) = best_gate else {
        return (baseline.to_output(), stats);
    };

    stats.best_estimated_t = gate.estimated_t;
    let plan = make_bridge_gate_plan(gate);
    stats.verified_t = plan.calc_t_with_scratch(input, &mut scratch);

    if stats.verified_t > stats.baseline_t {
        stats.used_bridge_gate = true;
        (plan.to_output(), stats)
    } else {
        (baseline.to_output(), stats)
    }
}
...
fn main() {
    ...
    local! {
        ...
        trace.count_by("bridge_count", stats.bridge_count as i64);
        trace.count_by("separating_bridge_count", stats.separating_bridge_count as i64);
        trace.count_by("candidate_count", stats.candidate_count as i64);
        trace.count_by("baseline_t", stats.baseline_t as i64);
        trace.count_by("best_estimated_t", stats.best_estimated_t as i64);
        trace.count_by("verified_t", stats.verified_t as i64);
        if !stats.used_bridge_gate {
            trace.mark_fallback();
        }
