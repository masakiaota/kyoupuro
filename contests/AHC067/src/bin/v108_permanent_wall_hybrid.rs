...
const WALL_BIT: usize = 9;
const WALL_DOOR_KIND: i32 = (2 * WALL_BIT + 1) as i32;
...
impl Solver {
    ...
    fn set_wall_leaf_door(door_kind: &mut [i32], door_count: &mut usize, e: usize, g: i32) -> bool {
        if door_kind[e] >= 0 && door_kind[e] != g {
            return false;
        }
        if door_kind[e] < 0 {
    ...
    fn choose_entry_edge_for_wall_leaf(
        &self,
        u: usize,
        dist_all: &[[i32; C]],
        used_cells: &[bool; C],
        ...
        let mut best = None;
        for &(to, e) in &self.adj[u] {
            if to == GOAL || used_cells[to] {
                continue;
            }
            let score = dist_all[START][to] * 10 + self.adj[to].len() as i32;
            if best.is_none_or(|(best_score, _)| score < best_score) {
                best = Some((score, e));
            }
        }
    ...
    fn build_wall_leaf_solution(
        &mut self,
        seq: &[usize],
        len: usize,
        dist_all: &[[i32; C]],
    ) -> Option<Solution> {
        if len == 0 || len > WALL_BIT {
            return None;
        }
        ...
        let mut door_kind = vec![-1_i32; self.edges.len()];
        let mut door_count = 0usize;
        let mut used_cells = [false; C];

        for idx in 0..len {
            let u = seq[idx];
            if sw[u] >= 0 || u == START || u == GOAL {
                return None;
            }
            let entry_edge = self.choose_entry_edge_for_wall_leaf(u, dist_all, &used_cells)?;
            used_cells[u] = true;
            sw[u] = idx as i32;

            let adj_edges = self.adj[u].clone();
            for &(_, e) in &adj_edges {
                if e == entry_edge {
                    if idx >= 1 {
                        let g = (2 * (idx - 1) + 1) as i32;
                        if !Self::set_wall_leaf_door(&mut door_kind, &mut door_count, e, g) {
                            return None;
                        }
                    }
                } else if !Self::set_wall_leaf_door(
                    &mut door_kind,
                    &mut door_count,
                    e,
                    WALL_DOOR_KIND,
        ...
        let final_g = (2 * (len - 1) + 1) as i32;
        let goal_edges = self.adj[GOAL].clone();
        for &(_, e) in &goal_edges {
            if !Self::set_wall_leaf_door(&mut door_kind, &mut door_count, e, final_g) {
                return None;
            }
        }
    ...
    fn construct_wall_leaf_chain(
        &mut self,
        mode: usize,
        beam_w: usize,
        cand_limit: usize,
        max_counter_bits: usize,
        dist_all: &[[i32; C]],
        is_art: &[bool; C],
        reach_no_goal: &[bool; C],
    ) -> Solution {
        ...
        cand.sort_unstable_by(|&a, &b| {
            let score = |u: usize| -> i32 {
                let deg = self.adj[u].len() as i32;
                if mode == 0 {
                    110 * ecc[u] + 55 * (dist_all[START][u] + dist_all[u][GOAL]) - 18 * deg
                } else if mode == 1 {
                    90 * ecc[u] + 80 * dist_all[START][u] + 35 * dist_all[u][GOAL] - 15 * deg
                } else {
                    60 * (dist_all[START][u] + dist_all[u][GOAL])
                        + 110 * dist_all[START][u].min(dist_all[u][GOAL])
                        - 12 * deg
                }
            };
            score(b).cmp(&score(a))
        ...
        let seq = self.beam_cell_sequence(&cand, dist_all, max_counter_bits, beam_w, true);
        let mut best = Solution::new();
        for len in 1..=seq.len().min(max_counter_bits) {
            if let Some(sol) = self.build_wall_leaf_solution(&seq, len, dist_all) {
                if sol.t > best.t {
                    best = sol;
                }
            }
    ...
    fn solve_permanent_wall_candidates(&mut self) -> Solution {
        let dist_all = self.all_pairs_dist();
        let is_art = self.calc_articulation();
        let reach_no_goal = self.calc_reach_no_goal();
        let mut best = Solution::new();
        for &(max_bits, mode, beam_w, cand_limit) in &[
            (9usize, 0usize, 220usize, 90usize),
            (9usize, 1usize, 200usize, 80usize),
            (9usize, 2usize, 200usize, 80usize),
            (8usize, 0usize, 220usize, 90usize),
            (8usize, 1usize, 200usize, 80usize),
        ] {
            let sol = self.construct_wall_leaf_chain(
                mode,
                beam_w,
                cand_limit,
                max_bits,
...
fn main() {
    ...
    let permanent_wall_limit = std::env::var("V108_PERMANENT_WALL_LIMIT")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .unwrap_or(0);
    if permanent_wall_limit > 0 && best.t < permanent_wall_limit {
        let wall = solver.solve_permanent_wall_candidates();
        if wall.t > best.t {
            best = wall;
        }
    }
