...
impl Solver {
    ...
    fn collect_candidates_original(
        ...
        for v in 0..b {
            ...
            let mut cells =
                self.candidate_cells_in_comp_from_parent(bt, active, v, Some(pedge[v]), 5);
    ...
    fn solve_augmented_goal_cage(&mut self, timer: &Timer, deadline: f64) -> Solution {
        ...
        while timer.sec() < deadline {
            ...
            let extra = non.into_iter().take(remain).collect::<Vec<_>>();
...
fn main() {
    ...
    solver.improve_path_doors(&mut best, &timer, POST_IMPROVE_LIMIT_SEC, &mut rng);
