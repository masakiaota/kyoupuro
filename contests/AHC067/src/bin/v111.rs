...
const CHINESE_LOOP_LIMIT_SEC: f64 = PROGRAM_TIME_LIMIT_SEC * (1.50 / JUDGE_TIME_LIMIT_SEC);
const ASSIGN_LIMIT_SEC: f64 = PROGRAM_TIME_LIMIT_SEC * (1.55 / JUDGE_TIME_LIMIT_SEC);
const ANNEAL_CALL_LIMIT_SEC: f64 = PROGRAM_TIME_LIMIT_SEC * (1.56 / JUDGE_TIME_LIMIT_SEC);
const ANNEAL_LIMIT_SEC: f64 = PROGRAM_TIME_LIMIT_SEC * (1.60 / JUDGE_TIME_LIMIT_SEC);
const VIRTUAL_CALL_LIMIT_SEC: f64 = PROGRAM_TIME_LIMIT_SEC * (1.60 / JUDGE_TIME_LIMIT_SEC);
...
impl Solver {
    ...
    fn candidate_cells_in_comp_from_parent(
        ...
        scored.dedup_by_key(|(_, _, cell)| *cell);
        scored.truncate(limit);
        scored
            .into_iter()
            .map(|(_, depth, cell)| (cell, depth))
            .collect()
    }
    ...
    fn collect_candidates(
        ...
        for v in 0..b {
            ...
            let depth = full.len();
            let mut cell_depths =
                self.candidate_cells_in_comp_from_parent(bt, v, Some(pedge[v]), 5);
            let (cell, _) = cell_depths[0];
            let mut cells = Vec::with_capacity(cell_depths.len() + 1);
            cells.push(cell);
            for (alt, _) in cell_depths.drain(..) {
                if alt != cell {
                    cells.push(alt);
                }
            }
    ...
    fn anneal_assignment(
        ...
        while timer.sec() < ANNEAL_LIMIT_SEC {
            ...
            let old = cur[si];
            let pool = &by_switch[si];
            let ni = if pool.len() >= 8 && (rng.next() & 3) != 0 {
                let lim = if si <= 2 { 18 } else { 30 }.min(pool.len());
                pool[rng.next_int(lim)]
            } else {
                pool[rng.next_int(pool.len())]
            };
            if ni == old {
                continue;
            }
    ...
    fn improve_path_doors(&mut self, sol: &mut Solution, timer: &Timer, rng: &mut XorShift) {
        ...
        while sol.doors.len() < M && timer.sec() < POST_IMPROVE_LIMIT_SEC {
            ...
            let eval_cap = candidates.len().min((18.0 + 70.0 * remain_ratio) as usize);
    ...
    fn build_spanning_tree(&self, mode: usize, rng: &mut XorShift) -> VTree {
        ...
        if mode == 0 {
            ...
            while h < t {
                ...
                h += 1;
                vt.order.push(v);
                let ns = self.spanning_tree_neighbor_order(mode, v, rng);
                for (to, e) in ns {
                    if vt.parent[to] == -2 {
