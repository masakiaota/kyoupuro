...
impl Solver {
    ...
    fn build_graph(&mut self) {
        ...
        self.deg.fill(0);
        for v in 0..C {
            self.deg[v] = self.adj[v].len();
            for (k, &(to, e)) in self.adj[v].iter().enumerate() {
                self.to_arr[v][k] = to;
                self.edge_arr[v][k] = e;
            }
        }
    ...
    fn calc_t_internal(
        ...
        while head < self.queue.len() {
            ...
            for k in 0..self.deg[v] {
                let to = self.to_arr[v][k];
                let e = self.edge_arr[v][k];
    ...
    fn build_from_assignment(
        ...
        self.build_from_assignment_with_extra_kind(cands, assign, n, final_edges, extra_edges, None)
    }

    fn build_from_assignment_with_extra_kind(
        &mut self,
        cands: &[Candidate],
        assign: &[usize],
        n: usize,
        final_edges: &[usize],
        extra_edges: &[usize],
        extra_kind: Option<usize>,
    ) -> Solution {
        let mut door_kind = vec![-1; self.edges.len()];
        let final_g = (2 * (n - 1) + 1) as i32;
        let extra_g = extra_kind.map_or(final_g, |g| g as i32);
    ...
    fn solve_augmented_goal_cage(&mut self, timer: &Timer, deadline: f64) -> Solution {
        ...
        while timer.sec() < deadline {
            ...
            for mode in 0..attempts {
                ...
                if n < K && !extra.is_empty() && timer.sec() < deadline {
                    let sol = self.build_from_assignment_with_extra_kind(
                        &cands,
                        &assign,
                        n,
                        &final_edges,
