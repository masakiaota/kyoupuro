        ...
        for j in 0..m {
            if sorted[j].attach_idx < path_edges_len {
                if layout_mode == 0 {
                    ...
                    // v905: keep S1..S9 as a compact goal-side inner gadget.
    ...
    fn solve_spine_corridor(&mut self, timer: &Timer, deadline: f64) -> (Solution, PermanentCorridorStats) {
        ...
        for l in (6usize..=9).rev() {
            ...
            while timer.sec() < deadline * 0.45 && bases.len() < target && attempts_for_l < 12_000 {
                attempts_for_l += 1;
                stats.trials += 1;
                let extra_lim = if l >= 8 { 17 } else { 20 };
...
fn main() {
    ...
    let (mut best, direct_stats) =
        solver.solve_spine_corridor(&timer, PERMANENT_CORRIDOR_LIMIT_SEC);
    solver.improve_final_bit_walls(&mut best, &timer, POST_IMPROVE_LIMIT_SEC);
    ...
    #[cfg(feature = "local")]
    {
        ...
        if switch9_count != 0 || max_switch > 8 {
            eprintln!("[v905.error] spine experiment reserves switch kind 9 for permanent walls");
        }
    }
