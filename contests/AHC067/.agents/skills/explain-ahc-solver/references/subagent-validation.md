...
解説の検証では、subagent が元 solver を読まずに、解説だけから十分な solver を作れるかを見る。

検証対象は、文章点検後の解説本文である。
粗い本文で検証し、その後に本文を整える順序にしてはいけない。
...
検証 agent には、元 solver、関連する過去版、再現済み solver を読ませない。
...
閾値未満なら、まず検証 agent の報告から曖昧点を拾う。

ユーザーが許可した場合だけ、同じ agent に元 solver を読ませて、解説との差分を分析させる。
この分析は解説改善のためであり、再検証には必ず別の新規 agent を使う。
...
一発に近い実装で閾値を超えた場合、解説は再現性を持つ可能性が高い。
何度も評価して最後に超えた場合、本文へ運用原則を戻す。
