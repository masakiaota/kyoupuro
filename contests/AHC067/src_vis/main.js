import init, {
  prepare_case as wasmPrepareCaseModule,
  vis_prepared as wasmVisPreparedModule,
} from "./wasm/ahc067_vis.js";

let wasmPrepareCase = null;
let wasmVisPrepared = null;

const CUSTOM_CASE_VALUE = "";
const BASE_RENDER_FPS = 60;
...
function setHeroActions(value) {
  els.heroActions.textContent = `actions: ${formatScore(value)}`;
}
...
function prepareVisualization() {
  ...
  state.preparedHasOutput = els.outputArea.value.trim() !== "";
  state.prepareError = "";
  ...
  try {
    const maxTurn = Number(wasmPrepareCase(els.inputArea.value, els.outputArea.value));
    state.prepared = true;
    return Number.isFinite(maxTurn) ? Math.max(0, maxTurn) : 0;
  } catch (error) {
    state.prepareError = String(error);
    return 0;
  }
}
...
function setTurnMax(preferLastTurn = false) {
  const preparedMaxTurn = prepareVisualization();
  const maxTurn = state.preparedHasOutput ? preparedMaxTurn : 0;
...
function render() {
  ...
  if (!state.prepared) {
    els.svgHost.innerHTML = "";
    els.score.textContent = state.preparedHasOutput ? "0" : "-";
    setHeroActions(null);
    els.elapsedTime.textContent = formatElapsedMs(state.currentElapsedMs);
    els.error.textContent = state.prepareError;
    return;
  }

  let ret = null;
  try {
    ret = wasmVisPrepared(turn);
    els.svgHost.innerHTML = ret.svg;
    if (state.preparedHasOutput) {
      els.score.textContent = formatScore(ret.score);
      setHeroActions(getMaxTurn());
      els.elapsedTime.textContent = formatElapsedMs(state.currentElapsedMs);
...
function renderAtMost60Hz(ts, force = false) {
  const minInterval = 1000 / getRenderFps();
...
async function main() {
  wasmPrepareCase = wasmPrepareCaseModule;
  wasmVisPrepared = wasmVisPreparedModule;
  await init();
  state.ready = true;
  updatePlayButton();
