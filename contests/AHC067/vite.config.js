...
const CASE_SORT_OPTIONS = [
  ...
  { key: "wall_count_asc", label: "walls ∧" },
  { key: "wall_count_desc", label: "walls ∨" },
];
...
function buildBinary(manifestPath, binName) {
  ...
  if (manifestPath === MANIFEST_PATH) {
    args.splice(3, 0, "--features", "local");
  }
...
function parseCaseMeta(filePath) {
  ...
  const rows = tokens.slice(3, 3 + n);
  if (rows.length !== n || rows.some((row) => row.length !== n)) {
    return null;
  }
  const wallCount = rows.reduce(
    (acc, row) => acc + Array.from(row).filter((cell) => cell === "#").length,
    0,
  );
  return {
    n,
