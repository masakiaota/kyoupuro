...
#[wasm_bindgen]
pub fn prepare_case(input: &str, output: &str) -> Result<i32, JsValue> {
    match impl_vis::prepare_case(input, output) {
        Ok(max_turn) => Ok(max_turn as i32),
...
#[wasm_bindgen]
pub fn vis_prepared(turn: i32) -> Result<Ret, JsValue> {
    match impl_vis::visualize_prepared(turn.max(0) as usize) {
        Ok((score, err, svg)) => Ok(Ret { score, err, svg }),
        Err(e) => Err(JsValue::from_str(&e)),
    }
}
