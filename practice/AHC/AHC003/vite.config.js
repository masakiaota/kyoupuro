...
const CASE_SORT_OPTIONS = [
  ...
  { key: "avg_edge_asc", label: "avg edge ∧" },
  { key: "avg_edge_desc", label: "avg edge ∨" },
  { key: "avg_query_dist_asc", label: "avg dist ∧" },
  { key: "avg_query_dist_desc", label: "avg dist ∨" },
];
...
function parseCaseMeta(filePath) {
  ...
  const hCount = 30 * 29;
  const vCount = 29 * 30;
  const queryOffset = hCount + vCount;
  if (tokens.length < queryOffset + 1000 * 6) {
    return null;
  }
  ...
  for (let i = 0; i < hCount + vCount; i += 1) {
    const value = Number(tokens[i]);
    if (!Number.isFinite(value)) {
      return null;
    }
    edgeSum += value;
    minEdge = Math.min(minEdge, value);
    maxEdge = Math.max(maxEdge, value);
  }

  let queryDistSum = 0;
  for (let k = 0; k < 1000; k += 1) {
    const base = queryOffset + k * 6;
    const si = Number(tokens[base]);
    const sj = Number(tokens[base + 1]);
    const ti = Number(tokens[base + 2]);
    const tj = Number(tokens[base + 3]);
    if (![si, sj, ti, tj].every(Number.isFinite)) {
      return null;
    }
    queryDistSum += Math.abs(si - ti) + Math.abs(sj - tj);
  }

  return {
    avgEdge: edgeSum / (hCount + vCount),
    minEdge,
    maxEdge,
    avgQueryDist: queryDistSum / 1000,
  };
}
    ...
    configureServer(server) {
      ...
      server.middlewares.use("/api/run-rust-bin", async (req, res, next) => {
        ...
        try {
          ...
          const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "ahc003-vis-"));
          const tempInputPath = path.join(tempDir, caseName || "input.txt");
          fs.writeFileSync(tempInputPath, inputText, "utf-8");

          const result = await new Promise((resolve, reject) => {
            const child = spawn(TESTER_BIN_PATH, [tempInputPath, solverBinPath], {
              cwd: ROOT_DIR,
