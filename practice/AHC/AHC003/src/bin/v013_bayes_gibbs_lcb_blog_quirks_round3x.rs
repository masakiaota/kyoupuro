// v013_bayes_gibbs_lcb_blog_quirks_round3x.rs
...
struct Solver {
    ...
    edge_mean: [[f64; EDGE_POS]; SEG_COUNT],
    edge_var: [[f64; EDGE_POS]; SEG_COUNT],
    edge_sample_count: [usize; SEG_COUNT],
...
impl Solver {
    fn new() -> Self {
        Self {
            ...
            edge_mean: [[MU0; EDGE_POS]; SEG_COUNT],
            edge_var: [[0.0; EDGE_POS]; SEG_COUNT],
            edge_sample_count: [0; SEG_COUNT],
    ...
    fn observe_and_update(&mut self, s: (usize, usize), path: &[u8], observed: f64, turn: usize) {
        ...
        let sigma2 = (observed * observed / 300.0) + path_len * D2_OVER3_PRIOR;
    ...
    fn run_mcmc(&mut self, turn: usize) {
        ...
        self.edge_sample_count = [0; SEG_COUNT];
    ...
    fn update_segment(&mut self, g: usize) {
        if self.history_by_segment[g].is_empty() {
            let z = Z_MIN + (self.rng.next_u64() as usize % (Z_MAX - Z_MIN + 1));
            let x = [
                1000.0 + 8000.0 * self.rng.next_f64(),
                1000.0 + 8000.0 * self.rng.next_f64(),
            ];
            self.record_edge_sample(g, z, x);
            self.segments[g] = SegmentState {
                z,
                x,
                mean: x,
                var: [0.0, 0.0],
        ...
        let (mut sample_x, var) = posts[selected_z].sample(&mut self.rng);
        sample_x[0] = sample_x[0].clamp(1000.0, 9000.0);
        sample_x[1] = sample_x[1].clamp(1000.0, 9000.0);
        ...
        self.record_edge_sample(g, selected_z, sample_x);
    ...
    fn record_edge_sample(&mut self, g: usize, z: usize, x: [f64; 2]) {
        for i in 0..EDGE_POS {
            let cost = if i < z { x[0] } else { x[1] };
            let n = self.edge_sample_count[g] as f64;
            let dmean = (cost - self.edge_mean[g][i]) / (n + 1.0);
            self.edge_mean[g][i] += dmean;
            let dvar = (n * dmean * dmean
                + (cost - self.edge_mean[g][i]) * (cost - self.edge_mean[g][i])
                - self.edge_var[g][i])
                / (n + 1.0);
            self.edge_var[g][i] += dvar;
            self.edge_sample_count[g] += 1;
        }
    }
    ...
    fn build_lcb_weights(&self, beta: f64) -> ([[f64; EDGE_POS]; N], [[f64; N]; EDGE_POS]) {
        ...
        for (i, row) in h_weight.iter_mut().enumerate().take(N) {
            self.fill_segment_lcb_weights(i, row, beta);
        }
        ...
        for j in 0..N {
            ...
            for (i, row) in v_weight.iter_mut().enumerate().take(EDGE_POS) {
                row[j] = if self.edge_sample_count[N + j] > 0 {
                    self.edge_lcb_weight(N + j, i, beta)
                } else if i < seg.z {
                    w0
                } else {
                    w1
    ...
    fn fill_segment_lcb_weights(&self, g: usize, row: &mut [f64; EDGE_POS], beta: f64) {
        if self.edge_sample_count[g] > 0 {
            for (i, cell) in row.iter_mut().enumerate().take(EDGE_POS) {
                *cell = self.edge_lcb_weight(g, i, beta);
            }
            return;
        }
        ...
        let w1 = (seg.mean[1] - beta * seg.var[1].sqrt()).max(1.0);
        for (i, cell) in row.iter_mut().enumerate().take(EDGE_POS) {
            *cell = if i < seg.z { w0 } else { w1 };
        }
    }

    fn edge_lcb_weight(&self, g: usize, i: usize, beta: f64) -> f64 {
        let mean = self.edge_mean[g][i];
        let var = self.edge_var[g][i].max(0.0);
        (mean - beta * var.sqrt()).max(1.0)
    }
}
...
fn beta_schedule(turn: usize) -> f64 {
    let x = 1.0 - (turn as f64 / 1000.0).clamp(0.0, 1.0);
    1.5 * x.powf(2.0)
}

fn round_schedule(turn: usize) -> usize {
    let progress = (turn as f64 / 1000.0).clamp(0.0, 1.0);
    3 * (20.0_f64.powf(1.0 - progress) * 4.0_f64.powf(progress)) as usize
}
