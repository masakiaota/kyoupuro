// v010_bayes_gibbs_lcb_path_sigma_mean_edge_stats.rs
...
impl Posterior2 {
    ...
    fn sample(&self, rng: &mut XorShift64) -> [f64; 2] {
        let det = self.det().max(1e-300);
        let (mean, _) = self.mean_var();
        ...
        [mean[0] + l00 * e0, mean[1] + l10 * e0 + l11 * e1]
    }
}
...
struct Solver {
    ...
    edge_m2: [[f64; EDGE_POS]; SEG_COUNT],
...
impl Solver {
    fn new() -> Self {
        Self {
            ...
            edge_m2: [[0.0; EDGE_POS]; SEG_COUNT],
    ...
    fn update_segment(&mut self, g: usize) {
        ...
        let edge_sample = posts[selected_z].sample(&mut self.rng);
        let (mean, var) = posts[selected_z].mean_var();
        ...
        self.record_edge_sample(g, selected_z, edge_sample);
    ...
    fn record_edge_sample(&mut self, g: usize, z: usize, x: [f64; 2]) {
        let count = self.edge_sample_count[g];
        if count == 0 {
            for i in 0..EDGE_POS {
                self.edge_mean[g][i] = if i < z { x[0] } else { x[1] };
                self.edge_m2[g][i] = 0.0;
            }
        } else {
            let next_count = (count + 1) as f64;
            for i in 0..EDGE_POS {
                let cost = if i < z { x[0] } else { x[1] };
                let delta = cost - self.edge_mean[g][i];
                self.edge_mean[g][i] += delta / next_count;
                let delta2 = cost - self.edge_mean[g][i];
                self.edge_m2[g][i] += delta * delta2;
            }
        }
        self.edge_sample_count[g] = count + 1;
    }
    ...
    fn edge_lcb_weight(&self, g: usize, i: usize, beta: f64) -> f64 {
        let count = self.edge_sample_count[g].max(1) as f64;
        let mean = self.edge_mean[g][i];
        let var = (self.edge_m2[g][i] / count).max(0.0);
