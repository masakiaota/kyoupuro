...
const CASE_SORT_OPTIONS = [
  ...
  { key: "m_asc", label: "M ∧" },
  { key: "m_desc", label: "M ∨" },
  { key: "eps_asc", label: "eps ∧" },
  { key: "eps_desc", label: "eps ∨" },
  { key: "oil_cells_asc", label: "oil cells ∧" },
  { key: "oil_cells_desc", label: "oil cells ∨" },
  { key: "total_area_asc", label: "total area ∧" },
  { key: "total_area_desc", label: "total area ∨" },
];
...
function parseCaseMeta(filePath) {
  ...
  const eps = Number(tokens[pos++]);
  if (!Number.isFinite(n) || !Number.isFinite(m) || !Number.isFinite(eps)) {
    return null;
  }

  let totalArea = 0;
  for (let oil = 0; oil < m; oil += 1) {
    const d = Number(tokens[pos++]);
  ...
  let totalVolume = 0;
  let maxCell = 0;
  for (let idx = 0; idx < n * n && pos < tokens.length; idx += 1) {
  ...
  return { n, m, eps, totalArea, oilCells, totalVolume, maxCell };
}
