...
fn main() {
    ...
    eprintln!(
        "v015 mode={:?} target_size={} candidates={} m={} mi_cutoff={}",
        candidate_mode,
        target_size,
    ...
    while turns_used < query_limit {
        ...
        if let Some((best_mask, best_prob)) = best {
            ...
            eprintln!("v015 failed: no finite posterior candidates");
            return;
        }
...
fn positive_probability_cell_scores(n: usize, candidates: &Candidates) -> (Vec<f64>, Vec<usize>) {
    ...
    if let Some(posterior) = build_mi_posterior(candidates, MI_POSTERIOR_MASS_CUTOFF) {
