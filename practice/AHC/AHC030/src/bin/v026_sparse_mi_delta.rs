...
fn main() {
    ...
    eprintln!(
        "v026 mode={:?} target_size={} candidates={} m={} mi_cutoff={} mi_active_limit={} sa_temp={}->{} sa_proposals={}x sa_seeds=min({}M,{}) sa_delta_eval=true shape_swap_alpha={} sparse_mi_delta=true",
        candidate_mode,
        target_size,
    ...
    while turns_used < query_limit {
        ...
        if let Some((best_mask, best_prob)) = best {
            ...
            eprintln!("v026 failed: no finite posterior candidates");
            return;
        }
...
fn precompute_active_cell_values_and_order(
    total_cells: usize,
    m: usize,
    posterior: &Posterior,
    ...
    placements_by_oil: &[Vec<Placement>],
    rng: &mut XorShift64,
) -> (CellValues, Vec<usize>) {
    let mut entries_by_cell = vec![Vec::new(); total_cells];
    ...
    for (active_index, &candidate_id) in posterior.active_ids.iter().enumerate() {
        ...
        for &cell_id in &touched_cells {
            let value = cell_count[cell_id];
            entries_by_cell[cell_id].push(CellValueEntry {
                active_index,
                value,
            });
            mass_by_cell_value[cell_id * (m + 1) + value as usize] += weight;
    ...
    let cell_order = scores.into_iter().map(|(cell_id, _, _)| cell_id).collect();

    (CellValues { entries_by_cell }, cell_order)
}
...
struct CellValues {
    entries_by_cell: Vec<Vec<CellValueEntry>>,
}

impl CellValues {
    fn entries_for_cell(&self, cell_id: usize) -> &[CellValueEntry] {
        &self.entries_by_cell[cell_id]
    }
}
...
fn optimize_query_mask_by_mi_per_cost(
    ...
    let (cell_values, cell_order) = precompute_active_cell_values_and_order(
        total_cells,
        input.m,
        &posterior,
        candidates,
...
fn fill_next_state_buffers(
    ...
    next_sum_v_by_active.copy_from_slice(current_sum_v_by_active);
    next_mass_by_v.copy_from_slice(current_mass_by_v);

    match query_move {
        QueryMove::Add { cell } => {
            for entry in cell_values.entries_for_cell(cell) {
                let index = entry.active_index;
                let old_sum_v = current_sum_v_by_active[index] as usize;
                let delta = entry.value as usize;
