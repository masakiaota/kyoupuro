...
const VARIANT_POOL_RATIO_PERCENT: usize = 20;
...
fn main() {
    ...
    eprintln!(
        "v017 mode={:?} target_size={} candidates={} m={} mi_cutoff={} mi_active_limit={} variant_ratio={} random_fill=fixed_trials",
        candidate_mode,
        target_size,
    ...
    while turns_used < query_limit {
        ...
        if let Some((best_mask, best_prob)) = best {
            ...
            eprintln!("v017 failed: no finite posterior candidates");
            return;
        }
...
fn build_variant_neighbors(input: &Input) -> Vec<Vec<[Option<u16>; 4]>> {
    let mut neighbors_by_oil = Vec::with_capacity(input.m);
    for oil_id in 0..input.m {
        let shape = &input.shape_cells[oil_id];
        let max_i = shape.iter().map(|&(i, _)| i).max().unwrap();
        let max_j = shape.iter().map(|&(_, j)| j).max().unwrap();
        let rows = input.n - max_i;
        let cols = input.n - max_j;
...
fn refresh_candidate_pool(
    ...
    for _ in 0..variant_trials {
        ...
        if !fill_variant_candidate_indices(&mut indices, parent_indices, variant_neighbors, rng) {
            continue;
        }
...
fn fill_variant_candidate_indices(
    indices: &mut [u16],
    parent_indices: &[u16],
    variant_neighbors: &[Vec<[Option<u16>; 4]>],
    rng: &mut XorShift64,
