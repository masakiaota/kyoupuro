...
fn main() {
    ...
    if theta_size.is_none() {
        eprintln!(
            "v002 fallback=mine_all reason=theta_size_over_limit limit={} m={} query_size={}",
    ...
    while turns_used < query_limit {
        ...
        if let Some((best_mask, best_prob)) = best {
            ...
            eprintln!("v002 failed: no finite posterior candidates");
            return;
        }
...
fn update_logw_by_observation(
    ...
    for candidate_id in 0..candidates.len() {
        ...
        candidates.logw[candidate_id] += log_observed_probability(y, mu, sigma);
    }
}
