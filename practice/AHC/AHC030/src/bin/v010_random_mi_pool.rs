...
fn main() {
    ...
    let (mut answer_candidates, mut mi_candidates, candidate_mode) = if is_exhaustive {
        ...
        let candidates = initialize_candidate_pool(&input, &placements_by_oil, &zobrist, &mut rng);
    ...
    eprintln!(
        "v010 mode={:?} answer_candidates={} mi_candidates={} m={}",
        candidate_mode,
    ...
    while turns_used < query_limit {
        ...
        if let Some((best_mask, best_prob)) = best {
            ...
            eprintln!("v010 failed: no finite posterior candidates");
            return;
        }
...
fn refresh_candidate_pools(
    ...
    let mut next_answer = empty_candidates(input.m, K_MAX);
    let mut next_mi = empty_candidates(input.m, K_MAX - KEEP_COUNT);
    let mut seen_answer = HashSet::with_capacity(K_MAX * 2);
    let mut seen_mi = HashSet::with_capacity((K_MAX - KEEP_COUNT) * 2);
