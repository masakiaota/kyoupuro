...
const SAMPLED_CANDIDATES: usize = 80_000;
const SAMPLED_KEEP_CANDIDATES: usize = 24_000;
const SAMPLED_PARENT_POOL: usize = 4096;
const SAMPLED_RANDOM_CANDIDATES: usize = 12_000;
const SAMPLED_MAX_MUTATED_OILS: usize = 3;
const LOCAL_MUTATION_RATE_PERCENT: usize = 70;
const LATE_ANSWER_START_SEC: f64 = 1.9;
const MI_MOVES_PER_RESTART: usize = 24 * 4;
const MI_NO_IMPROVE_LIMIT: usize = 8;
...
fn main() {
    ...
    let placement_grid_sizes = compute_placement_grid_sizes(&placements_by_oil);
    let theta_size = theta_size(&placements_by_oil, MAX_EXHAUSTIVE_CANDIDATES);
    let (mut candidates, candidate_mode) = if theta_size.is_some() {
        (
            enumerate_candidates(&input, &placements_by_oil),
            CandidateMode::Exhaustive,
        )
    } else {
        (
            sample_initial_candidates(&input, &placements_by_oil, SAMPLED_CANDIDATES, &mut rng),
            CandidateMode::Sampled,
        )
    };
    ...
    let mut cell_values = precompute_cell_values(input.n, &candidates, &placements_by_oil);

    eprintln!(
        "v008 mode={:?} candidates={} m={} mi_moves_per_restart={}",
        candidate_mode,
        candidates.len(),
        input.m,
        MI_MOVES_PER_RESTART
    );
    ...
    while turns_used < query_limit {
        ...
        if let Some((best_mask, best_prob)) = best {
            if best_prob >= POSTERIOR_THRESHOLD {
                ...
                if candidate_mode == CandidateMode::Sampled {
                    candidates = refresh_sampled_candidates(
                        &input,
                        &placements_by_oil,
                        &placement_grid_sizes,
                        &observations,
                        &rejected_masks,
                        &candidates,
                        SAMPLED_CANDIDATES,
                    ...
                    cell_values = precompute_cell_values(input.n, &candidates, &placements_by_oil);
                }
                continue;
            }
        } else {
            eprintln!("v008 failed: no finite posterior candidates");
            return;
        }
        ...
        update_logw_by_observation(&mut candidates, &observation);
...
fn compute_placement_grid_sizes(placements_by_oil: &[Vec<Placement>]) -> Vec<(usize, usize)> {
    placements_by_oil
        .iter()
        .map(|placements| {
            let rows = placements
                .iter()
                .map(|placement| placement.deltai_k)
                .max()
                .unwrap_or(0)
                + 1;
            let cols = placements
                .iter()
                .map(|placement| placement.deltaj_k)
                .max()
                .unwrap_or(0)
                + 1;
            (rows, cols)
...
fn sample_initial_candidates(
    ...
    let max_attempts = target_size.saturating_mul(32).max(target_size);
...
fn push_candidate_if_new(
    ...
    if !seen.insert(indices.to_vec()) {
        return false;
    }

    let positive_mask = positive_mask_from_indices(indices, placements_by_oil);
    if rejected_masks.is_some_and(|masks| masks.contains(&positive_mask)) {
        return false;
    }
    if !logw.is_finite() {
        return false;
    ...
    candidates.push(indices, positive_mask);
    let last = candidates.logw.len() - 1;
    candidates.logw[last] = logw;
    true
}

fn positive_mask_from_indices(indices: &[u16], placements_by_oil: &[Vec<Placement>]) -> Mask {
...
fn precompute_cell_values(
    ...
    for candidate_id in 0..candidate_count {
        for oil_id in 0..candidates.m {
            ...
            let placed_mask = &placements_by_oil[oil_id][placement_id].placed_mask;
            for word_id in 0..MASK_WORDS {
                let mut bits = placed_mask.words[word_id];
                while bits != 0 {
...
fn optimize_query_mask_by_mi_per_cost(
    ...
    let total_cells = input.n * input.n;
    let posterior = build_posterior(candidates).expect("finite posterior must exist");
    ...
    let mut state = random_initial_query_state(
        input.n,
        initial_query_size,
        rng,
        &posterior,
    ...
    let mut no_improve_count = 0_usize;

    for _ in 0..MI_MOVES_PER_RESTART {
        if started_at.elapsed().as_secs_f64() >= late_answer_start_sec {
            break;
        }
        if no_improve_count >= MI_NO_IMPROVE_LIMIT {
            break;
        }

        let Some(query_move) = random_query_move(&state, rng) else {
            break;
        };
        ...
        if next_score > state.score {
            apply_query_move(&mut state, query_move, next_query_size);
            std::mem::swap(&mut state.sum_v_by_active, &mut next_sum_v_by_active);
            std::mem::swap(&mut state.mass_by_v, &mut next_mass_by_v);
...
fn build_posterior(candidates: &Candidates) -> Option<Posterior> {
...
fn random_initial_query_state(
    ...
    let query_mask = sample_query_mask(n, query_size, rng);
    let total_cells = n * n;
    let mut in_query = vec![false; total_cells];
    for (cell_id, value) in in_query.iter_mut().enumerate() {
        *value = query_mask.contains(cell_id);
    }
...
fn random_query_move(state: &QueryState, rng: &mut XorShift64) -> Option<QueryMove> {
    let total_cells = state.in_query.len();
    for _ in 0..16 {
        match rng.next_usize(2) {
            0 if state.query_size < total_cells => {
                let cell = random_cell_with_status(&state.in_query, false, rng)?;
                return Some(QueryMove::Add { cell });
            }
            1 if state.query_size > 2 => {
                let cell = random_cell_with_status(&state.in_query, true, rng)?;
                return Some(QueryMove::Remove { cell });
            }
            _ => {}
        }
    }

    if state.query_size < total_cells {
        let cell = random_cell_with_status(&state.in_query, false, rng)?;
        return Some(QueryMove::Add { cell });
    }
...
fn random_cell_with_status(in_query: &[bool], status: bool, rng: &mut XorShift64) -> Option<usize> {
    for _ in 0..64 {
        let cell = rng.next_usize(in_query.len());
        if in_query[cell] == status {
            return Some(cell);
        }
    }

    (0..in_query.len()).find(|&cell| in_query[cell] == status)
}
...
fn refresh_sampled_candidates(
    input: &Input,
    placements_by_oil: &[Vec<Placement>],
    placement_grid_sizes: &[(usize, usize)],
    ...
    let mut next = empty_candidates(input.m, target_size);
    let mut seen = HashSet::new();
    let keep_count = SAMPLED_KEEP_CANDIDATES
        .min(target_size)
    ...
    let parent_pool = SAMPLED_PARENT_POOL.min(candidate_ids.len());
    let mutation_target = target_size.saturating_sub(SAMPLED_RANDOM_CANDIDATES);
    if parent_pool > 0 {
        let mut attempts = 0_usize;
        let max_attempts = target_size.saturating_mul(24).max(target_size);
        while next.len() < mutation_target && attempts < max_attempts {
            attempts += 1;
            let parent_id = candidate_ids[rng.next_usize(parent_pool)];
            let parent_indices = current.placement_indices_for(parent_id);
            let indices = mutate_candidate_indices(
                parent_indices,
                placements_by_oil,
                placement_grid_sizes,
                rng,
    ...
    let mut attempts = 0_usize;
    let max_attempts = target_size.saturating_mul(32).max(target_size);
    while next.len() < target_size && attempts < max_attempts {
        attempts += 1;
        fill_random_candidate_indices(&mut indices, placements_by_oil, rng);
        let logw = compute_candidate_logw(&indices, observations);
        push_candidate_if_new(
    ...
    if next.len() == 0 {
        sample_initial_candidates(input, placements_by_oil, target_size, rng)
    } else {
        next
    }
}

fn mutate_candidate_indices(
    parent_indices: &[u16],
    placements_by_oil: &[Vec<Placement>],
    placement_grid_sizes: &[(usize, usize)],
    rng: &mut XorShift64,
    ...
    let mutation_count = 1 + rng.next_usize(SAMPLED_MAX_MUTATED_OILS.min(indices.len()));

    for _ in 0..mutation_count {
        let oil_id = rng.next_usize(indices.len());
        let placement_count = placements_by_oil[oil_id].len();
        let current = indices[oil_id] as usize;
        let next = if rng.next_usize(100) < LOCAL_MUTATION_RATE_PERCENT {
            random_neighbor_placement_id(
                current,
                &placements_by_oil[oil_id],
                placement_grid_sizes[oil_id],
                rng,
            )
            .unwrap_or_else(|| rng.next_usize(placement_count))
        } else {
            rng.next_usize(placement_count)
        };
        indices[oil_id] = next as u16;
    }

    indices
}

fn random_neighbor_placement_id(
    current: usize,
    ...
    for _ in 0..8 {
        let (next_i, next_j) = match rng.next_usize(4) {
            0 if placement.deltai_k > 0 => (placement.deltai_k - 1, placement.deltaj_k),
            1 if placement.deltai_k + 1 < rows => (placement.deltai_k + 1, placement.deltaj_k),
            2 if placement.deltaj_k > 0 => (placement.deltai_k, placement.deltaj_k - 1),
            3 if placement.deltaj_k + 1 < cols => (placement.deltai_k, placement.deltaj_k + 1),
            _ => continue,
        };
        return Some(next_i * cols + next_j);
    }
...
fn compute_candidate_logw(indices: &[u16], observations: &[ObservationData]) -> f64 {
    ...
    for observation in observations {
        let mut sum_v = 0_usize;
        for (oil_id, &placement_id) in indices.iter().enumerate() {
            sum_v += observation.hits_by_oil[oil_id][placement_id as usize] as usize;
        }
