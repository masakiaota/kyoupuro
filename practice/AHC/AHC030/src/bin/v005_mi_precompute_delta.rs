...
fn main() {
    ...
    if theta_size.is_none() {
        eprintln!(
            "v005 fallback=mine_all reason=theta_size_over_limit limit={} m={}",
    ...
    let cell_values = precompute_cell_values(input.n, &candidates, &placements_by_oil);

    eprintln!(
        "v005 candidates={} m={} mi_moves_per_restart={}",
        candidates.len(),
        input.m,
    ...
    while turns_used < query_limit {
        ...
        if let Some((best_mask, best_prob)) = best {
            ...
            eprintln!("v005 failed: no finite posterior candidates");
            return;
        }
