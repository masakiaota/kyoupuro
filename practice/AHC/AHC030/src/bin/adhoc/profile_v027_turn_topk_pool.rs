// profile_v027_turn_topk_pool.rs
...
fn main() {
    ...
    eprintln!(
        "v027 mode={:?} target_size={} candidates={} m={} mi_cutoff={} mi_active_limit={} turn_pool_limit={} sa_temp={}->{} sa_proposals={}x sa_seeds=min({}M,{}) sa_delta_eval=true shape_swap_alpha={}",
        candidate_mode,
        target_size,
    ...
    while turns_used < query_limit {
        ...
        if let Some((best_mask, best_prob)) = best {
            ...
            eprintln!("v027 failed: no finite posterior candidates");
            return;
        }
...
fn optimize_query_mask_by_mi_per_cost(
    ...
    let mut add_try_count = 0_u64;
    let mut remove_try_count = 0_u64;
    let mut add_success_count = 0_u64;
    let mut remove_success_count = 0_u64;
    ...
    prof::add_time_ns("query.try_add", add_ns, add_try_count);
    prof::add_time_ns("query.try_remove", remove_ns, remove_try_count);
    prof::add_value("query.loop_count", loop_count);
    prof::add_value("query.add_success", add_success_count);
    prof::add_value("query.remove_success", remove_success_count);
