...
const MI_ACTIVE_CANDIDATE_LIMIT: usize = 100;
...
const SA_MAX_PROPOSAL_FACTOR: usize = 2;
...
fn main() {
    ...
    eprintln!(
        "profile_v028 mode={:?} target_size={} candidates={} m={} mi_cutoff={} mi_active_limit={} turn_pool_limit={} sa_temp={}->{} sa_proposals={}x sa_seeds=min({}M,{}) sa_delta_eval=true shape_swap_alpha={} sweep_mi=true",
        candidate_mode,
        target_size,
    ...
    while turns_used < query_limit {
        prof::inc("main.turns");
        prof::add_value(
            "main.candidates_at_turn_start",
            answer_candidates.len() as u64,
        );
        if started_at.elapsed().as_secs_f64() >= LATE_ANSWER_START_SEC {
            prof::inc("main.enter_giveup_time");
            prof::add_value("main.turns_before_giveup", turns_used as u64);
        ...
        if candidate_mode == CandidateMode::Sampled {
            let timer = prof::start();
            truncate_to_top_candidates(&mut answer_candidates, MI_ACTIVE_CANDIDATE_LIMIT);
            prof::finish("main.truncate_top", timer);
            prof::add_value(
                "main.candidates_after_truncate",
                answer_candidates.len() as u64,
            );
        }
        ...
        prof::finish("main.best_posterior_mask", timer);
        if let Some((best_mask, best_prob)) = best {
            prof::add_value("main.best_prob_x1000000", (best_prob * 1.0e6) as u64);
            if best_prob >= POSTERIOR_THRESHOLD {
                prof::inc("main.answer_threshold");
                let timer = prof::start();
                ...
                if candidate_mode == CandidateMode::Sampled {
                    ...
                    prof::finish("main.refresh_after_reject", timer);
                }
                continue;
            }
        } else {
            eprintln!("profile_v028 failed: no finite posterior candidates");
            return;
        }
        ...
        prof::finish("main.optimize_query", timer);

        if started_at.elapsed().as_secs_f64() >= LATE_ANSWER_START_SEC {
            prof::inc("main.enter_giveup_after_optimize");
            prof::add_value("main.turns_before_giveup", turns_used as u64);
            let timer = prof::start();
            giveup_by_digging_likely_cells(
                &mut scanner,
        ...
        prof::finish("main.build_observation", timer);
        let timer = prof::start();
        update_logw_by_observation(&mut answer_candidates, &observation);
        prof::finish("main.update_logw", timer);
        ...
        if candidate_mode == CandidateMode::Sampled {
            ...
            prof::finish("main.refresh", timer);
            prof::add_value(
                "main.candidates_after_refresh",
                answer_candidates.len() as u64,
            );
        }
    }
...
fn truncate_to_top_candidates(candidates: &mut Candidates, limit: usize) {
    let limit = limit.max(1);
    let mut candidate_ids = (0..candidates.len())
        .filter(|&candidate_id| candidates.logw[candidate_id].is_finite())
        .collect::<Vec<_>>();
    if candidate_ids.len() <= limit && candidate_ids.len() == candidates.len() {
        return;
    }

    candidate_ids.sort_unstable_by(|&a, &b| candidates.logw[b].total_cmp(&candidates.logw[a]));

    let keep_count = candidate_ids.len().min(limit);
    let mut next = empty_candidates(candidates.m, keep_count);
    for &candidate_id in candidate_ids.iter().take(keep_count) {
        next.push_with_logw(
            candidates.placement_indices_for(candidate_id),
            candidates.positive_masks[candidate_id],
            candidates.logw[candidate_id],
        );
    }
...
fn refresh_candidate_pool(
    ...
    prof::add_value("refresh.current_len", current.len() as u64);
    let sort_timer = prof::start();
    ...
    prof::finish("refresh.collect_sort_ids", sort_timer);
    prof::add_value("refresh.finite_ids", candidate_ids.len() as u64);
    ...
    let seed_count = (input.m * SA_SEED_PER_OIL)
        ...
        .min(target_size);
    prof::add_value("refresh.seed_count", seed_count as u64);
    ...
    prof::add_value("refresh.max_proposals", max_proposals as u64);
    ...
    let mut fill_neighbor_ns = 0_u128;
    let mut compute_delta_ns = 0_u128;
    let mut fill_neighbor_count = 0_u64;
    let mut compute_delta_count = 0_u64;
    let mut accepted_count = 0_u64;
    let mut pushed_count = 0_u64;
    let mut invalid_neighbor_count = 0_u64;
    let mut nonfinite_count = 0_u64;
    ...
    for &seed_id in seed_ids {
        ...
        for _ in 0..proposals_per_seed {
            ...
            if !fill_sa_neighbor_candidate_indices(
                ...
                fill_neighbor_ns += fill_timer.elapsed().as_nanos();
                fill_neighbor_count += 1;
            ...
            compute_delta_ns += delta_timer.elapsed().as_nanos();
            compute_delta_count += 1;
    ...
    prof::add_time_ns(
        "refresh.fill_neighbor",
        fill_neighbor_ns,
        fill_neighbor_count,
    );
    prof::add_time_ns(
        "refresh.compute_delta",
        compute_delta_ns,
        compute_delta_count,
    );
    prof::add_value("refresh.global_steps", global_step as u64);
    prof::add_value("refresh.accepted", accepted_count);
    prof::add_value("refresh.pushed", pushed_count);
    prof::add_value("refresh.invalid_neighbor", invalid_neighbor_count);
    prof::add_value("refresh.nonfinite", nonfinite_count);
    prof::add_value("refresh.next_len", next_answer.len() as u64);
    next_answer
}
...
fn optimize_query_mask_by_mi_per_cost(
    ...
    prof::finish("query.build_mi_posterior", timer);
    prof::add_value("query.active_count", posterior.active_ids.len() as u64);
    ...
    prof::finish("query.precompute_cell_values", timer);
    ...
    prof::finish("query.compute_single_cell_order", timer);
    ...
    prof::finish("query.initial_state", timer);
    ...
    let mut add_sweep_count = 0_u64;
    let mut remove_sweep_count = 0_u64;
    let mut add_changed_total = 0_u64;
    let mut remove_changed_total = 0_u64;
    let mut add_ns = 0_u128;
    let mut remove_ns = 0_u128;
    ...
    loop {
        ...
        let timer = prof::start();
        add_sweep_count += 1;
        let add_count = sweep_add_by_single_cell_mi_order(
            &mut state,
            &cell_order,
            &posterior.weights,
            &cell_values,
        ...
        add_ns += timer.elapsed().as_nanos();
        add_changed_total += add_count as u64;
        if add_count > 0 {
            continue;
        }

        let timer = prof::start();
        remove_sweep_count += 1;
        let remove_count = sweep_remove_by_single_cell_mi_order(
            &mut state,
            &cell_order,
            &posterior.weights,
            &cell_values,
        ...
        remove_ns += timer.elapsed().as_nanos();
        remove_changed_total += remove_count as u64;
        if remove_count > 0 {
            continue;
        }
    ...
    prof::add_time_ns("query.sweep_add", add_ns, add_sweep_count);
    prof::add_time_ns("query.sweep_remove", remove_ns, remove_sweep_count);
    prof::add_value("query.loop_count", loop_count);
    prof::add_value("query.add_changed", add_changed_total);
    prof::add_value("query.remove_changed", remove_changed_total);
    prof::add_value("query.final_query_size", state.query_size as u64);
    state.query_mask
}
...
fn sweep_add_by_single_cell_mi_order(
    ...
    for &cell in cell_order {
        if started_at.elapsed().as_secs_f64() >= late_answer_start_sec {
            prof::add_value("query.add_scanned_cells", scanned);
            break;
        }
...
fn sweep_remove_by_single_cell_mi_order(
    state: &mut QueryState,
    cell_order: &[usize],
    weights: &[f64],
    cell_values: &CellValues,
    ...
    for &cell in cell_order.iter().rev() {
        if started_at.elapsed().as_secs_f64() >= late_answer_start_sec {
            prof::add_value("query.remove_scanned_cells", scanned);
            break;
        }
        if !state.in_query[cell] {
            continue;
