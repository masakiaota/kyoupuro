...
const MI_ACTIVE_CANDIDATE_LIMIT: usize = 200;
const MI_HILLCLIMB_PASSES: usize = 1;
...
struct Profile {
    setup: Duration,
    best_posterior: Duration,
    optimize_total: Duration,
    mi_build_posterior: Duration,
    mi_precompute_cell_values: Duration,
    mi_single_cell_order: Duration,
    mi_initial_state: Duration,
    mi_hill_add: Duration,
    mi_hill_remove: Duration,
    ask_survey_wait: Duration,
    build_observation: Duration,
    update_logw: Duration,
    refresh_pool: Duration,
    refresh_sort: Duration,
    refresh_keep: Duration,
    refresh_random_fill: Duration,
    giveup_total: Duration,
    refresh_attempts: usize,
    refresh_pushed: usize,
    refresh_nonfinite_logw: usize,
    turns_before_giveup: usize,
    surveys_before_giveup: usize,
    mi_calls: usize,
    mi_active_sum: usize,
    mi_add_changed: usize,
    mi_remove_changed: usize,
}
...
impl Profile {
    ...
    fn print(&self, label: &str, total_elapsed: Duration) {
        eprintln!("PROFILE label={label} total_ms={:.3}", Self::millis(total_elapsed));
        eprintln!(
            "PROFILE main setup={:.3} best_posterior={:.3} optimize_total={:.3} ask_survey_wait={:.3} build_observation={:.3} update_logw={:.3} refresh_pool={:.3} giveup_total={:.3}",
            Self::millis(self.setup),
            Self::millis(self.best_posterior),
            Self::millis(self.optimize_total),
            Self::millis(self.ask_survey_wait),
            Self::millis(self.build_observation),
            Self::millis(self.update_logw),
            Self::millis(self.refresh_pool),
            Self::millis(self.giveup_total),
        );
        eprintln!(
            "PROFILE refresh sort={:.3} keep={:.3} random_fill={:.3} attempts={} pushed={} nonfinite_logw={}",
            Self::millis(self.refresh_sort),
            Self::millis(self.refresh_keep),
            Self::millis(self.refresh_random_fill),
            self.refresh_attempts,
            self.refresh_pushed,
            self.refresh_nonfinite_logw,
        );
        eprintln!(
            "PROFILE mi calls={} avg_active={:.1} build_posterior={:.3} precompute_cell_values={:.3} single_cell_order={:.3} initial_state={:.3} hill_add={:.3} hill_remove={:.3} add_changed={} remove_changed={}",
            self.mi_calls,
            if self.mi_calls == 0 { 0.0 } else { self.mi_active_sum as f64 / self.mi_calls as f64 },
            Self::millis(self.mi_build_posterior),
            Self::millis(self.mi_precompute_cell_values),
            Self::millis(self.mi_single_cell_order),
            Self::millis(self.mi_initial_state),
            Self::millis(self.mi_hill_add),
            Self::millis(self.mi_hill_remove),
            self.mi_add_changed,
            self.mi_remove_changed,
        );
        eprintln!(
            "PROFILE flow turns_before_giveup={} surveys_before_giveup={}",
            self.turns_before_giveup,
            self.surveys_before_giveup
        );
    }
}
...
fn main() {
    ...
    profile.setup += setup_started.elapsed();

    eprintln!(
        "v016 mode={:?} target_size={} candidates={} m={} mi_cutoff={} mi_active_limit={} mi_passes={}",
        candidate_mode,
        target_size,
    ...
    while turns_used < query_limit {
        if started_at.elapsed().as_secs_f64() >= LATE_ANSWER_START_SEC {
            profile.turns_before_giveup = turns_used;
            profile.surveys_before_giveup = observations.len();
            ...
            profile.giveup_total += section_started.elapsed();
            profile.print("time_guard_before_turn", started_at.elapsed());
            return;
        }

        let section_started = Instant::now();
        let best = best_posterior_mask(&answer_candidates);
        profile.best_posterior += section_started.elapsed();
        if let Some((best_mask, best_prob)) = best {
            if best_prob >= POSTERIOR_THRESHOLD {
                ...
                turns_used += 1;
                if accepted {
                    profile.print("accepted_by_posterior", started_at.elapsed());
                    return;
                }
                ...
                if candidate_mode == CandidateMode::Sampled {
                    ...
                    profile.refresh_pool += section_started.elapsed();
                }
                continue;
            }
        } else {
            eprintln!("v016 failed: no finite posterior candidates");
            profile.print("no_finite_posterior", started_at.elapsed());
            return;
        }
        ...
        if started_at.elapsed().as_secs_f64() >= LATE_ANSWER_START_SEC {
            ...
            profile.giveup_total += section_started.elapsed();
            profile.print("time_guard_after_optimize", started_at.elapsed());
            return;
        }
        ...
        profile.ask_survey_wait += section_started.elapsed();
        turns_used += 1;
        ...
        profile.build_observation += section_started.elapsed();
        let section_started = Instant::now();
        update_logw_by_observation(&mut answer_candidates, &observation);
        profile.update_logw += section_started.elapsed();
    ...
    profile.print("loop_end", started_at.elapsed());
}
...
fn refresh_candidate_pool(
    ...
    profile.refresh_sort += section_started.elapsed();
    ...
    profile.refresh_keep += section_started.elapsed();
    ...
    while next_answer.len() < target_size {
        if started_at.elapsed().as_secs_f64() >= late_answer_start_sec {
            profile.refresh_random_fill += section_started.elapsed();
            return next_answer;
        }
        fill_random_candidate_indices(&mut indices, placements_by_oil, rng);
        let logw = compute_candidate_logw(&indices, observations);
        profile.refresh_attempts += 1;
        if !logw.is_finite() {
            profile.refresh_nonfinite_logw += 1;
        }
...
fn optimize_query_mask_by_mi_per_cost(
    ...
    profile.mi_build_posterior += section_started.elapsed();
    profile.mi_calls += 1;
    profile.mi_active_sum += posterior.active_ids.len();
    let initial_query_size = (total_cells / 2).max(2).min(total_cells);

    if started_at.elapsed().as_secs_f64() >= late_answer_start_sec {
        let mask = sample_query_mask(input.n, initial_query_size, rng);
        profile.optimize_total += optimize_started.elapsed();
        return mask;
    }
    ...
    profile.mi_precompute_cell_values += section_started.elapsed();
    ...
    profile.mi_single_cell_order += section_started.elapsed();
    ...
    profile.mi_initial_state += section_started.elapsed();
    ...
    for _ in 0..MI_HILLCLIMB_PASSES {
        if started_at.elapsed().as_secs_f64() >= late_answer_start_sec {
            break;
        }
        ...
        if try_add_by_single_cell_mi_order(
            ...
            changed = true;
            profile.mi_add_changed += 1;
        }
        profile.mi_hill_add += section_started.elapsed();
        ...
        if try_remove_by_single_cell_mi_order(
            ...
            changed = true;
            profile.mi_remove_changed += 1;
        }
        profile.mi_hill_remove += section_started.elapsed();
