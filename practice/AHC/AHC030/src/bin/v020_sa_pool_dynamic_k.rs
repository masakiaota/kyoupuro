...
const SA_START_TEMPERATURE: f64 = 16.0;
const SA_END_TEMPERATURE: f64 = 0.5;
...
fn main() {
    ...
    let k_max = candidate_limit(input.n);
    ...
    let (mut answer_candidates, candidate_mode) = if is_exhaustive {
        ...
        let candidates =
            initialize_candidate_pool(&input, &placements_by_oil, &zobrist, k_max, &mut rng);
    ...
    eprintln!(
        "v020 mode={:?} k_max={} candidates={} m={} mi_cutoff={} mi_active_limit={} sa_temp={}->{} sa_proposals={}x dynamic_k=late_remaining",
        candidate_mode,
        k_max,
    ...
    while turns_used < query_limit {
        ...
        if let Some((best_mask, best_prob)) = best {
            if best_prob >= POSTERIOR_THRESHOLD {
                ...
                if candidate_mode == CandidateMode::Sampled {
                    let target_size =
                        dynamic_candidate_limit(k_max, started_at, LATE_ANSWER_START_SEC);
            ...
            eprintln!("v020 failed: no finite posterior candidates");
            return;
        }
        ...
        if candidate_mode == CandidateMode::Sampled {
            let target_size = dynamic_candidate_limit(k_max, started_at, LATE_ANSWER_START_SEC);
            answer_candidates = refresh_candidate_pool(
                &input,
                &placements_by_oil,
                &zobrist,
...
fn dynamic_candidate_limit(k_max: usize, started_at: Instant, late_answer_start_sec: f64) -> usize {
    let remaining = (late_answer_start_sec - started_at.elapsed().as_secs_f64()).max(0.0);
    ((remaining / late_answer_start_sec) * k_max as f64)
        .ceil()
        .max(10.0) as usize
}
...
fn refresh_candidate_pool(
    ...
    let Some(&best_id) = candidate_ids.first() else {
        return next_answer;
    };

    let mut current_indices = current.placement_indices_for(best_id).to_vec();
    let mut current_logw = current.logw[best_id];
    push_candidate_if_new(
        &mut next_answer,
        placements_by_oil,
        zobrist,
    ...
    let no_accept_limit = (target_size * SA_NO_ACCEPT_FACTOR).max(256);
    let mut steps_since_accept = 0_usize;

    for step in 0..max_proposals {
        ...
        let proposal_logw = compute_candidate_logw(&proposal_indices, observations);
        ...
        let delta = proposal_logw - current_logw;
        if delta >= 0.0 || rng.next_f64() < (delta / temperature).exp() {
            current_indices.copy_from_slice(&proposal_indices);
...
fn fill_sa_neighbor_candidate_indices(
    ...
    if roll < MOVE_NEIGHBOR_RATIO_PERCENT {
        fill_move_neighbor_candidate_indices(indices, parent_indices, move_neighbors, rng)
        ...
        fill_swap_neighbor_candidate_indices(indices, parent_indices, placement_bounds, rng)
    } else {
        false
    }
}
...
fn fill_random_one_oil_neighbor_candidate_indices(
    ...
    indices[oil_id] != parent_indices[oil_id]
}
