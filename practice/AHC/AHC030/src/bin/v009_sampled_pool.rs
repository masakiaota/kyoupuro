...
const KEEP_COUNT: usize = K_MAX / 100;
const MUTATION_TRIALS: usize = K_MAX * 49 / 100;
const MAX_MUTATION_SHIFT: usize = 1;
...
fn main() {
    ...
    let is_exhaustive = theta_size(&placements_by_oil, K_MAX).is_some();
    let placement_id_by_offset = if is_exhaustive {
        Vec::new()
    } else {
        build_placement_id_by_offset(&placements_by_oil)
    };
    ...
    let (mut candidates, candidate_mode) = if is_exhaustive {
        (
        ...
        (
            initialize_candidate_pool(&input, &placements_by_oil, &zobrist, &mut rng),
        ...
        if let Some((best_mask, best_prob)) = best {
            if best_prob >= POSTERIOR_THRESHOLD {
                ...
                if candidate_mode == CandidateMode::Sampled {
                    candidates = refresh_candidate_pool(
                        &input,
                        &placements_by_oil,
                        &placement_id_by_offset,
                        &zobrist,
                        &observations,
                        &rejected_masks,
                        &candidates,
            ...
            eprintln!("v009 failed: no finite posterior candidates");
            return;
        }
...
fn build_placement_id_by_offset(
    placements_by_oil: &[Vec<Placement>],
) -> Vec<HashMap<(usize, usize), usize>> {
    let mut maps = Vec::with_capacity(placements_by_oil.len());
    for placements in placements_by_oil {
        let mut map = HashMap::with_capacity(placements.len() * 2);
        for (placement_id, placement) in placements.iter().enumerate() {
            map.insert((placement.deltai_k, placement.deltaj_k), placement_id);
        }
        maps.push(map);
    }
    maps
...
fn initialize_candidate_pool(
    ...
    let mut candidates = empty_candidates(input.m, K_MAX);
    let mut seen = HashSet::with_capacity(K_MAX * 2);
    let mut indices = vec![0_u16; input.m];

    while candidates.len() < K_MAX {
...
fn refresh_candidate_pool(
    input: &Input,
    placements_by_oil: &[Vec<Placement>],
    placement_id_by_offset: &[HashMap<(usize, usize), usize>],
    zobrist: &[Vec<u64>],
    ...
    let mut next = empty_candidates(input.m, K_MAX);
    let mut seen = HashSet::with_capacity(K_MAX * 2);
    let mut kept_parent_indices = Vec::new();
    ...
    for &candidate_id in candidate_ids.iter().take(KEEP_COUNT) {
        ...
        if push_candidate_if_new(
            ...
            kept_parent_indices.push(indices.to_vec());
        }
    }
    ...
    for _ in 0..MUTATION_TRIALS {
        ...
        if kept_parent_indices.is_empty() {
            break;
        }
        let parent = &kept_parent_indices[rng.next_usize(kept_parent_indices.len())];
        let child =
            mutate_candidate_indices(parent, placements_by_oil, placement_id_by_offset, rng);
        let logw = compute_candidate_logw(&child, observations);
        push_candidate_if_new(
            &mut next,
            placements_by_oil,
            zobrist,
...
fn mutate_candidate_indices(
    ...
    for oil_id in 0..child.len() {
        if rng.next_usize(2) == 0 {
            continue;
        }

        let placement_id = child[oil_id] as usize;
        let placement = &placements_by_oil[oil_id][placement_id];
        let choice = rng.next_usize(4 * MAX_MUTATION_SHIFT);
        let shift = choice / 4 + 1;
        let direction = choice % 4;

        let next_offset = match direction {
            0 => placement
                .deltai_k
                .checked_sub(shift)
                .map(|deltai_k| (deltai_k, placement.deltaj_k)),
            1 => Some((placement.deltai_k + shift, placement.deltaj_k)),
            2 => placement
                .deltaj_k
                .checked_sub(shift)
                .map(|deltaj_k| (placement.deltai_k, deltaj_k)),
            _ => Some((placement.deltai_k, placement.deltaj_k + shift)),
        };

        let Some((next_i, next_j)) = next_offset else {
            continue;
        };
        if let Some(&next_placement_id) = placement_id_by_offset[oil_id].get(&(next_i, next_j)) {
            child[oil_id] = next_placement_id as u16;
        }
    }
