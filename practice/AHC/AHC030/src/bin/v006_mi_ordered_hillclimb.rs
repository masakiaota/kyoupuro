...
fn main() {
    ...
    if theta_size.is_none() {
        eprintln!(
            "v006 fallback=mine_all reason=theta_size_over_limit limit={} m={}",
    ...
    eprintln!("v006 candidates={} m={}", candidates.len(), input.m);
    ...
    while turns_used < query_limit {
        ...
        if let Some((best_mask, best_prob)) = best {
            ...
            eprintln!("v006 failed: no finite posterior candidates");
            return;
        }
...
fn optimize_query_mask_by_mi_per_cost(
    ...
    let cell_order = compute_single_cell_mi_order(total_cells, input.m, &posterior, cell_values);
...
fn compute_single_cell_mi_order(
    ...
    for cell_id in 0..total_cells {
        let values = cell_values.values_for_cell(cell_id);
        let mut mass_by_value = vec![0.0_f64; m + 1];
        for (active_index, &candidate_id) in posterior.active_ids.iter().enumerate() {
            let value = values[candidate_id] as usize;
            mass_by_value[value] += posterior.weights[active_index];
        }

        let mut entropy = 0.0;
        for probability in mass_by_value {
            if probability > 0.0 {
                entropy -= probability * probability.ln();
            }
        }
        scores.push((cell_id, entropy));
    }

    scores.sort_unstable_by(|&(cell_a, score_a), &(cell_b, score_b)| {
        score_b
            .total_cmp(&score_a)
            .then_with(|| cell_a.cmp(&cell_b))
    });
    scores.into_iter().map(|(cell_id, _)| cell_id).collect()
}
