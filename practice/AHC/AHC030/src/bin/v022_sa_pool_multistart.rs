...
fn main() {
    ...
    eprintln!(
        "v022 mode={:?} target_size={} candidates={} m={} mi_cutoff={} mi_active_limit={} sa_temp={}->{} sa_proposals={}x sa_seeds=min({}M,{})",
        candidate_mode,
        target_size,
    ...
    while turns_used < query_limit {
        ...
        if let Some((best_mask, best_prob)) = best {
            ...
            eprintln!("v022 failed: no finite posterior candidates");
            return;
        }
...
fn refresh_candidate_pool(
    ...
    for &seed_id in seed_ids {
        ...
        let mut current_indices = current.placement_indices_for(seed_id).to_vec();
        let mut current_logw = current.logw[seed_id];
