...
const DEFAULT_MI_FIXED_SWEEP_COUNT: usize = 7;
const VERIFY_COMPLEMENT_MIN_SIZE: usize = 2;
const FALSE_POSITIVE_VERIFY_PROB_THRESHOLD: f64 = 0.96;
...
const SA_MAX_PROPOSAL_NUMERATOR: usize = 6;
const SA_MAX_PROPOSAL_DENOMINATOR: usize = 5;
...
fn read_mi_fixed_sweep_count() -> usize {
    std::env::var("MI_FIXED_SWEEP_COUNT")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .filter(|&value| value > 0)
        .unwrap_or(DEFAULT_MI_FIXED_SWEEP_COUNT)
}
...
fn main() {
    ...
    let mi_fixed_sweep_count = read_mi_fixed_sweep_count();

    eprintln!(
        "v033 mode={:?} target_size={} candidates={} m={} mi_cutoff={} mi_active_limit={} turn_pool_limit={} sa_temp={}->{} sa_proposals={}/{}K sa_seeds=min({}M,{}) sa_delta_eval=true shape_swap_alpha={} fixed_mi_sweeps={} verify_complement_min_size={} fp_verify_threshold={}",
        candidate_mode,
        target_size,
        ...
        SA_END_TEMPERATURE,
        SA_MAX_PROPOSAL_NUMERATOR,
        SA_MAX_PROPOSAL_DENOMINATOR,
        SA_SEED_PER_OIL,
        SA_MAX_SEEDS,
        SHAPE_SWAP_ALPHA,
        mi_fixed_sweep_count,
        VERIFY_COMPLEMENT_MIN_SIZE,
        FALSE_POSITIVE_VERIFY_PROB_THRESHOLD
    );

    let mut observations = Vec::new();
    let mut exact_digs = Vec::new();
    let mut exact_value_by_cell = vec![-1_i8; total_cells];
    let mut rejected_masks = HashSet::new();
    let mut complement_verified_masks = HashSet::new();
    let mut false_positive_verified_masks = HashSet::new();
    while turns_used < query_limit {
        ...
        if let Some((best_mask, best_prob)) = best {
            if best_prob >= POSTERIOR_THRESHOLD {
                if turns_used + 2 <= query_limit && !complement_verified_masks.contains(&best_mask)
                {
                    let complement_mask = complement_mask(input.n, best_mask);
                    let complement_size = complement_mask.count_ones();
                    if complement_size >= VERIFY_COMPLEMENT_MIN_SIZE {
                        let query_cells = complement_mask.to_cells(input.n);
                        let y = ask_survey(&mut scanner, &mut out, &query_cells);
                        turns_used += 1;
                        complement_verified_masks.insert(best_mask);
                ...
                if turns_used + 2 <= query_limit
                    && !false_positive_verified_masks.contains(&best_mask)
                {
                    if let Some((cell_id, _probability)) = select_false_positive_verification_cell(
                        input.n,
                        &answer_candidates,
                        best_mask,
                        &exact_value_by_cell,
                    ) {
                        let value = ask_dig(&mut scanner, &mut out, input.n, cell_id);
                        turns_used += 1;
                        exact_value_by_cell[cell_id] = value as i8;
                        exact_digs.push(ExactDigObservation { cell_id, value });
                        false_positive_verified_masks.insert(best_mask);

                        update_logw_by_exact_dig(
                            &mut answer_candidates,
                            &placements_by_oil,
                            cell_id,
                            value,
                        );
            ...
            eprintln!("v033 failed: no finite posterior candidates");
            return;
        }
...
fn refresh_candidate_pool(
    ...
    exact_digs: &[ExactDigObservation],
    ...
    let max_proposals = ((target_size * SA_MAX_PROPOSAL_NUMERATOR) + SA_MAX_PROPOSAL_DENOMINATOR
        - 1)
        / SA_MAX_PROPOSAL_DENOMINATOR;
    let max_proposals = max_proposals.max(target_size);
...
fn refill_random_candidate_pool(
    input: &Input,
    placements_by_oil: &[Vec<Placement>],
    zobrist: &[Vec<u64>],
    observations: &[ObservationData],
    ...
    let mut indices = vec![0_u16; input.m];
    let max_attempts = target_size.saturating_mul(20).max(256);
    for _ in 0..max_attempts {
        ...
        fill_random_candidate_indices(&mut indices, placements_by_oil, rng);
        if !candidate_indices_match_exact_digs(&indices, placements_by_oil, exact_digs) {
            continue;
        }
        let logw = compute_candidate_logw_from_observations(&indices, observations);
        push_candidate_if_new(
            candidates,
            placements_by_oil,
            zobrist,
...
fn compute_candidate_logw_from_observations(
    indices: &[u16],
    observations: &[ObservationData],
) -> f64 {
    let mut logw = 0.0;
    for observation in observations {
        ...
        if sum_v >= observation.logp_by_sum_v.len() {
...
fn compute_neighbor_logw_delta(
    ...
    if !candidate_indices_match_exact_digs(proposal_indices, placements_by_oil, exact_digs) {
        return f64::NEG_INFINITY;
    }
...
fn candidate_indices_match_exact_digs(
    indices: &[u16],
    placements_by_oil: &[Vec<Placement>],
    exact_digs: &[ExactDigObservation],
) -> bool {
    for exact in exact_digs {
        if candidate_indices_cell_value(indices, placements_by_oil, exact.cell_id) != exact.value {
            return false;
        }
    }
    true
...
fn candidate_indices_cell_value(
    indices: &[u16],
    placements_by_oil: &[Vec<Placement>],
    cell_id: usize,
) -> i32 {
    let mut value = 0_i32;
    for (oil_id, &placement_id) in indices.iter().enumerate() {
        if placements_by_oil[oil_id][placement_id as usize]
            .placed_mask
            .contains(cell_id)
        {
            value += 1;
...
fn complement_mask(n: usize, mask: Mask) -> Mask {
    let mut complement = Mask::default();
    for cell_id in 0..n * n {
        if !mask.contains(cell_id) {
            complement.set(cell_id);
        }
    }
    complement
}
...
fn optimize_query_mask_by_mi_per_cost(
    ...
    for sweep_id in 0..fixed_sweep_count {
        if started_at.elapsed().as_secs_f64() >= late_answer_start_sec {
            break;
        }
        if sweep_id % 2 == 0 {
            sweep_add_by_single_cell_mi_order(
                &mut state,
                &cell_order,
                &posterior.weights,
                &cell_values,
            ...
            sweep_remove_by_single_cell_mi_order(
                &mut state,
                &cell_order,
                &posterior.weights,
                &cell_values,
...
fn update_logw_by_exact_dig(
    ...
    for candidate_id in 0..candidates.len() {
        ...
        let predicted =
            candidate_cell_value(candidates, placements_by_oil, candidate_id, cell_id) as i32;
        if predicted != value {
...
fn select_false_positive_verification_cell(
    n: usize,
    candidates: &Candidates,
    best_mask: Mask,
    ...
    let total_cells = n * n;
    let (positive_prob_by_cell, _) = positive_probability_cell_scores(n, candidates);
    let mut best_cell = None;
    let mut lowest_probability = f64::INFINITY;

    for cell_id in 0..total_cells {
        if !best_mask.contains(cell_id) || exact_value_by_cell[cell_id] >= 0 {
            continue;
        }
    ...
    if lowest_probability < FALSE_POSITIVE_VERIFY_PROB_THRESHOLD {
        best_cell.map(|cell_id| (cell_id, lowest_probability))
    } else {
        None
    }
}
