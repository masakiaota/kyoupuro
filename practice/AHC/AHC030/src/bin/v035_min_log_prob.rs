...
fn main() {
    ...
    eprintln!(
        "v035 mode={:?} target_size={} candidates={} m={} mi_cutoff={} mi_active_limit={} turn_pool_limit={} sa_temp={}->{} sa_proposals={}/{}K sa_seeds=min({}M,{}) sa_delta_eval=true shape_swap_alpha={} fixed_mi_sweeps={} verify_complement_min_size={} min_log_prob={}",
        candidate_mode,
        target_size,
    ...
    while turns_used < query_limit {
        ...
        if let Some((best_mask, best_prob)) = best {
            ...
            eprintln!("v035 failed: no finite posterior candidates");
            return;
        }
...
fn log_observed_probability(y: i32, mu: f64, sigma: f64) -> f64 {
    ...
    if probability > 0.0 {
        probability.ln().max(MIN_LOG_PROB)
    } else {
        MIN_LOG_PROB
    }
}
