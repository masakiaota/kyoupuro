// v007_mi_ordered_random_tiebreak.rs
...
fn main() {
    ...
    if theta_size.is_none() {
        eprintln!(
            "v007 fallback=mine_all reason=theta_size_over_limit limit={} m={}",
    ...
    eprintln!("v007 candidates={} m={}", candidates.len(), input.m);
    ...
    while turns_used < query_limit {
        ...
        if let Some((best_mask, best_prob)) = best {
            ...
            eprintln!("v007 failed: no finite posterior candidates");
            return;
        }
