...
fn main() {
    ...
    if theta_size.is_none() {
        eprintln!(
            "v004 fallback=mine_all reason=theta_size_over_limit limit={} m={}",
            MAX_EXHAUSTIVE_CANDIDATES, input.m
        );
    ...
    eprintln!(
        "v004 candidates={} m={} mi_moves_per_restart={}",
        candidates.len(),
        input.m,
    ...
    while turns_used < query_limit {
        ...
        if let Some((best_mask, best_prob)) = best {
            ...
            eprintln!("v004 failed: no finite posterior candidates");
            return;
        }
...
impl ObservationModel {
    fn new(total_cells: usize, epsilon: f64, max_sum_v: usize) -> Self {
        Self {
            epsilon,
            max_sum_v,
            tables_by_query_size: (0..=total_cells).map(|_| None).collect(),
        }
    }

    fn table(&mut self, query_size: usize) -> &ObservationTable {
        if self.tables_by_query_size[query_size].is_none() {
            let table = build_observation_table(query_size, self.epsilon, self.max_sum_v);
            self.tables_by_query_size[query_size] = Some(table);
        }
    ...
    fn mi_per_cost(
        ...
        let mut mass_by_v = vec![0.0_f64; self.max_sum_v + 1];
...
struct CellValueCache {
    values_by_cell: Vec<Option<Vec<u8>>>,
}

impl CellValueCache {
    fn new(total_cells: usize) -> Self {
        Self {
            values_by_cell: (0..total_cells).map(|_| None).collect(),
        }
    }

    fn ensure_cell_values(
        &mut self,
        cell_id: usize,
        posterior: &Posterior,
        ...
        if self.values_by_cell[cell_id].is_some() {
            return;
        }

        let mut values = Vec::with_capacity(posterior.active_ids.len());
        ...
        self.values_by_cell[cell_id] = Some(values);
    }

    fn values(&self, cell_id: usize) -> &[u8] {
        self.values_by_cell[cell_id].as_ref().unwrap()
    }
}
...
fn optimize_query_mask_by_mi_per_cost(
    ...
    let mut observation_model = ObservationModel::new(total_cells, input.epsilon, max_sum_v);
    let mut cell_value_cache = CellValueCache::new(total_cells);
    ...
    for _ in 0..MI_MOVES_PER_RESTART {
        ...
        fill_next_sum_v_by_active(
            ...
            candidates,
            placements_by_oil,
            &mut cell_value_cache,
        );

        let next_score =
            observation_model.mi_per_cost(next_query_size, &next_sum_v_by_active, &posterior);
...
fn random_initial_query_state(
    ...
    candidates: &Candidates,
    placements_by_oil: &[Vec<Placement>],
    observation_model: &mut ObservationModel,
) -> QueryState {
    ...
    let score = observation_model.mi_per_cost(query_size, &sum_v_by_active, posterior);
...
fn fill_next_sum_v_by_active(
    ...
    candidates: &Candidates,
    placements_by_oil: &[Vec<Placement>],
    cell_value_cache: &mut CellValueCache,
) {
    match query_move {
        QueryMove::Add { cell } => {
            cell_value_cache.ensure_cell_values(cell, posterior, candidates, placements_by_oil);
            let values = cell_value_cache.values(cell);
            for index in 0..current_sum_v_by_active.len() {
                next_sum_v_by_active[index] = current_sum_v_by_active[index] + values[index] as u16;
            }
        }
