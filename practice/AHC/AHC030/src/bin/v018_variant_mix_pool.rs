...
const VARIANT_POOL_RATIO_PERCENT: usize = 90;
const MOVE_VARIANT_RATIO_PERCENT: usize = 30;
const RANDOM_ONE_OIL_VARIANT_RATIO_PERCENT: usize = 30;
const MAX_VARIANT_SHIFT: usize = 1;
...
fn main() {
    ...
    let variant_neighbors = if is_exhaustive {
        Vec::new()
    } else {
        build_variant_neighbors(&placement_bounds)
    };
    ...
    eprintln!(
        "v018 mode={:?} target_size={} candidates={} m={} mi_cutoff={} mi_active_limit={} variant_ratio={} random_fill=fixed_trials",
        candidate_mode,
        target_size,
    ...
    while turns_used < query_limit {
        ...
        if let Some((best_mask, best_prob)) = best {
            ...
            eprintln!("v018 failed: no finite posterior candidates");
            return;
        }
...
fn build_variant_neighbors(placement_bounds: &[PlacementBounds]) -> Vec<Vec<[Option<u16>; 4]>> {
    ...
    for bounds in placement_bounds {
        ...
        for deltai_k in 0..rows {
            for deltaj_k in 0..cols {
                ...
                if deltai_k >= MAX_VARIANT_SHIFT {
                    neighbors[placement_id][0] =
                        Some(((deltai_k - MAX_VARIANT_SHIFT) * cols + deltaj_k) as u16);
                }
                if deltai_k + MAX_VARIANT_SHIFT < rows {
                    neighbors[placement_id][1] =
                        Some(((deltai_k + MAX_VARIANT_SHIFT) * cols + deltaj_k) as u16);
                }
                if deltaj_k >= MAX_VARIANT_SHIFT {
                    neighbors[placement_id][2] =
                        Some((deltai_k * cols + (deltaj_k - MAX_VARIANT_SHIFT)) as u16);
                }
                if deltaj_k + MAX_VARIANT_SHIFT < cols {
...
fn refresh_candidate_pool(
    ...
    variant_neighbors: &[Vec<[Option<u16>; 4]>],
    ...
    let mut indices = vec![0_u16; input.m];
    let variant_trials = (target_size * VARIANT_POOL_RATIO_PERCENT / 100)
        .min(target_size.saturating_sub(next_answer.len()));
    let move_variant_trials = variant_trials * MOVE_VARIANT_RATIO_PERCENT / 100;
    let random_one_oil_variant_trials = variant_trials * RANDOM_ONE_OIL_VARIANT_RATIO_PERCENT / 100;
    let swap_variant_trials = variant_trials - move_variant_trials - random_one_oil_variant_trials;

    for _ in 0..move_variant_trials {
        if started_at.elapsed().as_secs_f64() >= late_answer_start_sec {
            return next_answer;
        }
        ...
        let parent_id = keep_ids[rng.next_usize(keep_ids.len())];
        let parent_indices = current.placement_indices_for(parent_id);
        if !fill_move_variant_candidate_indices(
            &mut indices,
            parent_indices,
            variant_neighbors,
            rng,
    ...
    for _ in 0..random_one_oil_variant_trials {
        if started_at.elapsed().as_secs_f64() >= late_answer_start_sec {
            return next_answer;
        }
        if keep_ids.is_empty() {
        ...
        let parent_id = keep_ids[rng.next_usize(keep_ids.len())];
        let parent_indices = current.placement_indices_for(parent_id);
        fill_random_one_oil_variant_candidate_indices(
            &mut indices,
            parent_indices,
            placements_by_oil,
            rng,
    ...
    for _ in 0..swap_variant_trials {
        if started_at.elapsed().as_secs_f64() >= late_answer_start_sec {
            return next_answer;
        }
        if keep_ids.is_empty() {
        ...
        let parent_id = keep_ids[rng.next_usize(keep_ids.len())];
        let parent_indices = current.placement_indices_for(parent_id);
        if !fill_swap_variant_candidate_indices(&mut indices, parent_indices, placement_bounds, rng)
        {
            continue;
        }
    ...
    let random_trials = target_size.saturating_sub(next_answer.len());
...
fn fill_move_variant_candidate_indices(
    indices: &mut [u16],
    parent_indices: &[u16],
    variant_neighbors: &[Vec<[Option<u16>; 4]>],
    rng: &mut XorShift64,
    ...
    for oil_id in 0..indices.len() {
        ...
        if let Some(next_placement_id) = variant_neighbors[oil_id][placement_id][dir] {
...
fn fill_random_one_oil_variant_candidate_indices(
    indices: &mut [u16],
    parent_indices: &[u16],
    placements_by_oil: &[Vec<Placement>],
    rng: &mut XorShift64,
...
fn fill_swap_variant_candidate_indices(
    indices: &mut [u16],
    parent_indices: &[u16],
    placement_bounds: &[PlacementBounds],
    rng: &mut XorShift64,
    ...
    let oil_a = rng.next_usize(indices.len());
    let mut oil_b = rng.next_usize(indices.len() - 1);
    if oil_b >= oil_a {
        oil_b += 1;
    }
    ...
    let next_a = clamp_coord_to_placement_id(deltai_b, deltaj_b, placement_bounds[oil_a]);
    let next_b = clamp_coord_to_placement_id(deltai_a, deltaj_a, placement_bounds[oil_b]);

    indices[oil_a] = next_a;
    indices[oil_b] = next_b;

    indices[oil_a] != parent_indices[oil_a] || indices[oil_b] != parent_indices[oil_b]
}
...
fn clamp_coord_to_placement_id(deltai_k: usize, deltaj_k: usize, bounds: PlacementBounds) -> u16 {
    let clamped_i = deltai_k.min(bounds.rows - 1);
    let clamped_j = deltaj_k.min(bounds.cols - 1);
