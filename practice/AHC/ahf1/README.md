...
- `./scripts/score_tools.sh`
  - 公式 `tools` の評価バイナリをラップする（`score` があれば `score`、無ければ `vis` の `Score = ...` を使用）。
...
- `./scripts/vis_tools.sh <input_file> <output_file> [svg_file]`
  - 公式 `tools` の `vis` バイナリをラップする。
  - `out.svg` の出力先衝突を避けるため一時ディレクトリで実行し、指定パス（省略時 `results/vis/<output_basename>.svg`）へ保存する。
