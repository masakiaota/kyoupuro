...
fn deletion_gain(seq: &[Event], order: usize, problem: &Problem) -> i64 {
    let nodes = build_nodes(seq);
    let mut pick_idx = usize::MAX;
...
fn choose_replace_removal_order(
    ...
    for (idx, &order) in state.selected_ids.iter().enumerate() {
        let gain = deletion_gain(&state.seq, order, problem);
        if gain > best_gain {
            best_gain = gain;
            best_idx = idx;
        }
