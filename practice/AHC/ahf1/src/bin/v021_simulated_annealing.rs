...
const DEFAULT_START_TEMP: f64 = 200.0;
const DEFAULT_END_TEMP: f64 = 1.0;
...
fn read_temp_var(name: &str, default: f64) -> f64 {
    std::env::var(name)
        .ok()
...
fn read_schedule_var() -> bool {
    matches!(
        std::env::var("V021_SCHEDULE").ok().as_deref(),
        Some("linear")
    )
}
...
fn greedy_initial_state(orders: &[Order]) -> State {
    ...
    let len = current_len.max(total_length(&seq, orders));
    State {
        seq,
        selected,
        selected_ids,
...
fn propose_reinsert(state: &State, orders: &[Order], rng: &mut Xoshiro256PlusPlus) -> State {
    let order = state.selected_ids[rng.random_range(0..M)];
    let mut candidate = state.clone();
    candidate.seq = remove_order_events(&state.seq, order);
    candidate.len = apply_best_insertion(&mut candidate.seq, orders, order);
    candidate
}

fn propose_replace(state: &State, orders: &[Order], rng: &mut Xoshiro256PlusPlus) -> State {
    ...
    candidate.len = apply_best_insertion(&mut candidate.seq, orders, add_order);
...
fn propose_double_replace(state: &State, orders: &[Order], rng: &mut Xoshiro256PlusPlus) -> State {
    ...
    cand1.len = apply_best_insertion(&mut cand1.seq, orders, add_a);
    cand1.len = apply_best_insertion(&mut cand1.seq, orders, add_b);
    ...
    cand2.seq = reduced;
    cand2.len = apply_best_insertion(&mut cand2.seq, orders, add_b);
    cand2.len = apply_best_insertion(&mut cand2.seq, orders, add_a);
...
fn temperature(progress: f64, start_temp: f64, end_temp: f64, linear: bool) -> f64 {
    let p = progress.clamp(0.0, 1.0);
    if linear {
        start_temp + (end_temp - start_temp) * p
    } else {
...
fn main() {
    ...
    let start_temp = read_temp_var("V021_START_TEMP", DEFAULT_START_TEMP);
    let end_temp = read_temp_var("V021_END_TEMP", DEFAULT_END_TEMP);
    let linear_schedule = read_schedule_var();

    let mut current = greedy_initial_state(&orders);
    ...
    while Instant::now() < deadline {
        ...
        let temp = temperature(progress, start_temp, end_temp, linear_schedule);
        let move_type = rng.random_range(0..threshold_total);

        let candidate = if move_type < threshold_reinsert {
            propose_reinsert(&current, &orders, &mut rng)
        } else if move_type < threshold_replace1 {
            propose_replace(&current, &orders, &mut rng)
        } else {
            propose_double_replace(&current, &orders, &mut rng)
        };
    ...
    let mut path: Vec<(i32, i32)> = Vec::with_capacity(best.seq.len() + 2);
    path.push(CENTER);
    for &ev in &best.seq {
