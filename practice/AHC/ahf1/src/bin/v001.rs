...
const REPLACE_ITERS: usize = 1500;
...
fn best_insertion(nodes: &[(i32, i32)], pickup: (i32, i32), dropoff: (i32, i32)) -> Insertion {
    // nodes: [center, events..., center]
    // pickup is inserted between nodes[i] and nodes[i + 1], i in [0, edges-1]
    // dropoff is inserted after pickup.
...
fn main() {
    ...
    // Local improvement: reinsert each selected order to its best position.
    for _ in 0..REINSERT_PASSES {
        ...
        for &order in &selected_ids {
            ...
            if new_len < current_len {
                insert_order_events(&mut reduced, order, ins.pick_pos, ins.drop_pos);
                seq = reduced;
                current_len = new_len;
                improved = true;
            }
    ...
    // Subset improvement: random replacement (improving moves only).
    let mut rng = Xoshiro256PlusPlus::seed_from_u64(seed_from_orders(&orders));
    for _ in 0..REPLACE_ITERS {
        ...
        if new_len < current_len {
            insert_order_events(&mut reduced, add_order, ins.pick_pos, ins.drop_pos);
            seq = reduced;
            current_len = new_len;
    ...
    // Final polish after subset changes.
    for &order in &selected_ids {
        let mut reduced = remove_order_events(&seq, order);
        let reduced_nodes = build_nodes(&reduced, &orders);
        let reduced_len = nodes_length(&reduced_nodes);
