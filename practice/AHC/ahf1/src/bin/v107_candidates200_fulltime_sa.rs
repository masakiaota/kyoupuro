// v107_candidates200_fulltime_sa.rs
...
const INITIAL_CANDIDATES: usize = 200;
...
fn build_nodes_and_positions(seq: &[Event]) -> (Vec<usize>, Vec<(usize, usize)>) {
    let mut nodes = Vec::with_capacity(seq.len() + 2);
    let mut positions = vec![(usize::MAX, usize::MAX); N];
    nodes.push(CENTER_ID);
    for (i, &ev) in seq.iter().enumerate() {
        let pos = i + 1;
        if ev.kind == 0 {
            positions[ev.order].0 = pos;
        } else {
            positions[ev.order].1 = pos;
        }
        nodes.push(event_node_id(ev));
    }
    nodes.push(CENTER_ID);
...
fn best_insertion(problem: &Problem, nodes: &[usize], order: usize) -> Insertion {
    ...
    let mut best_pick_delta = i32::MAX;
    let mut best_pick_pos = 0usize;

    for edge in 0..edges {
        let a = nodes[edge];
        let b = nodes[edge + 1];
        let edge_cost = get_dist(problem, a, b);

        if edge > 0 {
            let delta_drop = get_dist(problem, a, dropoff) + get_dist(problem, dropoff, b) - edge_cost;
            let total = best_pick_delta + delta_drop;
            if total < best.delta {
                best = Insertion {
                    delta: total,
        ...
        let delta_pick = get_dist(problem, a, pickup) + get_dist(problem, pickup, b) - edge_cost;
        let delta_same_edge = delta_pick
            + get_dist(problem, pickup, dropoff)
            + get_dist(problem, dropoff, b)
            - get_dist(problem, pickup, b);
        if delta_same_edge < best.delta {
            best = Insertion {
                delta: delta_same_edge,
                pick_pos: edge,
                drop_pos: edge + 1,
            };
        }

        if delta_pick < best_pick_delta {
            best_pick_delta = delta_pick;
            best_pick_pos = edge;
        }
    }
...
fn greedy_candidate_orders(problem: &Problem) -> Vec<usize> {
...
fn greedy_initial_state(problem: &Problem) -> State {
    ...
    let candidates = greedy_candidate_orders(problem);
...
fn deletion_gain(
    ...
    let (pick_idx, drop_idx) = positions[order];
    debug_assert!(pick_idx != usize::MAX && drop_idx != usize::MAX && pick_idx < drop_idx);

    if pick_idx + 1 == drop_idx {
        let prev = nodes[pick_idx - 1];
        let pick = nodes[pick_idx];
        let drop = nodes[drop_idx];
        let next = nodes[drop_idx + 1];
        return (get_dist(problem, prev, pick)
            + get_dist(problem, pick, drop)
            + get_dist(problem, drop, next)
            - get_dist(problem, prev, next)) as i64;
    }

    let prev_pick = nodes[pick_idx - 1];
    let pick = nodes[pick_idx];
    let next_pick = nodes[pick_idx + 1];
    let prev_drop = nodes[drop_idx - 1];
    let drop = nodes[drop_idx];
    let next_drop = nodes[drop_idx + 1];

    (get_dist(problem, prev_pick, pick)
        + get_dist(problem, pick, next_pick)
        - get_dist(problem, prev_pick, next_pick)
        + get_dist(problem, prev_drop, drop)
        + get_dist(problem, drop, next_drop)
        - get_dist(problem, prev_drop, next_drop)) as i64
}

fn choose_replace_removal_order(
    state: &State,
    problem: &Problem,
    ...
    let (nodes, positions) = build_nodes_and_positions(&state.seq);
    let mut best_idx = 0;
    let mut best_gain = i64::MIN;
    for (idx, &order) in state.selected_ids.iter().enumerate() {
        let gain = deletion_gain(problem, &nodes, &positions, order);
        if gain > best_gain {
            best_gain = gain;
            best_idx = idx;
        }
...
fn propose_replace(state: &State, problem: &Problem, rng: &mut Xoshiro256PlusPlus) -> State {
    let rem_idx = choose_replace_removal_order(state, problem, rng);
...
fn main() {
    ...
    let mut current = greedy_initial_state(&problem);
    let mut best = current.clone();

    let deadline = program_start + Duration::from_millis(TOTAL_LIMIT_MS);
