...
fn best_initial_state(problem: &Problem, rng: &mut Xoshiro256PlusPlus) -> State {
    let first_center = (rng.random_range(300..=500), rng.random_range(300..=500));
    let mut best = greedy_initial_state(problem, first_center);
    for _ in 1..INITIAL_STARTS {
        let center = (rng.random_range(300..=500), rng.random_range(300..=500));
...
fn main() {
    ...
    let mut current = best_initial_state(&problem, &mut rng);
