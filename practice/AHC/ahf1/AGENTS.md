...
## 採点運用ルール
- `results/score_summary.csv` に記録してよいのは、`./scripts/score_tools.sh <bin_name>` の全件評価、またはユーザーが明示した subset 評価のみとする。
- 単体デバッグ（`eval_set=single`）や実行途中の中間結果（ケース不足）は記録しない。
- `run.sh` と `score_tools.sh` は並列起動しない。採点は `run.sh` 完了後に実行する。
