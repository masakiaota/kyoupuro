// v112_depth5_greedy_tail_mcts.rs
...
const ACTION_SEQ_DEPTH: usize = 5;
...
impl Node {
    ...
    fn redo_playout(
        ...
        if terminal {
            ...
            mixed_rollout_to_end(&board, turn, fs, rng, self.actions.len(), action_seq_depth) as f64
        }
    }
    ...
    fn evaluate(
        ...
        if self.child_nodes.is_empty() {
            let value = self.redo_playout(root_board, root_turn, fs, rng, action_seq_depth);
            self.w += value;
            self.n += 1;
...
fn choose_dir(
    ...
    let max_depth = ACTION_SEQ_DEPTH.min(remaining_decisions);
