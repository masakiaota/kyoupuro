// v113_fast_greedy_rollout_mc.rs
...
impl Node {
    ...
    fn redo_playout(
        ...
        if terminal {
            ...
            greedy_rollout_to_end(&board, turn, fs, rng) as f64
        }
    }
    ...
    fn evaluate(
        ...
        let value = self.child_nodes[idx].evaluate(root_board, root_turn, fs, rng, max_depth);
        self.w += value;
        self.n += 1;
        value
    }
    ...
    match next_flavor {
        Some(flavor) => {
            ...
            for (idx, dir) in DIRS.into_iter().enumerate() {
                ...
                let surface = tilted.next_surface_gain(dir, flavor);
...
fn choose_dir(
    ...
    while !timer.is_time_up() && timer.elapsed() < local_deadline {
        ...
        let _ = root.evaluate(board_after_place, turn, fs, &mut sample_rng, remaining_decisions);
    }
