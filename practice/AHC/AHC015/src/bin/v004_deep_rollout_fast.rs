...
fn choose_rollout_horizon(
    timer: &Timer,
    decision_turn: usize,
    remaining_decisions: usize,
) -> usize {
    let base = match decision_turn {
        1..=5 => 0,
        6..=15 => 10,
        16..=30 => 40,
        _ => remaining_decisions,
    }
    .min(remaining_decisions);

    let remaining = timer.remaining();
    if remaining < 0.010 {
        0
    } else if remaining < 0.020 {
        base.min(4)
    } else if remaining < 0.040 {
        base.min(8)
    } else {
        base
    }
...
fn choose_dir(
    ...
    let decision_turn = turn + 1;
    let placed = decision_turn;
    ...
    while !timer.is_time_up() && timer.elapsed() < local_deadline {
        let horizon = choose_rollout_horizon(timer, decision_turn, NN - placed);
        if horizon == 0 {
            break;
        }
