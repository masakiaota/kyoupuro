// v117codex_region_potential.rs
use std::io::{self, BufRead, BufReader, BufWriter, Write};

const N: usize = 10;
const NN: usize = N * N;
const DIRS: [u8; 4] = [b'F', b'B', b'L', b'R'];
const TARGETS: [(i32, i32); 4] = [(0, 0), (0, 4), (9, 0), (9, 9)];
...
fn region_potential(board: &Board) -> i64 {
    let mut total = 0i64;
    for idx in 0..NN {
        let flavor = board.cells[idx];
        if flavor == 0 {
            continue;
        }
        let (tr, tc) = TARGETS[flavor as usize];
        let r = (idx / N) as i32;
        let c = (idx % N) as i32;
        let dist = (r - tr).abs() + (c - tc).abs();
        total += i64::from(2 * N as i32 - dist);
    }
    total
}

fn choose_dir(board: &Board, next_flavor: Option<u8>) -> (u8, Board) {
    let mut best_idx = 0usize;
    let mut best_board = Board::new();
    let mut best_region = i64::MIN;
    ...
    for (idx, dir) in DIRS.into_iter().enumerate() {
        ...
        let region = region_potential(&tilted);
        let surface = match next_flavor {
            Some(flavor) => tilted.next_surface_gain_with_counts(dir, flavor, &counts),
            None => -1,
        };
        let comp_sq = tilted.component_sq_sum();
        if region > best_region
            || (region == best_region && surface > best_surface)
            || (region == best_region && surface == best_surface && comp_sq > best_comp_sq)
        {
            best_idx = idx;
            best_board = tilted;
            best_region = region;
...
fn main() {
    ...
    for turn in 0..NN {
        ...
        let next_flavor = if turn + 1 < NN { Some(fs[turn + 1]) } else { None };
        let (dir, next_board) = choose_dir(&board, next_flavor);
        board = next_board;
