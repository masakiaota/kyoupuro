...
fn proxy_value(board: &Board) -> i64 {
    let mut same_adj = 0i64;
    let mut diff_adj = 0i64;
    for idx in 0..NN {
        let color = board.cells[idx];
        if color == 0 {
            continue;
...
fn rollout_value(board: &Board, next_turn: usize, fs: &[u8; NN], rng: &mut XorShift64) -> i64 {
    ...
    while turn < NN {
        ...
        let mut best_value = proxy_value(&best_board);
        for &dir in &DIRS[1..] {
            let cand = board.tilted(dir);
            let value = proxy_value(&cand);
            if value > best_value {
                best_value = value;
                best_board = cand;
            }
