...
fn choose_dir(
    ...
    while !timer.is_time_up() && timer.elapsed() < local_deadline {
        ...
        for candidate in &mut candidates {
            ...
            let value = rollout_value(&candidate.board, placed, fs, target, &mut sample_rng, horizon);
...
fn main() {
    ...
    let mut fixed_target_idx: Option<usize> = None;
    ...
    for turn in 0..NN {
        ...
        let target_idx = match fixed_target_idx {
            Some(idx) => idx,
            None => {
                let idx = choose_best_target(&board, turn + 1, &targets);
                fixed_target_idx = Some(idx);
                idx
            }
        };
        let dir = choose_dir(&board, turn, &fs, &targets[target_idx], &timer, &mut rng);
