...
struct TurnStats {
    turn: usize,
    elapsed_before_ms: f64,
    elapsed_after_ms: f64,
    spent_ms: f64,
    local_budget_ms: f64,
    remaining_decisions: usize,
    initial_horizon: usize,
    last_horizon: usize,
    horizon2_count: usize,
    horizon3_count: usize,
    horizon4_count: usize,
    horizon6_count: usize,
    horizon8_count: usize,
    outer_loops: usize,
...
fn choose_dir(
    ...
    let mut outer_loops = 0usize;
    let mut horizon2_count = 0usize;
    let mut horizon3_count = 0usize;
    let mut horizon4_count = 0usize;
    let mut horizon6_count = 0usize;
    let mut horizon8_count = 0usize;
    let mut initial_horizon = 0usize;
    let mut last_horizon = 0usize;
    ...
    while !timer.is_time_up() && timer.elapsed() < local_deadline {
        ...
        for candidate in &mut candidates {
            ...
            let value =
                rollout_value(&candidate.board, placed, fs, target, &mut sample_rng, horizon);
    ...
    let stats = TurnStats {
        turn: placed,
        elapsed_before_ms: elapsed_before * 1000.0,
        elapsed_after_ms: elapsed_after * 1000.0,
        spent_ms: (elapsed_after - elapsed_before) * 1000.0,
        local_budget_ms: (local_deadline - elapsed_before).max(0.0) * 1000.0,
        remaining_decisions,
        initial_horizon,
        last_horizon,
        horizon2_count,
        ...
        horizon8_count,
        outer_loops,
        total_rollouts: candidates.iter().map(|c| c.rollout_count).sum(),
        f_rollouts: candidates[0].rollout_count,
        b_rollouts: candidates[1].rollout_count,
        l_rollouts: candidates[2].rollout_count,
        r_rollouts: candidates[3].rollout_count,
        chosen_dir: candidates[best_idx].dir,
    };
    (candidates[best_idx].dir, stats)
}
...
fn main() {
    ...
    let mut stats_list = Vec::with_capacity(NN);
    ...
    for turn in 0..NN {
        ...
        board.place_pth_empty(p, fs[turn]);
        if turn == NN - 1 {
            stats_list.push(TurnStats {
                turn: NN,
                elapsed_before_ms: timer.elapsed() * 1000.0,
                elapsed_after_ms: timer.elapsed() * 1000.0,
                spent_ms: 0.0,
                local_budget_ms: 0.0,
        ...
        let (dir, stats) = choose_dir(&board, turn, &fs, &targets, &timer, &mut rng);
    ...
    eprintln!(
        "turn,dir,elapsed_before_ms,elapsed_after_ms,spent_ms,local_budget_ms,remaining_decisions,initial_horizon,last_horizon,outer_loops,total_rollouts,f_rollouts,b_rollouts,l_rollouts,r_rollouts,h2,h3,h4,h6,h8"
    );
    for s in stats_list {
        eprintln!(
            "{},{},{:.3},{:.3},{:.3},{:.3},{},{},{},{},{},{},{},{},{},{},{},{},{},{}",
            s.turn,
            s.chosen_dir as char,
            s.elapsed_before_ms,
            s.elapsed_after_ms,
            s.spent_ms,
            s.local_budget_ms,
            s.remaining_decisions,
            s.initial_horizon,
            s.last_horizon,
            s.outer_loops,
            s.total_rollouts,
