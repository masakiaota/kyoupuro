...
fn sample_future_places(next_turn: usize, rng: &mut XorShift64) -> [usize; NN] {
    let mut places = [0usize; NN];
    for (turn, slot) in places.iter_mut().enumerate().skip(next_turn) {
        let remaining_empty = NN - turn;
        *slot = (rng.next_u64() as usize % remaining_empty) + 1;
    }
    places
}

fn rollout_value_with_places(
    board: &Board,
    next_turn: usize,
    fs: &[u8; NN],
    future_places: &[usize; NN],
) -> i64 {
    let mut board = *board;
    let mut turn = next_turn;
    while turn < NN {
        let cur_flavor = fs[turn];
        board.place_pth_empty(future_places[turn], cur_flavor);
        if turn == NN - 1 {
            break;
        }
...
fn choose_dir(
    ...
    loop {
        let future_places = sample_future_places(turn + 1, outer_rng);
        for candidate in &mut candidates {
            let value = rollout_value_with_places(&candidate.board, turn + 1, fs, &future_places);
            candidate.sum += value as f64;
            candidate.count += 1;
        }
        if timer.is_time_up() || timer.elapsed() >= deadline {
            break;
        }
    }
