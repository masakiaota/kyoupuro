...
fn evaluate_features(board: &Board) -> EvalFeatures {
    ...
    let mut center_diff_adj = 0i32;
    let mut max_comp = [0i32; 4];
    ...
    for idx in 0..NN {
        ...
        if c + 1 < N {
            ...
            if right != 0 {
                if right == color {
                    ...
                    diff_adj += 1;
                    if (2..=7).contains(&r) && (2..=6).contains(&c) {
                        center_diff_adj += 1;
                    }
                }
            }
        ...
        if r + 1 < N {
            ...
            if down != 0 {
                if down == color {
                    ...
                    diff_adj += 1;
                    if (2..=6).contains(&r) && (2..=7).contains(&c) {
                        center_diff_adj += 1;
                    }
                }
            }
        ...
        comp_sq += size * size;
        num_components += 1;
        let flavor = color as usize;
        if size > max_comp[flavor] {
            max_comp[flavor] = size;
        }
    }
    ...
    EvalFeatures {
        ...
        diff_adj,
        center_diff_adj,
        max_comp_sum: max_comp[1] + max_comp[2] + max_comp[3],
        num_components,
    }
}

fn static_value(board: &Board, placed: usize) -> i64 {
    let f = evaluate_features(board);
    let phase = placed as i64;
    let comp_w = if phase < 20 { 10 } else { 14 };
    let same_w = if phase < 20 { 14 } else if phase < 75 { 18 } else { 16 };
    let diff_w = if phase < 20 { 8 } else if phase < 70 { 14 } else { 10 };
    let center_diff_w = if phase < 70 { 12 } else { 5 };
    let largest_w = if phase < 25 { 18 } else if phase < 70 { 28 } else { 18 };
    let component_penalty_w = if phase < 20 { 1 } else if phase < 70 { 4 } else { 2 };

    i64::from(f.comp_sq) * comp_w
        + i64::from(f.same_adj) * same_w
        - i64::from(f.diff_adj) * diff_w
        - i64::from(f.center_diff_adj) * center_diff_w
        + i64::from(f.max_comp_sum) * largest_w
        - i64::from(f.num_components) * component_penalty_w
}
...
fn rollout_value(
    ...
    while turn < end_turn {
        ...
        let mut best_score = static_value(&best_board, placed);
        for &dir in &DIRS[1..] {
            let cand = board.tilted(dir);
            let score = static_value(&cand, placed);
            if score > best_score {
                best_score = score;
                best_board = cand;
            }
...
fn choose_dir(
    ...
    for &dir in &DIRS {
        ...
        candidates[dir_index(dir)] = Candidate {
            dir,
            board: tilted,
            sum: (static_value(&tilted, placed) * 2) as f64,
            weight: 2.0,
        };
    }
    ...
    while !timer.is_time_up() && timer.elapsed() < local_deadline {
        ...
        for candidate in &mut candidates {
            ...
            let value = rollout_value(&candidate.board, placed, fs, &mut sample_rng, horizon);
