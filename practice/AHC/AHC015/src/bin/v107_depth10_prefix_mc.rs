...
fn choose_dir(
    ...
    let mut best = PrefixCandidate {
        actions: [fallback_dir; PREFIX_DEPTH],
        sum: f64::NEG_INFINITY,
        count: 0,
    };
    let mask = prefixes.len() - 1;
    let mut idx = (rng.next_u64() as usize) & mask;
    let mut stride = ((rng.next_u64() as usize) | 1) & mask;
    if stride == 0 {
        stride = 1;
    }
    while !timer.is_time_up() && timer.elapsed() < deadline {
        let mut sample_rng = XorShift64::new(rng.next_u64());
        let value = prefix_value(board_after_place, turn, &prefixes[idx], depth, fs, &mut sample_rng);
        if value as f64 > best.sum {
            best = PrefixCandidate {
                actions: prefixes[idx],
                sum: value as f64,
                count: 1,
            };
        }
...
fn main() {
    ...
    let mut prefixes = Vec::with_capacity(4usize.pow(PREFIX_DEPTH as u32));
    let mut cur = [DIRS[0]; PREFIX_DEPTH];
    build_sequences(PREFIX_DEPTH, 0, &mut cur, &mut prefixes);
    ...
    for turn in 0..NN {
        ...
        let dir = choose_dir(&board, turn, &fs, &prefixes, &timer, &mut rng);
