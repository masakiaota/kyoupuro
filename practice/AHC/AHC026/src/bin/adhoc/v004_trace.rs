// v004_trace.rs
#![allow(dead_code)]
// v004_beam の v002 prefix 追跡ログ付き adhoc 版。
...
impl State {
    ...
    fn v002_greedy_action(&self) -> Action {
        let v = self.next_v;
        let src = self.pos_i[v as usize];
        let j = self.pos_j[v as usize];
        let h = self.pile[src as usize].len();
...
impl Selector {
    ...
    fn get_by_hash(&self, hash_key: u64) -> Option<BeamCandidate> {
        self.by_hash
            .get(&hash_key)
            .and_then(|&index| self.candidates[index])
    }
...
fn rollback_beam_search(state: &mut State, max_turn: usize, beam_width: usize) -> Vec<Action> {
    ...
    let mut protected_state = state.clone();
    let mut first_drop: Option<usize> = None;
    for turn in 0..max_turn {
        let protected_parent_hash = protected_state.hash_key();
        let protected_parent_eval = protected_state.evaluate();
        let protected_action = protected_state.v002_greedy_action();
        let protected_action_text = action_summary(&protected_state, protected_action);
        let mut protected_child_state = protected_state.clone();
        protected_child_state.move_forward(protected_action);
        let protected_child_hash = protected_child_state.hash_key();
        let protected_child_eval = protected_child_state.evaluate();
        let protected_parent_node = beam
            .iter()
            .copied()
            .find(|&node| nodes[node].hash_key == protected_parent_hash);
        let parent_status = match protected_parent_node {
            Some(node) if nodes[node].evaluator.total_cost == protected_parent_eval.total_cost => {
                "exact"
            }
            Some(node) if nodes[node].evaluator.total_cost < protected_parent_eval.total_cost => {
                "same_hash_better_cost"
            }
            Some(_) => "same_hash_worse_cost",
            None => "missing",
        };
        let mut protected_generated = false;
        selector.clear();
        ...
        for edge in tour_edges {
            match edge {
                ...
                TourEdge::Visit(parent) => {
                    ...
                    if Some(parent) == protected_parent_node {
                        protected_generated = actions_buf.contains(&protected_action);
                    }
        ...
        let kept_candidate = selector.get_by_hash(protected_child_hash);
        let kept_hash = kept_candidate.is_some();
        let kept_exact = kept_candidate.is_some_and(|candidate| {
            candidate.evaluator.total_cost == protected_child_eval.total_cost
                && candidate.evaluator.score_key == protected_child_eval.score_key
        });
        let worst_kept = selector.worst.max_all().map(|slot| slot.evaluator);
        eprintln!(
            "[trace] depth={} target={} parent={} generated={} kept_hash={} kept_exact={} parent_eval={:?} child_eval={:?} kept_eval={:?} worst_kept={:?}",
            turn,
            (turn + 1),
            parent_status,
            protected_generated,
            kept_hash,
            kept_exact,
            protected_parent_eval,
            protected_child_eval,
            kept_candidate.map(|candidate| candidate.evaluator),
            worst_kept,
        );
        if first_drop.is_none() && !kept_hash {
            first_drop = Some(turn);
            eprintln!(
                "[trace.drop] depth={} target={} parent={} generated={} action={} child_hash={:016x} child_eval={:?} worst_kept={:?}",
                turn,
                (turn + 1),
                parent_status,
                protected_generated,
                protected_action_text,
                protected_child_hash,
                protected_child_eval,
                worst_kept,
            );
        }
        ...
        protected_state.move_forward(protected_action);
    }
    let best_leaf = beam
        .iter()
        .copied()
...
fn action_summary(state: &State, action: Action) -> String {
    let mut state = state.clone();
    let mut parts = Vec::new();
    let mut cost = 0u32;
    for step in action.moves[..action.move_count as usize].iter().copied() {
        let bottom = state.pile[action.src as usize][step.j as usize];
        cost += step.len as u32 + 1;
        parts.push(format!(
            "{}->{}(len={},c={})",
            bottom as usize + 1,
            step.dst as usize + 1,
