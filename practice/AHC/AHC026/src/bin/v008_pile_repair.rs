// v008_pile_repair.rs
#![allow(dead_code)]
// rollback beam + target pile repair 実験版。
// Action は「v の上の塊を複数 move に分割してから v を carry」する macro action。
// さらに対象列の suffix を退避し、他列 top suffix の良い block を対象列へ積む repair action も混ぜる。
...
/// - repair action では複数の source から block を動かすことがある。
...
impl State {
    ...
    #[inline]
    fn apply_macro_step(&mut self, step: MoveStep) {
        self.move_block(step.src as usize, step.j as usize, step.dst as usize);
        self.total_cost += step.len as u32 + 1;
    }

    #[inline]
    fn undo_macro_step(&mut self, step: MoveStep) {
        let dst_back_j = self.pile[step.dst as usize].len() - step.len as usize;
        self.move_block(step.dst as usize, dst_back_j, step.src as usize);
        self.total_cost -= step.len as u32 + 1;
    }
    ...
    fn enumerate_actions(&self, out: &mut Vec<Action>) {
        ...
        self.enumerate_repair_actions(out);
    }
    ...
    fn enumerate_repair_actions(&self, out: &mut Vec<Action>) {
        let mut targets = (0..M as u8)
            .map(|target| (count_pile_inv(&self.pile[target as usize]), target))
            .collect::<Vec<_>>();
        targets.sort_unstable_by(|a, b| b.cmp(a));
        targets.truncate(REPAIR_TARGET_WIDTH);
        for &(_, target) in &targets {
            let prefixes = self.repair_prefix_candidates(target);
            for prefix_len in prefixes {
                let Some(action) = self.build_repair_action(target, prefix_len) else {
                    continue;
                };
                if !out.iter().any(|&existing| existing == action) {
                    out.push(action);
    ...
    fn repair_prefix_candidates(&self, target: u8) -> Vec<usize> {
        let pile = &self.pile[target as usize];
        let h = pile.len();
        let mut candidates = Vec::<(u32, usize)>::with_capacity(h + 1);
        for prefix_len in 0..=h {
            if target == self.pos_i[self.next_v as usize] {
                let next_j = self.pos_j[self.next_v as usize] as usize;
                if prefix_len <= next_j {
                    continue;
                }
            }
            let prefix_inv = count_pile_inv(&pile[..prefix_len]);
            let clear_len = h - prefix_len;
            let score = prefix_inv * REPAIR_PREFIX_INV_SCORE + clear_len as u32;
            candidates.push((score, prefix_len));
        }
        candidates.sort_unstable();
        candidates.truncate(REPAIR_PREFIX_WIDTH);
        if !candidates.iter().any(|&(_, p)| p == h) {
            candidates.push((count_pile_inv(pile) * REPAIR_PREFIX_INV_SCORE, h));
        }
        candidates.into_iter().map(|(_, p)| p).collect()
    }

    fn build_repair_action(&self, target: u8, prefix_len: usize) -> Option<Action> {
        ...
        if prefix_len < sim.pile[target as usize].len() {
            let dst = sim.best_evac_dst(target, prefix_len)?;
            let len = sim.pile[target as usize].len() - prefix_len;
            let step = MoveStep {
                src: target,
                j: prefix_len as u8,
                dst,
            ...
            action.moves[action.move_count as usize] = step;
            action.move_count += 1;
            sim.apply_macro_step(step);
        }

        for _ in 0..REPAIR_FILL_STEPS {
            if action.move_count as usize >= MAX_MACRO_MOVES {
                break;
            }
            let Some(step) = sim.best_repair_fill_step(target) else {
                break;
            };
            action.moves[action.move_count as usize] = step;
            action.move_count += 1;
    ...
    fn best_evac_dst(&self, target: u8, prefix_len: usize) -> Option<u8> {
        let pile = &self.pile[target as usize];
        let suffix = &pile[prefix_len..];
        if suffix.is_empty() {
            return None;
        }
        let removed_cross = count_cross_inv(&pile[..prefix_len], suffix);
        let mut best: Option<(i32, u8)> = None;
        for dst in 0..M as u8 {
            if dst == target {
                continue;
            }
            let added_cross = count_cross_inv(&self.pile[dst as usize], suffix);
            let key = (added_cross as i32 - removed_cross as i32, dst);
    ...
    fn best_repair_fill_step(&self, target: u8) -> Option<MoveStep> {
        let mut best: Option<(i32, u8, MoveStep)> = None;
        for src in 0..M as u8 {
            if src == target {
                continue;
            }
            let h = self.pile[src as usize].len();
            let max_len = h.min(REPAIR_MAX_BLOCK_LEN);
            for len in 1..=max_len {
                let j = h - len;
                if src == self.pos_i[self.next_v as usize]
                    && j <= self.pos_j[self.next_v as usize] as usize
                {
                    continue;
                }
                let block = &self.pile[src as usize][j..];
                let prefix = &self.pile[src as usize][..j];
                let removed_cross = count_cross_inv(prefix, block);
                let added_cross = count_cross_inv(&self.pile[target as usize], block);
                let internal_inv = count_pile_inv(block);
                let delta = added_cross as i32 - removed_cross as i32;
                let score = delta * REPAIR_GLOBAL_DELTA_SCORE
                    + internal_inv as i32 * REPAIR_INTERNAL_INV_SCORE
                    + (len as i32 + 1) * REPAIR_COST_SCORE;
                if score >= REPAIR_ACCEPT_SCORE {
                    continue;
                }
                let step = MoveStep {
                    src,
...
/// target pile repair operator の候補幅と評価。
const REPAIR_TARGET_WIDTH: usize = 3;
const REPAIR_PREFIX_WIDTH: usize = 2;
const REPAIR_FILL_STEPS: usize = 2;
const REPAIR_MAX_BLOCK_LEN: usize = 5;
const REPAIR_PREFIX_INV_SCORE: u32 = 20;
const REPAIR_GLOBAL_DELTA_SCORE: i32 = 100;
const REPAIR_INTERNAL_INV_SCORE: i32 = 20;
const REPAIR_COST_SCORE: i32 = 1;
const REPAIR_ACCEPT_SCORE: i32 = 0;
...
fn actions_to_output(input: &Input, actions: &[Action]) -> (Output, u32) {
    ...
    for &action in actions {
        for step in action.moves[..action.move_count as usize].iter().copied() {
            let bottom = state.pile[step.src as usize][step.j as usize];
            ops.push((bottom, step.dst));
            state.apply_macro_step(step);
        }
        for step in action.carries[..action.carry_count as usize]
            .iter()
            .copied()
