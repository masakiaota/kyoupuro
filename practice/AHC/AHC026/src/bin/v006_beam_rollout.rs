// v006_beam_rollout.rs
#![allow(dead_code)]
// rollback beam + greedy rollout 実験版。
// Action は「v の上の塊を複数 move に分割してから v を carry」する macro action。
// 分割候補は v002 と同じ near-lexico スカラー評価で小さいローカル beam を走らせて作る。
// child state を cheap 評価で一度絞り、その後 v002 greedy で完走した total_cost を beam rank に使う。
use proconio::input;
...
impl State {
    ...
    /// v002 と同じ逐次 greedy で、現在の `next_v` を取り出す 1 macro action を作る。
...
fn greedy_complete(state: &mut State, out: &mut Vec<Action>) {
    ...
    while (state.next_v as usize) < N {
        let action = state.greedy_action();
