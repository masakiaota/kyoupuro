#![allow(non_snake_case)]

use proconio::input;
use rand::prelude::*;
use svg::node::{
    element::{Group, Rectangle, Style, Title},
    Text,
};

trait SetMinMax {
    fn setmin(&mut self, v: Self) -> bool;
    fn setmax(&mut self, v: Self) -> bool;
}
impl<T> SetMinMax for T
where
    T: PartialOrd,
{
    fn setmin(&mut self, v: T) -> bool {
        *self > v && {
            *self = v;
            true
        }
    }
    fn setmax(&mut self, v: T) -> bool {
        *self < v && {
            *self = v;
            true
        }
    }
}

#[derive(Clone, Debug)]
struct Input {
    n: usize,
    m: usize,
    bs: Vec<Vec<usize>>,
}

impl std::fmt::Display for Input {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        writeln!(f, "{} {}", self.n, self.m)?;
        for i in 0..self.m {
            let row: Vec<String> = self.bs[i].iter().map(|v| v.to_string()).collect();
            writeln!(f, "{}", row.join(" "))?;
        }
        Ok(())
    }
}

fn parse_input(f: &str) -> Input {
    let f = proconio::source::once::OnceSource::from(f);
    input! {
        from f,
        n: usize, m: usize,
        bs: [[usize; n / m]; m]
    }
    Input { n, m, bs }
}

fn read<T: Copy + PartialOrd + std::fmt::Display + std::str::FromStr>(
    token: Option<&str>,
    lb: T,
    ub: T,
) -> Result<T, String> {
    if let Some(v) = token {
        if let Ok(v) = v.parse::<T>() {
            if v < lb || ub < v {
                Err(format!("Out of range: {}", v))
            } else {
                Ok(v)
            }
        } else {
            Err(format!("Parse error: {}", v))
        }
    } else {
        Err("Unexpected EOF".to_owned())
    }
}

struct Output {
    out: Vec<(usize, usize)>,
}

fn parse_output(input: &Input, f: &str) -> Result<Output, String> {
    let mut out = vec![];
    let mut tokens = f.split_whitespace().peekable();
    while tokens.peek().is_some() {
        out.push((
            read(tokens.next(), 1, input.n)?,
            read(tokens.next(), 0, input.m)?,
        ));
    }
    if out.len() > 5000 {
        return Err("Too many output".to_owned());
    }
    Ok(Output { out })
}

fn gen_input(seed: u64) -> Input {
    let mut rng = rand_chacha::ChaCha20Rng::seed_from_u64(seed);
    let n = 200;
    let m = 10;
    let mut is: Vec<usize> = (1..=n).collect();
    is.shuffle(&mut rng);
    let mut bs = vec![vec![]; m];
    for i in 0..n {
        bs[i % m].push(is[i]);
    }
    Input { n, m, bs }
}

fn compute_score_details(
    input: &Input,
    out: &[(usize, usize)],
) -> (i64, i64, String, Vec<Vec<usize>>) {
    let mut bs = input.bs.clone();
    let mut cost: usize = 0;
    let mut t = 0;
    for &(v, mut to) in out {
        let (i, j) = 'ij: {
            for i in 0..bs.len() {
                for j in 0..bs[i].len() {
                    if bs[i][j] == v {
                        break 'ij (i, j);
                    }
                }
            }
            return (
                0,
                cost as i64,
                format!("Box {v} has already been taken out."),
                bs,
            );
        };
        if to == 0 {
            if j + 1 != bs[i].len() {
                return (
                    0,
                    cost as i64,
                    format!("Box {v} is not at the top of the stack."),
                    bs,
                );
            } else if v != t + 1 {
                return (
                    0,
                    cost as i64,
                    format!(
                        "Before carrying out box {v}, all boxes less than {v} must be carried out."
                    ),
                    bs,
                );
            }
            bs[i].pop();
            t += 1;
        } else {
            cost += bs[i].len() - j + 1;
            to -= 1;
            if i != to {
                for k in j..bs[i].len() {
                    let b = bs[i][k];
                    bs[to].push(b);
                }
                bs[i].truncate(j);
            }
        }
    }
    let score = (10000 - cost as i64).max(1);
    let err = if t < input.n {
        format!("Not finished ({} / {})", t, input.n)
    } else {
        String::new()
    };
    (score, cost as i64, err, bs)
}

fn color(mut val: f64) -> String {
    val.setmin(1.0);
    val.setmax(0.0);
    let (r, g, b) = if val < 0.5 {
        let x = val * 2.0;
        (
            30. * (1.0 - x) + 144. * x,
            144. * (1.0 - x) + 255. * x,
            255. * (1.0 - x) + 30. * x,
        )
    } else {
        let x = val * 2.0 - 1.0;
        (
            144. * (1.0 - x) + 255. * x,
            255. * (1.0 - x) + 30. * x,
            30. * (1.0 - x) + 70. * x,
        )
    };
    format!(
        "#{:02x}{:02x}{:02x}",
        r.round() as i32,
        g.round() as i32,
        b.round() as i32
    )
}

fn rect(x: usize, y: usize, w: usize, h: usize, fill: &str) -> Rectangle {
    Rectangle::new()
        .set("x", x)
        .set("y", y)
        .set("width", w)
        .set("height", h)
        .set("fill", fill)
}

#[allow(dead_code)]
fn group(title: String) -> Group {
    Group::new().add(Title::new("").add(Text::new(title)))
}

fn vis_prefix(input: &Input, out: &[(usize, usize)]) -> (i64, i64, String, String) {
    let W = 800;
    let H = 800;
    let (score, cost, err, bs) = compute_score_details(input, out);
    let max_h = (2 * input.n / input.m)
        .max(bs.iter().map(|b| b.len()).max().unwrap_or(0));
    let w = W / input.m;
    let h = H / max_h.max(1);
    let mut doc = svg::Document::new()
        .set("id", "vis")
        .set("viewBox", (-5, -5, W + 10, H + 10))
        .set("width", W + 10)
        .set("height", H + 10)
        .set("style", "background-color:white");
    doc = doc.add(Style::new(format!(
        "text {{text-anchor: middle;dominant-baseline: central; font-size: {}}}",
        16 * 2 * input.n / input.m / max_h.max(1)
    )));
    for i in 0..input.m {
        for j in 0..bs[i].len() {
            doc = doc.add(
                rect(
                    i * w + 1,
                    (max_h - j - 1) * h,
                    w - 2,
                    h,
                    &color(bs[i][j] as f64 / input.n as f64),
                )
                .set("stroke", "black")
                .set("stroke-width", 1)
                .set("class", "box"),
            );
            doc = doc.add(
                svg::node::element::Text::new(bs[i][j].to_string())
                    .set("x", i * w + w / 2)
                    .set("y", (max_h - j - 1) * h + h / 2),
            );
        }
    }
    (score, cost, err, doc.to_string())
}

pub fn generate(seed: i32) -> String {
    let seed = seed.max(0) as u64;
    let input = gen_input(seed);
    format!("{}", input)
}

pub fn calc_max_turn(input: &str, output: &str) -> usize {
    let input = parse_input(input);
    match parse_output(&input, output) {
        Ok(out) => out.out.len(),
        Err(_) => 0,
    }
}

pub fn visualize(
    input: &str,
    output: &str,
    turn: usize,
) -> Result<(i64, i64, String, String), String> {
    let input = parse_input(input);
    let out = parse_output(&input, output)?;
    let turn = turn.min(out.out.len());
    let (mut score, cost, err, svg) = vis_prefix(&input, &out.out[..turn]);
    if !err.is_empty() {
        score = 0;
    }
    Ok((score, cost, err, svg))
}
