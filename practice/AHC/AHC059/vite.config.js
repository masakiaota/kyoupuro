...
const CASE_SORT_OPTIONS = [
  ...
  { key: "pair_dist_avg_asc", label: "pair_dist_avg ∧" },
  { key: "pair_dist_avg_desc", label: "pair_dist_avg ∨" },
  { key: "pair_dist_max_desc", label: "pair_dist_max ∨" },
];
...
function parseCaseMeta(filePath) {
  const tokens = fs.readFileSync(filePath, "utf-8").trim().split(/\s+/).map(Number);
  const n = tokens[0];
  if (!Number.isInteger(n) || n <= 0 || tokens.length < 1 + n * n) {
    return null;
  }
  const positions = new Map();
  for (let i = 0; i < n; i += 1) {
    for (let j = 0; j < n; j += 1) {
      const value = tokens[1 + i * n + j];
      if (!positions.has(value)) {
        positions.set(value, []);
      }
      positions.get(value).push([i, j]);
    }
  }
  let sumDist = 0;
  let maxDist = 0;
  ...
  for (const pair of positions.values()) {
    ...
    const dist = Math.abs(pair[0][0] - pair[1][0]) + Math.abs(pair[0][1] - pair[1][1]);
    sumDist += dist;
