...
impl TraceStats {
    fn summary(&self) {
        eprintln!(
            "[summary] greedy_insert_dist={} moves={} turns={} score_est={} iter={} improved={} elapsed_ms={:.3}",
...
fn build_by_order(input: &Input, order: &[usize]) -> (Vec<usize>, usize) {
    ...
    for &v in order {
        ...
        greedy_insert_dist += insertion.cost;
...
fn greedy_shuffle(input: &Input) -> (Vec<usize>, usize, SearchStats) {
    ...
    loop {
        if iterations > 0 && start.elapsed().as_secs_f64() >= SEARCH_TIME_LIMIT_SEC {
            break;
        }
        ...
        let (state, dist) = build_by_order(input, &order);
        iterations += 1;
...
fn solve(input: &Input) -> Vec<u8> {
    let (state, greedy_insert_dist, search_stats) = greedy_shuffle(input);
    local! {
