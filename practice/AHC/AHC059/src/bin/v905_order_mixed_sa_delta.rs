...
const REVERSE_PERCENT: usize = 0;
const RELOCATE_PERCENT: usize = 60;
const SWAP_PERCENT: usize = 40;
...
const NEIGHBOR_NAMES: [&str; NEIGHBOR_COUNT] = ["reverse", "relocate", "swap"];
...
struct NeighborStats {
    stats: [NeighborStat; NEIGHBOR_COUNT],
}

#[cfg(feature = "local")]
impl NeighborStats {
    fn record_try(&mut self, kind: usize) {
        self.stats[kind].tried += 1;
    }

    fn record_accept(&mut self, kind: usize) {
        self.stats[kind].accepted += 1;
    }

    fn record_best_update(&mut self, kind: usize) {
        self.stats[kind].best_update += 1;
    }

    fn summary(&self) {
        for (kind, name) in NEIGHBOR_NAMES.iter().enumerate() {
            let stat = self.stats[kind];
            ...
            eprintln!(
                "[neighbor] kind={} tried={} accepted={} accept_rate={:.6} best_update={}",
                name,
                stat.tried,
                stat.accepted,
                accept_rate,
...
impl AppliedMove {
    ...
    fn start(self) -> usize {
        match self {
            AppliedMove::Reverse { l, .. } => l,
            AppliedMove::Relocate { i, j } | AppliedMove::Swap { i, j } => i.min(j),
        }
    }
    ...
    fn kind(self) -> usize {
        match self {
            AppliedMove::Reverse { .. } => 0,
            AppliedMove::Relocate { .. } => 1,
            AppliedMove::Swap { .. } => 2,
        }
    }

    fn apply(self, order: &mut [usize]) {
        match self {
            AppliedMove::Reverse { l, r } => order[l..=r].reverse(),
            AppliedMove::Relocate { i, j } => apply_relocate(order, i, j),
            AppliedMove::Swap { i, j } => order.swap(i, j),
        }
    }

    fn undo(self, order: &mut [usize]) {
        match self {
            AppliedMove::Reverse { l, r } => order[l..=r].reverse(),
            AppliedMove::Relocate { i, j } => undo_relocate(order, i, j),
            AppliedMove::Swap { i, j } => order.swap(i, j),
        }
    }
}
...
fn order_swap_sa(input: &Input) -> (Vec<usize>, usize, SearchStats) {
    ...
    loop {
        ...
        let applied = {
            let r = rng.gen_usize(REVERSE_PERCENT + RELOCATE_PERCENT + SWAP_PERCENT);
            if r < REVERSE_PERCENT {
                ...
                AppliedMove::Reverse { l, r }
            } else if r < REVERSE_PERCENT + RELOCATE_PERCENT {
                let i = rng.gen_usize(M);
                let mut j = rng.gen_usize(M);
                if i == j {
                    j = (j + 1) % M;
                }
                AppliedMove::Relocate { i, j }
            } else {
                let i = rng.gen_usize(M);
                let mut j = rng.gen_usize(M);
                if i == j {
                ...
                AppliedMove::Swap { i, j }
            }
        };
        let kind = applied.kind();
        #[cfg(feature = "local")]
        neighbor_stats.record_try(kind);

        let start = applied.start();
        applied.apply(&mut order);
        ...
        if accept(delta, temp, &mut rng) {
            ...
            accepted += 1;
            #[cfg(feature = "local")]
            neighbor_stats.record_accept(kind);
            if current_dist < best_dist {
                best_order.clone_from(&order);
                best_dist = current_dist;
                improved += 1;
                #[cfg(feature = "local")]
                neighbor_stats.record_best_update(kind);
            }
        } else {
            applied.undo(&mut order);
        }
    }
