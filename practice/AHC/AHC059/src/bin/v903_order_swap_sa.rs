...
impl TraceStats {
    fn summary(&self) {
        eprintln!(
            "[summary] greedy_insert_dist={} moves={} turns={} score_est={} iter={} accepted={} improved={} elapsed_ms={:.3}",
...
fn order_swap_sa(input: &Input) -> (Vec<usize>, usize, SearchStats) {
    ...
    let (_, mut current_dist) = build_by_order(input, &order);
    ...
    loop {
        ...
        order.swap(i, j);
        let (_, next_dist) = build_by_order(input, &order);
        iterations += 1;
    ...
    let (best_state, restored_dist) = build_by_order(input, &best_order);
