...
3. scorer または vis があるなら `./scripts/eval.sh [-v] <bin_name> [input_dir]` で公式スコアを確認する
   - `input_dir` 省略時は `tools/in` を使う。
   - 各ケースについて `run -> score/vis` を `cpu//2` 並列で実行する。
...
- `./scripts/eval.sh [-v] <bin_name> [input_dir]`
  - solver と公式 `score` または `vis` を 1 回だけ build し、ケース単位で `run -> score/vis` を `cpu//2` 並列実行する。
  - 出力は `results/out/<bin_name>/` に保存し、要約は `results/score_summary.csv` に追記する。
- `./scripts/gen_tools.sh <args...>`
  - 公式 `tools` の `gen` バイナリを `tools/` を cwd にしてラップする。追加入力生成用である。
- `./scripts/vis_tools.sh <input_file> <output_file> [svg_out]`
  - 公式 `tools` の `vis` バイナリを `tools/` を cwd にしてラップし、SVG を生成する。
- `./scripts/unpack_tools.sh [tools_zip_path ...]`
  - `tools/*.zip` を自動検出するか、指定した複数の公式配布 zip を `tools/` にマージ展開する。
...
./scripts/gen_tools.sh ./tools/seeds.txt
./scripts/vis_tools.sh ./tools/in/0000.txt ./results/out/v000_template/0000.txt
./scripts/build_wasm.sh
./scripts/dev_vis.sh
...
## Visualizer の使い方
- まず `./scripts/unpack_tools.sh` で `tools/src/` と `tools/in/` を揃える
