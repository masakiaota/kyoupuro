...
If tools has `score`, it is used. Otherwise `vis` is used as scorer.
EOF
}
...
SCORER_MODE=
SCORER_BIN=
if [ -f "$TOOLS_DIR/src/bin/score.rs" ]; then
    cargo build --release --quiet --manifest-path "$TOOLS_MANIFEST" --bin score
    SCORER_MODE=score
    SCORER_BIN="$TOOLS_BIN_DIR/score"
elif [ -f "$TOOLS_DIR/src/bin/vis.rs" ]; then
    cargo build --release --quiet --manifest-path "$TOOLS_MANIFEST" --bin vis
    SCORER_MODE=vis
    SCORER_BIN="$TOOLS_BIN_DIR/vis"
else
    echo "error: neither score nor vis binary found under tools/src/bin" >&2
    exit 1
fi
...
if [ ! -x "$SCORER_BIN" ]; then
    echo "error: scorer binary not found: $SCORER_BIN" >&2
    exit 1
fi
...
export OUTPUT_DIR META_DIR SOLVER_BIN SCORER_BIN SCORER_MODE VERBOSE

if [ "$VERBOSE" -ge 1 ]; then
    printf 'eval: start run -> score pipeline (scorer=%s)\n' "$SCORER_MODE" >&2
fi
...
if tr '\n' '\0' < "$INPUT_LIST" | xargs -0 -n 1 -P "$PARALLEL" sh -c '
    ...
    case "$SCORER_MODE" in
        score)
            if score_result=$("$SCORER_BIN" "$input_file" "$output_file" 2>> "$err_file"); then
                score_status=0
            else
                score_status=$?
            fi
            ;;
        vis)
            vis_dir="$META_DIR/$base.vis"
            mkdir -p "$vis_dir"
            if score_result=$(cd "$vis_dir" && "$SCORER_BIN" "$input_file" "$output_file" 2>> "$err_file"); then
                score_status=0
            else
                score_status=$?
            fi
