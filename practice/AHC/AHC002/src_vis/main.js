...
let playbackLastTs = null;
let playbackCarryTurns = 0;
...
function advancePlayback(deltaTurns) {
  const maxTurn = getMaxTurn();
  if (maxTurn <= 0 || deltaTurns <= 0) return;

  const now = Number(els.turn.value);
  let next = now + deltaTurns;
...
function tickPlayback(timestamp) {
  ...
  if (playbackLastTs === null) {
    playbackLastTs = timestamp;
    playbackFrame = requestAnimationFrame(tickPlayback);
    return;
  }

  const elapsedSec = Math.max(0, (timestamp - playbackLastTs) / 1000);
  playbackLastTs = timestamp;
  playbackCarryTurns += elapsedSec * getSpeed();

  const wholeTurns = Math.floor(playbackCarryTurns);
  if (wholeTurns > 0) {
    playbackCarryTurns -= wholeTurns;
    advancePlayback(wholeTurns);
  }
...
function startPlayback() {
  ...
  if (!els.loopPlay.checked && Number(els.turn.value) >= maxTurn) return;
...
function setTurnMax() {
  ...
  if (input.trim().length > 0) {
    try {
      mx = wasmGetMaxTurn(input, output);
    } catch {
      mx = 0;
    }
  }
...
function render() {
  ...
  if (!input.trim()) {
    ...
    els.error.textContent = "Input を入れると描画される";
...
async function loadRustBins() {
  try {
    ...
    setRunStatus(`bin 一覧を取得: ${bins.length} 件`);
  } catch (e) {
    els.rustBin.innerHTML = "";
    const opt = document.createElement("option");
    opt.value = "";
    ...
    els.rustBin.appendChild(opt);
    setRunStatus(
      `bin 一覧取得に失敗: ${String(e)} (yarn dev で開いているか確認)`,
      true,
    );
  }
}
...
async function runSelectedRustBin() {
  ...
  try {
    ...
    setRunStatus(
      `${binName} 実行完了${elapsed !== null ? ` (${elapsed} ms)` : ""}${stderr ? " / stderr あり" : ""}`,
    );
  } catch (e) {
    setRunStatus(`bin 実行失敗: ${String(e)}`, true);
  } finally {
    runningBin = false;
...
els.genBtn.addEventListener("click", () => {
  generateInput(Number(els.seed.value) | 0);
});
...
async function main() {
  ...
  const wasmBinUrl = new URL(
    "/wasm/heuristic_contest_template_vis_bg.wasm",
    window.location.origin,
  );
  const wasmJsSource = await fetch(wasmJsUrl).then((res) => {
    if (!res.ok) {
      throw new Error(`WASM loader fetch failed: HTTP ${res.status}`);
    }
    return res.text();
  });
  const blobUrl = URL.createObjectURL(
    new Blob([wasmJsSource], { type: "text/javascript" }),
  );
  ...
  els.error.textContent =
    "seed を生成するか Input を貼ると AHC002 の盤面を描画する";
  render();
  await loadRustBins();
}

main().catch((e) => {
  els.error.textContent = `WASM 初期化失敗: ${String(e)}`;
});
