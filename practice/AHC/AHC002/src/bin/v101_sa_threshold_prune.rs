...
const SA_TEMP_START: f64 = 300.0;
const SA_TEMP_END: f64 = 50.0;
...
struct RerouteSearch<'a> {
    ...
    in_path: &'a [u8],
    dfs_used: &'a mut [u8],
    optimistic_gain_prefix: &'a [i32],
    required_new_score: f64,
    b_cell: usize,
    min_i: usize,
    max_i: usize,
    min_j: usize,
...
impl<'a> RerouteSearch<'a> {
    ...
    fn dfs(&mut self, cur: usize) {
        ...
        if self.path.len() + self.board.manhattan[cur * CELL_COUNT + self.b_cell] as usize > self.max_len {
            return;
        }
        let remaining_steps = self.max_len - self.path.len();
        let optimistic_new_score =
            (self.cur_score_delta as i64) + (self.optimistic_gain_prefix[remaining_steps] as i64);
        if (optimistic_new_score as f64) <= self.required_new_score {
            return;
        }
...
fn build_board() -> Board {
    ...
    let mut manhattan = vec![0_u8; CELL_COUNT * CELL_COUNT];
    for a in 0..CELL_COUNT {
        ...
        for b in 0..CELL_COUNT {
            let bi = b / N;
            let bj = b % N;
            manhattan[a * CELL_COUNT + b] = (ai.abs_diff(bi) + aj.abs_diff(bj)) as u8;
        }
    }
...
fn build_optimistic_gain_prefix(scores: &[i32]) -> Vec<i32> {
    let mut positives: Vec<i32> = scores.iter().copied().filter(|&s| s > 0).collect();
    positives.sort_unstable_by(|a, b| b.cmp(a));

    let mut prefix = vec![0_i32; CELL_COUNT + 1];
    let mut acc = 0_i32;
    for k in 1..=CELL_COUNT {
        if k <= positives.len() {
            acc += positives[k - 1];
        }
        prefix[k] = acc;
...
fn main() {
    ...
    let optimistic_gain_prefix = build_optimistic_gain_prefix(&board.scores);
    ...
    let mut reroute_path = Vec::with_capacity(SEG_MAX + SEG_EXTRA_LEN + 1);
    let mut reroute_best_path = Vec::with_capacity(SEG_MAX + SEG_EXTRA_LEN + 1);
    let mut new_seg_cells = Vec::with_capacity(SEG_MAX + SEG_EXTRA_LEN + 1);
    let mut new_seg_move_scores = Vec::with_capacity(SEG_MAX + SEG_EXTRA_LEN + 1);
    ...
    loop {
        ...
        let accept_threshold_delta = temp * u.ln();
        let required_new_score = (old_seg_score_delta as f64) + accept_threshold_delta;
        ...
        let new_seg_score_delta = {
            let mut search = RerouteSearch {
                ...
                optimistic_gain_prefix: &optimistic_gain_prefix,
                required_new_score,
                b_cell,
                min_i,
                max_i,
        ...
        if delta <= accept_threshold_delta {
            for &cell in &cur_cells[l + 1..r] {
                in_path[board.tiles[cell]] = 1;
            }
            continue;
