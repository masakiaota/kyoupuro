// v012_middle_reroute_sa_fast.rs
...
impl<'a> RerouteSearch<'a> {
    ...
    fn dfs(&mut self, cur: usize) {
        ...
        for &dir_rank in &dir_order {
            ...
            let next_tile = self.board.tiles[next];
            let pos = self.tile_pos[next_tile];
            if pos != INVALID && !(self.cut_l < pos && pos < self.cut_r) {
                continue;
            }
...
fn build_tile_pos(board: &Board, cells: &[usize]) -> Vec<usize> {
    let mut tile_pos = vec![INVALID; board.tile_count];
    for (idx, &cell) in cells.iter().enumerate() {
        tile_pos[board.tiles[cell]] = idx;
    }
    tile_pos
}
...
#[inline(always)]
fn objective_value(score_delta: i32) -> f64 {
    score_delta as f64
}
...
fn main() {
    ...
    let mut tile_pos = build_tile_pos(&board, &cur_cells);
    ...
    while global_start.elapsed().as_secs_f64() < TOTAL_TIME_LIMIT_SEC {
        ...
        let delta = objective_value(candidate_score_delta) - objective_value(cur_score_delta);
        ...
        let mut new_seg_cells = Vec::with_capacity(new_seg_path.len());
        let mut new_seg_move_scores = Vec::with_capacity(new_seg_path.len());
        let mut cur = a_cell;
        for &dir in &new_seg_path {
            let next = board.adj[cur][dir_to_idx(dir)];
            debug_assert!(next != INVALID);
            cur = next;
        ...
        for &cell in &cur_cells[l + 1..=r] {
            tile_pos[board.tiles[cell]] = INVALID;
        }

        cur_path.splice(l..r, new_seg_path.iter().copied());
        cur_cells.splice((l + 1)..(r + 1), new_seg_cells.iter().copied());
        cur_move_scores.splice(l..r, new_seg_move_scores.iter().copied());

        for (idx, &cell) in cur_cells.iter().enumerate().skip(l + 1) {
            tile_pos[board.tiles[cell]] = idx;
        }
